...
LirOperand StmtEmitter::emit_amd64_va_arg_from_registers(
  ...
  LirOperand fp_base_ptr;
  if (layout.sse_slots > 0) {
    fp_base_ptr = fresh_value(ctx);
    emit_lir_op(ctx, lir::LirGepOp{
                         fp_base_ptr, "i8", access.reg_save_area_ptr, false,
                         {LirGepIndex::typed(LirTypeRef::integer(64), fp_offset_i64)}});
  ...
  LirOperand active_sse_slot_ptr;
  for (size_t chunk = 0; chunk < layout.classes.size(); ++chunk) {
    ...
    if (cls == llvm_cc::Amd64ArgClass::Integer) {
      src_ptr = gp_base_ptr;
      if (gp_chunk_index > 0) {
        src_ptr = fresh_value(ctx);
        emit_lir_op(ctx, lir::LirGepOp{
                             src_ptr, "i8", gp_base_ptr, false,
                             {LirGepIndex::typed(
                                 LirTypeRef::integer(64),
                                 LirOperand::integer(std::to_string(gp_chunk_index * 8),
                                                     gp_chunk_index * 8))}});
      }
      ++gp_chunk_index;
      ...
      if (sse_slot_index > 0) {
        ...
        emit_lir_op(ctx, lir::LirGepOp{
                             ...
                             {LirGepIndex::typed(
                                 LirTypeRef::integer(64),
                                 LirOperand::integer(std::to_string(sse_slot_index * 16),
                                                     sse_slot_index * 16))}});
      }
      ...
      LirOperand upper_ptr = fresh_value(ctx);
      emit_lir_op(
          ctx, lir::LirGepOp{upper_ptr, "i8", active_sse_slot_ptr, false,
                             {LirGepIndex::typed(LirTypeRef::integer(64),
                                                 LirOperand::integer("8", 8))}});
      src_ptr = upper_ptr;
    } else {
      continue;
    }

    LirOperand dst_ptr = LirOperand::raw(tmp_addr);
    if (chunk_offset > 0) {
      dst_ptr = fresh_value(ctx);
      emit_lir_op(
          ctx, lir::LirGepOp{dst_ptr, "i8", tmp_addr, false,
                             {LirGepIndex::typed(
                                 LirTypeRef::integer(64),
                                 LirOperand::integer(std::to_string(chunk_offset), chunk_offset))}});
    }
