...
std::string StmtEmitter::emit_rval_payload(FnCtx& ctx, const VaArgExpr& v, const Expr& e) {
  ...
  if (const auto hfa = classify_aarch64_hfa(mod_, res_ts)) {
    ...
    const std::string src_ptr = fresh_tmp(ctx);
    emit_lir_op(ctx, lir::LirPhiOp{src_ptr, "ptr", {{reg_tmp, reg_lbl}, {aligned_stack_ptr, stack_lbl}}});
    ...
    emit_lir_op(ctx, lir::LirMemcpyOp{tmp_addr, src_ptr, std::to_string(hfa->aggregate_size), false});
    const std::string out = fresh_tmp(ctx);
  ...
  const bool is_gp_scalar =
      (res_ty == "ptr") || (res_ts.ptr_level == 0 && res_ts.array_rank == 0 && is_any_int(res_ts.base));
