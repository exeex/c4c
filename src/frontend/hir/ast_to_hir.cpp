...
bool parse_builtin_typespec_text(const std::string& text, TypeSpec* out) {
  if (!out) return false;
  ...
  while (iss >> tok) {
    if (tok == "const") ts.is_const = true;
      ...
      else if (is_unsigned) ts.base = TB_UINT;
      else ts.base = TB_INT;
      saw_base = true;
    } else if (tok == "long") {
      ++long_count;
      ts.base = long_count >= 2 ? (is_unsigned ? TB_ULONGLONG : TB_LONGLONG)
...
class Lowerer {
 ...
 private:
      ...
      if (ctx && !ctx->tpl_bindings.empty() && ts.tpl_struct_origin)
        {
          ...
          resolve_pending_tpl_struct(
              ts, find_template_struct_primary(ts.tpl_struct_origin),
              ctx->tpl_bindings, ctx->nttp_bindings);
        }
        return ts;
      }
      case NK_BINOP: {
  ...
  void lower_struct_def(const Node* sd) {
    ...
    for (int bi = 0; bi < sd->n_bases; ++bi) {
      ...
      if (base.tpl_struct_origin) {
        ...
        resolve_pending_tpl_struct(
            base, find_template_struct_primary(base.tpl_struct_origin),
            base_tpl_bindings, base_nttp_bindings);
      }
  ...
  void resolve_pending_tpl_struct(TypeSpec& ts,
    ...
    auto eval_deferred_nttp_expr_hir =
    ...
    // Resolve each explicit arg using tpl_bindings / nttp_bindings.
      ...
      HirTemplateArg arg;
        ...
        if (is_deferred_nttp_expr_ref(arg_refs[ai])) {
          ...
          (void)eval_deferred_nttp_expr_hir(
              primary_tpl, pi, resolved_type_bindings,
              resolved_nttp_bindings, &expr, &eval_val);
          arg.is_value = true;
          arg.value = eval_val;
        } else {
        ...
        resolved_nttp_bindings.push_back({param_name, arg.value});
      } else {
        ...
        if (ref.size() > 1 && ref[0] == '@') {
          ...
          resolve_pending_tpl_struct(
              nested_ts, find_template_struct_primary(inner_origin),
    ...
    while (static_cast<int>(concrete_args.size()) < primary_tpl->n_template_params) {
      const int pi = static_cast<int>(concrete_args.size());
      ...
      if (arg.is_value) {
        if (primary_tpl->template_param_default_values[pi] != LLONG_MIN) {
          ...
          if (!eval_deferred_nttp_expr_hir(primary_tpl, pi,
                                           resolved_type_bindings,
                                           resolved_nttp_bindings,
                                           nullptr, &eval_val)) {
            break;
          }
          arg.value = eval_val;
    ...
    if (!module_->struct_defs.count(mangled) &&
      ...
      for (int bi = 0; bi < tpl_def->n_bases; ++bi) {
        ...
        if (base_ts.tpl_struct_origin) {
          resolve_pending_tpl_struct(
              base_ts, find_template_struct_primary(base_ts.tpl_struct_origin),
              method_tpl_bindings, method_nttp_bindings);
        }
  ...
  void lower_function(const Node* fn_node,
    ...
    // Substitute template type parameters in the return type.
    {
      ...
      if (tpl_override && ret_ts.tpl_struct_origin) {
        ...
        resolve_pending_tpl_struct(
            ret_ts, find_template_struct_primary(ret_ts.tpl_struct_origin),
            *tpl_override, nttp_override ? *nttp_override : nttp_empty);
      }
    ...
    for (int i = 0; i < fn_node->n_params; ++i) {
      ...
      // Substitute template type parameters in parameter types.
      {
        ...
        if (tpl_override && param_ts.tpl_struct_origin) {
          ...
          resolve_pending_tpl_struct(
              param_ts, find_template_struct_primary(param_ts.tpl_struct_origin),
              *tpl_override, nttp_override ? *nttp_override : nttp_empty);
        }
  ...
  void lower_struct_method(const std::string& mangled_name,
    ...
    // Substitute template type parameters in the return type.
    {
      ...
      if (tpl_bindings && ret_ts.tpl_struct_origin) {
        ...
        resolve_pending_tpl_struct(
            ret_ts, find_template_struct_primary(ret_ts.tpl_struct_origin),
            *tpl_bindings, nttp_bindings ? *nttp_bindings : nttp_empty);
      }
  ...
  void lower_local_decl_stmt(FunctionCtx& ctx, const Node* n) {
    ...
    // Substitute template type parameters in local variable types.
    {
      ...
      if (!ctx.tpl_bindings.empty() && decl_ts.tpl_struct_origin)
        resolve_pending_tpl_struct(
            decl_ts, find_template_struct_primary(decl_ts.tpl_struct_origin),
            ctx.tpl_bindings, ctx.nttp_bindings);
  ...
  ExprId lower_expr(FunctionCtx* ctx, const Node* n) {
    ...
    switch (n->kind) {
      ...
      case NK_CAST: {
        ...
        if (ctx && !ctx->tpl_bindings.empty() && cast_ts.tpl_struct_origin)
          resolve_pending_tpl_struct(
              cast_ts, find_template_struct_primary(cast_ts.tpl_struct_origin),
              ctx->tpl_bindings, ctx->nttp_bindings);
