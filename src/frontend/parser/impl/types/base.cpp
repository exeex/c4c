...
static const Node* find_template_struct_primary_for_parse_base_type(
    Parser& parser,
    const TypeSpec& ts) {
    if (ts.record_def && ts.record_def->n_template_params > 0 &&
        ts.record_def->n_template_args == 0) {
        TextId record_text_id = parse_base_type_record_text_id(parser,
                                                               ts.record_def);
        const int record_context_id =
            ts.record_def->namespace_context_id >= 0
                ? ts.record_def->namespace_context_id
                : parser.current_namespace_context_id();
        if (Node* primary =
                parser.find_template_struct_primary(record_context_id,
                                                    record_text_id)) {
            if (primary->n_fields >= ts.record_def->n_fields) return primary;
        }
        if (ts.record_def->n_fields >= 0) return ts.record_def;
    }
    if (ts.tpl_struct_origin_key.base_text_id != kInvalidText) {
        if (Node* primary =
                parser.find_template_struct_primary(ts.tpl_struct_origin_key)) {
            return primary;
        }
    }
    if (ts.tag_text_id != kInvalidText &&
        ...
        for (int i = 0; i < ts.n_qualifier_segments; ++i) {
            const char* segment =
                ts.qualifier_segments ? ts.qualifier_segments[i] : nullptr;
        ...
        if (Node* primary = parser.find_template_struct_primary(qn)) {
            return primary;
        }
    }
    ...
    const int namespace_context_id =
        ts.namespace_context_id >= 0 ? ts.namespace_context_id
                                     : parser.current_namespace_context_id();
    const Node* best = parser.find_template_struct_primary(
        namespace_context_id, tag_text_id);
    int probe_context_id = namespace_context_id;
    while (probe_context_id >= 0 &&
           probe_context_id <
               static_cast<int>(
                   parser.namespace_state_.namespace_contexts.size())) {
        const ParserNamespaceContext& ns =
            parser.namespace_state_.namespace_contexts[probe_context_id];
        if (Node* candidate =
                parser.find_template_struct_primary(ns.id, tag_text_id)) {
            if (!best || candidate->n_fields > best->n_fields) {
                best = candidate;
            }
        }
        probe_context_id = ns.parent_id;
    }
    return best;
}
...
TypeSpec Parser::parse_base_type() {
    ...
    lookup_struct_member_typedef_recursive_for_type =
            [&](const TypeSpec& owner_ts, const std::string& member,
                ...
                auto complete_member_typedef_type_from_owner_fields =
                    [&](const Node* sdef, TypeSpec* member_ts) {
                    if (!sdef || !member_ts ||
                        (member_ts->base != TB_STRUCT &&
                         member_ts->base != TB_UNION &&
                         member_ts->base != TB_TYPEDEF) ||
                        member_ts->record_def) {
                        return;
                    }
                    TextId member_type_text_id = type_spec_base_text_id(*member_ts);
                    const Node* sole_nested_record = nullptr;
                    int nested_record_count = 0;
                    for (int fi = 0; fi < sdef->n_fields; ++fi) {
                        const Node* field = sdef->fields ? sdef->fields[fi] : nullptr;
                        if (!field || !field->is_anon_field ||
                            !field->type.record_def) {
                            continue;
                        }
                        TextId field_text_id =
                            field->unqualified_text_id != kInvalidText
                                ? field->unqualified_text_id
                                : field->type.tag_text_id;
                        if (field_text_id == kInvalidText && field->name) {
                            field_text_id =
                                parser_text_id_for_token(kInvalidText,
                                                         field->name);
                        }
                        if (member_type_text_id == kInvalidText) {
                            sole_nested_record = field;
                            ++nested_record_count;
                            continue;
                        }
                        if (field_text_id != member_type_text_id) continue;
                        member_ts->record_def = field->type.record_def;
                        member_ts->namespace_context_id =
                            field->type.namespace_context_id;
                        break;
                    }
                    if (member_type_text_id == kInvalidText &&
                        nested_record_count == 1 && sole_nested_record) {
                        member_ts->base = sole_nested_record->type.base;
                        member_ts->record_def =
                            sole_nested_record->type.record_def;
                        member_ts->tag_text_id =
                            sole_nested_record->type.tag_text_id;
                        member_ts->namespace_context_id =
                            sole_nested_record->type.namespace_context_id;
                    }
                };
                auto try_node_member_typedefs = [&](const Node* sdef) -> bool {
                    ...
                    for (int i = 0; i < sdef->n_member_typedefs; ++i) {
                        ...
                        if (name_matches) {
                            ...
                            TypeSpec member_ts = sdef->member_typedef_types[i];
                    ...
                    [&](const Node* owner) -> bool {
                        ...
                        for (int i = 0; i < selected->n_member_typedefs; ++i) {
                            ...
                            if (!substituted_type && is_type_member &&
                                ...
                                member_ts = type_bindings.front().second;
                            }
                            *out = member_ts;
                            return true;
                        }
                ...
                auto try_template_instantiation_member_typedef =
                    [&](const Node* owner) -> bool {
                        ...
                        if (const TypeSpec* structured_member =
                            ...
                            TypeSpec member_ts = *structured_member;
                            complete_member_typedef_type_from_owner_fields(
                                owner, &member_ts);
                            *out = member_ts;
                            return true;
                    ...
                    if (is_cpp_mode()) {
                        ...
                        const bool simple_unqualified_known_type_head =
                            ...
                            is_known_simple_visible_type_head(*this,
                                                              name_text_id,
                                                              name);
                        if (!simple_unqualified_known_type_head && !already_have_base) {
                            QualifiedNameRef qn;
                            if (peek_qualified_name(&qn, true)) {
                                const int after_pos =
                                    core_input_state_.pos +
                                    (qn.is_global_qualified ? 1 : 0) +
                                    2 * static_cast<int>(
                                            qn.qualifier_segments.size()) +
                                    1;
                                if ((qn.is_global_qualified ||
                                     !qn.qualifier_segments.empty()) &&
                                    after_pos <
                                        static_cast<int>(
                                    ...
                                    core_input_state_.tokens[after_pos].kind ==
                                        TokenKind::Less) {
                                    if (Node* primary_tpl =
                                            find_template_struct_primary(qn)) {
                                        TentativeParseGuardLite template_guard(
                                            *this);
                                        std::string qualified_name;
                                        if (consume_qualified_type_spelling_with_typename(
                                                /*require_typename=*/false,
                                                /*allow_global=*/true,
                                                /*consume_final_template_args=*/
                                                false,
                                            ...
                                            if (parse_template_argument_list(
                                                ...
                                                if (check(TokenKind::Identifier)) {
                                                    ...
                                                    TypeSpec resolved_owner =
                                                        owner_ts;
                                                    if (origin && origin[0]) {
                                                        (void)ensure_template_struct_instantiated_from_args(
                                                            origin,
                                                            primary_tpl,
                                                            parsed_args,
                                                            primary_tpl->line,
            ...
            if (!typedef_type && is_unqualified_typedef &&
                ...
                if (!typedef_type &&
                    ...
                    const TextId current_record_text_id =
                        active_context_state_.current_struct_tag_text_id;
                    const std::string_view current_record_name =
                        parser_text(current_record_text_id, {});
                    if (!current_record_name.empty() &&
                        current_record_name.find("::") ==
                        ...
                        typedef_type =
                            find_dependent_record_member_typedef_type(
                                owner_key, tname_text_id);
                    }
                }
            }
            if (!typedef_type) {
            ...
            if (typedef_type) {
                ...
                if (is_cpp_mode() && check(TokenKind::Less)) {
                    ...
                    if (ati) {
                       ...
                       find_template_struct_primary_for_parse_base_type(
                           *this, ts)) {
                    ...
                    if (resolved.base == TB_STRUCT && !resolved.record_def) {
                        ...
                        const TextId resolved_text_id =
                            resolved.tag_text_id != kInvalidText
                                ...
                                : resolved.template_param_text_id;
                        const Node* sole_nested_record = nullptr;
                        int nested_record_count = 0;
                        if (inst_def) {
                            for (int fi = 0; fi < inst_def->n_fields; ++fi) {
                                const Node* field =
                                    inst_def->fields ? inst_def->fields[fi]
                                                     : nullptr;
                                if (!field || !field->is_anon_field ||
                                    !field->type.record_def) {
                                    continue;
                                ...
                                if (field_text_id != resolved_text_id) continue;
                                resolved.record_def = field->type.record_def;
                                resolved.tag_text_id = field->type.tag_text_id;
            ...
            // Dependent template-template parameter application:
                ...
                if (is_cpp_mode() && tname &&
                    ...
                    if (parse_template_argument_list(&ignored_args)) {
                        ts.base = TB_TYPEDEF;
                        set_parse_base_type_legacy_tag_if_present(ts, tname,
                                                                  0);
                ...
                auto template_primary_for_parse_base_type =
                    [&](const TypeSpec& type,
                        TextId* out_text_id) -> const Node* {
                    TextId text_id =
                        type.tag_text_id != kInvalidText
                            ? type.tag_text_id
                            : parse_base_type_record_text_id(*this,
                                                             type.record_def);
                    const Node* primary =
                        find_template_struct_primary_for_parse_base_type(
                            *this, type);
                    if (!primary &&
                        ...
                        if (const char* legacy_tag =
                                parse_base_type_legacy_tag_if_no_metadata(
                                    type)) {
                            if (legacy_tag[0]) {
                                text_id = parser_text_id_for_token(
                                    kInvalidText, legacy_tag);
                                primary = find_template_struct_primary(
                                    current_namespace_context_id(),
                                    text_id);
                    ...
                    if (out_text_id) *out_text_id = text_id;
                    return primary;
                };
                TextId tpl_text_id = kInvalidText;
                const Node* primary_tpl =
                    is_cpp_mode() && ts.base == TB_STRUCT
                        ? template_primary_for_parse_base_type(ts,
                                                               &tpl_text_id)
                        : nullptr;
                // Template struct instantiation: Pair<int> or Array<int, 4> in type context.
                if (is_cpp_mode() && ts.base == TB_STRUCT && primary_tpl &&
                    check(TokenKind::Less)) {
                    std::string tpl_name;
                    if (primary_tpl) {
                        if (primary_tpl->name && primary_tpl->name[0]) {
                            tpl_name = primary_tpl->name;
                        } else if (primary_tpl->template_origin_name &&
                                   primary_tpl->template_origin_name[0]) {
                            tpl_name = primary_tpl->template_origin_name;
                        } else if (primary_tpl->unqualified_name &&
                                   primary_tpl->unqualified_name[0]) {
                            tpl_name = primary_tpl->unqualified_name;
                        }
                    }
                    if (tpl_name.empty() && tpl_text_id != kInvalidText) {
                        tpl_name = std::string(parser_text(tpl_text_id, {}));
                    }
                    if (tpl_name.empty()) {
                        if (const char* legacy_tag =
                                parse_base_type_legacy_tag_if_no_metadata(ts)) {
                    ...
                    if (tpl_name.empty()) {
                        tpl_name = primary_tpl->name ? primary_tpl->name : "";
                    }
                    std::vector<ParsedTemplateArg> actual_args;
                    ...
                    for (auto& arg : actual_args) {
                        ...
                        if (deferred_member_name.empty() ||
                            !(parse_base_type_has_structured_identity_metadata(arg_ts) ||
                              parse_base_type_legacy_tag_if_no_metadata(arg_ts))) {
                            continue;
                        }
                    ...
                    if (primary_tpl && has_pack_param) {
                        ...
                        set_parse_base_type_legacy_tag_if_present(
                            ts, arena_.strdup(tpl_name.c_str()), 0);
                        return ts;
                    }
                    ...
                    if (primary_tpl) {
                        ...
                        auto prelim_binding_matches_type =
                            ...
                            if (type_has_owner) {
                                ...
                                return type_text_id == kInvalidText ||
                                       binding.name_text_id == type_text_id ||
                                       type.template_param_text_id ==
                                           binding.name_text_id;
                            }
                            return binding.name_text_id != kInvalidText &&
                                   binding.name_text_id == type_text_id;
                        };
                        std::function<TypeSpec(TypeSpec)> prelim_type_for_type;
                        prelim_type_for_type =
                            ...
                            if (type.base == TB_TYPEDEF) {
                                ...
                                if (type_text_id != kInvalidText) {
                                    for (const ParserTypeBindingMetadata&
                                        ...
                                        if (prelim_binding_matches_type(
                                                binding, type,
                                                type_text_id)) {
                                            return binding.type;
                                        }
                            ...
                            if (type.tpl_struct_origin_key.base_text_id !=
                                ...
                                const TextId saved_member_text_id =
                                    type.deferred_member_type_text_id;
                                const Node* origin_primary =
                                    find_template_struct_primary(
                                        type.tpl_struct_origin_key);
                                if (origin_primary) {
                                    std::vector<ParsedTemplateArg> origin_args;
                                    bool can_instantiate_origin = true;
                                    for (int ai = 0;
                                         ai < type.tpl_struct_args.size; ++ai) {
                                        const TemplateArgRef& src =
                                            type.tpl_struct_args.data[ai];
                                        ...
                                        if (src.kind ==
                                            ...
                                            arg.is_value = false;
                                            arg.type =
                                                prelim_type_for_type(src.type);
                                            if (arg.type.base == TB_TYPEDEF &&
                                                !arg.type.record_def &&
                                                (arg.type.tag_text_id !=
                                                     kInvalidText ||
                                                 arg.type
                                                         .template_param_text_id !=
                                                     kInvalidText)) {
                                        ...
                                        origin_args.push_back(arg);
                                    }
                                    if (can_instantiate_origin) {
                                        std::string origin_name =
                                            type.tpl_struct_origin &&
                                                    type.tpl_struct_origin[0]
                                                ? type.tpl_struct_origin
                                            : origin_primary
                                                      ->template_origin_name &&
                                                    origin_primary
                                                ...
                                                ? origin_primary
                                                      ->template_origin_name
                                            : origin_primary->name &&
                                                    origin_primary->name[0]
                                                ? origin_primary->name
                                                : "";
                                        std::string origin_mangled;
                                        TypeSpec origin_resolved{};
                                        if (!origin_name.empty() &&
                                            ensure_template_struct_instantiated_from_args(
                                                origin_name, origin_primary,
                                                origin_args,
                                                origin_primary->line,
                                                &origin_mangled,
                                                "prelim_alias_member_type_arg",
                                                &origin_resolved)) {
                            ...
                            const std::string deferred_member_name =
                                deferred_member_lookup_name(type);
                            if (!deferred_member_name.empty()) {
                                TypeSpec resolved_member{};
                                if (lookup_struct_member_typedef_recursive_for_type(
                                        type, deferred_member_name,
                                        type.deferred_member_type_text_id,
                                        &resolved_member)) {
                                    resolved_member
                                        .deferred_member_type_name = nullptr;
                                    resolved_member
                        ...
                        std::function<bool(const Node*, long long*)>
                            ...
                            auto push_template_eval_bindings =
                                [&](const Node* owner,
                                    const std::vector<ParsedTemplateArg>& args)
                                -> std::pair<size_t, size_t> {
                                const size_t old_tb = prelim_tb.size();
                                const size_t old_nb = prelim_nb.size();
                                if (!owner || !owner->template_param_names)
                                    return {old_tb, old_nb};
                                for (int pi = 0;
                                     pi < owner->n_template_params &&
                                     pi < static_cast<int>(args.size());
                                     ++pi) {
                                    const char* pn =
                                        owner->template_param_names[pi];
                                    if (!pn) continue;
                                    if (owner->template_param_is_nttp &&
                                        owner->template_param_is_nttp[pi]) {
                                        if (!args[pi].is_value) continue;
                                        prelim_nb.push_back({pn, args[pi].value});
                            ...
                            auto pop_template_eval_bindings =
                                [&](std::pair<size_t, size_t> sizes) {
                                prelim_tb.resize(sizes.first);
                                prelim_tb_meta.resize(sizes.first);
                                prelim_nb.resize(sizes.second);
                                prelim_nb_meta.resize(sizes.second);
                            };
                            auto parsed_args_from_template_ref =
                                [&](const Node* ref,
                                    std::vector<ParsedTemplateArg>* out_args)
                                -> bool {
                                if (!ref || !out_args ||
                                    ref->n_template_args <= 0 ||
                                    !ref->template_arg_is_value) {
                                    return false;
                                }
                                out_args->clear();
                                out_args->reserve(ref->n_template_args);
                                for (int ai = 0; ai < ref->n_template_args;
                                     ++ai) {
                                    ParsedTemplateArg arg{};
                                    arg.is_value =
                                        ref->template_arg_is_value[ai];
                                    if (arg.is_value) {
                                        arg.value =
                                            ref->template_arg_values
                                                ? ref->template_arg_values[ai]
                                                : 0;
                                        arg.nttp_text_id =
                                            ref->template_arg_nttp_text_ids
                                                ? ref
                                                      ->template_arg_nttp_text_ids
                                                          [ai]
                                                : kInvalidText;
                                        arg.expr =
                                            ref->template_arg_exprs
                                                ? ref->template_arg_exprs[ai]
                                                : nullptr;
                                        if (arg.expr) {
                                            long long ev = 0;
                                        ...
                                        if (!ref->template_arg_types)
                                            return false;
                                        arg.type = prelim_type_for_type(
                                            ref->template_arg_types[ai]);
                                    }
                                    out_args->push_back(arg);
                                }
                                return true;
                            };
                            std::function<bool(const Node*, const std::string&,
                                               long long*)>
                                ...
                                std::vector<ParsedTemplateArg> cur_args;
                                if (cur->n_template_args > 0 &&
                                    cur->template_arg_is_value) {
                                    for (int ai = 0; ai < cur->n_template_args;
                                         ++ai) {
                                        ParsedTemplateArg arg{};
                                        arg.is_value =
                                            cur->template_arg_is_value[ai];
                                        if (arg.is_value) {
                                            arg.value =
                                                cur->template_arg_values
                                                    ? cur
                                                          ->template_arg_values
                                                              [ai]
                                                    : 0;
                                        } else if (cur->template_arg_types) {
                                            arg.type =
                                                cur->template_arg_types[ai];
                                        }
                                        cur_args.push_back(arg);
                                    }
                                }
                                const auto saved_sizes =
                                    push_template_eval_bindings(cur, cur_args);
                                for (int fi = 0; fi < cur->n_fields; ++fi) {
                                    ...
                                    if (f->init &&
                                        eval_prelim_structured_nttp_expr(
                                            f->init, value)) {
                                        pop_template_eval_bindings(saved_sizes);
                                        return true;
                                    }
                                    if (f->ival >= 0) {
                                        *value = f->ival;
                                ...
                                for (int ci = 0; ci < cur->n_children; ++ci) {
                                    ...
                                    if (!child || child->kind != NK_DECL ||
                                        !child->is_static || !child->name ||
                            ...
                            auto eval_template_struct_static_member =
                                [&](const Node* ref, long long* value) -> bool {
                                if (!ref || !value || !ref->has_template_args ||
                                    ref->n_template_args <= 0 ||
                                ...
                                const int owner_context =
                                    ref->namespace_context_id >= 0
                                        ? ref->namespace_context_id
                                        : current_namespace_context_id();
                                const Node* owner_primary =
                                    find_template_struct_primary(
                                        owner_context, owner_text_id);
                                if (!owner_primary) return false;
                                std::vector<ParsedTemplateArg> owner_args;
                                if (!parsed_args_from_template_ref(
                                        ref, &owner_args)) {
                                    return false;
                                }
                                const char* owner_name =
                                    owner_primary->template_origin_name &&
                                        ...
                                        ? owner_primary->template_origin_name
                                    : owner_primary->name &&
                                            owner_primary->name[0]
                                        ? owner_primary->name
                                    : owner_primary->unqualified_name;
                                if (!owner_name || !owner_name[0]) {
                                    return false;
                                }
                                std::string owner_mangled;
                                TypeSpec owner_ts{};
                                bool ok_static = ensure_template_struct_instantiated_from_args(
                                        owner_name, owner_primary, owner_args,
                                        owner_primary->line, &owner_mangled,
                                        "structured_static_member_nttp",
                                        &owner_ts);
                                if (!ok_static) {
                                    return false;
                                }
                                ...
                                return !member_name.empty() &&
                                       lookup_static_member_recursive(
                                           owner_def, member_name, value);
                            };
                            auto eval_template_global_ref =
                                [&](const Node* ref, long long* value) -> bool {
                                if (!ref || !value || !ref->has_template_args ||
                                    ref->n_template_args <= 0 ||
                                    ref->unqualified_text_id == kInvalidText) {
                                ...
                                const Node* primary =
                                    find_template_global_primary(
                                        context, ref->unqualified_text_id);
                                if (!primary || !primary->init) return false;
                                std::vector<ParsedTemplateArg> global_args;
                                if (!parsed_args_from_template_ref(
                                        ref, &global_args)) {
                                    return false;
                                }
                                const auto saved_sizes =
                                    push_template_eval_bindings(primary,
                                                                global_args);
                                const bool ok =
                                    eval_prelim_structured_nttp_expr(
                                        primary->init, value);
                            ...
                            if (node->kind == NK_VAR) {
                                if (eval_template_struct_static_member(node,
                                                                       out)) {
                                    return true;
                                }
                                if (eval_template_global_ref(node, out)) {
                                    return true;
                                }
                        ...
                        const size_t explicit_eval_tb_size =
                            prelim_tb.size();
                        const size_t explicit_eval_nb_size =
                            prelim_nb.size();
                        for (int pi = 0; pi < static_cast<int>(actual_args.size()) &&
                            ...
                            if (primary_tpl->template_param_is_nttp[pi]) {
                                if (actual_args[pi].is_value) {
                                    const size_t saved_tb_size =
                                        prelim_tb.size();
                                    const size_t saved_nb_size =
                                        prelim_nb.size();
                                    prelim_tb.resize(explicit_eval_tb_size);
                                    prelim_tb_meta.resize(
                                        explicit_eval_tb_size);
                                    ...
                                    prelim_nb.resize(saved_nb_size);
                                    prelim_nb_meta.resize(saved_nb_size);
                    ...
                    if (!instantiated_record &&
                            ...
                            for (int bi = 0; bi < inst->n_bases; ++bi) {
                                ...
                                if (inst->base_types[bi].tpl_struct_origin) {
                                    ...
                                    auto type_is_unsigned_integer =
                                        [&](TypeSpec type) -> bool {
                                        type = substitute_bound_type_arg(type);
                                        switch (type.base) {
                                            case TB_UCHAR:
                                            case TB_USHORT:
                                            case TB_UINT:
                                    ...
                                    auto functional_cast_type =
                                        ...
                                        candidate.inner_rank = -1;
                                        *out_type =
                                            substitute_bound_type_arg(candidate);
                                        if (out_type->base != TB_TYPEDEF ||
                                            out_type->record_def) {
                                            return true;
                                        }
                                    ...
                                    auto push_eval_bindings =
                                        ...
                                        for (int pi = 0;
                                            ...
                                            if (owner->template_param_is_nttp &&
                                                ...
                                                type_binding_meta.push_back(
                                                    meta);
                                            }
                                        }
                                        return {old_tb, old_nb};
                                    };
                                    auto pop_eval_bindings =
                                        [&](std::pair<size_t, size_t> sizes) {
                                        type_bindings.resize(sizes.first);
                                        type_binding_meta.resize(sizes.first);
                                        nttp_bindings.resize(sizes.second);
                                        nttp_binding_meta.resize(sizes.second);
                                    };
                                    auto parsed_args_from_ref =
                                        [&](const Node* ref,
                                            std::vector<ParsedTemplateArg>*
                                                out_args) -> bool {
                                        if (!ref || !out_args ||
                                    ...
                                    std::function<bool(const Node*,
                                        ...
                                        if (cur->n_template_args > 0 &&
                                            cur->template_arg_is_value) {
                                            for (int ai = 0;
                                                 ai < cur->n_template_args;
                                                 ++ai) {
                                                cur_args.push_back(
                                                    parsed_template_arg_from_node_slot(
                                                        cur, ai));
                                            }
                                        }
                                        const auto saved =
                                            push_eval_bindings(cur, cur_args);
                                        for (int fi = 0; fi < cur->n_fields;
                                             ++fi) {
                                            const Node* f = cur->fields[fi];
                                            if (!f || f->kind != NK_DECL ||
                                                !f->is_static || !f->name ||
                                    ...
                                    auto eval_template_struct_static_member =
                                        ...
                                        TypeSpec owner_ts{};
                                        if (!ensure_template_struct_instantiated_from_args(
                                                owner_name, owner_primary,
                                                owner_args,
                                                owner_primary->line,
                                                &owner_mangled,
                                                "bound_static_member_nttp",
                                                &owner_ts)) {
                                            return false;
                                        }
                                        const Node* owner_def =
                                    ...
                                    eval_structured_nttp_expr =
                                        ...
                                        if (node->kind == NK_VAR) {
                                            if (eval_template_struct_static_member(
                                                    node, out)) {
                                                return true;
                                            }
                                            if (eval_template_global_ref(node,
                                                                         out)) {
                                                return true;
                                            }
                        ...
                        // Store concrete primary-family arguments so member
                        // typedef lookup keys stay aligned for partial
                        // specializations such as Template<T, false>::type.
                        {
                            int n = static_cast<int>(concrete_args.size());
                            if (primary_tpl &&
                                primary_tpl->n_template_params > n) {
                                n = primary_tpl->n_template_params;
                            } else if (!primary_tpl &&
                                       tpl_def->n_template_params > n) {
                                n = tpl_def->n_template_params;
                            }
                            inst->n_template_params = n;
                            inst->template_param_names =
                                primary_tpl && primary_tpl->template_param_names
                                    ? primary_tpl->template_param_names
                            ...
                            inst->template_param_is_nttp =
                                primary_tpl && primary_tpl->template_param_is_nttp
                            ...
                            for (int pi = 0; pi < n; ++pi) {
                                const bool param_is_nttp =
                                    inst->template_param_is_nttp &&
                                    pi < inst->n_template_params &&
                                    inst->template_param_is_nttp[pi];
                                const bool have_concrete_arg =
                                    pi < static_cast<int>(concrete_args.size());
                                if (param_is_nttp) {
                                    inst->template_arg_is_value[pi] = true;
                                    inst->template_arg_values[pi] =
                                        have_concrete_arg &&
                                                concrete_args[pi].is_value
                                            ? concrete_args[pi].value
                                            : 0;
                                    inst->template_arg_nttp_names[pi] = nullptr;
                                    inst->template_arg_nttp_text_ids[pi] =
                                        have_concrete_arg &&
                                                concrete_args[pi].is_value
                                            ? concrete_args[pi].nttp_text_id
                                            : kInvalidText;
                ...
                if (is_cpp_mode() && check(TokenKind::ColonColon) &&
                    ...
                    if (lookup_struct_member_typedef_recursive_for_type(
                        ...
                        if (resolved.base == TB_STRUCT) {
                            ...
                            if (inst_def && !resolved.record_def) {
                                const TextId resolved_text_id =
                                    resolved.tag_text_id != kInvalidText
                                        ? resolved.tag_text_id
                                        : resolved.template_param_text_id;
                                ...
                                for (int fi = 0; fi < inst_def->n_fields; ++fi) {
                                    ...
                                    TextId field_text_id =
                                        field->unqualified_text_id != kInvalidText
                                            ? field->unqualified_text_id
                                            : field->type.tag_text_id;
                                    if (field_text_id == kInvalidText &&
                                        field->name) {
                                        field_text_id =
                                            parser_text_id_for_token(
                                                kInvalidText, field->name);
                                    }
                                    if (resolved_text_id == kInvalidText) {
                                        sole_nested_record = field;
                                        ++nested_record_count;
                                    ...
                                    if (field_text_id == resolved_text_id) {
                                        resolved.record_def =
                                            field->type.record_def;
                                        resolved.tag_text_id =
                                            field->type.tag_text_id;
