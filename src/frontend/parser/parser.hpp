...
class Parser {
 public:
  ...
  void register_dependent_record_member_typedef_binding(
      ...
      const TypeSpec& type);
  void register_record_member_typedef_info(
  ...
  const TypeSpec* find_dependent_record_member_typedef_type(
      const QualifiedNameKey& key) const;
  const TypeSpec* find_dependent_record_member_typedef_type(
      const QualifiedNameKey& owner_key, TextId member_text_id) const;
  const ParserAliasTemplateMemberTypedefInfo*
  find_record_member_typedef_info(const QualifiedNameKey& key) const;
  ...
  const ParserAliasTemplateInfo* find_alias_template_info_in_context(
      int context_id, TextId name_text_id) const;
  QualifiedNameKey qualified_name_key(const QualifiedNameRef& name) const;
  ...
  bool is_template_scope_type_param(TextId name_text_id) const;
  ...
  bool parse_dependent_typename_specifier(std::string* out_name = nullptr,
                                          TypeSpec* out_type = nullptr,
                                          bool* out_has_type = nullptr);
