...
match_minimal_structured_direct_call_folded_two_arg_helper(
  ...
  const auto* add = std::get_if<c4c::backend::BackendBinaryInst>(&block.insts[0]);
  const auto* sub = std::get_if<c4c::backend::BackendBinaryInst>(&block.insts[1]);
  if (add == nullptr || sub == nullptr ||
      add->opcode != c4c::backend::BackendBinaryOpcode::Add ||
      sub->opcode != c4c::backend::BackendBinaryOpcode::Sub ||
      !is_i32_scalar_binary(*add) || !is_i32_scalar_binary(*sub) ||
      add->result.empty() || sub->result.empty() ||
      add->lhs != "2" || add->rhs != callee_fn.signature.params[0].name ||
      sub->lhs != add->result || sub->rhs != callee_fn.signature.params[1].name ||
      *block.terminator.value != sub->result) {
    return std::nullopt;
  }

  return MatchedMinimalStructuredDirectCallFoldedTwoArgHelper{
      &callee_fn, 2 + arg0_imm - arg1_imm};
}
...
std::optional<MinimalDirectCallAddImmSlice> parse_minimal_direct_call_add_imm_slice(
  ...
  const auto* callee_fn = matched->callee_fn;
...
std::optional<MinimalDirectCallSlice> parse_minimal_direct_call_two_arg_folded_slice(
  ...
  const auto helper_match = match_minimal_structured_direct_call_folded_two_arg_helper(
      *matched->callee_fn, *arg0_imm, *arg1_imm);
...
parse_minimal_call_crossing_direct_call_slice(
  ...
  if (source_add == nullptr || call == nullptr || final_add == nullptr || !matched.has_value() ||
  ...
  const auto* helper = matched->callee_fn;
  if (helper->signature.params.size() != 1 ||
