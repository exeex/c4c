  ...
  switch (authority.use_kind) {
    ...
    case PreparedValueFreshnessUseKind::PointerValueMemoryUse:
      return authority.source_kind ==
             ...
             authority.rank == PreparedValueFreshnessSourceRank::PointerValueMemory;
...
[[nodiscard]] bool prepared_value_freshness_candidate_matches_query(
  ...
  if (query.use_kind == PreparedValueFreshnessUseKind::PointerValueMemoryUse &&
      (!query.block_label.has_value() || !query.instruction_index.has_value() ||
       authority.reference.block_label != query.block_label ||
       authority.reference.instruction_index != query.instruction_index)) {
    return false;
...
void attribute_block_entry_publication_proof_if_agreeing(
  ...
  bir::Function compatibility_function;
  compatibility_function.blocks.push_back(*proof_successor_block);
...
find_prepared_current_block_entry_publication(
  ...
  for (const auto& publication :
    ...
    if (prepared_block_entry_publication_available(publication)) {
      ...
      attribute_block_entry_publication_proof_if_agreeing(query, result);
      return result;
    }
  }
