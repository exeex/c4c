...
std::optional<std::size_t> find_prepared_bounded_multi_defined_named_frame_offset_if_supported(
...
void note_prepared_bounded_multi_defined_name_once(std::vector<std::string>* names,
...
bool append_prepared_bounded_multi_defined_call_argument_if_supported(
  ...
  if (arg_type == c4c::backend::bir::TypeKind::Ptr) {
    ...
    if (arg.name.front() == '@') {
      ...
      note_prepared_bounded_multi_defined_name_once(used_same_module_global_names, symbol_name);
      *body += "    lea ";
      *body += kArgRegs64[arg_index];
      *body += ", [rip + ";
    ...
    const auto frame_offset = find_prepared_bounded_multi_defined_named_frame_offset_if_supported(
        local_layout, function_addressing, module, function_locations, function_name, arg.name);
