...
| Scalar integer operations and casts | Later `codegen/` after target MIR exists | BIR operation/type facts are present in retained module; no AArch64 machine instruction node/lowering owner yet. | Missing structured machine instruction node; do not use legacy `codegen/*.md` text patterns as records. | Target MIR instruction record design, then scalar instruction-selection idea. |
| Floating-point scalar operations | Later `codegen/` after target MIR exists | BIR operation/type facts are present for supported types; no AArch64 FP machine instruction node/lowering owner yet. | Missing structured machine instruction node. | Target MIR instruction record design, then FP lowering idea. |
...
| Move, copy, spill, reload, and ABI-binding records sourced from prepared value/regalloc/storage plans, allocation-result facts, and parallel copies. | Prepared move/storage/regalloc facts exist and `ALLOCATION_CONTRACT.md` defines the target allocation result shape; no AArch64 move/spill/reload MIR records exist. | missing | AArch64 MIR design | Target MIR needs explicit records before instruction selection. Spill-slot ids and reserved scratch policy must come from the allocation contract, not local lowering slices. |
...
structured machine instruction nodes for scalar and floating-point lowering,
...
selection, structured machine instruction nodes, or assembly text emission.
