...
| Memory and addressing | `module/module.hpp` for current snapshots; `codegen/records.*` for record-only memory operands; shared preparation owns typed memory facts | Prepared addressing exists for base/offset/size/alignment, with `PreparedMemoryAccess::address_space` and `PreparedMemoryAccess::is_volatile` carrying the shared address-space and volatility facts. | `OperandRecord` address fields plus `codegen::MemoryOperand` / `MemoryInstructionRecord` for the current record layer. | Memory instruction-selection and addressing-legality design; preserve the typed prepared fields instead of recovering facts from text. |
...
| Scalar integer operations and casts | `codegen/records.*` over target MIR | BIR operation/type facts are present in retained module and current record-only scalar ALU/cast records cover the accepted subset. | `ScalarInstructionRecord`, `ScalarAluRecord`, `ScalarCastRecord` for the current subset; broader machine-node and final opcode selection remain later. | Scalar instruction-selection idea over structured records; do not use legacy `codegen/*.md` text patterns as records. |
...
| Memory and addressing | BIR load/store/global/string/pointer operations and BIR types. | `PreparedAddressing`, `PreparedAddressingFunction`, `PreparedMemoryAccess`, `PreparedAddress`; `PreparedMemoryAccess::address_space` and `PreparedMemoryAccess::is_volatile` carry the typed shared address-space and volatility facts. | Address fields on `OperandRecord`; `codegen::MemoryOperand` / `MemoryInstructionRecord` for the current record layer. | `module/module.hpp` snapshots, `codegen/records.*`; shared preparation owns typed carrier facts. | present for base/offset/size/alignment, volatility, and address space; no target workaround from printed BIR. |
...
| Scalar integer operations and casts | Retained BIR instructions, operands, opcodes, and `TypeKind`. | `PreparedBirModule::module`; prepared value/storage/location facts. | missing structured machine instruction node. | Later `codegen/` after target MIR records. | missing target record; legacy `codegen/*.md` text patterns are not accepted records. |
| Floating-point scalar operations | Retained BIR FP instructions and `TypeKind`. | `PreparedBirModule::module`; prepared value/storage/location facts. | missing structured machine instruction node. | Later `codegen/` after target MIR records. | missing target record; lowering deferred until target instruction model exists. |
...
| AAPCS64-specific call/register semantics are complete enough for every call shape. | Prepared calls carry generic banks/register strings and storage facts, and AArch64 now records call, move, ABI-binding, and preservation snapshots; no final AAPCS64 call-lowering contract validates all cases yet. | ambiguous | target ABI work | A later target ABI/call-frame idea should consume these records before instruction selection. |
...
| Typed virtual or prepared-value operands retaining `PreparedValueId`, `ValueNameId`, and `TypeKind`. | `module::OperandRecord` preserves prepared value identity, value names, type facts, allocation locations, and storage-plan snapshots. `codegen::OperandRecord`/`MemoryOperand` cover later machine-node input records. | present | AArch64 module and codegen records | Do not use rendered value names or old emitter register strings as semantic operands. Missing prepared carriers should become separate open-idea candidates, not local workarounds. |
...
| Frame, stack-slot, dynamic-stack, and callee-save MIR records sourced from prepared plans. | `FrameRecord`, `FrameSlotRecord`, `DynamicStackRecord`, and `CalleeSaveRecord` preserve prepared frame and slot snapshots. | present | AArch64 module records plus later ABI frame policy | Prologue/epilogue work may consume these snapshots and `ALLOCATION_CONTRACT.md`; it must not recompute frame placement or invent spill slots locally. |
| Memory operands preserving base kind, frame-slot id, symbol `LinkNameId`, pointer value id/name, string identity, offset, size, alignment, volatility, and address space. | Prepared memory facts exist, including `PreparedMemoryAccess::address_space` and `PreparedMemoryAccess::is_volatile`; `codegen::MemoryOperand` preserves these facts for the current record-only memory layer. | present for record layer | AArch64 codegen records | Load/store instruction selection and addressing legality are later work; consumers must preserve the typed prepared address-space and volatility facts through those records. |
| Branch and compare records sourced from prepared branch conditions and `BlockLabelId`. | `module::BranchRecord` preserves prepared branch facts and `codegen::ScalarInstructionRecord` carries current record-only branch/compare candidates where supported. | present for record layer | AArch64 module/codegen records | Final compare/branch opcode selection remains later work over structured records. |
| Call records sourced from `call_plans`. | `CallRecord`, `CallArgumentRecord`, `CallResultRecord`, `CallPreservedValueRecord`, and `AbiBindingRecord` preserve prepared call and ABI-binding snapshots. | present for snapshots; AAPCS64 completeness remains ambiguous | target ABI work over module records | AArch64 ABI design must consume these records and `ALLOCATION_CONTRACT.md`; it must not synthesize calling convention, call-clobber resources, or call-time spills locally. |
| Move, copy, spill, reload, and ABI-binding records sourced from prepared value/regalloc/storage plans, allocation-result facts, and parallel copies. | `MoveRecord`, `AbiBindingRecord`, `SpillReloadRecord`, and `ParallelCopyRecord` are live target-MIR snapshots with prepared provenance, structured slot ids, target register records, scratch authority, and matched move/copy provenance where available. | present | AArch64 module records plus `ALLOCATION_CONTRACT.md` | Instruction selection can now consume explicit move/copy/spill/reload records. Spill-slot ids, destination homes, and reserved scratch policy must come from these records and the allocation contract, not local lowering slices. |
...
spill/reload, ABI-binding, frame, call snapshot, and record-only scalar/memory
surfaces now exist as structured records. The remaining non-present or
ambiguous items are later consumer contracts already named here:
AAPCS64 call/return/variadic completeness, final memory addressing legality
and load/store selection, structured machine instruction nodes beyond the
current record-only scalar/memory subset, f128/i128, atomics, intrinsics,
...
Proceed is allowed only through the structured target-record route. Later
scalar, memory, branch, call, return, prologue, vector, inline-asm, printer,
encoder, and object slices must consume `module::Module` records,
`codegen::records` machine-node inputs where present, and the allocation-result
authority in `ALLOCATION_CONTRACT.md`. They must not recover facts from
rendered names, printed BIR, legacy LIR strings, assembly text, parser operand
recovery, or stale markdown examples.
