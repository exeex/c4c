...
call/return/frame, and record-only scalar/memory surfaces now exist as
structured records or accepted contracts. The remaining non-present or
deferred items are later consumer contracts already named here: dedicated
return-boundary records, direct variadic-count exposure, outgoing call-area
carriers, explicit special-register role records, full variadic function-entry
carriers, final memory addressing legality and load/store selection,
structured machine instruction nodes beyond the current record-only
scalar/memory subset, f128/i128, atomics, intrinsics, inline asm,
assembler/encoder, object/binary-utils, and linker behavior.
...
scalar, memory, branch, call, return, prologue, epilogue, memory-return,
indirect-call, variadic, vector, inline-asm, printer, encoder, and object
slices must consume `module::Module` records, `codegen::records` machine-node
inputs where present, the allocation-result authority in
