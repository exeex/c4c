...
- Treating printed `.s` output as the bridge from codegen to the built-in
  encoder or object writer, or as the only reusable codegen product.
...
| `codegen/emit.md` | obsolete route | Must not define the new BIR / prepared backend contract; use only as a warning about rejected direct text emission. | Owner: legacy public entry shim, superseded in live code by public `codegen.hpp` `compile_prepared_module(...)` plus internal `module_compile.{hpp,cpp}` coordination. Limitation: mixed BIR/LIR recognition with assembly output. Removal condition: Step 4 contract rejects rendered-name and direct text fallback routes. |
...
| `codegen/asm_emitter.md` | obsolete route | Must not influence the new backend contract; direct assembly rendering that bypasses compiled target MIR is excluded. | Owner: legacy assembly text emitter. Limitation: text output route bypasses structured backend state. Live `asm_emitter.{hpp,cpp}` is a distinct current helper that consumes the compiled module through the shared MIR printer. Removal condition: target-local printer or assembler layer is explicitly designed. |
