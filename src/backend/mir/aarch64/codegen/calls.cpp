...
[[nodiscard]] std::optional<bir::Value> prepared_call_boundary_source_value(
  ...
  const auto consumption =
      prepare::find_prepared_current_block_publication_consumption(
          ...
          value_name,
          before_instruction_index);
  return consumption.available && consumption.produced_value != nullptr
             ? std::optional<bir::Value>{*consumption.produced_value}
             : std::nullopt;
}
...
[[nodiscard]] const bir::Value* indexed_indirect_callee_source_producer_result(
...
struct IndirectCalleeSourceProducerIdentity {
...
[[nodiscard]] bool emit_indirect_callee_value_to_register_with_csel(
  ...
  const auto* select =
      producer.has_value() &&
          ...
          ? std::get_if<bir::SelectInst>(producer->instruction)
          : nullptr;
  if (select == nullptr) {
...
materialize_indirect_call_callee_to_prepared_register(
  ...
  const auto* callee_source = callee_producer.has_value()
                                  ? callee_producer->produced_value
                                  : nullptr;
