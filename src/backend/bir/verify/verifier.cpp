  ...
  for (std::size_t slot_index = 0; slot_index < module.functions_.slots_.size();
    ...
    for (std::size_t inst_slot = 0; inst_slot < function.insts_.slots_.size();
      ...
      if (const auto* binary = std::get_if<BinaryNode>(&instruction.payload)) {
        ...
        const bool direct_scalar_floating_fadd =
            binary->opcode == BinaryOpcode::FAdd && floating_type(binary->type) &&
            binary->direct_scalar_lhs.has_value();
        const bool direct_scalar_floating_fsub =
            binary->opcode == BinaryOpcode::FSub && floating_type(binary->type) &&
            binary->direct_scalar_lhs.has_value();
        const bool direct_scalar_rhs_floating_fmul =
            binary->opcode == BinaryOpcode::FMul && floating_type(binary->type) &&
            binary->direct_scalar_rhs.has_value();
        const bool direct_scalar_rhs_floating_fadd =
            binary->opcode == BinaryOpcode::FAdd && floating_type(binary->type) &&
            binary->direct_scalar_rhs.has_value();
        const bool direct_scalar_rhs_floating_fsub =
            binary->opcode == BinaryOpcode::FSub && floating_type(binary->type) &&
            binary->direct_scalar_rhs.has_value();
        ...
        bool exact = (fadd || fmul || float_fmul || direct_scalar_floating_fadd ||
                      direct_scalar_floating_fsub ||
                      direct_scalar_floating_fmul ||
                      direct_scalar_rhs_floating_fadd ||
                      direct_scalar_rhs_floating_fsub ||
                      direct_scalar_rhs_floating_fmul || add || sext_add || mul)
        ...
        const bool direct_scalar_fmul = exact && direct_scalar_floating_fmul &&
            ...
            direct_scalar->parameter_index < function.parameters_.size() &&
            instruction.operands[0] == function.parameters_[direct_scalar->parameter_index];
        const bool direct_scalar_fadd = exact && direct_scalar_floating_fadd &&
            direct_scalar && direct_scalar->source_value_id != 0 &&
            direct_scalar->scalar_type == binary->type &&
            direct_scalar->owner.valid() && direct_scalar->owner.epoch == module.epoch_ &&
            direct_scalar->owner.slot < module.link_names_.size() &&
            ...
            direct_scalar->parameter_index < function.parameters_.size() &&
            instruction.operands[0] == function.parameters_[direct_scalar->parameter_index];
        const bool direct_scalar_fsub = exact && direct_scalar_floating_fsub &&
            direct_scalar && direct_scalar->source_value_id != 0 &&
            direct_scalar->scalar_type == binary->type &&
            direct_scalar->owner.valid() && direct_scalar->owner.epoch == module.epoch_ &&
            direct_scalar->owner.slot < module.link_names_.size() &&
        ...
        const bool direct_scalar_rhs_fmul = exact && direct_scalar_rhs_floating_fmul &&
            direct_scalar_rhs && direct_scalar_rhs->source_value_id != 0 &&
            direct_scalar_rhs->scalar_type == binary->type &&
            direct_scalar_rhs->owner.valid() &&
            direct_scalar_rhs->owner.epoch == module.epoch_ &&
            direct_scalar_rhs->owner.slot < module.link_names_.size() &&
            module.link_names_[direct_scalar_rhs->owner.slot].spelling == function.link_name_ &&
            direct_scalar_rhs->parameter_index < function.parameters_.size() &&
            instruction.operands[1] == function.parameters_[direct_scalar_rhs->parameter_index];
        const bool direct_scalar_rhs_fadd = exact && direct_scalar_rhs_floating_fadd &&
            direct_scalar_rhs && direct_scalar_rhs->source_value_id != 0 &&
            direct_scalar_rhs->scalar_type == binary->type &&
            direct_scalar_rhs->owner.valid() &&
            direct_scalar_rhs->owner.epoch == module.epoch_ &&
            ...
            direct_scalar_rhs->parameter_index < function.parameters_.size() &&
            instruction.operands[1] == function.parameters_[direct_scalar_rhs->parameter_index];
        const bool direct_scalar_rhs_fsub = exact && direct_scalar_rhs_floating_fsub &&
            direct_scalar_rhs && direct_scalar_rhs->source_value_id != 0 &&
            direct_scalar_rhs->scalar_type == binary->type &&
            direct_scalar_rhs->owner.valid() &&
            direct_scalar_rhs->owner.epoch == module.epoch_ &&
        ...
        if (exact && !direct_scalar_add && !direct_scalar_fneg &&
            ...
            !direct_scalar_fsub &&
            !direct_scalar_fmul &&
            !direct_scalar_rhs_add && !direct_scalar_rhs_fadd &&
