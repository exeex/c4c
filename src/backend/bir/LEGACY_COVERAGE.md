...
| `(?:lir_to_bir\.(?:cpp\|hpp)\|lir_adapter_error\.hpp)` | Accepted typed import and error transport; reject text recovery and partial publication | Producer/owner: indexed `lir_to_bir/README.md`; every current-LIR fact follows its exact accepted phase-A receiving disposition, and legacy evidence cannot reopen LIR or create/repair semantic facts. |
