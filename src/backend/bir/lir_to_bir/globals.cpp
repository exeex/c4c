...
BackendAggregateLayoutLookup lookup_global_layout_result(
  ...
  // Metadata-bearing globals use lookup_structured_global_layout_result() and
  // fail closed on StructNameId, parity, or spelling mismatches before reaching
  // this rendered-text fallback.
...
bool requires_structured_global_type_ref(const c4c::codegen::lir::LirGlobal& global) {
  ...
  const auto trimmed_type = c4c::codegen::lir::trim_lir_arg_text(global.llvm_type);
  return !trimmed_type.empty() && trimmed_type.front() == '%';
}
    ...
    for (auto& [byte_offset, address] : info.pointer_initializer_offsets) {
      if (address.value_type != bir::TypeKind::Void) {
        ...
        if (target_it != global_types.end() &&
            ...
            target_it->second.link_name_id != kInvalidLinkName &&
...
std::optional<bir::Global> lower_minimal_global_impl(
  ...
  const bool use_structured_global_type_ref =
      requires_structured_global_type_ref(global) && structured_layouts != nullptr;
  const auto layout_lookup =
      use_structured_global_type_ref
...
std::optional<bir::Global> lower_string_constant_global(
  ...
  // compatibility name. Ordinary string literal operands remain name-only;
  // aggregate pointer-initializer lowering interns relocation-slot targets
  // separately when prepared object data needs stable identity.
