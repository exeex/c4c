...
| `bir.hpp: AtomicOperation` and atomic enums; `src/backend/legacy/prealloc/atomics.cpp` | atomic kind/order/result and later target carriers | ordinary atomic instructions with complete operands/results/orderings | preparation supplies target facts, D4 owns any one-to-many target expansion, E2 owns abstract homes, and F1 performs only concrete one-to-one mapping |
...
| `src/backend/legacy/prealloc/call_plans.cpp`; `regalloc/call_return_abi.cpp` | ABI classification, source recovery, call placement | core supplies typed call operands, exact producer def-use, symbols and object facts | C3/C4 own ABI/call plans, D2 owns explicit abstract call transport, E2 owns abstract homes, E4 publishes the MIR-ready revision, and F1 only maps it concretely |
| `bir.hpp: InlineAsmMetadata`, `InlineAsmOperandMetadata`; `prealloc/inline_asm.cpp` | asm template, constraints, ties, clobbers, memory/address intent and realization | ordinary `InlineAsm` operand/result edges plus original opaque asm/constraint text, ordered clobbers, side effects, and `AsmGotoTerm` CFG | C7 owns target vocabulary, C9 owns parsed constraint binding, E2 owns abstract homes, F1 maps one-to-one, and F3 alone parses/encodes the opaque text |
| `bir.hpp: IntrinsicOperation`; `prealloc/special_carriers.hpp: PreparedIntrinsicCarrier` | intrinsic semantic family plus target carrier | typed semantic intrinsic ID, immediates, operands/results/effects | C8/D1 own helper planning and pseudo lowering, D2 owns helper-call transport, D4 owns target expansion, E2 owns abstract homes, and F1 only maps concretely |
...
| `bir_route7_comparison.cpp`, `bir_comparison_view.cpp` | comparison producer and branch condition recovery | compare opcode/result IDs plus terminator condition use | comparison analysis is read-only; D4 owns fused/materialized target choice and F1 only maps the resulting pseudo node |
...
| `src/backend/legacy/prealloc/liveness.*`, `regalloc/*`, `stack_layout/*` | liveness, homes, allocation, slots, moves, frame | core supplies stable instructions/values/uses/CFG inputs and revisions | intervals/interference are E1 BIR products, abstract homes are E2 BIR products, abstract spill objects plus explicit `Spill`/`Reload` are E3 BIR state, D5 owns out-of-SSA and copy resolution, E4 publishes, and F1 only maps each surviving node/home concretely |
...
| `src/ir/mem2reg/phi_eliminate.rs` | explicit out-of-SSA boundary, critical-edge splitting, parallel-copy correctness | reject direct synchronization of vectors/spans/IDs; D5 uses editor APIs and owns both explicit out-of-SSA state and allocation-aware copy resolution before E4 |
...
| ABI placement, chosen homes, spill/reload, call moves, frame, opcodes and emission | `schema-owned` | excluded from Raw/Canonical core; C3/C4 prepare ABI/call facts, D2 owns explicit call transport, E1 owns allocation liveness, E2 owns abstract homes, E3 owns explicit Pseudo BIR spill/reload, D5 owns copy resolution, E4 publishes, F1 performs only concrete one-to-one mapping, and F3 emits |
