  ...
  for (const auto& block : function.blocks) {
    ...
    for (const auto& instruction : block.insts) {
      ...
      if (const auto* bin = std::get_if<LirBinOp>(&instruction)) {
        ...
        if ((!fadd && !fmul && !fpext_fmul && !sitofp_fmul && !uitofp_fmul && !fptosi_add && !fptoui_add && !wide_ffs_trunc_add && !float_fmul && !add && !ctz_direct_add && !ctz_trunc_add && !clz_direct_add && !clz_trunc_add && !mul && !sext_add && !ffs_add) ||
            ...
            for (const auto& instruction : block.insts) {
              ...
              if (const auto* bin = std::get_if<LirBinOp>(&instruction)) {
                ...
                if (lhs == source_values.end() ||
                    ...
                    (!fadd && !fmul && !fpext_fmul && !sitofp_fmul && !uitofp_fmul && !fptosi_add && !fptoui_add && !wide_ffs_trunc_add && !float_fmul && !sext_add && !abs_add && !cttz_add && !ctz_trunc_add && !clz_direct_add && !clz_trunc_add && !add && normalized_i32_add_results.count(
                        bin->lhs.value_id()->value) == 0)) {
                  edit_error = ImportError{ImportErrorCode::UnsupportedOrdinaryInstruction,
                                           name, block.label,
