...
| `D5` | Out of SSA and owned copy-resolution closure | reverified `D4` output; its subordinate closure later consumes the stable post-E3 allocation candidate | intermediate pseudo BIR with phi semantics lowered to explicit edge-local `ParallelCopy`/`EdgeCopy` plus preallocated scratch-reservation identities; after stable E3, the same owner transactionally resolves every bundle to directly realizable single-move `EdgeCopy` nodes before E4 | [out-of-SSA pass](passes/out_of_ssa/README.md) |
| `E1` | Allocation liveness and interference | exact initial D5 or E3-retry revision plus layout/constraint/call facts | revision-bound live ranges, simultaneous-copy interference, and pressure facts covering all uses, definitions, fixed homes, clobbers, and non-spillable `CopyScratch` reservations | shared allocation analysis consumed by the [BIR register allocator](regalloc/README.md) |
| `E2` | Shared pseudo-physical register allocation | exact current `E1` facts and finite target-layout pools | legal abstract `(category, class/group, slot)` homes for ordinary identities and every non-spillable scratch reservation, or an ordinary eviction request; shortage or forbidden alias fails closed | the same shared BIR allocator for RV64, AArch64, and x86 |
| `E3` | Explicit spill/reload insertion | allocation candidate, pressure/eviction decisions, and exact liveness | abstract spill-slot identities plus admitted, directly realizable pseudo `Spill`/`Reload` nodes while preserving `ParallelCopy` and scratch-reservation semantics | [spill/reload](regalloc/spill_reload/README.md); it cannot spill scratch or resolve bundles, and candidate mutation invalidates allocation facts and retries at `E1` until stable or rejected |
