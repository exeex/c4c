...
// Structural mirror of the ref Rust source; the shared RISC-V C++ `RiscvCodegen`
// / `CodegenState` surface does not exist yet, so this file keeps the memory
// helper boundaries explicit without inventing a fake public interface.
...
std::string memcpy_loop_label(std::uint64_t id) {
  return ".Lmemcpy_loop_" + std::to_string(id);
}

std::string memcpy_done_label(std::uint64_t id) {
  return ".Lmemcpy_done_" + std::to_string(id);
}
