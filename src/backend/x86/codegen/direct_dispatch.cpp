...
std::optional<std::string> try_emit_direct_prepared_lir_helper_module(
  ...
  if (const auto asm_text = try_emit_minimal_stack_aggregate_param_slot_module(module);
      asm_text.has_value()) {
    return asm_text;
  }
  if (const auto asm_text = try_emit_minimal_small_struct_stack_param_slot_module(module);
      asm_text.has_value()) {
    return asm_text;
  }
  ...
  if (const auto asm_text = try_emit_minimal_single_arg_helper_call_module(module);
      asm_text.has_value()) {
    return asm_text;
  }
  ...
  if (const auto asm_text = try_emit_minimal_folded_two_arg_direct_call_module(module);
      asm_text.has_value()) {
    return asm_text;
  }
