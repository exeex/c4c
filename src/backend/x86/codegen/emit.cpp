...
std::optional<MinimalExternGlobalArrayLoadSlice> parse_minimal_extern_global_array_load_slice(
  ...
  return MinimalExternGlobalArrayLoadSlice{function.name, global.name, *element_index * 4};
}
...
std::optional<MinimalGlobalCharPointerDiffSlice> parse_minimal_global_char_pointer_diff_slice(
  ...
  const auto* function = find_single_zero_arg_i32_lir_definition(module);
  if (function == nullptr || function->blocks.size() != 1 || !function->alloca_insts.empty() ||
      !function->stack_objects.empty()) {
    return std::nullopt;
  }
  ...
  return MinimalGlobalCharPointerDiffSlice{function->name, global.name, *global_size, *byte_index};
}
...
std::optional<MinimalStringLiteralCharSlice> parse_minimal_string_literal_char_slice(
  ...
  const auto* function = find_single_zero_arg_i32_backend_definition(module);
  if (string_const == nullptr || function == nullptr || function->blocks.size() != 1) {
    return std::nullopt;
  }
...
std::optional<MinimalDirectCallAddImmSlice> parse_minimal_direct_call_add_imm_slice(
    const c4c::codegen::lir::LirModule& module) {
  const auto parsed = c4c::backend::parse_backend_minimal_direct_call_add_imm_lir_module(module);
  if (!parsed.has_value() || parsed->helper == nullptr || parsed->main_function == nullptr) {
    return std::nullopt;
  }
...
std::string emit_minimal_direct_call_add_imm_asm(
  ...
  if (slice.callee_name.empty() || slice.caller_name.empty()) {
    throw std::invalid_argument("direct single-argument call slice without helper metadata");
  }
...
std::string emit_minimal_direct_call_add_imm_asm(
  ...
  return emit_minimal_direct_call_add_imm_asm(target_triple,
                                              MinimalDirectCallAddImmSlice{
                                                  slice.helper->name,
