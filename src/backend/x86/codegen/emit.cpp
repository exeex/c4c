...
std::optional<MinimalTwoArgLocalArgHelperCallSlice>
parse_minimal_two_arg_both_local_second_rewrite_helper_call_slice(
    const c4c::codegen::lir::LirModule& module) {
  using namespace c4c::codegen::lir;
...
std::optional<MinimalTwoArgLocalArgHelperCallSlice>
parse_minimal_two_arg_both_local_double_rewrite_helper_call_slice(
    const c4c::codegen::lir::LirModule& module) {
  using namespace c4c::codegen::lir;
  ...
  const auto try_match =
    ...
    const auto& block = entry.blocks.front();
    const auto* ret = std::get_if<LirRet>(&block.terminator);
    const auto* call = block.insts.size() == 11 ? std::get_if<LirCallOp>(&block.insts.back()) : nullptr;
    const auto call_operands =
        call == nullptr
            ? std::nullopt
            : c4c::backend::parse_backend_direct_global_two_typed_call_operands(
...
parse_minimal_global_two_field_struct_store_sub_sub_slice(
  ...
  return MinimalGlobalTwoFieldStructStoreSubSubSlice{
      ...
      .align_bytes = global.align_bytes > 0 ? global.align_bytes : 4,
  };
}
...
parse_minimal_repeated_printf_local_i32_calls_bir_slice(
  ...
  if (module.globals.size() != 0 || module.string_constants.size() != 2 ||
      module.functions.size() != 2) {
    return std::nullopt;
  }
  ...
  const auto match_printf_call =
      [](const CallInst& call,
        ...
        if (call.callee != "printf" || !call.is_variadic || call.return_type != TypeKind::I32 ||
            ...
            call.result->type != TypeKind::I32 ||
            call.args.size() != immediates.size() + 1 ||
        ...
        for (const auto immediate : immediates) {
          if (call.args[index] != bir::Value::immediate_i32(immediate)) {
            return false;
          }
          ++index;
        }
...
std::optional<std::string> try_emit_prepared_lir_module(
  ...
  if (const auto slice = parse_minimal_two_arg_both_local_second_rewrite_helper_call_slice(module);
      slice.has_value()) {
    return emit_minimal_two_arg_local_arg_helper_call_asm(module.target_triple, *slice);
  }
  if (const auto slice = parse_minimal_two_arg_both_local_double_rewrite_helper_call_slice(module);
      slice.has_value()) {
    return emit_minimal_two_arg_local_arg_helper_call_asm(module.target_triple, *slice);
  }
