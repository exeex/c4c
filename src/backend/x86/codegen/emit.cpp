...
std::optional<MinimalTwoArgDirectCallSlice> parse_minimal_two_arg_direct_call_slice(
  ...
  if (helper == nullptr || main_fn == nullptr || helper->is_declaration ||
      main_fn->is_declaration || !c4c::backend::backend_lir_is_zero_arg_i32_main_definition(
                                       main_fn->signature_text) ||
      helper->entry.value != 0 || main_fn->entry.value != 0 ||
    ...
    if (const auto slice = parse_minimal_call_crossing_direct_call_slice(module);
      ...
      if (legacy_fallback != nullptr) {
        ...
        const auto regalloc_source_value =
            c4c::backend::parse_backend_single_helper_call_crossing_source_value(
                *legacy_fallback);
...
std::string emit_module(const c4c::codegen::lir::LirModule& module) {
  try {
    ...
    if (const auto slice = parse_minimal_call_crossing_direct_call_slice(adapted);
      ...
      const auto regalloc_source_value =
          c4c::backend::parse_backend_single_helper_call_crossing_source_value(module);
      if (!regalloc_source_value.has_value()) {
        throw c4c::backend::LirAdapterError(
            c4c::backend::LirAdapterErrorKind::Unsupported,
            "legacy x86 call-crossing source value for shared regalloc");
