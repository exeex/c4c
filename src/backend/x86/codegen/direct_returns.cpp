...
std::optional<MinimalAffineReturnSlice> parse_minimal_affine_return_slice(
  ...
  const auto lowered_return = lower_affine_operand(*block.terminator.value, param_names, values);
...
std::string emit_minimal_return_imm_asm(const c4c::backend::bir::Module& module,
  ...
  const auto symbol = asm_symbol_name(module.target_triple, module.functions.front().name);
