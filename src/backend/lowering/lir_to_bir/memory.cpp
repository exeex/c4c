...
try_lower_minimal_local_single_field_struct_store_load_zero_return_module(
  ...
  const auto& entry = function.blocks.front();
  const auto* first_gep = entry.insts.size() == 4 ? std::get_if<LirGepOp>(&entry.insts[0]) : nullptr;
  const auto* store = entry.insts.size() == 4 ? std::get_if<LirStoreOp>(&entry.insts[1]) : nullptr;
  const auto* second_gep =
      entry.insts.size() == 4 ? std::get_if<LirGepOp>(&entry.insts[2]) : nullptr;
  const auto* load = entry.insts.size() == 4 ? std::get_if<LirLoadOp>(&entry.insts[3]) : nullptr;
  const auto* ret = std::get_if<LirRet>(&entry.terminator);
  if (entry.label != "entry" || first_gep == nullptr || store == nullptr || second_gep == nullptr ||
...
try_lower_minimal_local_paired_single_field_struct_compare_sub_zero_return_module(
  ...
  if (first_slot == nullptr || second_slot == nullptr || first_slot->result.empty() ||
      ...
      first_slot->type_str != "%struct.T" || second_slot->type_str != "%struct.T.__shadow_0") {
    return std::nullopt;
  }

  const auto* true_ret = std::get_if<LirRet>(&function.blocks[1].terminator);
  const auto* false_ret = std::get_if<LirRet>(&function.blocks[2].terminator);
  if (true_ret == nullptr || false_ret == nullptr || function.blocks[0].label != "entry" ||
      function.blocks[1].label != "block_1" ||
      !function.blocks[1].insts.empty() || true_ret->type_str != "i32" ||
      !true_ret->value_str.has_value() || *true_ret->value_str != "1" ||
      function.blocks[2].label != "block_2" || !function.blocks[2].insts.empty() ||
      false_ret->type_str != "i32" || !false_ret->value_str.has_value() ||
      *false_ret->value_str != "0") {
    return std::nullopt;
  }
...
std::optional<bir::Module>
try_lower_minimal_local_enum_constant_compare_store_load_zero_return_module(
    const c4c::codegen::lir::LirModule& module) {
  using namespace c4c::codegen::lir;
  ...
  const auto* enum_slot = std::get_if<LirAllocaOp>(&function.alloca_insts[0]);
  if (enum_slot == nullptr || enum_slot->result != "%lv.e" || !enum_slot->count.empty() ||
      enum_slot->type_str != "i32") {
    return std::nullopt;
  }
  ...
  const auto* store = block6.insts.size() == 2 ? std::get_if<LirStoreOp>(&block6.insts[0]) : nullptr;
  const auto* load = block6.insts.size() == 2 ? std::get_if<LirLoadOp>(&block6.insts[1]) : nullptr;
  const auto* ret = std::get_if<LirRet>(&block6.terminator);
  if (block6.label != "block_6" || store == nullptr || load == nullptr || ret == nullptr ||
...
try_lower_minimal_local_string_literal_char_compare_ladder_zero_return_module(
  ...
  if (module.functions.size() != 1 || !module.globals.empty() || module.string_pool.size() != 1 ||
      !module.extern_decls.empty()) {
    return std::nullopt;
  }
  ...
  if (string_const.pool_name != "@.str0" || string_const.raw_bytes != "abcdef" ||
      string_const.byte_length != 7) {
    return std::nullopt;
  }
  ...
  if (function.is_declaration ||
      !lir_function_matches_minimal_no_param_integer_return(function, 32) ||
      function.entry.value != 0 || function.blocks.size() != 15 ||
      function.alloca_insts.size() != 1 || !function.stack_objects.empty()) {
    return std::nullopt;
  }

  const auto* string_slot = std::get_if<LirAllocaOp>(&function.alloca_insts[0]);
  if (string_slot == nullptr || string_slot->result != "%lv.s" || !string_slot->count.empty() ||
      string_slot->type_str != "ptr") {
    return std::nullopt;
  }
...
try_lower_minimal_global_nested_struct_anonymous_union_compare_zero_return_module(
  ...
  if (std::find(module.type_decls.begin(), module.type_decls.end(),
          ...
          module.type_decls.end() ||
      std::find(module.type_decls.begin(), module.type_decls.end(),
                "%struct.S1 = type { i32, i32 }") == module.type_decls.end() ||
      std::find(module.type_decls.begin(), module.type_decls.end(),
                "%struct._anon_0 = type { [4 x i8] }") == module.type_decls.end() ||
      std::find(module.type_decls.begin(), module.type_decls.end(),
                "%struct.S2 = type { i32, i32, %struct._anon_0, %struct.S1 }") ==
          module.type_decls.end()) {
    return std::nullopt;
  }

  const auto& global = module.globals.front();
  if (global.name != "v" || global.is_internal || global.is_const || global.is_extern_decl ||
      global.linkage_vis != "" || global.qualifier != "global " ||
      global.llvm_type != "%struct.S2" ||
      global.init_text !=
          "{ i32 1, i32 2, %struct._anon_0 { [4 x i8] [i8 3, i8 0, i8 0, i8 0] }, "
          "%struct.S1 { i32 4, i32 5 } }" ||
      global.align_bytes != 4) {
    return std::nullopt;
  }
  ...
  constexpr std::string_view kExpectedModule =
      ...
      "%struct._anon_0 = type { [4 x i8] }\n"
      "%struct.S2 = type { i32, i32, %struct._anon_0, %struct.S1 }\n"
      "@v = global %struct.S2 { i32 1, i32 2, %struct._anon_0 { [4 x i8] [i8 3, i8 0, i8 0, i8 "
      "0] }, %struct.S1 { i32 4, i32 5 } }, align 4\n"
      "\n"
      "define i32 @main()\n"
      "{\n"
      "entry:\n"
      "  %t0 = getelementptr %struct.S2, ptr @v, i32 0, i32 0\n"
      "  %t1 = load i32, ptr %t0\n"
      "  %t2 = icmp ne i32 %t1, 1\n"
      ...
      "  ret i32 1\n"
      "block_2:\n"
      "  %t5 = getelementptr %struct.S2, ptr @v, i32 0, i32 1\n"
      "  %t6 = load i32, ptr %t5\n"
      "  %t7 = icmp ne i32 %t6, 2\n"
      ...
      "  ret i32 2\n"
      "block_4:\n"
      "  %t10 = getelementptr %struct.S2, ptr @v, i32 0, i32 2\n"
      "  %t11 = getelementptr %struct._anon_0, ptr %t10, i32 0, i32 0\n"
      "  %t12 = load i32, ptr %t11\n"
      "  %t13 = icmp ne i32 %t12, 3\n"
      "  %t14 = zext i1 %t13 to i32\n"
      "  %t15 = icmp ne i32 %t14, 0\n"
      "  br i1 %t15, label %logic.skip.17, label %logic.rhs.16\n"
      "logic.rhs.16:\n"
      "  %t20 = getelementptr %struct.S2, ptr @v, i32 0, i32 2\n"
      "  %t21 = getelementptr %struct._anon_0, ptr %t20, i32 0, i32 0\n"
      "  %t22 = load i32, ptr %t21\n"
      "  %t23 = icmp ne i32 %t22, 3\n"
      "  %t24 = zext i1 %t23 to i32\n"
      "  %t25 = icmp ne i32 %t24, 0\n"
      "  %t26 = zext i1 %t25 to i32\n"
      "  br label %logic.rhs.end.18\n"
      "logic.rhs.end.18:\n"
      "  br label %logic.end.19\n"
      "logic.skip.17:\n"
      "  br label %logic.end.19\n"
      "logic.end.19:\n"
      "  %t27 = phi i32 [ %t26, %logic.rhs.end.18 ], [ 1, %logic.skip.17 ]\n"
      "  %t28 = icmp ne i32 %t27, 0\n"
      "  br i1 %t28, label %block_5, label %block_6\n"
      "block_5:\n"
      "  ret i32 3\n"
      "block_6:\n"
      "  %t29 = getelementptr %struct.S2, ptr @v, i32 0, i32 3\n"
      "  %t30 = getelementptr %struct.S1, ptr %t29, i32 0, i32 0\n"
      "  %t31 = load i32, ptr %t30\n"
      "  %t32 = icmp ne i32 %t31, 4\n"
      "  %t33 = zext i1 %t32 to i32\n"
      "  %t34 = icmp ne i32 %t33, 0\n"
      "  br i1 %t34, label %block_7, label %block_8\n"
      "block_7:\n"
      "  ret i32 4\n"
      "block_8:\n"
      "  %t35 = getelementptr %struct.S2, ptr @v, i32 0, i32 3\n"
      "  %t36 = getelementptr %struct.S1, ptr %t35, i32 0, i32 1\n"
      "  %t37 = load i32, ptr %t36\n"
      "  %t38 = icmp ne i32 %t37, 5\n"
  ...
  if (!match_return_block(function.blocks[1], "block_1", "1") ||
      !match_return_block(function.blocks[3], "block_3", "2") ||
      !match_return_block(function.blocks[9], "block_5", "3") ||
      !match_return_block(function.blocks[11], "block_7", "4") ||
      !match_return_block(function.blocks[13], "block_9", "5") ||
      !match_return_block(function.blocks[14], "block_10", "0")) {
    return std::nullopt;
  }
...
try_lower_minimal_nested_anonymous_aggregate_alias_compare_zero_return_module(
  ...
  if (std::find(module.type_decls.begin(), module.type_decls.end(),
          ...
          module.type_decls.end() ||
      std::find(module.type_decls.begin(), module.type_decls.end(),
                "%struct._anon_1 = type { [4 x i8] }") == module.type_decls.end() ||
      std::find(module.type_decls.begin(), module.type_decls.end(),
                "%struct._anon_4 = type { i32 }") == module.type_decls.end() ||
      std::find(module.type_decls.begin(), module.type_decls.end(),
                "%struct._anon_3 = type { [4 x i8] }") == module.type_decls.end() ||
      std::find(module.type_decls.begin(), module.type_decls.end(),
                "%struct._anon_2 = type { %struct._anon_3 }") == module.type_decls.end() ||
      std::find(module.type_decls.begin(), module.type_decls.end(),
                "%struct._anon_5 = type { i32 }") == module.type_decls.end() ||
      std::find(module.type_decls.begin(), module.type_decls.end(),
  ...
  constexpr std::string_view kExpectedModule =
      ...
      "{\n"
      "entry:\n"
      "  %lv.v = alloca %struct._anon_0, align 4\n"
      "  %t0 = getelementptr %struct._anon_0, ptr %lv.v, i32 0, i32 0\n"
      "  store i32 1, ptr %t0\n"
      "  %t1 = getelementptr %struct._anon_0, ptr %lv.v, i32 0, i32 1\n"
      "  %t2 = getelementptr %struct._anon_1, ptr %t1, i32 0, i32 0\n"
      "  store i32 2, ptr %t2\n"
      "  %t3 = getelementptr %struct._anon_0, ptr %lv.v, i32 0, i32 2\n"
      "  %t4 = getelementptr %struct._anon_2, ptr %t3, i32 0, i32 0\n"
      "  %t5 = getelementptr %struct._anon_3, ptr %t4, i32 0, i32 0\n"
      "  %t6 = getelementptr %struct._anon_4, ptr %t5, i32 0, i32 0\n"
      "  store i32 3, ptr %t6\n"
      "  %t7 = getelementptr %struct._anon_0, ptr %lv.v, i32 0, i32 3\n"
      "  %t8 = getelementptr %struct._anon_5, ptr %t7, i32 0, i32 0\n"
      "  store i32 4, ptr %t8\n"
      "  %t9 = getelementptr %struct._anon_0, ptr %lv.v, i32 0, i32 0\n"
      "  %t10 = load i32, ptr %t9\n"
      "  %t11 = icmp ne i32 %t10, 1\n"
      "  %t12 = zext i1 %t11 to i32\n"
      "  %t13 = icmp ne i32 %t12, 0\n"
      ...
      "  ret i32 1\n"
      "block_2:\n"
      "  %t14 = getelementptr %struct._anon_0, ptr %lv.v, i32 0, i32 1\n"
      "  %t15 = getelementptr %struct._anon_1, ptr %t14, i32 0, i32 0\n"
      "  %t16 = load i32, ptr %t15\n"
      "  %t17 = icmp ne i32 %t16, 2\n"
      "  %t18 = zext i1 %t17 to i32\n"
      "  %t19 = icmp ne i32 %t18, 0\n"
      "  br i1 %t19, label %logic.rhs.20, label %logic.skip.21\n"
      "logic.rhs.20:\n"
      "  %t24 = getelementptr %struct._anon_0, ptr %lv.v, i32 0, i32 1\n"
      "  %t25 = getelementptr %struct._anon_1, ptr %t24, i32 0, i32 0\n"
      "  %t26 = load i32, ptr %t25\n"
      "  %t27 = icmp ne i32 %t26, 2\n"
      "  %t28 = zext i1 %t27 to i32\n"
      "  %t29 = icmp ne i32 %t28, 0\n"
      "  %t30 = zext i1 %t29 to i32\n"
      "  br label %logic.rhs.end.22\n"
      "logic.rhs.end.22:\n"
      "  br label %logic.end.23\n"
      "logic.skip.21:\n"
      "  br label %logic.end.23\n"
      "logic.end.23:\n"
      "  %t31 = phi i32 [ %t30, %logic.rhs.end.22 ], [ 0, %logic.skip.21 ]\n"
      "  %t32 = icmp ne i32 %t31, 0\n"
      "  br i1 %t32, label %block_3, label %block_4\n"
      "block_3:\n"
      "  ret i32 2\n"
      "block_4:\n"
      "  %t33 = getelementptr %struct._anon_0, ptr %lv.v, i32 0, i32 2\n"
      "  %t34 = getelementptr %struct._anon_2, ptr %t33, i32 0, i32 0\n"
      "  %t35 = getelementptr %struct._anon_3, ptr %t34, i32 0, i32 0\n"
      "  %t36 = getelementptr %struct._anon_4, ptr %t35, i32 0, i32 0\n"
      "  %t37 = load i32, ptr %t36\n"
      "  %t38 = icmp ne i32 %t37, 3\n"
      "  %t39 = zext i1 %t38 to i32\n"
      "  %t40 = icmp ne i32 %t39, 0\n"
      "  br i1 %t40, label %block_5, label %block_6\n"
      "block_5:\n"
      "  ret i32 3\n"
      "block_6:\n"
      "  %t41 = getelementptr %struct._anon_0, ptr %lv.v, i32 0, i32 3\n"
      "  %t42 = getelementptr %struct._anon_5, ptr %t41, i32 0, i32 0\n"
      "  %t43 = load i32, ptr %t42\n"
      "  %t44 = icmp ne i32 %t43, 4\n"
      "  %t45 = zext i1 %t44 to i32\n"
      "  %t46 = icmp ne i32 %t45, 0\n"
      "  br i1 %t46, label %block_7, label %block_8\n"
      "block_7:\n"
      "  ret i32 4\n"
      "block_8:\n"
      "  ret i32 0\n"
  ...
  if (!match_return_block(function.blocks[1], "block_1", "1") ||
      !match_return_block(function.blocks[7], "block_3", "2") ||
      !match_return_block(function.blocks[9], "block_5", "3") ||
      !match_return_block(function.blocks[11], "block_7", "4") ||
      !match_return_block(function.blocks[12], "block_8", "0")) {
    return std::nullopt;
  }
...
try_lower_minimal_global_anonymous_struct_field_compare_zero_return_module(
  ...
  if (std::find(module.type_decls.begin(), module.type_decls.end(),
          ...
          module.type_decls.end() ||
      std::find(module.type_decls.begin(), module.type_decls.end(),
                "%struct._anon_0 = type { i32, i32, i32 }") == module.type_decls.end()) {
    return std::nullopt;
  }

  const auto& global = module.globals.front();
  if (global.name != "s" || global.is_internal || global.is_const || global.is_extern_decl ||
      global.linkage_vis != "" || global.qualifier != "global " ||
      global.llvm_type != "%struct._anon_0" ||
      global.init_text != "{ i32 1, i32 2, i32 3 }" || global.align_bytes != 4) {
    return std::nullopt;
  }
...
try_lower_minimal_global_named_two_field_struct_designated_init_compare_zero_return_module(
  ...
  if (std::find(module.type_decls.begin(), module.type_decls.end(),
          ...
          module.type_decls.end() ||
      std::find(module.type_decls.begin(), module.type_decls.end(),
                "%struct.S = type { i32, i32 }") == module.type_decls.end()) {
    return std::nullopt;
  }
  ...
  if (global.name != "s" || global.is_internal || global.is_const || global.is_extern_decl ||
      global.linkage_vis != "" || global.qualifier != "global " ||
      global.llvm_type != "%struct.S" || global.init_text != "{ i32 1, i32 2 }" ||
      global.align_bytes != 4) {
    return std::nullopt;
  }
...
try_lower_minimal_local_i32_array_pointer_inc_store_compare_123_zero_return_module(
  ...
  const auto* array_slot = std::get_if<LirAllocaOp>(&function.alloca_insts[0]);
  const auto* pointer_slot = std::get_if<LirAllocaOp>(&function.alloca_insts[1]);
  if (array_slot == nullptr || pointer_slot == nullptr || array_slot->result != "%lv.arr" ||
      pointer_slot->result != "%lv.p" || !array_slot->count.empty() || !pointer_slot->count.empty() ||
      array_slot->type_str != "[2 x i32]" || pointer_slot->type_str != "ptr") {
    return std::nullopt;
  }
  ...
  if (entry.label != "entry" || entry.insts.size() != 17 || entry_condbr == nullptr ||
      entry_condbr->cond_name != "%t13" || entry_condbr->true_label != "block_1" ||
      entry_condbr->false_label != "block_2") {
    return std::nullopt;
  }
  ...
  constexpr std::string_view kExpectedModule =
      ...
      "{\n"
      "entry:\n"
      "  %lv.arr = alloca [2 x i32], align 4\n"
      "  %lv.p = alloca ptr, align 8\n"
      "  %t0 = getelementptr [2 x i32], ptr %lv.arr, i64 0, i64 0\n"
      "  %t1 = sext i32 0 to i64\n"
      "  %t2 = getelementptr i32, ptr %t0, i64 %t1\n"
      "  store ptr %t2, ptr %lv.p\n"
      "  %t3 = load ptr, ptr %lv.p\n"
      "  %t4 = sext i32 1 to i64\n"
      "  %t5 = getelementptr i32, ptr %t3, i64 %t4\n"
      "  store ptr %t5, ptr %lv.p\n"
      "  %t6 = load ptr, ptr %lv.p\n"
      "  store i32 123, ptr %t6\n"
      "  %t7 = getelementptr [2 x i32], ptr %lv.arr, i64 0, i64 0\n"
      "  %t8 = sext i32 1 to i64\n"
      "  %t9 = getelementptr i32, ptr %t7, i64 %t8\n"
      "  %t10 = load i32, ptr %t9\n"
      "  %t11 = icmp ne i32 %t10, 123\n"
      "  %t12 = zext i1 %t11 to i32\n"
      "  %t13 = icmp ne i32 %t12, 0\n"
      "  br i1 %t13, label %block_1, label %block_2\n"
      "block_1:\n"
...
try_lower_minimal_local_i32_array_pointer_dec_store_compare_123_zero_return_module(
  ...
  const auto* pointer_slot = std::get_if<LirAllocaOp>(&function.alloca_insts[1]);
  if (array_slot == nullptr || pointer_slot == nullptr || array_slot->result != "%lv.arr" ||
      pointer_slot->result != "%lv.p" || !array_slot->count.empty() ||
      !pointer_slot->count.empty() || array_slot->type_str != "[2 x i32]" ||
      pointer_slot->type_str != "ptr") {
    return std::nullopt;
  }
  ...
  const auto& entry = function.blocks[0];
  const auto* entry_condbr = std::get_if<LirCondBr>(&entry.terminator);
  if (entry.label != "entry" || entry.insts.size() != 18 || entry_condbr == nullptr ||
      entry_condbr->cond_name != "%t14" || entry_condbr->true_label != "block_1" ||
      entry_condbr->false_label != "block_2") {
    return std::nullopt;
  }
  ...
  constexpr std::string_view kExpectedModule =
      ...
      "  %t4 = sext i32 1 to i64\n"
      "  %t5 = sub i64 0, %t4\n"
      "  %t6 = getelementptr i32, ptr %t3, i64 %t5\n"
      "  store ptr %t6, ptr %lv.p\n"
      "  %t7 = load ptr, ptr %lv.p\n"
      "  store i32 123, ptr %t7\n"
      "  %t8 = getelementptr [2 x i32], ptr %lv.arr, i64 0, i64 0\n"
      "  %t9 = sext i32 0 to i64\n"
      "  %t10 = getelementptr i32, ptr %t8, i64 %t9\n"
      "  %t11 = load i32, ptr %t10\n"
      "  %t12 = icmp ne i32 %t11, 123\n"
      "  %t13 = zext i1 %t12 to i32\n"
      "  %t14 = icmp ne i32 %t13, 0\n"
      "  br i1 %t14, label %block_1, label %block_2\n"
      "block_1:\n"
      "  ret i32 1\n"
      "block_2:\n"
      "  ret i32 0\n"
