...
struct ParsedBackendMinimalDualIdentityDirectCallSubLirModuleView {
  const c4c::codegen::lir::LirFunction* lhs_helper = nullptr;
  const c4c::codegen::lir::LirFunction* rhs_helper = nullptr;
  const c4c::codegen::lir::LirFunction* main_function = nullptr;
  const c4c::codegen::lir::LirCallOp* lhs_call = nullptr;
  const c4c::codegen::lir::LirCallOp* rhs_call = nullptr;
  const c4c::codegen::lir::LirBinOp* sub = nullptr;
  std::int64_t lhs_call_arg_imm = 0;
  std::int64_t rhs_call_arg_imm = 0;
};
...
struct ParsedBackendMinimalVoidDirectCallImmReturnLirModuleView {
  const c4c::codegen::lir::LirFunction* helper = nullptr;
  const c4c::codegen::lir::LirFunction* main_function = nullptr;
  const c4c::codegen::lir::LirCallOp* call = nullptr;
  std::int64_t return_imm = 0;
...
struct ParsedBackendMinimalVoidDirectCallImmReturnModuleView {
  const BackendFunction* helper = nullptr;
  const BackendFunction* main_function = nullptr;
  const BackendCallInst* call = nullptr;
  std::int64_t return_imm = 0;
...
struct ParsedBackendMinimalDirectCallIdentityArgModuleView {
  const BackendFunction* helper = nullptr;
  const BackendFunction* main_function = nullptr;
  const BackendCallInst* call = nullptr;
  std::int64_t call_arg_imm = 0;
...
struct ParsedBackendMinimalDeclaredDirectCallLirModuleView {
  const c4c::codegen::lir::LirExternDecl* callee = nullptr;
  const c4c::codegen::lir::LirFunction* declared_callee = nullptr;
  const c4c::codegen::lir::LirFunction* main_function = nullptr;
  const c4c::codegen::lir::LirBlock* main_block = nullptr;
  const c4c::codegen::lir::LirCallOp* call = nullptr;
  ParsedBackendDirectGlobalTypedCallView parsed_call;
...
inline std::optional<std::string_view> parse_backend_structured_single_identity_function(
    const BackendFunction& function,
    std::optional<std::string_view> expected_name) {
  const std::string_view function_name =
      expected_name.has_value() ? *expected_name : std::string_view(function.signature.name);
  ...
  if (block.label != "entry" || !block.insts.empty() || !block.terminator.value.has_value() ||
      backend_return_scalar_type(block.terminator) != BackendScalarType::I32 ||
      *block.terminator.value != param.name) {
    return std::nullopt;
  }
...
parse_backend_minimal_structured_direct_call_module(const BackendModule& module,
  ...
  const auto try_match = [&](const BackendFunction& caller,
                             const BackendFunction& helper)
      -> std::optional<ParsedBackendMinimalStructuredDirectCallModuleView> {
    if (caller.is_declaration || helper.is_declaration ||
        !backend_function_is_definition(caller.signature) ||
        !backend_function_is_definition(helper.signature) ||
        backend_signature_return_scalar_type(caller.signature) != BackendScalarType::I32 ||
        backend_signature_return_scalar_type(helper.signature) != BackendScalarType::I32 ||
        !caller.signature.params.empty() || caller.signature.is_vararg ||
        helper.signature.is_vararg || caller.blocks.size() != 1 || helper.blocks.size() != 1) {
      return std::nullopt;
    }

    const auto& caller_block = caller.blocks.front();
    if (caller_block.label != "entry" || caller_block.insts.size() != 1 ||
        !caller_block.terminator.value.has_value() ||
        backend_return_scalar_type(caller_block.terminator) != BackendScalarType::I32) {
      return std::nullopt;
    }

    const auto* call = std::get_if<BackendCallInst>(&caller_block.insts.front());
...
inline std::optional<ParsedBackendMinimalVoidDirectCallImmReturnModuleView>
parse_backend_minimal_void_direct_call_imm_return_module(const BackendModule& module) {
  ...
  if (helper == nullptr || main_fn == nullptr || helper->is_declaration || main_fn->is_declaration ||
      ...
      backend_signature_return_type_kind(helper->signature) != BackendValueTypeKind::Void ||
      !main_fn->signature.params.empty() || !helper->signature.params.empty() ||
      main_fn->signature.is_vararg || helper->signature.is_vararg ||
  ...
  if (helper_block.label != "entry" || !helper_block.insts.empty() ||
      helper_block.terminator.kind != BackendTerminatorKind::Return ||
      !backend_return_is_void(helper_block.terminator) ||
  ...
  const auto parsed_call =
      call == nullptr ? std::nullopt : parse_backend_direct_global_typed_call(*call);
  const auto return_imm = parse_backend_i64_literal(*main_block.terminator.value);
  if (call == nullptr || !parsed_call.has_value() ||
      !backend_typed_call_matches_signature(parsed_call->typed_call, helper->signature) ||
  ...
  return ParsedBackendMinimalVoidDirectCallImmReturnModuleView{
      helper,
      main_fn,
      call,
      *return_imm,
...
inline std::optional<ParsedBackendMinimalVoidDirectCallImmReturnLirModuleView>
parse_backend_minimal_void_direct_call_imm_return_lir_module(
    const c4c::codegen::lir::LirModule& module) {
  using namespace c4c::codegen::lir;
  ...
  for (const auto& function : module.functions) {
    ...
    if (helper != nullptr || function.is_declaration ||
        !backend_lir_signature_matches(function.signature_text, "define", "void", function.name, {})) {
      return std::nullopt;
    }
    helper = &function;
  }
  ...
  if (helper == nullptr || main_fn == nullptr ||
      main_fn->entry.value != 0 || helper->entry.value != 0 ||
      main_fn->blocks.size() != 1 || helper->blocks.size() != 1 ||
      !main_fn->alloca_insts.empty() || !helper->alloca_insts.empty() ||
      !main_fn->stack_objects.empty() || !helper->stack_objects.empty()) {
    return std::nullopt;
  }
  ...
  const auto symbol_name =
      call == nullptr ? std::nullopt : c4c::codegen::lir::parse_lir_direct_global_callee(call->callee);
  const auto typed_call =
      call == nullptr ? std::nullopt : c4c::codegen::lir::parse_lir_typed_call_or_infer_params(*call);
  const auto return_imm = parse_backend_i64_literal(*main_ret->value_str);
  if (call == nullptr || !symbol_name.has_value() || !typed_call.has_value() ||
      call->return_type != "void" || !call->result.empty() || symbol_name->empty() ||
      *symbol_name == main_fn->name || *symbol_name != helper->name || !typed_call->args.empty() ||
      !return_imm.has_value() || *return_imm < std::numeric_limits<std::int32_t>::min() ||
      *return_imm > std::numeric_limits<std::int32_t>::max()) {
    return std::nullopt;
  }

  return ParsedBackendMinimalVoidDirectCallImmReturnLirModuleView{
      helper,
      main_fn,
      call,
      *return_imm,
...
inline std::optional<ParsedBackendMinimalDirectCallIdentityArgModuleView>
parse_backend_minimal_direct_call_identity_arg_module(const BackendModule& module) {
  if (!module.globals.empty() || !module.string_constants.empty()) {
    return std::nullopt;
  }
  ...
  const auto parsed = parse_backend_minimal_structured_direct_call_module(module, 1);
  if (!parsed.has_value() ||
      !parse_backend_structured_single_identity_function(*parsed->helper, std::nullopt)
           .has_value()) {
    return std::nullopt;
  }
  ...
  return ParsedBackendMinimalDirectCallIdentityArgModuleView{
      parsed->helper,
      parsed->main_function,
      parsed->call,
      *call_arg_imm,
...
inline std::optional<ParsedBackendMinimalDeclaredDirectCallLirModuleView>
parse_backend_minimal_declared_direct_call_lir_module(
    const c4c::codegen::lir::LirModule& module) {
  using namespace c4c::codegen::lir;
  ...
  const LirExternDecl* callee = nullptr;
  ...
  bool callee_is_vararg = false;
  if (callee != nullptr) {
    if (c4c::codegen::lir::trim_lir_arg_text(callee->return_type_str) != "i32") {
      return std::nullopt;
    }
  } else {
    if (declared_callee == nullptr || declared_callee->name != *symbol_name ||
      ...
      for (std::size_t index = 0; index < fixed_param_count; ++index) {
        if (c4c::codegen::lir::trim_lir_arg_text((*params)[index].type) !=
            c4c::codegen::lir::trim_lir_arg_text(typed_call->param_types[index])) {
          return std::nullopt;
        }
      }
      callee_is_vararg = saw_varargs;
    } else {
      const auto params = parse_backend_function_signature_params(declared_callee->signature_text);
      callee_is_vararg =
          params.has_value() &&
          std::any_of(params->begin(), params->end(), [](const auto& param) {
            return param.is_varargs;
          });
    }
  }
  ...
  const bool return_call_result =
      !call->result.empty() && *main_ret->value_str == call->result;
  std::int64_t return_imm = 0;
  if (!return_call_result) {
  ...
  return ParsedBackendMinimalDeclaredDirectCallLirModuleView{
      callee,
      declared_callee,
      main_fn,
      &main_block,
...
inline std::optional<ParsedBackendStructuredFoldedTwoArgFunctionView>
parse_backend_folded_two_arg_function(
    const c4c::codegen::lir::LirFunction& function,
    std::optional<std::string_view> expected_name = std::nullopt) {
  using namespace c4c::codegen::lir;
  ...
  if (block.label != "entry" || block.insts.size() != 2 || ret == nullptr ||
      !ret->value_str.has_value() || ret->type_str != "i32") {
    return std::nullopt;
  }

  const auto* add = std::get_if<LirBinOp>(&block.insts[0]);
  const auto* sub = std::get_if<LirBinOp>(&block.insts[1]);
  ...
  if (add == nullptr || sub == nullptr || add_opcode != LirBinaryOpcode::Add ||
      ...
      *ret->value_str != sub->result || sub->lhs != add->result ||
...
inline std::optional<ParsedBackendMinimalFoldedTwoArgDirectCallLirModuleView>
parse_backend_minimal_folded_two_arg_direct_call_lir_module(
    const c4c::codegen::lir::LirModule& module) {
  using namespace c4c::codegen::lir;
  ...
  const auto helper_shape = parse_backend_folded_two_arg_function(*helper, std::nullopt);
  if (!helper_shape.has_value()) {
    return std::nullopt;
  }
  ...
  const auto* main_ret = std::get_if<LirRet>(&main_block.terminator);
  if (main_block.label != "entry" || main_ret == nullptr || !main_ret->value_str.has_value() ||
      main_ret->type_str != "i32" || main_block.insts.size() != 1) {
    return std::nullopt;
  }
  ...
  return ParsedBackendMinimalFoldedTwoArgDirectCallLirModuleView{
      helper,
      main_fn,
      call,
      *lhs_call_arg_imm,
...
parse_backend_minimal_direct_call_identity_arg_lir_module(
    const c4c::codegen::lir::LirModule& module) {
  using namespace c4c::codegen::lir;
  ...
  const auto call_arg_imm = operand.has_value() ? parse_backend_i64_literal(*operand) : std::nullopt;
  if (!call_arg_imm.has_value()) {
    return std::nullopt;
  }

  return ParsedBackendMinimalDirectCallIdentityArgLirModuleView{
      helper,
      main_fn,
      call,
      *call_arg_imm,
...
inline std::optional<ParsedBackendMinimalDualIdentityDirectCallSubLirModuleView>
parse_backend_minimal_dual_identity_direct_call_sub_lir_module(
  ...
  for (const auto* helper : helpers) {
    if (helper == nullptr || helper->is_declaration || helper->entry.value != 0 ||
        helper->blocks.size() != 1 ||
        !parse_backend_single_identity_function(*helper, std::nullopt).has_value()) {
      return std::nullopt;
    }
  }
  ...
  if (lhs_call == nullptr || rhs_call == nullptr || sub == nullptr ||
      ...
      lhs_call->result.empty() || rhs_call->result.empty() ||
      sub->lhs != lhs_call->result || sub->rhs != rhs_call->result ||
      *main_ret->value_str != sub->result) {
    return std::nullopt;
  }

  const LirFunction* lhs_helper = nullptr;
  const LirFunction* rhs_helper = nullptr;
  std::int64_t lhs_call_arg_imm = 0;
  std::int64_t rhs_call_arg_imm = 0;
  for (const auto* helper : helpers) {
    if (lhs_helper == nullptr) {
      const auto operand =
          parse_backend_direct_global_single_typed_call_operand(*lhs_call, helper->name, "i32");
      const auto parsed_imm =
          operand.has_value() ? parse_backend_i64_literal(*operand) : std::nullopt;
      if (parsed_imm.has_value()) {
        lhs_helper = helper;
        lhs_call_arg_imm = *parsed_imm;
      }
    }
    if (rhs_helper == nullptr) {
      const auto operand =
          parse_backend_direct_global_single_typed_call_operand(*rhs_call, helper->name, "i32");
      const auto parsed_imm =
          operand.has_value() ? parse_backend_i64_literal(*operand) : std::nullopt;
      if (parsed_imm.has_value()) {
        rhs_helper = helper;
        rhs_call_arg_imm = *parsed_imm;
      }
    }
  }
  ...
  return ParsedBackendMinimalDualIdentityDirectCallSubLirModuleView{
      lhs_helper,
      rhs_helper,
      main_fn,
      lhs_call,
