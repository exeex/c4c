...
| Bounded B4 promotion / phi-growth planning | B4, as the sole dynamic SSA construction and proof owner | exact B3 revision plus CFG/dominance/value-flow, stable object/value/edge order, static `SsaEligible` admission, and `CompletePromotionPolicyV1` | B4 alone builds the complete eligible promotion set in one private candidate after the bounded plan is accepted | B5 admits only the exact completely promoted B4 checkpoint; explicit memory survives only for forms independently classified non-promotable by the closed matrix | B4 resource-plan validation plus whole-graph SSA publication; B5 verifies inherited exact plan/accounting without repairing B4 | input graph, policy version, eligibility, CFG, dominance, or value-flow change invalidates the plan; candidate failure discards all mutations and recomputation begins from immutable B3 | checked-arithmetic overflow, hard-bound breach, incomplete promotion, construction/resource failure, or SSA failure publishes nothing; partial selection and mid-pass fallback are forbidden | planning sits immediately before the B4 mutation it bounds, and B5 remains the adjacent admission consumer; no resource pass, retained-partial schema, or second SSA authority is needed |
...
3. Resolved by [Step 4](step4_bounded_b4_promotion_policy.md): B4 uses one
   versioned fail-closed complete-promotion plan with checked metrics, finite
   hard bounds, stable ordering, exact duplicate-edge accounting, and atomic
