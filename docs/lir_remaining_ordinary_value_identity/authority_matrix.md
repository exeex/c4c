...
integer and Steps 7.7/7.8 explicit scalar FPTrunc/FPExt casts in
...
| `LirCastOp`: `result,operand`; `kind,from_type,to_type` | Active in PX/PB/PL/PC/PI/PV/PO and PF fixed-vector parameter setup; Steps 7.1, 7.7, and 7.8 own PX explicit scalar integer and authoritative-source FPTrunc/FPExt `CastExpr` routes | The focused explicit routes use `fresh_value`, exact input `LirOperand`, and the operand-returning coercion wrapper. Floating authority is gated on an authoritative Step-7.5 source and an actual representation-width change; other cast producers remain text-only with `fresh_tmp` | Native kind and exact endpoints; authoritative integer casts require coherent Trunc/ZExt/SExt widths, while authoritative FPTrunc/FPExt require exact floating endpoints with strict narrowing/widening direction. Generic ownership rejects invalid/duplicate definitions and unknown/cross-function uses | `lir_scalar_cast_result_use_identity.c`, `lir_scalar_fptrunc_result_use_identity.c`, and `lir_scalar_fpext_result_use_identity.c` close only the representative explicit integer, FPTrunc, and FPExt chains. Int/float conversion, pointer, bitcast, vector, aggregate, implicit coercion, and other producers remain unclaimed |
...
`LirCastOp`. Steps 7.7 and 7.8 separately own authoritative-source explicit
FPTrunc and FPExt. Other floating, pointer, bitcast, vector, aggregate,
...
implicit coercions, same-representation no-ops, monostate-source floating
casts, and all other producer families remain compatibility or outside this
packet.
