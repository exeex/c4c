...
Capability complete. Commit `727949c9c` selected exactly the direct
non-array/non-VLA integer local scalar declaration `LirStoreOp` initialized by
a native representable integer immediate, with native local-object
pointer/owner/type/liveness authority. The verifier rejects malformed selected
native authority while display spelling remains nonsemantic. The handoff at
`docs/lir_local_operation_authority/handoff_to_734.md` authorizes 734 Step
7.28 only. Fresh build plus
`ctest --test-dir build -j --output-on-failure -R '^frontend_lir_call_type_ref$'`
passed 1/1, with a matching non-decreasing canonical guard. All other stores,
GEPs, VLA lifetime, and later families remain out of scope and fail closed.
