...
function-body parameter-use authority row after accepted 734 Step 7.47.

## Why This Exists

Idea 734 can receive only structured LIR authority into typed Raw BIR. Its
latest accepted receiver packet, commit `b26d47f15`, consumed the closed-858
DirectScalar binary-`fadd` RHS row. The 734 completion gate remains unmet, but
...
- Reject selecting multiple rows, a generic body-parameter family, or any row
  already accepted by 734 through Step 7.47.
...
Close accepted. Implementation commit `c3a7883a4` proves exactly one selected
producer-side body-parameter row:
`LirBinOp.scalar_lhs_parameter_authority` for a current-function
`DirectScalar` floating parameter used as the LHS of binary floating `fsub`,
with producer shape `return x - 2.0;`.
...
`LirBinOp` opcode `fsub`; `lhs` is the same parameter SSA/value as the
authority tuple; `type_str` matches the authority type; `rhs` is a
nonselected scalar operand.

The existing emitter already publishes the carrier. The verifier now admits
selected floating `fsub` LHS authority in addition to prior selected floating
LHS rows, requires a nonselected scalar RHS, and rejects duplicate selected
floating-`fsub` LHS consumers in the current function. Neighboring `fadd` and
`fmul` LHS malformed coverage uses nonselected `fdiv` instead of now-selected
`fsub`.

Malformed coverage rejects omitted/missing, invalid, duplicate definition,
foreign owner, wrong index, wrong type, wrong ABI, wrong role, non-`fsub`,
LHS mismatch, type mismatch, selected-RHS incoherence, and duplicate selected
consumer forms.
...
Raw-BIR receiver work landed in this idea.

Exact 734 return action: reactivate
`ideas/open/734_lir_to_new_bir_container_completeness.md` for a future bounded
Raw-BIR receiver packet that receives only this selected binary-`fsub` LHS
