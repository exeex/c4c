...
Close accepted for this bounded producer seam. Commit `1e24e2081` confirmed
that `StmtEmitter::emit_rval_expr` routed `MemberExpr` through the string-only
`emit_rval_payload` path and that the final `emit_bitfield_load` temporary was
the first production owner dropping the RHS identity for `insn.f1.offset`.
`emit_member_rval_operand` now returns that valid current-function
`LirValueId`; structured load, shift/mask/sign-extension, and final-value
operands retain verifier-valid authority.

Accepted proof: fresh `cmake --build --preset default` succeeded;
`^frontend_lir_call_type_ref$` passed; the matching subset guard retained the
known downstream failure and passed its other coverage; the broad backend guard
passed. The rejected full-baseline candidate is not a defect in this completed
producer seam: it exposes the downstream `LirIndirectBrOp.addr_value` carrier
failure. Exact return action: resume
`ideas/open/764_lir_production_computed_goto_addr_value_publication.md` at
Step 1 to publish the already-authoritative address pointer into that carrier,
rerun `comp-goto-1`, `20040302-1`, `20041214-1`, `920501-4`, and `920501-5`,
then require no new baseline failures before any future baseline accept. Do
not resume 734 directly.
