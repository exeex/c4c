...
Focused local-memory GEP cases:
...
- Identify the semantic `lir_to_bir` admission gap for local-memory GEP forms.
...
- The focused local-memory GEP cases no longer fail at `lir_to_bir` admission
  for the old local-memory rejection reason.
