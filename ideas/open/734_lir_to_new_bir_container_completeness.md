...
## Runbook Exhaustion Decision: post-Step 7.43 DirectScalar unary-fneg receipt

Close rejected. Commit `617a8fae9` receives exactly closed 854's DirectScalar
unary-`fneg` body-parameter authority row into typed Raw BIR. The Raw-BIR
`FNeg` builder admission is limited to the selected current-function parameter
tuple: parameter `LirValueId`, owner, parameter index, matching scalar type,
DirectScalar ABI, explicit `Lhs` role, unary `fneg` opcode, lhs parameter
value, and empty rhs. The importer passes invalid Raw-BIR RHS for the validated
unary row. Focused coverage checks the positive tuple plus missing, invalid,
foreign, duplicate, owner/index/type/ABI/role, definition scalar-type
mismatch, signature parameter-type mismatch, non-`fneg`, lhs mismatch,
rhs-populated, and duplicate-consumer failures.

The supervisor accepted a fresh build plus focused receiver proof
`ctest --test-dir build -j --output-on-failure -R
'^backend_lir_to_bir_interface$'`, followed by `git diff --check`, and a
matching broader backend regression guard with before 6/6, after 6/6, and no
new failures under `--allow-non-decreasing-passed`.
...
explicit dispatcher and neighboring coverage, whole-module transactional
proof, documentation convergence, and source-wide focused/broader proof are
still unmet. Valid unreceived families include further function-body
parameter uses, memory/VA, aggregate/vector, module/type/global/metadata,
residual instruction/terminator, and inline-assembly forms. They remain fail
closed; no receiver may recover authority from text, names, rendered operands,
signatures, compatibility mirrors, `monostate`, or unclassified values.
...
Classification: `separate-blocker`. New open idea
`ideas/open/855_lir_next_body_parameter_authority_handoff.md` owns only
tracing, publishing, verifying, and handing off exactly one next valid
function-body parameter-use row after the accepted DirectScalar unary-`fneg`
receipt. It must not edit Raw-BIR/importer code, reopen any accepted
DirectPointer or DirectScalar body-parameter receipt, select from presentation
fields, or absorb another family.

Resumption record: Steps 1 through 7.43 are accepted historical work, most
recently receiver commit `617a8fae9` with the fresh focused proof, matching
broader accepted regression guard, and `git diff --check` stated above.
Interrupted point: source completion reassessment after Step 7.43; no further
734 receiver row is authorized. After 855 closes with one exact structured
handoff and accepted focused producer proof, reactivate 734 and repair its
runbook only for that matching typed Raw-BIR receiver row. Do not repeat Step
...
## Runbook Exhaustion Decision: post-Step 7.44 DirectScalar binary-fmul-LHS receipt

Close rejected. Commit `e0540da75` receives exactly closed 855's
...
The accepted receiver preserves LirValueId 91, owner
...
Focused coverage rejects missing, invalid, duplicate, foreign,
owner/index/type/ABI/role, non-`fmul`, LHS mismatch, result type mismatch, RHS
incoherence, and duplicate-consumer cases transactionally before publication.
...
plus the matching broader backend regression guard with before 6/6, after 6/6,
and no new failures under `--allow-non-decreasing-passed`. Direct code review
found no blocker.
...
Classification: `separate-blocker`. New open idea
`ideas/open/856_lir_next_body_parameter_authority_handoff.md` owns only
tracing, publishing, verifying, and handing off exactly one next valid
function-body parameter-use row after the accepted DirectScalar binary-`fmul`
LHS receipt. It must not edit Raw-BIR/importer code, reopen accepted
DirectPointer or DirectScalar body-parameter receipts, select from presentation
fields, or absorb another family.

Resumption record: Steps 1 through 7.44 are accepted historical work, most
recently receiver commit `e0540da75` with the fresh focused proof, matching
broader accepted regression guard, `git diff --check`, and direct review stated
above. Interrupted point: source completion reassessment after Step 7.44; no
further 734 receiver row is authorized. After 856 closes with one exact
structured handoff and accepted focused producer proof, reactivate 734 and
repair its runbook only for that matching typed Raw-BIR receiver row. Do not
...
## Runbook Exhaustion Decision: post-Step 7.45 DirectScalar binary-fmul-RHS receipt

Close rejected. Commit `2b7e897ce` receives exactly closed 856's
`LirBinOp.scalar_rhs_parameter_authority` tuple into typed Raw BIR for a
current-function `DirectScalar` floating parameter used as binary `fmul` RHS.
The accepted receiver preserves the original RHS parameter source value,
owner, parameter index, lowered floating scalar type, `DirectScalar` ABI,
explicit `Rhs` role, `fmul` opcode, RHS operand identity, matching operation
type, and nonselected scalar LHS. LIR producer authority remains owned by
closed 856 and was not edited.
...
operation type mismatch, selected-LHS incoherence, nonselected-LHS mismatch or
type mismatch, and duplicate selected consumer forms transactionally before
publication.
...
Classification: `separate-blocker`. New open idea
`ideas/open/857_lir_next_body_parameter_authority_handoff.md` owns only
tracing, publishing, verifying, and handing off exactly one next valid
function-body parameter-use row after the accepted DirectScalar binary-`fmul`
...
Resumption record: Steps 1 through 7.45 are accepted historical work, most
recently receiver commit `2b7e897ce` with the fresh focused proof, matching
broader accepted regression guard, and `git diff --check` stated above.
Interrupted point: source completion reassessment after Step 7.45; no further
734 receiver row is authorized. After 857 closes with one exact structured
handoff and accepted focused producer proof, reactivate 734 and repair its
runbook only for that matching typed Raw-BIR receiver row. Do not repeat Step
...
## Resumption Record: DirectScalar binary-fadd-RHS parameter authority completion

Closed idea 858 completed the next body-parameter producer handoff with
accepted implementation `80d8a339c`. The selected row is
`LirBinOp.scalar_rhs_parameter_authority` for a current-function
`DirectScalar` floating parameter used as the RHS of binary `fadd`, with
...
`LirBinOp` `fadd` whose `rhs` is the same parameter SSA/value as the authority
tuple, whose `type_str` matches the authority type, and whose `lhs` is a
nonselected scalar operand.

The existing emitter already publishes the carrier. The verifier admits only
...
current function. Malformed coverage rejects omitted/missing, invalid,
duplicate definition, foreign owner, wrong index, wrong type, wrong ABI, wrong
role, non-`fadd`, RHS mismatch, type mismatch, selected-LHS incoherence, and
duplicate selected consumer forms. Neighboring `fmul` RHS malformed coverage
uses nonselected `fsub` instead of now-selected `fadd`.
...
Exact return action: do not activate this idea as part of this record. When
the supervisor next activates 734, plan-owner must repair the runbook for one
bounded Raw-BIR receiver packet only: receive the selected binary-`fadd` RHS
...
repeat Step 7.46, start receiver implementation during the 858 closure, receive
floating binary-`fadd` LHS, other floating binary parameter uses, memory/VA,
aggregate/vector, module/type/global/metadata, residual
...
## Runbook Exhaustion Decision: post-Step 7.47 DirectScalar binary-fadd-RHS receipt

Close rejected. Commit `b26d47f15` receives exactly closed 858's
`LirBinOp.scalar_rhs_parameter_authority` tuple into typed Raw BIR for a
current-function `DirectScalar` floating parameter used as binary `fadd` RHS.
The accepted receiver preserves the original RHS parameter source value,
owner, parameter index, lowered floating scalar type, `DirectScalar` ABI,
explicit `Rhs` role, `fadd` opcode, RHS operand identity, matching operation
type, and nonselected scalar LHS. LIR producer authority remains owned by
closed 858 and was not edited.

Focused receiver coverage rejects omitted/missing, invalid, duplicate
definition, foreign owner, wrong index/type/ABI/role, non-`fadd`, RHS
mismatch, operation type mismatch, nonselected-LHS mismatch/type mismatch, and
duplicate selected consumer forms transactionally before publication. The
neighboring `fmul` RHS receiver negative now uses nonselected `fsub` for its
non-`fmul` case.
...
still unmet. Valid unreceived families include further function-body
parameter uses, memory/VA, aggregate/vector, module/type/global/metadata,
residual instruction/terminator, inline-assembly, and any other current-LIR
semantic facts not yet covered by accepted typed Raw-BIR receiver
dispositions. They remain fail closed; no receiver may recover authority from
text, names, rendered operands, signatures, compatibility mirrors,
`monostate`, or unclassified values.

Classification: `separate-blocker`. New open idea
`ideas/open/859_lir_next_body_parameter_authority_handoff.md` owns only
tracing, publishing, verifying, and handing off exactly one next valid
function-body parameter-use row after the accepted DirectScalar binary-`fadd`
RHS receipt. It must not edit Raw-BIR/importer code, reopen accepted
DirectPointer or DirectScalar body-parameter receipts, select from
...
Resumption record: Steps 1 through 7.47 are accepted historical work, most
recently receiver commit `b26d47f15` with the fresh focused proof, matching
broader accepted regression guard, and `git diff --check` stated above.
Interrupted point: source completion reassessment after Step 7.47; no further
734 receiver row is authorized. After 859 closes with one exact structured
handoff and accepted focused producer proof, reactivate 734 and repair its
runbook only for that matching typed Raw-BIR receiver row. Do not repeat Step
...
## Runbook Exhaustion Decision: post-Step 7.48 DirectScalar binary-fsub-LHS receipt

Close rejected. Commit `6a91d07ca` receives exactly closed 859's
`LirBinOp.scalar_lhs_parameter_authority` tuple into typed Raw BIR for a
current-function `DirectScalar` floating parameter used as binary `fsub` LHS.
The accepted receiver adds the bounded Raw-BIR `FSub` opcode and preserves the
original LHS parameter source value, owner, parameter index, lowered floating
scalar type, `DirectScalar` ABI, explicit `Lhs` role, `fsub` opcode, LHS
operand identity, matching operation type, and nonselected scalar RHS. LIR
producer authority remains owned by closed 859 and was not edited.

Focused receiver coverage rejects omitted/missing, invalid, duplicate
definition, foreign owner, wrong index/type/ABI/role, non-`fsub`, LHS
mismatch, operation type mismatch, nonselected-RHS mismatch/type mismatch, and
duplicate selected consumer forms transactionally before publication.
Neighboring fadd/fmul LHS receiver coverage now uses nonselected `fdiv` for
nonmatching-opcode negatives.
...
Classification: `separate-blocker`. New open idea
`ideas/open/860_lir_next_body_parameter_authority_handoff.md` owns only
tracing, publishing, verifying, and handing off exactly one next valid
...
Resumption record: Steps 1 through 7.48 are accepted historical work, most
recently receiver commit `6a91d07ca` with the fresh focused proof, matching
broader accepted regression guard, and `git diff --check` stated above.
Interrupted point: source completion reassessment after Step 7.48; no further
734 receiver row is authorized. After 860 closes with one exact structured
handoff and accepted focused producer proof, reactivate 734 and repair its
runbook only for that matching typed Raw-BIR receiver row. Do not repeat Step
...
## Runbook Exhaustion Decision: post-Step 7.49 DirectScalar binary-fsub-RHS receipt

Close rejected. Commit `44edfbacd` receives exactly closed 860's
`LirBinOp.scalar_rhs_parameter_authority` tuple into typed Raw BIR for a
current-function `DirectScalar` floating parameter used as binary `fsub` RHS.
The accepted receiver preserves the original RHS parameter source value,
owner, parameter index, lowered floating scalar type, `DirectScalar` ABI,
explicit `Rhs` role, `fsub` opcode, RHS operand identity, matching operation
type, and nonselected scalar LHS. LIR producer authority remains owned by
closed 860 and was not edited.

The supervisor accepted the fresh focused receiver proof
`{ cmake --build --preset default && ctest --test-dir build -j --output-on-failure -R '^backend_lir_to_bir_interface$'; } > test_after.log 2>&1`,
the broader backend proof
`{ cmake --build --preset default && ctest --test-dir build -j --output-on-failure -R '^backend_'; }`
with `6/6` backend tests passing, and `git diff --check` before commit.
...
Resumption record: Steps 1 through 7.49 are accepted historical work, most
...
## Runbook Exhaustion Decision: post-Step 7.51 direct double(double) call-result receipt

Close rejected. Commit `750b6b3ba` receives exactly closed 864's
`LirCallOp.direct_one_double_arg_scalar_floating_call_authority` tuple into
typed Raw BIR for the selected direct nonvariadic `double(double)` call-result
row. The accepted receiver preserves the native result `LirValueId`,
current-function owner `LinkNameId`, direct callee `LinkNameId`, exact
`double(double)` type tuple, and explicit `DirectCallResult` role. Downstream
consumer coherence is deliberately out of scope for this row. LIR producer
authority remains owned by closed 864 and was not edited.

The supervisor accepted the fresh matching focused before/after receiver proof
`{ cmake --build --preset default && ctest --test-dir build -j --output-on-failure -R '^backend_lir_to_bir_interface$'; } > test_before.log/test_after.log 2>&1`
with `1/1` passing before and after, `git diff --check`, and the broader
backend proof
`{ cmake --build --preset default && ctest --test-dir build -j --output-on-failure -R '^backend_'; } > /tmp/c4c_backend_after_751.log 2>&1`
with `6/6` backend tests passing.
...
Classification: `separate-blocker`. New open idea
`ideas/open/865_lir_next_non_body_parameter_authority_handoff.md` owns exactly
one next producer-side non-body-parameter handoff after the accepted
`double(double)` direct call-result receipt. It must not edit Raw-BIR/importer
code, reopen accepted call-result or body-parameter receipts, select from
presentation fields, or absorb a generic residual sweep.

Resumption record: Steps 1 through 7.51 remain accepted historical work, most
recently receiver commit `750b6b3ba` with the fresh focused before/after
proof, broader backend proof, and `git diff --check` stated above.
Interrupted point: source completion reassessment after Step 7.51; no further
734 receiver row is authorized. After 865 closes with one exact structured
non-body-parameter handoff and accepted focused producer proof, reactivate 734
and repair its runbook only for that matching typed Raw-BIR receiver row. Do
not repeat Step 7.51 or receive another call-result row, parameter-use row,
memory/VA, aggregate/vector, module/type/global/metadata, CFG/PHI, residual
instruction/terminator, inline-assembly, generic residual sweep, or any other
form without its separately scoped first-owner handoff.
