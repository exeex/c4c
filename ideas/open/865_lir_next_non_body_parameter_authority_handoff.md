...
Idea 734 remains incomplete after accepted Step 7.51, but it can receive only
structured LIR authority into typed Raw BIR. The latest accepted receiver
packet, commit `750b6b3ba`, consumed the closed-864 direct nonvariadic
`double(double)` call-result row. No further 734 receiver row is authorized
without a separately accepted producer-side handoff.
...
- Reject selecting a function-body parameter row, multiple rows, a generic
  residual sweep, or any row already accepted by 734 through Step 7.51.
- Reject selecting fixed direct-call argument 0 or argument 1 parameter
  ...
  7.41.
- Reject selecting the zero-argument floating call-result row or the
  `double(double)` direct call-result row. Idea 734 already accepted those
  receiver rows in Steps 7.50 and 7.51.
- Reject recovering identity, role, type, opcode, or operand relation from
  rendered text, names, signatures, diagnostics, compatibility mirrors,
  `monostate`, or testcase-specific shape.
