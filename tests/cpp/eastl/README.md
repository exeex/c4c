...
| `eastl_tuple_simple.cpp` | `SEMA` | `build/c4cll --dump-canonical -I ref/EASTL/include -I ref/EABase/include/Common tests/cpp/eastl/eastl_tuple_simple.cpp` | The old `/usr/include/c++/14/cstddef` lvalue-reference binding blocker is gone. `--dump-canonical` now reaches deterministic undeclared-identifier failures in `/usr/include/c++/14/bits/ranges_util.h` around `_M_begin`, `_M_end`, `_M_size`, `_S_store_size`, and `this`; the earlier `bidirectional_iterator<_It>` concept-id condition in `subrange::advance` now folds correctly, so the next frontier is method/member-context support rather than concept-id evaluation or reference binding. |
