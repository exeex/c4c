...
| `eastl_utility_simple.cpp` | `SEMA` | `build/c4cll --dump-canonical -I ref/EASTL/include -I ref/EABase/include/Common tests/cpp/eastl/eastl_utility_simple.cpp` | `--parse-only` succeeds in about `10.476s`. The next smaller canonical frontier is now inside `eastl::pair`: `error: initializer for scalar member 'pair' must have exactly one argument`. A reduced internal templated delegating-ctor fix already handles unqualified self-delegation, but the remaining piecewise-style helper path still fails with the same root family. |
...
| `eastl_tuple_simple.cpp` | `SEMA` | `build/c4cll --dump-canonical -I ref/EASTL/include -I ref/EABase/include/Common tests/cpp/eastl/eastl_tuple_simple.cpp` | `--parse-only` succeeds again but currently takes about `26.810s`, so it is no longer the smallest active frontier. The old canonical `initializer for scalar member 'first' must have exactly one argument` failure is still gone; the current canonical workflow times out deeper in the libstdc++ / tuple stack after the smaller `eastl_utility_simple.cpp` `pair` issue. |
