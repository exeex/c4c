...
| `eastl_integer_sequence_simple.cpp` | `SEMA` | `build/c4cll --parse-only -I ref/EASTL/include -I ref/EABase/include/Common tests/cpp/eastl/eastl_integer_sequence_simple.cpp` | The nested variable-template parser blocker in `type_properties.h` is fixed. The current earliest failure is `tests/cpp/eastl/eastl_integer_sequence_simple.cpp:6:1` with `object has incomplete type: eastl::is_signed_helper`. A reduced internal reproducer now exists at `tests/cpp/internal/negative_case/namespaced_inherited_type_alias_base_member_lookup_parse.cpp`. |
| `eastl_type_traits_simple.cpp` | `SEMA` | `build/c4cll --parse-only -I ref/EASTL/include -I ref/EABase/include/Common tests/cpp/eastl/eastl_type_traits_simple.cpp` | Same `is_signed_helper` incomplete-type blocker at `tests/cpp/eastl/eastl_type_traits_simple.cpp:55:1`. The reduced reproducer in `tests/cpp/internal/negative_case/namespaced_inherited_type_alias_base_member_lookup_parse.cpp` now covers the same defaulted-NTTP plus inherited-alias shape without the full header stack. |
...
| `eastl_memory_simple.cpp` | `SEMA` | `build/c4cll --parse-only -I ref/EASTL/include -I ref/EABase/include/Common tests/cpp/eastl/eastl_memory_simple.cpp` | Recheck pending after the nested variable-template parser fix; the previous `has_unique_object_representations` blocker is no longer authoritative. |
| `eastl_tuple_simple.cpp` | `SEMA` | `build/c4cll --parse-only -I ref/EASTL/include -I ref/EABase/include/Common tests/cpp/eastl/eastl_tuple_simple.cpp` | Recheck pending after the nested variable-template parser fix; the previous `has_unique_object_representations` blocker is no longer authoritative. |
...
- `run_eastl_type_traits_simple_workflow.cmake`: older end-to-end workflow kept
  for future reactivation once the current `is_signed_helper` /
