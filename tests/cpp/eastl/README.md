...
| `eastl_memory_uses_allocator_frontier.cpp` | `PARSE` | `build/c4cll --parser-debug --parser-debug-tentative --parse-only -I ref/EASTL/include -I ref/EABase/include/Common tests/cpp/eastl/eastl_memory_uses_allocator_frontier.cpp` | Reduced header-only timeout baseline for the active memory frontier. `#include <EASTL/internal/memory_uses_allocator.h>` alone still times out after `25s` under parser debug, while the smaller `#include <EASTL/internal/function_detail.h>` smoke case completes in about `15s`; progress reaches `EASTL/allocator.h` and then `EASTL/internal/function_detail.h` late in the timeout window. |
...
- `run_eastl_timeout_baseline.cmake`: reusable parser-debug timeout baseline for
  reduced EASTL header/frontier cases that still exceed the current time budget.
