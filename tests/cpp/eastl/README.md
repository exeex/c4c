...
| `eastl_vector_simple.cpp` | `SEMA` | `build/c4cll --dump-canonical -I ref/EASTL/include -I ref/EABase/include/Common tests/cpp/eastl/eastl_vector_simple.cpp` | `--parse-only` now succeeds again within the workflow timeout, so the old parser timeout and incomplete-type frontier are gone. `--dump-canonical` has narrowed to vector/allocator-only sema failures: `vector.h` comparison operators (`operator_eq` / `operator_neq`), the remaining `allocator.h:210:16` incompatible-return diagnostic, and the follow-on `vector.h:438:61` unknown `base_type::allocator_type` cast. |
