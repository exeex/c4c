...
| `eastl_utility_simple.cpp` | `SEMA` | `build/c4cll --dump-canonical -I ref/EASTL/include -I ref/EABase/include/Common tests/cpp/eastl/eastl_utility_simple.cpp` | `--parse-only` succeeds in about `10.476s`. The old `eastl::pair` piecewise delegating-helper failure (`initializer for scalar member 'pair' must have exactly one argument`) is gone. The current canonical frontier now advances further and crashes later in deeper template lowering with `SIGSEGV` instead of stopping at that constructor-init path. |
