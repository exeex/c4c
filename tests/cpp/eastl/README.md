...
| `eastl_tuple_simple.cpp` | `SEMA` | `build/c4cll --dump-canonical -I ref/EASTL/include -I ref/EABase/include/Common tests/cpp/eastl/eastl_tuple_simple.cpp` | `--parse-only` now succeeds cleanly again, and the old canonical `initializer for scalar member 'first' must have exactly one argument` failure is gone after the ctor-init member-lowering fixes. The current canonical workflow now times out deeper in the libstdc++ / tuple stack with no replacement terminal diagnostic inside the 30s policy window, so the next frontier has moved from a bounded member-init semantic error to a heavier later-stage timeout path. |
