...
| `eastl_vector_simple.cpp` | `SEMA` | `build/c4cll --dump-canonical -I ref/EASTL/include -I ref/EABase/include/Common tests/cpp/eastl/eastl_vector_simple.cpp` | `--parse-only` still succeeds within the workflow timeout. `--dump-canonical` now moves past the old free `operator_eq` / `operator_neq` conflict path and has narrowed to the allocator-only follow-up: `allocator.h:210:16` incompatible return type, then `vector.h:438:61` unknown `base_type::allocator_type` cast. |
