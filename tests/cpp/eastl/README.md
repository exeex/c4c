...
| `eastl_integer_sequence_simple.cpp` | `SEMA` | `build/c4cll --parse-only -I ref/EASTL/include -I ref/EABase/include/Common tests/cpp/eastl/eastl_integer_sequence_simple.cpp` | The nested variable-template parser blocker in `type_properties.h` is fixed. The current earliest failure is `tests/cpp/eastl/eastl_integer_sequence_simple.cpp:6:1` with `object has incomplete type: eastl::is_signed_helper`. |
| `eastl_type_traits_simple.cpp` | `SEMA` | `build/c4cll --parse-only -I ref/EASTL/include -I ref/EABase/include/Common tests/cpp/eastl/eastl_type_traits_simple.cpp` | Same `is_signed_helper` incomplete-type blocker at `tests/cpp/eastl/eastl_type_traits_simple.cpp:55:1`. The standalone workflow target remains useful historically but no longer matches the active frontier. |
