...
| `eastl_tuple_simple.cpp` | `SEMA` | `build/c4cll --dump-canonical -I ref/EASTL/include -I ref/EABase/include/Common tests/cpp/eastl/eastl_tuple_simple.cpp` | The old `/usr/include/c++/14/bits/ranges_util.h` undeclared-identifier cluster around `_M_begin`, `_M_end`, `_M_size`, `_S_store_size`, and `this` is gone after the constrained member-template conversion-operator parser fix. `--dump-canonical` now fails later with `initializer for scalar member 'first' must have exactly one argument`, so the next frontier has moved past `ranges::subrange::advance` member ownership and into a later semantic aggregate/initializer check. |
