...
bool prepared_and_bir_same_block_load_local_stored_source_match(
    const prepare::PreparedNameTables& names,
    const prepare::PreparedStackLayout& stack_layout,
    const prepare::PreparedAddressingFunction& addressing,
    const prepare::PreparedEdgePublicationSourceProducerLookups& source_producers,
  ...
  const auto prepared =
      prepare::find_prepared_same_block_load_local_stored_value_source(
          ...
          value,
          before_instruction_index);
  const auto bir = mir::find_bir_same_block_load_local_stored_value_source_identity(
      mir::BirSameBlockLoadLocalSourceRequest{
          .block = &block,
  ...
  return bir.store_local != nullptr &&
         bir.store_local->value == prepared->stored_value &&
         bir.store_memory_access.instruction_index ==
             prepared->store_instruction_index &&
         bir.load_memory_access.instruction_index ==
             prepared->load_access->inst_index &&
         bir.stored_value.name == prepared->stored_value.name &&
         bir.stored_value.type == prepared->stored_value.type;
}
...
bool prepared_and_bir_cfg_edge_publication_source_identity_match(
  ...
  const auto bir =
      mir::find_bir_cfg_edge_publication_source_identity(request);
  const bool mismatched_request =
      (request.predecessor_block != nullptr &&
       request.predecessor_label_id != c4c::kInvalidBlockLabel &&
      ...
      (request.successor_block != nullptr &&
       request.successor_label_id != c4c::kInvalidBlockLabel &&
       request.successor_block->label_id != c4c::kInvalidBlockLabel &&
       request.successor_label_id != request.successor_block->label_id);
  const auto expected_status = mismatched_request
      ? mir::BirCfgEdgePublicationSourceStatus::MismatchedRequest
      : request.destination_value == nullptr
            ? mir::BirCfgEdgePublicationSourceStatus::MissingDestinationValue
            : expected_bir_edge_status(prepared.status);
  if (bir.status != expected_status) {
    return false;
  }
...
int verify_prepared_module_lookup_reader_agreement_boundary() {
  ...
  const prepare::PreparedControlFlowFunction invalid_function_control_flow{
      .function_name = c4c::kInvalidFunctionName,
      .blocks = {return_block(entry_label)},
  };
  const auto invalid_function_agreement =
      prepare::prepared_bir_function_agreement(prepared,
                                               invalid_function_control_flow);
  if (invalid_function_agreement.available ||
      invalid_function_agreement.conflicted ||
      !prepare::make_prepared_edge_publication_source_producer_lookups(
            prepared, invalid_function_control_flow)
            .producers_by_value_name.empty()) {
    return fail("module lookup-reader should fail closed for invalid prepared function ids");
  }

  auto absent_function_prepared = prepared;
  absent_function_prepared.module.functions.clear();
  const auto absent_function_agreement =
      prepare::prepared_bir_function_agreement(absent_function_prepared,
                                               control_flow);
  if (absent_function_agreement.available ||
      absent_function_agreement.conflicted ||
      !prepare::make_prepared_edge_publication_source_producer_lookups(
            absent_function_prepared, control_flow)
            .producers_by_value_name.empty()) {
    return fail("module lookup-reader should fail closed when BIR functions are absent");
  }

  const prepare::PreparedControlFlowBlock invalid_prepared_block =
      return_block(c4c::kInvalidBlockLabel);
  const auto invalid_block_agreement = prepare::prepared_bir_block_agreement(
      prepared,
      prepared.module.functions.front(),
      invalid_prepared_block);
  if (invalid_block_agreement.available || invalid_block_agreement.conflicted) {
    return fail("module lookup-reader should fail closed for invalid prepared block ids");
  }
  ...
  const auto raw_fallback_source_producers =
      prepare::make_prepared_edge_publication_source_producer_lookups(
          raw_fallback_prepared, control_flow);
  const auto raw_fallback_producer =
      raw_fallback_source_producers.producers_by_value_name.find(value_name);
  if (raw_fallback_producer ==
          raw_fallback_source_producers.producers_by_value_name.end() ||
      raw_fallback_producer->second.block_label != entry_label ||
      raw_fallback_producer->second.kind !=
          prepare::PreparedEdgePublicationSourceProducerKind::LoadLocal) {
    return fail("public lookup-reader producer surface should preserve raw-label fallback compatibility");
  }
  const auto raw_fallback_function_lookups =
      prepare::make_prepared_function_lookups(raw_fallback_prepared,
                                              control_flow);
  if (raw_fallback_function_lookups.edge_publication_source_producers
          .producers_by_value_name.find(value_name) ==
      raw_fallback_function_lookups.edge_publication_source_producers
          .producers_by_value_name.end()) {
    return fail("public prepared aggregate should preserve raw-label fallback compatibility");
  }
  ...
  auto missing_prepared_name_prepared = prepared;
  auto& missing_prepared_name_block =
      missing_prepared_name_prepared.module.functions.front().blocks.front();
  missing_prepared_name_block.label = "entry";
  missing_prepared_name_block.label_id =
      missing_prepared_name_prepared.module.names.block_labels.intern(
          "module_only");
  const auto missing_prepared_name_label =
      prepare::prepared_bir_block_label_agreement(missing_prepared_name_prepared,
                                                  missing_prepared_name_block);
  if (missing_prepared_name_label.available ||
      !missing_prepared_name_label.conflicted ||
      prepare::compatible_prepared_bir_block_label_id(
          missing_prepared_name_prepared,
          missing_prepared_name_block) != c4c::kInvalidBlockLabel ||
      !prepare::make_prepared_edge_publication_source_producer_lookups(
            missing_prepared_name_prepared, control_flow)
            .producers_by_value_name.empty()) {
    return fail("module lookup-reader should fail closed when structured labels miss the prepared name table");
  }
...
int verify_current_block_join_parallel_copy_source_query() {
  ...
  auto require_selected_join_route5_fallback =
      [&](const prepare::PreparedCurrentBlockJoinParallelCopySourceFacts& facts,
        ...
        if (facts.status !=
            ...
            !facts.facts[0].source_is_incoming_expression ||
            !facts.facts[0].destination_is_source_value ||
            facts.facts[0].source_is_source_value) {
          return fail(message);
        }
        return 0;
      };
  ...
  auto no_source_route5_join_index = route5_join_index;
  no_source_route5_join_index.join_records.front().available = false;
  no_source_route5_join_index.join_records.front().status =
      bir::Route5PublicationStatus::NoSource;
  const auto no_source_route5_supported_query =
      prepare::prepare_current_block_join_parallel_copy_source_facts(
          prepare::PreparedCurrentBlockJoinParallelCopySourceQueryInputs{
              .names = &names,
              .regalloc = &regalloc,
  ...
  auto source_mismatch_route5_join_index = route5_join_index;
  source_mismatch_route5_join_index.join_records.front().source_value_name =
      "%current.other_source";
  const auto source_mismatch_route5_supported_query =
      prepare::prepare_current_block_join_parallel_copy_source_facts(
          prepare::PreparedCurrentBlockJoinParallelCopySourceQueryInputs{
              .names = &names,
              .regalloc = &regalloc,
  ...
  auto destination_type_mismatch_route5_join_index = route5_join_index;
  destination_type_mismatch_route5_join_index.join_records.front()
      .destination_value_type = bir::TypeKind::I64;
  const auto destination_type_mismatch_route5_supported_query =
      prepare::prepare_current_block_join_parallel_copy_source_facts(
          prepare::PreparedCurrentBlockJoinParallelCopySourceQueryInputs{
              .names = &names,
              .regalloc = &regalloc,
  ...
  auto memory_source_route5_join_index = route5_join_index;
  memory_source_route5_join_index.join_records.front().available = true;
  memory_source_route5_join_index.join_records.front().status =
      bir::Route5PublicationStatus::MemorySource;
  memory_source_route5_join_index.join_records.front().source_producer_kind =
      bir::Route5PublicationSourceKind::LoadLocal;
  const auto memory_source_route5_supported_query =
      prepare::prepare_current_block_join_parallel_copy_source_facts(
          prepare::PreparedCurrentBlockJoinParallelCopySourceQueryInputs{
              .names = &names,
              .regalloc = &regalloc,
...
int verify_edge_publication_source_producer_facts() {
  ...
  const auto stale_destination_id_facts = prepare::prepare_edge_copy_source_facts(
      &lookups.edge_publications, predecessor_label, successor_label, 999);
  if (!bir_load_edge ||
      ...
      bir_load_edge.source_producer_block_label_id !=
          load_publication->source_producer_block_label.value_or(
              c4c::kInvalidBlockLabel) ||
      bir_load_edge.destination_value !=
          &std::get<bir::PhiInst>(successor_block.insts[0]).result ||
      bir_load_edge.source_value !=
          &std::get<bir::PhiInst>(successor_block.insts[0]).incomings[0].value ||
      bir_load_edge.destination_value_identity.value !=
          &std::get<bir::PhiInst>(successor_block.insts[0]).result ||
      bir_load_edge.source_value_identity.value !=
          &std::get<bir::PhiInst>(successor_block.insts[0]).incomings[0].value ||
      bir_load_edge.destination_value_identity.name != load_destination.name ||
      bir_load_edge.source_value_identity.name != loaded.name ||
  ...
  if (stale_destination_id_facts.status ==
          prepare::PreparedEdgeCopySourceFactsStatus::Available ||
      !mir::find_bir_cfg_edge_publication_source_identity(load_request)) {
    return fail("stale prepared destination IDs must not become BIR-resolved destination identity");
  }
  ...
  const auto missing_destination_value =
  ...
  const auto bir_missing_destination =
      mir::find_bir_cfg_edge_publication_source_identity(
          make_bir_edge_publication_source_request(
              predecessor_block, predecessor_label, successor_block,
              successor_label, missing_destination_value));
  ...
  const auto bir_missing_source =
      mir::find_bir_cfg_edge_publication_source_identity(
          make_bir_edge_publication_source_request(
              predecessor_block, predecessor_label, successor_block,
              successor_label, unavailable_destination));
  ...
  auto duplicate_destination_successor = successor_block;
  duplicate_destination_successor.insts.push_back(
      std::get<bir::PhiInst>(successor_block.insts[0]));
  const auto duplicate_destination_query =
      mir::find_bir_cfg_edge_publication_source_identity(
          make_bir_edge_publication_source_request(
              predecessor_block, predecessor_label,
              duplicate_destination_successor, successor_label,
              load_destination));
  if (duplicate_destination_query ||
      duplicate_destination_query.status !=
          mir::BirCfgEdgePublicationSourceStatus::AmbiguousPublication) {
    return fail("public BIR CFG query should reject duplicate exact destination phis");
  }
  auto conflicting_incoming_successor = successor_block;
  auto& conflicting_incoming_phi =
      std::get<bir::PhiInst>(conflicting_incoming_successor.insts[0]);
  conflicting_incoming_phi.incomings[0].label_id = c4c::BlockLabelId{779};
  const auto conflicting_incoming_query =
      mir::find_bir_cfg_edge_publication_source_identity(
          make_bir_edge_publication_source_request(
              predecessor_block, predecessor_label,
              conflicting_incoming_successor, successor_label,
              load_destination));
  if (conflicting_incoming_query ||
      conflicting_incoming_query.status !=
          mir::BirCfgEdgePublicationSourceStatus::MissingSourceValue) {
    return fail("public BIR CFG query should not let matching text rescue a conflicting incoming id");
  }
  auto duplicate_incoming_successor = successor_block;
  auto& duplicate_incoming_phi =
      std::get<bir::PhiInst>(duplicate_incoming_successor.insts[0]);
  duplicate_incoming_phi.incomings.push_back(
      duplicate_incoming_phi.incomings.front());
  const auto duplicate_incoming_query =
      mir::find_bir_cfg_edge_publication_source_identity(
          make_bir_edge_publication_source_request(
              predecessor_block, predecessor_label,
              duplicate_incoming_successor, successor_label,
              load_destination));
  if (duplicate_incoming_query ||
      duplicate_incoming_query.status !=
          mir::BirCfgEdgePublicationSourceStatus::AmbiguousPublication) {
    return fail("public BIR CFG query should classify duplicate matching incomings as ambiguous");
  }
  const auto wrong_type_destination =
      bir::Value::named(bir::TypeKind::I64, load_destination.name);
  const auto wrong_type_query =
      mir::find_bir_cfg_edge_publication_source_identity(
          make_bir_edge_publication_source_request(
              predecessor_block, predecessor_label, successor_block,
              successor_label, wrong_type_destination));
  if (wrong_type_query ||
      wrong_type_query.status !=
          mir::BirCfgEdgePublicationSourceStatus::MismatchedRequest) {
    return fail("public BIR CFG query should classify destination type mismatch separately from ambiguity");
  }
  ...
  if (!indexed_bir_load_edge ||
      indexed_bir_load_edge.status !=
          mir::BirCfgEdgePublicationSourceStatus::Available ||
      indexed_bir_load_edge.source_producer.kind !=
          mir::SameBlockProducerKind::LoadLocal ||
  ...
  const auto indexed_bir_wrong_predecessor =
      mir::find_bir_cfg_edge_publication_source_identity(
          make_bir_edge_publication_source_request(
              route5_wrong_predecessor, c4c::BlockLabelId{777},
              successor_block, successor_label, load_destination));
...
int verify_route3_load_local_stored_value_source_matches_prepared_or_falls_back() {
  prepare::PreparedNameTables names;
  ...
  exact_block.label_id = block_label;
  exact_block.insts.push_back(bir::StoreLocalInst{
      .slot_name = "local0",
      .slot_id = static_cast<c4c::SlotNameId>(slot_id),
  ...
  exact_block.insts.push_back(bir::LoadLocalInst{
  ...
  const auto* load = std::get_if<bir::LoadLocalInst>(&exact_block.insts[1]);
  if (load == nullptr) {
    return fail("stored-source fixture should contain a load-local producer");
  }
  prepare::PreparedEdgePublicationSourceProducerLookups source_producers;
  source_producers.producers_by_value_name.emplace(
      loaded_name,
  ...
  auto* missing_store =
      std::get_if<bir::StoreLocalInst>(&missing_route3_range_block.insts[0]);
  auto* missing_load =
      std::get_if<bir::LoadLocalInst>(&missing_route3_range_block.insts[1]);
  if (missing_store == nullptr || missing_load == nullptr) {
    return fail("fallback stored-source fixture should contain store/load locals");
  }
  if (!missing_store->address.has_value() || !missing_load->address.has_value()) {
    return fail("fallback stored-source fixture should carry BIR memory addresses");
  }
  missing_store->address->size_bytes = 0;
  missing_load->address->size_bytes = 0;
  auto missing_route3_range_producers = source_producers;
  missing_route3_range_producers.producers_by_value_name[loaded_name].load_local =
      missing_load;
  const auto prepared =
      prepare::find_prepared_same_block_load_local_stored_value_source(
          names,
  ...
  const auto route3 =
      mir::find_bir_same_block_load_local_stored_value_source_identity(
          mir::BirSameBlockLoadLocalSourceRequest{
              .block = &missing_route3_range_block,
              .block_label = missing_route3_range_block.label,
              .root_value_name = "%loaded",
              .root_value_type = bir::TypeKind::I64,
              .before_instruction_index = missing_route3_range_block.insts.size(),
          });
...
int verify_bir_block_entry_publication_identity_lookup() {
  ...
  if (prepared_missing.status !=
      ...
      route4_unpublished_phi.destination_value_name_id !=
          prepared_missing.destination_value_name ||
      bir_unpublished_phi.status !=
          prepare::PreparedCurrentBlockEntryPublicationStatus::MissingPublication) {
    return fail("common identity should not reconstruct an unpublished PHI");
  }
  ...
  if (prepared_stack_only.status !=
      ...
      route4_stack_phi.status !=
          bir::Route4PublicationAvailabilityStatus::Available ||
      bir_stack_phi.status != prepare::PreparedCurrentBlockEntryPublicationStatus::PublicationUnavailable) {
    return fail("common identity should preserve unavailable prepared storage status");
  }
  ...
  const auto bir_missing_destination_key =
      mir::find_bir_block_entry_publication_identity(
          prepare::PreparedCurrentBlockEntryPublication{
              .status = prepare::PreparedCurrentBlockEntryPublicationStatus::MissingDestinationValue});
  if (bir_missing_destination_key.available ||
      bir_missing_destination_key.status !=
          prepare::PreparedCurrentBlockEntryPublicationStatus::MissingDestinationValue) {
  ...
  const auto bir_destination_type_mismatch =
      mir::find_bir_block_entry_publication_identity(
          prepared_destination_type_mismatch);
