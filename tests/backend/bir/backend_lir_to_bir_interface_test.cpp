...
void test_direct_scalar_fneg_parameter_authority_receipt_and_rejections() {
  const auto make_module = [] {
    lir::LirModule module;
    module.link_name_texts = std::make_shared<c4c::TextTable>();
    module.link_names.attach_text_table(module.link_name_texts.get());
    const auto owner = module.link_names.intern("direct_scalar_fneg_parameter_owner");
    lir::LirFunction function;
    function.name = "direct_scalar_fneg_parameter_owner";
    function.return_type = scalar_type(c4c::TB_VOID);
    function.signature_return_type_ref = lir::LirTypeRef("void");
    function.link_name_id = owner;
    function.params.emplace_back("%presentation-only", scalar_type(c4c::TB_FLOAT));
    function.signature_params.push_back({"%presentation-only-signature", scalar_type(c4c::TB_FLOAT), false});
    function.signature_param_type_refs.push_back(lir::LirTypeRef("float"));
    function.native_body_parameter_definitions.push_back(
        {lir::LirValueId{81}, 0, lir::LirTypeRef("float"), owner,
         lir::LirNativeBodyParameterAbi::DirectScalar});
    lir::LirBlock entry = return_block(0, "entry");
    entry.insts.push_back(lir::LirBinOp{
        lir::LirOperand::ssa("%fneg-result", lir::LirValueId{82}), "fneg",
        lir::LirTypeRef("float"),
        lir::LirOperand::ssa("%presentation-only-lhs", lir::LirValueId{81}),
        lir::LirOperand{},
  ...
  const auto raw = bir::lower_lir_to_raw_bir(module);
  expect(raw.has_value() && bir::FoundationVerifier::verify(raw.value()).ok(),
         "direct-scalar fneg parameter authority must publish verified Raw BIR: " +
             (raw.has_value() ? "foundation verifier rejected it" : raw.error().detail));
  const auto view = raw.value().view();
  const auto function = view.function(view.functions()[0]).value();
  const auto instruction =
      function.instruction(function.instructions(function.blocks()[0]).value()[0]).value();
  expect(instruction.binary() &&
             instruction.binary()->opcode == bir::BinaryOpcode::FNeg &&
             instruction.binary()->type ==
                 bir::Type{bir::TypeKind::Floating, 32, "float"} &&
             instruction.binary()->direct_scalar_lhs &&
             instruction.binary()->direct_scalar_lhs->source_value_id == 81 &&
         ...
         "Raw BIR must retain the exact typed fneg LHS parameter authority");
  ...
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[0]).scalar_lhs_parameter_authority.reset(); }, "missing fneg authority must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[0]).scalar_lhs_parameter_authority->value = lir::LirValueId::invalid(); }, "invalid fneg authority must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].native_body_parameter_definitions.push_back(candidate.functions[0].native_body_parameter_definitions[0]); }, "duplicate fneg parameter definition must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[0]).scalar_lhs_parameter_authority->owner = candidate.link_names.intern("foreign_fneg_owner"); }, "foreign fneg owner must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[0]).scalar_lhs_parameter_authority->parameter_index = 1; }, "fneg parameter index mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[0]).scalar_lhs_parameter_authority->type = lir::LirTypeRef("double"); }, "fneg parameter type mismatch must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].native_body_parameter_definitions[0].type = lir::LirTypeRef("double"); }, "fneg definition scalar type mismatch must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].signature_param_type_refs[0] = lir::LirTypeRef("double"); }, "fneg signature parameter type mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[0]).scalar_lhs_parameter_authority->abi = lir::LirNativeBodyParameterAbi::DirectPointer; }, "fneg ABI mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[0]).scalar_lhs_parameter_authority->role = lir::LirScalarBinaryParameterRole::Rhs; }, "fneg role mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[0]).opcode = "fadd"; }, "non-fneg opcode must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[0]).lhs = lir::LirOperand::ssa("%other", lir::LirValueId{83}); }, "fneg LHS relation mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[0]).rhs = lir::LirOperand::ssa("%rhs", lir::LirValueId{84}); }, "fneg populated RHS must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].blocks[0].insts.push_back(candidate.functions[0].blocks[0].insts[0]); }, "second fneg authority row must reject transactionally");
}

void test_direct_scalar_binary_fmul_lhs_parameter_authority_receipt_and_rejections() {
  const auto make_module = [] {
    lir::LirModule module;
    module.link_name_texts = std::make_shared<c4c::TextTable>();
    module.link_names.attach_text_table(module.link_name_texts.get());
    const auto owner = module.link_names.intern("direct_scalar_fmul_lhs_parameter_owner");
    lir::LirFunction function;
    function.name = "direct_scalar_fmul_lhs_parameter_owner";
    function.return_type = scalar_type(c4c::TB_VOID);
    function.signature_return_type_ref = lir::LirTypeRef("void");
    function.link_name_id = owner;
    function.params.emplace_back("%presentation-only", scalar_type(c4c::TB_FLOAT));
    ...
    function.signature_param_type_refs.push_back(lir::LirTypeRef("float"));
    function.native_body_parameter_definitions.push_back(
        {lir::LirValueId{91}, 0, lir::LirTypeRef("float"), owner,
         lir::LirNativeBodyParameterAbi::DirectScalar});
    lir::LirBlock entry = return_block(0, "entry");
    entry.insts.push_back(lir::LirConstFloat{
        lir::LirValueId{92}, scalar_type(c4c::TB_FLOAT), 2.5});
    entry.insts.push_back(lir::LirBinOp{
        lir::LirOperand::ssa("%fmul-result", lir::LirValueId{93}), "fmul",
        lir::LirTypeRef("float"),
        lir::LirOperand::ssa("%presentation-only-lhs", lir::LirValueId{91}),
        lir::LirOperand::ssa("%rhs", lir::LirValueId{92}),
        lir::LirScalarBinaryLhsParameterAuthority{
            lir::LirValueId{91}, 0, lir::LirTypeRef("float"), owner,
            lir::LirNativeBodyParameterAbi::DirectScalar,
            lir::LirScalarBinaryParameterRole::Lhs}});
  ...
  expect(raw.has_value() && verified.ok(),
         "direct-scalar binary fmul LHS parameter authority must publish verified Raw BIR: " +
  ...
  const auto function = view.function(view.functions()[0]).value();
  const auto insts = function.instructions(function.blocks()[0]).value();
  expect(insts.size() == 1, "Raw BIR must retain only the selected fmul instruction");
  const auto instruction = function.instruction(insts[0]).value();
  expect(instruction.binary() &&
             instruction.binary()->opcode == bir::BinaryOpcode::FMul &&
             instruction.binary()->type ==
                 bir::Type{bir::TypeKind::F32, 32, "float"} &&
             instruction.binary()->direct_scalar_lhs &&
             instruction.binary()->direct_scalar_lhs->source_value_id == 91 &&
             instruction.binary()->direct_scalar_lhs->parameter_index == 0 &&
             instruction.binary()->direct_scalar_lhs->scalar_type ==
                 bir::Type{bir::TypeKind::F32, 32, "float"} &&
             !instruction.binary()->direct_scalar_rhs &&
             ...
             function.value(instruction.operands()[1]).value().type ==
                 bir::Type{bir::TypeKind::F32, 32, "float"},
         "Raw BIR must retain the exact typed binary fmul LHS parameter authority");
  ...
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_lhs_parameter_authority.reset(); }, "missing fmul LHS authority must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_lhs_parameter_authority->value = lir::LirValueId::invalid(); }, "invalid fmul LHS authority must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].native_body_parameter_definitions.push_back(candidate.functions[0].native_body_parameter_definitions[0]); }, "duplicate fmul LHS parameter definition must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_lhs_parameter_authority->owner = candidate.link_names.intern("foreign_fmul_lhs_owner"); }, "foreign fmul LHS owner must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_lhs_parameter_authority->parameter_index = 1; }, "fmul LHS parameter index mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_lhs_parameter_authority->type = lir::LirTypeRef("double"); }, "fmul LHS authority type mismatch must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].native_body_parameter_definitions[0].type = lir::LirTypeRef("double"); }, "fmul LHS definition type mismatch must reject transactionally");
  ...
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).opcode = "fdiv"; }, "non-fmul LHS authority consumer must reject transactionally");
...
void test_direct_scalar_binary_fadd_lhs_parameter_authority_receipt_and_rejections() {
  ...
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).opcode = "fdiv"; }, "non-fadd LHS authority consumer must reject transactionally");
...
void test_direct_scalar_binary_fsub_lhs_parameter_authority_receipt_and_rejections() {
  const auto make_module = [] {
    lir::LirModule module;
    module.link_name_texts = std::make_shared<c4c::TextTable>();
    module.link_names.attach_text_table(module.link_name_texts.get());
    const auto owner = module.link_names.intern("direct_scalar_fsub_lhs_parameter_owner");
    lir::LirFunction function;
    function.name = "direct_scalar_fsub_lhs_parameter_owner";
    function.return_type = scalar_type(c4c::TB_VOID);
    function.signature_return_type_ref = lir::LirTypeRef("void");
    function.link_name_id = owner;
    function.params.emplace_back("%presentation-only", scalar_type(c4c::TB_DOUBLE));
    ...
    function.signature_param_type_refs.push_back(lir::LirTypeRef("double"));
    function.native_body_parameter_definitions.push_back(
        {lir::LirValueId{131}, 0, lir::LirTypeRef("double"), owner,
         lir::LirNativeBodyParameterAbi::DirectScalar});
    lir::LirBlock entry = return_block(0, "entry");
    entry.insts.push_back(lir::LirConstFloat{
        lir::LirValueId{132}, scalar_type(c4c::TB_DOUBLE), 2.5});
    entry.insts.push_back(lir::LirBinOp{
        lir::LirOperand::ssa("%fsub-result", lir::LirValueId{133}), "fsub",
        lir::LirTypeRef("double"),
        lir::LirOperand::ssa("%presentation-only-lhs", lir::LirValueId{131}),
        lir::LirOperand::ssa("%rhs", lir::LirValueId{132}),
        lir::LirScalarBinaryLhsParameterAuthority{
            lir::LirValueId{131}, 0, lir::LirTypeRef("double"), owner,
            lir::LirNativeBodyParameterAbi::DirectScalar,
            lir::LirScalarBinaryParameterRole::Lhs}});
  ...
  const auto verified = raw.has_value()
      ...
      : bir::VerificationResult{};
  expect(raw.has_value() && verified.ok(),
         "direct-scalar binary fsub LHS parameter authority must publish verified Raw BIR: " +
             (raw.has_value() ? (verified.errors.empty() ? "foundation verifier rejected it"
                                                         : verified.errors[0].message)
                              : (!raw.error().verification_errors.empty()
                                     ? raw.error().verification_errors[0].message
  ...
  const auto function = view.function(view.functions()[0]).value();
  const auto insts = function.instructions(function.blocks()[0]).value();
  expect(insts.size() == 1, "Raw BIR must retain only the selected fsub instruction");
  const auto instruction = function.instruction(insts[0]).value();
  expect(instruction.binary() &&
             instruction.binary()->opcode == bir::BinaryOpcode::FSub &&
             instruction.binary()->type ==
                 bir::Type{bir::TypeKind::F64, 64, "double"} &&
             instruction.binary()->direct_scalar_lhs &&
             instruction.binary()->direct_scalar_lhs->source_value_id == 131 &&
             instruction.binary()->direct_scalar_lhs->parameter_index == 0 &&
             instruction.binary()->direct_scalar_lhs->scalar_type ==
                 bir::Type{bir::TypeKind::F64, 64, "double"} &&
             !instruction.binary()->direct_scalar_rhs &&
             ...
             function.value(instruction.operands()[1]).value().type ==
                 bir::Type{bir::TypeKind::F64, 64, "double"},
         "Raw BIR must retain the exact typed binary fsub LHS parameter authority");
  ...
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_lhs_parameter_authority.reset(); }, "missing fsub LHS authority must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_lhs_parameter_authority->value = lir::LirValueId::invalid(); }, "invalid fsub LHS authority must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].native_body_parameter_definitions.push_back(candidate.functions[0].native_body_parameter_definitions[0]); }, "duplicate fsub LHS parameter definition must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_lhs_parameter_authority->owner = candidate.link_names.intern("foreign_fsub_lhs_owner"); }, "foreign fsub LHS owner must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_lhs_parameter_authority->parameter_index = 1; }, "fsub LHS parameter index mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_lhs_parameter_authority->type = lir::LirTypeRef("float"); }, "fsub LHS authority type mismatch must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].native_body_parameter_definitions[0].type = lir::LirTypeRef("float"); }, "fsub LHS definition type mismatch must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].signature_param_type_refs[0] = lir::LirTypeRef("float"); }, "fsub LHS signature type mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_lhs_parameter_authority->abi = lir::LirNativeBodyParameterAbi::DirectPointer; }, "fsub LHS ABI mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_lhs_parameter_authority->role = lir::LirScalarBinaryParameterRole::Rhs; }, "fsub LHS role mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).opcode = "fdiv"; }, "non-fsub LHS authority consumer must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).lhs = lir::LirOperand::ssa("%other", lir::LirValueId{134}); }, "fsub LHS identity mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).type_str = lir::LirTypeRef("float"); }, "fsub result type mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).rhs = lir::LirOperand::ssa("%missing-rhs", lir::LirValueId{134}); }, "fsub nonselected RHS must be a current-function scalar");
  rejected([](auto& candidate) { std::get<lir::LirConstFloat>(candidate.functions[0].blocks[0].insts[0]).type = scalar_type(c4c::TB_FLOAT); }, "fsub nonselected RHS type mismatch must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].blocks[0].insts.push_back(candidate.functions[0].blocks[0].insts[1]); }, "second fsub LHS authority row must reject transactionally");
}

void test_direct_scalar_binary_fadd_rhs_parameter_authority_receipt_and_rejections() {
  const auto make_module = [] {
    lir::LirModule module;
    module.link_name_texts = std::make_shared<c4c::TextTable>();
    module.link_names.attach_text_table(module.link_name_texts.get());
    const auto owner = module.link_names.intern("direct_scalar_fadd_rhs_parameter_owner");
    lir::LirFunction function;
    function.name = "direct_scalar_fadd_rhs_parameter_owner";
    function.return_type = scalar_type(c4c::TB_VOID);
    function.signature_return_type_ref = lir::LirTypeRef("void");
    function.link_name_id = owner;
    function.params.emplace_back("%presentation-only", scalar_type(c4c::TB_DOUBLE));
    ...
    function.signature_param_type_refs.push_back(lir::LirTypeRef("double"));
    function.native_body_parameter_definitions.push_back(
        {lir::LirValueId{121}, 0, lir::LirTypeRef("double"), owner,
         lir::LirNativeBodyParameterAbi::DirectScalar});
    lir::LirBlock entry = return_block(0, "entry");
    entry.insts.push_back(lir::LirConstFloat{
        lir::LirValueId{122}, scalar_type(c4c::TB_DOUBLE), 2.5});
    entry.insts.push_back(lir::LirBinOp{
        lir::LirOperand::ssa("%fadd-result", lir::LirValueId{123}), "fadd",
        lir::LirTypeRef("double"),
        lir::LirOperand::ssa("%lhs", lir::LirValueId{122}),
        lir::LirOperand::ssa("%presentation-only-rhs", lir::LirValueId{121}),
        std::nullopt,
        lir::LirScalarBinaryRhsParameterAuthority{
            lir::LirValueId{121}, 0, lir::LirTypeRef("double"), owner,
            lir::LirNativeBodyParameterAbi::DirectScalar,
            lir::LirScalarBinaryParameterRole::Rhs}});
    function.blocks.push_back(std::move(entry));
    function.entry = lir::LirBlockId{0};
    module.functions.push_back(std::move(function));
    return module;
  ...
  const auto verified = raw.has_value()
      ...
      : bir::VerificationResult{};
  expect(raw.has_value() && verified.ok(),
         "direct-scalar binary fadd RHS parameter authority must publish verified Raw BIR: " +
             (raw.has_value() ? (verified.errors.empty() ? "foundation verifier rejected it"
                                                         : verified.errors[0].message)
                              : (!raw.error().verification_errors.empty()
                                     ? raw.error().verification_errors[0].message
  ...
  const auto function = view.function(view.functions()[0]).value();
  const auto insts = function.instructions(function.blocks()[0]).value();
  expect(insts.size() == 1, "Raw BIR must retain only the selected fadd RHS instruction");
  const auto instruction = function.instruction(insts[0]).value();
  expect(instruction.binary() &&
             instruction.binary()->opcode == bir::BinaryOpcode::FAdd &&
             instruction.binary()->type ==
             ...
             !instruction.binary()->direct_scalar_lhs &&
             instruction.binary()->direct_scalar_rhs &&
             instruction.binary()->direct_scalar_rhs->source_value_id == 121 &&
             instruction.binary()->direct_scalar_rhs->parameter_index == 0 &&
             instruction.binary()->direct_scalar_rhs->scalar_type ==
                 bir::Type{bir::TypeKind::F64, 64, "double"} &&
             instruction.operands().size() == 2 &&
             function.value(instruction.operands()[0]).value().type ==
                 bir::Type{bir::TypeKind::F64, 64, "double"} &&
             instruction.operands()[1] == function.parameters()[0],
         "Raw BIR must retain the exact typed binary fadd RHS parameter authority");
  ...
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority.reset(); }, "missing fadd RHS authority must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->value = lir::LirValueId::invalid(); }, "invalid fadd RHS authority must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].native_body_parameter_definitions.push_back(candidate.functions[0].native_body_parameter_definitions[0]); }, "duplicate fadd RHS parameter definition must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->owner = candidate.link_names.intern("foreign_fadd_rhs_owner"); }, "foreign fadd RHS owner must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->parameter_index = 1; }, "fadd RHS parameter index mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->type = lir::LirTypeRef("float"); }, "fadd RHS authority type mismatch must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].native_body_parameter_definitions[0].type = lir::LirTypeRef("float"); }, "fadd RHS definition type mismatch must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].signature_param_type_refs[0] = lir::LirTypeRef("float"); }, "fadd RHS signature type mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->abi = lir::LirNativeBodyParameterAbi::DirectPointer; }, "fadd RHS ABI mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->role = lir::LirScalarBinaryParameterRole::Lhs; }, "fadd RHS role mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).opcode = "fdiv"; }, "non-fadd RHS authority consumer must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).rhs = lir::LirOperand::ssa("%other", lir::LirValueId{124}); }, "fadd RHS identity mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).type_str = lir::LirTypeRef("float"); }, "fadd RHS result type mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).lhs = lir::LirOperand::ssa("%missing-lhs", lir::LirValueId{124}); }, "fadd nonselected LHS must be a current-function scalar");
  rejected([](auto& candidate) { std::get<lir::LirConstFloat>(candidate.functions[0].blocks[0].insts[0]).type = scalar_type(c4c::TB_FLOAT); }, "fadd nonselected LHS type mismatch must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].blocks[0].insts.push_back(candidate.functions[0].blocks[0].insts[1]); }, "second fadd RHS authority row must reject transactionally");
}
...
void test_direct_scalar_binary_fmul_rhs_parameter_authority_receipt_and_rejections() {
  const auto make_module = [] {
    ...
    module.link_name_texts = std::make_shared<c4c::TextTable>();
    module.link_names.attach_text_table(module.link_name_texts.get());
    const auto owner = module.link_names.intern("direct_scalar_fmul_rhs_parameter_owner");
    lir::LirFunction function;
    function.name = "direct_scalar_fmul_rhs_parameter_owner";
    function.return_type = scalar_type(c4c::TB_VOID);
    function.signature_return_type_ref = lir::LirTypeRef("void");
    function.link_name_id = owner;
    function.params.emplace_back("%presentation-only", scalar_type(c4c::TB_DOUBLE));
    ...
    function.signature_param_type_refs.push_back(lir::LirTypeRef("double"));
    function.native_body_parameter_definitions.push_back(
        {lir::LirValueId{101}, 0, lir::LirTypeRef("double"), owner,
         lir::LirNativeBodyParameterAbi::DirectScalar});
    lir::LirBlock entry = return_block(0, "entry");
    entry.insts.push_back(lir::LirConstFloat{
        lir::LirValueId{102}, scalar_type(c4c::TB_DOUBLE), 2.5});
    entry.insts.push_back(lir::LirBinOp{
        lir::LirOperand::ssa("%fmul-result", lir::LirValueId{103}), "fmul",
        lir::LirTypeRef("double"),
        lir::LirOperand::ssa("%lhs", lir::LirValueId{102}),
        lir::LirOperand::ssa("%presentation-only-rhs", lir::LirValueId{101}),
        std::nullopt,
        lir::LirScalarBinaryRhsParameterAuthority{
            lir::LirValueId{101}, 0, lir::LirTypeRef("double"), owner,
            lir::LirNativeBodyParameterAbi::DirectScalar,
  ...
  const auto verified = raw.has_value()
      ...
      : bir::VerificationResult{};
  expect(raw.has_value() && verified.ok(),
         "direct-scalar binary fmul RHS parameter authority must publish verified Raw BIR: " +
             (raw.has_value() ? (verified.errors.empty() ? "foundation verifier rejected it"
                                                         : verified.errors[0].message)
                              : (!raw.error().verification_errors.empty()
                                     ? raw.error().verification_errors[0].message
  ...
  const auto function = view.function(view.functions()[0]).value();
  const auto insts = function.instructions(function.blocks()[0]).value();
  expect(insts.size() == 1, "Raw BIR must retain only the selected fmul RHS instruction");
  const auto instruction = function.instruction(insts[0]).value();
  expect(instruction.binary() &&
             instruction.binary()->opcode == bir::BinaryOpcode::FMul &&
             instruction.binary()->type ==
             ...
             !instruction.binary()->direct_scalar_lhs &&
             instruction.binary()->direct_scalar_rhs &&
             instruction.binary()->direct_scalar_rhs->source_value_id == 101 &&
             instruction.binary()->direct_scalar_rhs->parameter_index == 0 &&
             instruction.binary()->direct_scalar_rhs->scalar_type ==
                 bir::Type{bir::TypeKind::F64, 64, "double"} &&
             instruction.operands().size() == 2 &&
             function.value(instruction.operands()[0]).value().type ==
                 bir::Type{bir::TypeKind::F64, 64, "double"} &&
             instruction.operands()[1] == function.parameters()[0],
         "Raw BIR must retain the exact typed binary fmul RHS parameter authority");
  ...
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority.reset(); }, "missing fmul RHS authority must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->value = lir::LirValueId::invalid(); }, "invalid fmul RHS authority must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].native_body_parameter_definitions.push_back(candidate.functions[0].native_body_parameter_definitions[0]); }, "duplicate fmul RHS parameter definition must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->owner = candidate.link_names.intern("foreign_fmul_rhs_owner"); }, "foreign fmul RHS owner must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->parameter_index = 1; }, "fmul RHS parameter index mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->type = lir::LirTypeRef("float"); }, "fmul RHS authority type mismatch must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].native_body_parameter_definitions[0].type = lir::LirTypeRef("float"); }, "fmul RHS definition type mismatch must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].signature_param_type_refs[0] = lir::LirTypeRef("float"); }, "fmul RHS signature type mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->abi = lir::LirNativeBodyParameterAbi::DirectPointer; }, "fmul RHS ABI mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->role = lir::LirScalarBinaryParameterRole::Lhs; }, "fmul RHS role mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).opcode = "fdiv"; }, "non-fmul RHS authority consumer must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).rhs = lir::LirOperand::ssa("%other", lir::LirValueId{104}); }, "fmul RHS identity mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).type_str = lir::LirTypeRef("float"); }, "fmul RHS result type mismatch must reject transactionally");
  rejected([](auto& candidate) {
    auto& op = std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]);
    op.scalar_lhs_parameter_authority = lir::LirScalarBinaryLhsParameterAuthority{
        lir::LirValueId{102}, 0, lir::LirTypeRef("double"),
        candidate.functions[0].link_name_id,
        lir::LirNativeBodyParameterAbi::DirectScalar,
        lir::LirScalarBinaryParameterRole::Lhs};
  }, "fmul RHS selected-LHS authority must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).lhs = lir::LirOperand::ssa("%missing-lhs", lir::LirValueId{104}); }, "fmul nonselected LHS must be a current-function scalar");
  rejected([](auto& candidate) { std::get<lir::LirConstFloat>(candidate.functions[0].blocks[0].insts[0]).type = scalar_type(c4c::TB_FLOAT); }, "fmul nonselected LHS type mismatch must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].blocks[0].insts.push_back(candidate.functions[0].blocks[0].insts[1]); }, "second fmul RHS authority row must reject transactionally");
}

void test_direct_scalar_binary_fsub_rhs_parameter_authority_receipt_and_rejections() {
  const auto make_module = [] {
    lir::LirModule module;
    module.link_name_texts = std::make_shared<c4c::TextTable>();
    module.link_names.attach_text_table(module.link_name_texts.get());
    const auto owner = module.link_names.intern("direct_scalar_fsub_rhs_parameter_owner");
    lir::LirFunction function;
    function.name = "direct_scalar_fsub_rhs_parameter_owner";
    function.return_type = scalar_type(c4c::TB_VOID);
    function.signature_return_type_ref = lir::LirTypeRef("void");
    function.link_name_id = owner;
    function.params.emplace_back("%presentation-only", scalar_type(c4c::TB_DOUBLE));
    ...
    function.signature_param_type_refs.push_back(lir::LirTypeRef("double"));
    function.native_body_parameter_definitions.push_back(
        {lir::LirValueId{141}, 0, lir::LirTypeRef("double"), owner,
         lir::LirNativeBodyParameterAbi::DirectScalar});
    lir::LirBlock entry = return_block(0, "entry");
    entry.insts.push_back(lir::LirConstFloat{
        lir::LirValueId{142}, scalar_type(c4c::TB_DOUBLE), 2.5});
    entry.insts.push_back(lir::LirBinOp{
        lir::LirOperand::ssa("%fsub-result", lir::LirValueId{143}), "fsub",
        lir::LirTypeRef("double"),
        lir::LirOperand::ssa("%lhs", lir::LirValueId{142}),
        lir::LirOperand::ssa("%presentation-only-rhs", lir::LirValueId{141}),
        std::nullopt,
        lir::LirScalarBinaryRhsParameterAuthority{
            lir::LirValueId{141}, 0, lir::LirTypeRef("double"), owner,
            lir::LirNativeBodyParameterAbi::DirectScalar,
  ...
  const auto verified = raw.has_value()
      ...
      : bir::VerificationResult{};
  expect(raw.has_value() && verified.ok(),
         "direct-scalar binary fsub RHS parameter authority must publish verified Raw BIR: " +
             (raw.has_value() ? (verified.errors.empty() ? "foundation verifier rejected it"
                                                         : verified.errors[0].message)
                              : (!raw.error().verification_errors.empty()
                                     ? raw.error().verification_errors[0].message
  ...
  const auto function = view.function(view.functions()[0]).value();
  const auto insts = function.instructions(function.blocks()[0]).value();
  expect(insts.size() == 1, "Raw BIR must retain only the selected fsub RHS instruction");
  const auto instruction = function.instruction(insts[0]).value();
  expect(instruction.binary() &&
             instruction.binary()->opcode == bir::BinaryOpcode::FSub &&
             instruction.binary()->type ==
             ...
             !instruction.binary()->direct_scalar_lhs &&
             instruction.binary()->direct_scalar_rhs &&
             instruction.binary()->direct_scalar_rhs->source_value_id == 141 &&
             instruction.binary()->direct_scalar_rhs->parameter_index == 0 &&
             instruction.binary()->direct_scalar_rhs->scalar_type ==
                 bir::Type{bir::TypeKind::F64, 64, "double"} &&
             instruction.operands().size() == 2 &&
             function.value(instruction.operands()[0]).value().type ==
                 bir::Type{bir::TypeKind::F64, 64, "double"} &&
             instruction.operands()[1] == function.parameters()[0],
         "Raw BIR must retain the exact typed binary fsub RHS parameter authority");
  ...
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority.reset(); }, "missing fsub RHS authority must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->value = lir::LirValueId::invalid(); }, "invalid fsub RHS authority must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].native_body_parameter_definitions.push_back(candidate.functions[0].native_body_parameter_definitions[0]); }, "duplicate fsub RHS parameter definition must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->owner = candidate.link_names.intern("foreign_fsub_rhs_owner"); }, "foreign fsub RHS owner must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->parameter_index = 1; }, "fsub RHS parameter index mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->type = lir::LirTypeRef("float"); }, "fsub RHS authority type mismatch must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].native_body_parameter_definitions[0].type = lir::LirTypeRef("float"); }, "fsub RHS definition type mismatch must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].signature_param_type_refs[0] = lir::LirTypeRef("float"); }, "fsub RHS signature type mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->abi = lir::LirNativeBodyParameterAbi::DirectPointer; }, "fsub RHS ABI mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).scalar_rhs_parameter_authority->role = lir::LirScalarBinaryParameterRole::Lhs; }, "fsub RHS role mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).opcode = "fdiv"; }, "non-fsub RHS authority consumer must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).rhs = lir::LirOperand::ssa("%other", lir::LirValueId{144}); }, "fsub RHS identity mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).type_str = lir::LirTypeRef("float"); }, "fsub RHS result type mismatch must reject transactionally");
  rejected([](auto& candidate) { std::get<lir::LirBinOp>(candidate.functions[0].blocks[0].insts[1]).lhs = lir::LirOperand::ssa("%missing-lhs", lir::LirValueId{144}); }, "fsub nonselected LHS must be a current-function scalar");
  rejected([](auto& candidate) { std::get<lir::LirConstFloat>(candidate.functions[0].blocks[0].insts[0]).type = scalar_type(c4c::TB_FLOAT); }, "fsub nonselected LHS type mismatch must reject transactionally");
  rejected([](auto& candidate) { candidate.functions[0].blocks[0].insts.push_back(candidate.functions[0].blocks[0].insts[1]); }, "second fsub RHS authority row must reject transactionally");
}
...
void test_pointer_truthiness_parameter_authority_receipt_and_rejections() {
  ...
  rejected([](auto& candidate) { std::get<lir::LirCastOp>(candidate.functions[0].blocks[0].insts[0]).kind = lir::LirCastKind::ZExt; }, "pointer truthiness non-PtrToInt cast must reject transactionally");
...
void test_fixed_direct_call_argument0_parameter_authority_receipt_and_rejections() {
  const auto make_module = [](const std::size_t argument_index = 0) {
    ...
    if (argument_index == 1) {
      callee.params.emplace_back("%target-param1", scalar_type(c4c::TB_INT));
      callee.signature_params.push_back({"%target-param1", scalar_type(c4c::TB_INT), false});
      callee.signature_param_type_refs.push_back(lir::LirTypeRef::integer(32));
    }
    ...
    if (argument_index == 1) {
      caller.params.emplace_back("%presentation-only1", scalar_type(c4c::TB_INT));
      caller.signature_params.push_back({"%presentation-only1", scalar_type(c4c::TB_INT), false});
      caller.signature_param_type_refs.push_back(lir::LirTypeRef::integer(32));
    }
    caller.native_body_parameter_definitions.push_back(
        {lir::LirValueId{static_cast<std::uint32_t>(81 + argument_index)},
         static_cast<std::uint32_t>(argument_index), lir::LirTypeRef::integer(32), caller_owner,
         lir::LirNativeBodyParameterAbi::DirectScalar});
    lir::LirBlock entry = return_block(0, "entry");
    lir::LirCallOp call;
    call.result = lir::LirOperand::ssa("%presentation-result", lir::LirValueId{90});
    call.return_type = lir::LirTypeRef::integer(32);
    call.callee = lir::LirOperand::global("@presentation-target", callee_owner);
    call.direct_callee_link_name_id = callee_owner;
    call.arg_type_refs = {lir::LirTypeRef::integer(32)};
    if (argument_index == 1) call.arg_type_refs.push_back(lir::LirTypeRef::integer(32));
    ...
    if (argument_index == 1) {
      call.callee_signature->fixed_param_type_refs.push_back(lir::LirTypeRef::integer(32));
    }
    call.callee_signature_ref = callee.function_signature_ref;
    if (argument_index == 1) {
      call.structured_args.push_back(
          {"i32", lir::LirOperand::integer("7", 7), lir::LirTypeRef::integer(32)});
    }
    call.structured_args.push_back(
        {"i32",
         lir::LirOperand::ssa("%presentation-argument",
                              lir::LirValueId{static_cast<std::uint32_t>(81 + argument_index)}),
         lir::LirTypeRef::integer(32)});
    call.structured_args[argument_index].fixed_direct_call_argument_parameter_authority =
  ...
  expect(call.call() && call.call()->direct_scalar_argument &&
             call.call()->direct_scalar_argument->source_value_id == 81 &&
             call.call()->direct_scalar_argument->parameter_index == 0 &&
             call.call()->direct_scalar_argument->argument_index == 0 &&
             call.call()->direct_scalar_argument->scalar_type ==
                 bir::Type{bir::TypeKind::Integer, 32, "i32"} &&
  ...
  const auto argument1_module = make_module(1);
  const auto argument1_raw = bir::lower_lir_to_raw_bir(argument1_module);
  expect(argument1_raw.has_value() && bir::FoundationVerifier::verify(argument1_raw.value()).ok(),
         "fixed direct-call argument-1 authority must publish verified Raw BIR: " +
             (argument1_raw.has_value() ? "foundation verifier rejected it"
                                        : argument1_raw.error().detail));
  const auto argument1_view = argument1_raw.value().view();
  const auto argument1_function = argument1_view.function(argument1_view.functions()[1]).value();
  const auto argument1_call =
      argument1_function.instruction(
          argument1_function.instructions(argument1_function.blocks()[0]).value()[0]).value();
  expect(argument1_call.call() && argument1_call.call()->direct_scalar_argument &&
             argument1_call.call()->direct_scalar_argument->source_value_id == 82 &&
             argument1_call.call()->direct_scalar_argument->parameter_index == 1 &&
             argument1_call.call()->direct_scalar_argument->argument_index == 1 &&
             argument1_call.call()->direct_scalar_argument->scalar_type ==
                 bir::Type{bir::TypeKind::Integer, 32, "i32"} &&
             argument1_call.operands().size() == 2 &&
             argument1_call.operands()[1] == argument1_function.parameters()[1],
         "Raw BIR must retain the exact typed fixed direct-call argument-1 authority");
  ...
  const auto rejected_arg1 = [&](auto mutate, const std::string& message) {
    auto candidate = make_module(1);
    mutate(candidate);
    expect(!bir::lower_lir_to_raw_bir(candidate).has_value(), message + " (Raw rollback)");
    expect(!bir::lower_lir_to_canonical_bir(candidate).has_value(),
           message + " (Canonical rollback)");
  };
  rejected_arg1([](auto& candidate) {
    std::get<lir::LirCallOp>(candidate.functions[1].blocks[0].insts[0])
        .structured_args[1]
        .fixed_direct_call_argument_parameter_authority.reset();
  }, "missing fixed direct-call argument-1 authority must reject transactionally");
  rejected_arg1([](auto& candidate) {
    std::get<lir::LirCallOp>(candidate.functions[1].blocks[0].insts[0])
        .structured_args[1]
        .fixed_direct_call_argument_parameter_authority->value =
            lir::LirValueId::invalid();
  }, "invalid fixed direct-call argument-1 authority must reject transactionally");
  rejected_arg1([](auto& candidate) {
    candidate.functions[1].native_body_parameter_definitions.push_back(
        candidate.functions[1].native_body_parameter_definitions[0]);
  }, "duplicate fixed direct-call argument-1 definition must reject transactionally");
  rejected_arg1([](auto& candidate) {
    std::get<lir::LirCallOp>(candidate.functions[1].blocks[0].insts[0])
        .structured_args[1]
        .fixed_direct_call_argument_parameter_authority->owner =
            candidate.link_names.intern("foreign_fixed_direct_call_owner");
  }, "foreign fixed direct-call argument-1 owner must reject transactionally");
  rejected_arg1([](auto& candidate) {
    std::get<lir::LirCallOp>(candidate.functions[1].blocks[0].insts[0])
        .structured_args[1]
        .fixed_direct_call_argument_parameter_authority->parameter_index = 0;
  }, "fixed direct-call argument-1 parameter index mismatch must reject transactionally");
  rejected_arg1([](auto& candidate) {
    std::get<lir::LirCallOp>(candidate.functions[1].blocks[0].insts[0])
        .structured_args[1]
        .fixed_direct_call_argument_parameter_authority->type =
            lir::LirTypeRef::integer(64);
  }, "fixed direct-call argument-1 parameter type mismatch must reject transactionally");
  rejected_arg1([](auto& candidate) {
    std::get<lir::LirCallOp>(candidate.functions[1].blocks[0].insts[0])
        .structured_args[1]
        .fixed_direct_call_argument_parameter_authority->abi =
            lir::LirNativeBodyParameterAbi::DirectPointer;
  }, "fixed direct-call argument-1 ABI mismatch must reject transactionally");
  rejected_arg1([](auto& candidate) {
    std::get<lir::LirCallOp>(candidate.functions[1].blocks[0].insts[0])
        .structured_args[1]
        .fixed_direct_call_argument_parameter_authority->role =
            lir::LirFixedDirectCallArgumentParameterRole::FixedDirectCallArgument0;
  }, "fixed direct-call argument-1 role mismatch must reject transactionally");
  rejected_arg1([](auto& candidate) {
    std::get<lir::LirCallOp>(candidate.functions[1].blocks[0].insts[0])
        .structured_args[1]
        .operand = lir::LirOperand::ssa("%other", lir::LirValueId{83});
  }, "fixed direct-call argument-1 value mismatch must reject transactionally");
  rejected_arg1([](auto& candidate) {
    std::get<lir::LirCallOp>(candidate.functions[1].blocks[0].insts[0])
        .structured_args[1]
        .type_ref = lir::LirTypeRef::integer(64);
  }, "fixed direct-call argument-1 argument type mismatch must reject transactionally");
  rejected_arg1([](auto& candidate) {
    std::get<lir::LirCallOp>(candidate.functions[1].blocks[0].insts[0])
        .callee_signature->fixed_param_type_refs[1] = lir::LirTypeRef::integer(64);
  }, "fixed direct-call argument-1 callee parameter mismatch must reject transactionally");
  rejected_arg1([](auto& candidate) {
    auto& call =
        std::get<lir::LirCallOp>(candidate.functions[1].blocks[0].insts[0]);
    call.callee_signature.reset();
    call.structured_args[1].type_ref = lir::LirTypeRef::integer(64);
  }, "store-backed fixed direct-call argument-1 parameter mismatch must reject transactionally");
}
...
int main() {
  ...
  test_direct_scalar_binary_fsub_lhs_parameter_authority_receipt_and_rejections();
  test_direct_scalar_binary_fadd_rhs_parameter_authority_receipt_and_rejections();
  test_direct_scalar_binary_fmul_rhs_parameter_authority_receipt_and_rejections();
  test_direct_scalar_binary_fsub_rhs_parameter_authority_receipt_and_rejections();
