...
void test_x86_backend_scaffold_accepts_direct_conditional_phi_join_add_lir_input() {
  ...
  expect_contains(rendered, "  jge .Lelse\n",
                  "x86 direct-LIR phi-join regression should branch to the false predecessor on the native asm path");
  expect_contains(rendered, ".Lthen:\n  mov eax, 7\n  jmp .Ljoin\n",
                  "x86 direct-LIR phi-join regression should materialize the then predecessor on the native asm path");
  expect_contains(rendered, ".Ljoin:\n  add eax, 5\n  ret\n",
...
void test_x86_backend_explicit_lir_emit_surface_matches_local_pointer_temp_return_path() {
  const auto module = make_x86_local_pointer_temp_return_module();
  const auto direct_rendered = c4c::backend::x86::emit_module(module);
  const auto lowered_rendered =
      c4c::backend::x86::emit_module(c4c::backend::lower_lir_to_backend_module(module));
  const auto backend_rendered = c4c::backend::emit_module(
  ...
  expect_true(direct_rendered == lowered_rendered,
              "x86 explicit LIR emit surface should stay aligned with the legacy-lowered backend seam for the bounded local pointer round-trip slice");
  expect_true(direct_rendered == backend_rendered,
              "x86 backend selection should keep the bounded local pointer round-trip slice on the same explicit x86 asm path");
  expect_contains(direct_rendered, "mov eax, 0\n",
                  "x86 explicit LIR emit surface should fold the bounded local pointer round-trip without adapting through backend IR first");
  expect_not_contains(direct_rendered, "target triple =",
                      "x86 explicit LIR emit surface should not fall back to LLVM text for the bounded local pointer round-trip slice");
}
...
void test_x86_backend_explicit_lir_emit_surface_matches_double_indirect_local_pointer_conditional_return_path() {
  const auto module = make_double_indirect_local_pointer_conditional_return_module();
  const auto direct_rendered = c4c::backend::x86::emit_module(module);
  const auto lowered_rendered =
      c4c::backend::x86::emit_module(c4c::backend::lower_lir_to_backend_module(module));
  const auto backend_rendered = c4c::backend::emit_module(
  ...
  expect_true(direct_rendered == lowered_rendered,
              "x86 explicit LIR emit surface should stay aligned with the legacy-lowered backend seam for the bounded double-indirect local-pointer conditional family");
  expect_true(direct_rendered == backend_rendered,
              "x86 backend selection should keep the bounded double-indirect local-pointer conditional family on the same explicit x86 asm path");
  expect_contains(direct_rendered, "mov eax, 0\n",
                  "x86 explicit LIR emit surface should fold the bounded double-indirect local-pointer conditional family without adapting through backend IR first");
  expect_not_contains(direct_rendered, "target triple =",
...
void test_x86_backend_explicit_lir_emit_surface_matches_float_sitofp_equality_path() {
  const auto module = make_x86_float_sitofp_equality_module();
  const auto direct_rendered = c4c::backend::x86::emit_module(module);
  const auto backend_rendered = c4c::backend::emit_module(
      c4c::backend::BackendModuleInput{module},
      c4c::backend::BackendOptions{c4c::backend::Target::X86_64});
  ...
  expect_true(direct_rendered == backend_rendered,
              "x86 backend selection should keep the bounded float SIToFP equality family on the same explicit x86 asm path");
  expect_contains(direct_rendered, "mov eax, 0\n",
                  "x86 explicit LIR emit surface should fold the bounded float SIToFP equality family via the constant-fold interpreter with float support");
  expect_not_contains(direct_rendered, "target triple =",
                      "x86 explicit LIR emit surface should not fall back to LLVM text for the bounded float SIToFP equality family");
}
...
void test_x86_backend_rejects_param_slot_slice_when_helper_body_contract_disagrees() {
  ...
  auto* add =
      helper != nullptr && !helper->blocks.empty() && helper->blocks.front().insts.size() > 1
          ? std::get_if<c4c::codegen::lir::LirBinOp>(&helper->blocks.front().insts[1])
          : nullptr;
  expect_true(add != nullptr,
              "x86 param-slot regression test needs the helper add instruction to mutate");
  if (add != nullptr) {
    add->opcode = c4c::codegen::lir::LirBinaryOpcode::Sub;
  }
...
void test_x86_backend_explicit_lir_emit_surface_matches_structured_two_arg_direct_call_path() {
  auto module = make_typed_direct_call_two_arg_module();
  module.target_triple = "x86_64-unknown-linux-gnu";
  module.data_layout =
      "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128";
  ...
  const auto direct_rendered = c4c::backend::x86::emit_module(module);
  const auto lowered = c4c::backend::lower_lir_to_backend_module(make_typed_direct_call_two_arg_module());
  const auto lowered_rendered = c4c::backend::x86::emit_module(lowered);
  const auto backend_rendered = c4c::backend::emit_module(
      c4c::backend::BackendModuleInput{module},
      c4c::backend::BackendOptions{c4c::backend::Target::X86_64});
  ...
  expect_true(direct_rendered == lowered_rendered,
              "x86 explicit LIR emit surface should stay aligned with the structured two-argument direct-call backend path");
  expect_true(direct_rendered == backend_rendered,
              "x86 backend selection should keep the two-argument direct-call LIR slice on the same asm path");
  expect_contains(direct_rendered, "mov edi, 5\n",
                  "x86 explicit LIR emit surface should preserve the first two-argument direct-call immediate");
  expect_contains(direct_rendered, "mov esi, 7\n",
                  "x86 explicit LIR emit surface should preserve the second two-argument direct-call immediate");
...
void test_x86_backend_keeps_renamed_typed_two_arg_first_local_rewrite_slice_on_asm_path() {
  ...
  expect_true(helper != nullptr && main_fn != nullptr,
              "x86 renamed typed two-argument local-rewrite regression test needs helper and main functions");
  if (helper == nullptr || main_fn == nullptr) {
    return;
  }
  ...
  load0.ptr = "%lv.shadow.left";
  rewrite.result = "%t.rewrite.left";
  rewrite.lhs = "%t.load.left";
  store1.val = "%t.rewrite.left";
  store1.ptr = "%lv.shadow.left";
  load1.result = "%t.call.left";
  load1.ptr = "%lv.shadow.left";
  call.result = "%t.sum.result";
  call.callee = c4c::codegen::lir::LirOperand(std::string("@sum_pair"),
                                              c4c::codegen::lir::LirOperandKind::Global);
  call.args_str = "i32 %t.call.left, i32 7";
  ...
  const auto rendered = c4c::backend::emit_module(
      ...
      c4c::backend::BackendOptions{c4c::backend::Target::X86_64});
  expect_contains(rendered, ".type sum_pair, %function",
                  "x86 backend seam should key rewritten typed two-argument helper definitions from the observed helper symbol instead of a fixed add_pair name");
  expect_contains(rendered, "sum_pair:\n  mov eax, edi\n  add eax, esi\n  ret\n",
                  "x86 backend seam should keep renamed rewritten typed two-argument helpers on the asm path when helper and local SSA names change");
  expect_contains(rendered, "mov edi, 5",
                  "x86 backend seam should still materialize the rewritten renamed local first argument from the observed rewritten load chain");
  expect_contains(rendered, "mov esi, 7",
...
void test_x86_backend_keeps_renamed_typed_two_arg_both_local_arg_slice_on_asm_path() {
  ...
  expect_true(helper != nullptr && main_fn != nullptr,
              "x86 renamed typed two-argument both-local regression test needs helper and main functions");
  if (helper == nullptr || main_fn == nullptr) {
    return;
  }
  ...
  auto& lhs_local = std::get<c4c::codegen::lir::LirAllocaOp>(main_fn->alloca_insts[0]);
  auto& rhs_local = std::get<c4c::codegen::lir::LirAllocaOp>(main_fn->alloca_insts[1]);
  lhs_local.result = "%lv.shadow.left";
  rhs_local.result = "%lv.shadow.right";

  auto& lhs_store = std::get<c4c::codegen::lir::LirStoreOp>(main_fn->blocks.front().insts[0]);
  auto& rhs_store = std::get<c4c::codegen::lir::LirStoreOp>(main_fn->blocks.front().insts[1]);
  auto& lhs_load = std::get<c4c::codegen::lir::LirLoadOp>(main_fn->blocks.front().insts[2]);
  auto& rhs_load = std::get<c4c::codegen::lir::LirLoadOp>(main_fn->blocks.front().insts[3]);
  lhs_store.ptr = "%lv.shadow.left";
  rhs_store.ptr = "%lv.shadow.right";
  lhs_load.result = "%t.call.left";
  lhs_load.ptr = "%lv.shadow.left";
  rhs_load.result = "%t.call.right";
  rhs_load.ptr = "%lv.shadow.right";
  main_fn->blocks.front().insts.insert(
      main_fn->blocks.front().insts.begin() + 4,
      c4c::codegen::lir::LirLoadOp{"%t.unused.left", "i32", "%lv.shadow.left"});
  ...
  call.callee = c4c::codegen::lir::LirOperand(std::string("@sum_pair"),
                                              c4c::codegen::lir::LirOperandKind::Global);
  call.args_str = "i32 %t.call.left, i32 %t.call.right";
  main_fn->blocks.front().terminator =
      c4c::codegen::lir::LirRet{std::string("%t.sum.result"), "i32"};
  ...
  const auto rendered = c4c::backend::emit_module(
      ...
      c4c::backend::BackendOptions{c4c::backend::Target::X86_64});
  expect_contains(rendered, ".type sum_pair, %function",
                  "x86 backend seam should key both-local typed two-argument helper definitions from the observed helper symbol instead of a fixed add_pair name");
  expect_contains(rendered, "sum_pair:\n  mov eax, edi\n  add eax, esi\n  ret\n",
                  "x86 backend seam should keep renamed both-local typed two-argument helpers on the asm path when helper and local SSA names change");
  expect_contains(rendered, "mov edi, 5",
                  "x86 backend seam should still materialize the renamed first both-local typed two-argument argument from the observed slot store");
  expect_contains(rendered, "mov esi, 7",
                  "x86 backend seam should still materialize the renamed second both-local typed two-argument argument from the observed slot store");
  expect_contains(rendered, "call sum_pair",
                  "x86 backend seam should still lower renamed both-local typed two-argument direct calls without depending on the last local reload preceding the call");
  expect_not_contains(rendered, "target triple =",
                      "x86 backend seam should not fall back when a renamed both-local typed two-argument slice carries an extra unused reload before the direct call");
}

void test_x86_backend_keeps_renamed_typed_two_arg_double_rewrite_slice_on_asm_path() {
  ...
  expect_true(helper != nullptr && main_fn != nullptr,
              "x86 renamed typed two-argument double-rewrite regression test needs helper and main functions");
  if (helper == nullptr || main_fn == nullptr) {
    return;
  }
  ...
  auto& rhs_store0 = std::get<c4c::codegen::lir::LirStoreOp>(main_fn->blocks.front().insts[0]);
  auto& lhs_store0 = std::get<c4c::codegen::lir::LirStoreOp>(main_fn->blocks.front().insts[1]);
  auto& lhs_load0 = std::get<c4c::codegen::lir::LirLoadOp>(main_fn->blocks.front().insts[2]);
  auto& lhs_rewrite = std::get<c4c::codegen::lir::LirBinOp>(main_fn->blocks.front().insts[3]);
  auto& lhs_store1 = std::get<c4c::codegen::lir::LirStoreOp>(main_fn->blocks.front().insts[4]);
  auto& rhs_load0 = std::get<c4c::codegen::lir::LirLoadOp>(main_fn->blocks.front().insts[5]);
  auto& rhs_rewrite = std::get<c4c::codegen::lir::LirBinOp>(main_fn->blocks.front().insts[6]);
  auto& rhs_store1 = std::get<c4c::codegen::lir::LirStoreOp>(main_fn->blocks.front().insts[7]);
  auto& lhs_load1 = std::get<c4c::codegen::lir::LirLoadOp>(main_fn->blocks.front().insts[8]);
  auto& rhs_load1 = std::get<c4c::codegen::lir::LirLoadOp>(main_fn->blocks.front().insts[9]);
  auto& call = std::get<c4c::codegen::lir::LirCallOp>(main_fn->blocks.front().insts[10]);

  rhs_store0.val = "7";
  rhs_store0.ptr = "%lv.shadow.right";
  lhs_store0.val = "5";
  lhs_store0.ptr = "%lv.shadow.left";
  lhs_load0.result = "%t.load.left";
  lhs_load0.ptr = "%lv.shadow.left";
  lhs_rewrite.result = "%t.rewrite.left";
  lhs_rewrite.lhs = "%t.load.left";
  lhs_store1.val = "%t.rewrite.left";
  lhs_store1.ptr = "%lv.shadow.left";
  rhs_load0.result = "%t.load.right";
  rhs_load0.ptr = "%lv.shadow.right";
  rhs_rewrite.result = "%t.rewrite.right";
  rhs_rewrite.lhs = "%t.load.right";
  rhs_store1.val = "%t.rewrite.right";
  rhs_store1.ptr = "%lv.shadow.right";
  lhs_load1.result = "%t.call.left";
  lhs_load1.ptr = "%lv.shadow.left";
  rhs_load1.result = "%t.call.right";
  rhs_load1.ptr = "%lv.shadow.right";
  call.result = "%t.sum.result";
  call.callee = c4c::codegen::lir::LirOperand(std::string("@sum_pair"),
                                              c4c::codegen::lir::LirOperandKind::Global);
  call.args_str = "i32 %t.call.left, i32 %t.call.right";
  ...
  const auto rendered = c4c::backend::emit_module(
      ...
      c4c::backend::BackendOptions{c4c::backend::Target::X86_64});
  expect_contains(rendered, ".type sum_pair, %function",
                  "x86 backend seam should key double-rewritten typed two-argument helper definitions from the observed helper symbol instead of a fixed add_pair name");
  expect_contains(rendered, "sum_pair:\n  mov eax, edi\n  add eax, esi\n  ret\n",
                  "x86 backend seam should keep renamed double-rewritten typed two-argument helpers on the asm path when helper, parameter, and local SSA names change");
  expect_contains(rendered, "mov edi, 5",
                  "x86 backend seam should still materialize the rewritten renamed local first argument even when the initial slot stores appear in the opposite order");
  expect_contains(rendered, "mov esi, 7",
                  "x86 backend seam should still materialize the rewritten renamed local second argument from the observed second slot rewrite chain");
  expect_contains(rendered, "call sum_pair",
                  "x86 backend seam should still lower renamed double-rewritten typed two-argument direct calls without depending on fixed lowered helper names or store ordering");
  expect_not_contains(rendered, "target triple =",
                      "x86 backend seam should not fall back when the double-rewritten typed two-argument local slice relies only on the backend-owned rewrite contract");
}
...
void test_x86_backend_explicit_lir_emit_surface_keeps_renamed_global_load_caller_on_asm_path() {
  auto module = make_global_load_module();
  module.target_triple = "x86_64-unknown-linux-gnu";
  module.data_layout =
      "e-m:e-p270:32:32-p271:32:32-p272:64:64-i64:64-i128:128-f80:128-n8:16:32:64-S128";
  ...
  auto& load = std::get<c4c::codegen::lir::LirLoadOp>(caller_fn.blocks.front().insts.front());
  caller_fn.name = "entry_load";
  caller_fn.signature_text = "define i32 @entry_load()\n";
  load.result = "%t.entry_load";
  caller_fn.blocks.front().terminator =
      c4c::codegen::lir::LirRet{std::string("%t.entry_load"), "i32"};

  const auto rendered = c4c::backend::x86::emit_module(module);

  expect_contains(rendered, ".globl entry_load\n",
                  "x86 explicit LIR emit surface should publish the observed renamed scalar-global-load caller instead of requiring a literal main anchor");
  expect_contains(rendered, "entry_load:\n",
                  "x86 explicit LIR emit surface should preserve the renamed scalar-global-load caller label on the native asm path");
  expect_contains(rendered, "lea rax, g_counter[rip]\n",
                  "x86 explicit LIR emit surface should still form the scalar global address after the caller rename");
  expect_contains(rendered, "mov eax, dword ptr [rax]\n",
                  "x86 explicit LIR emit surface should still load the scalar global value after removing the caller-name anchor");
  expect_not_contains(rendered, "target triple =",
                      "x86 explicit LIR emit surface should not fall back to LLVM text for renamed scalar-global-load callers");
}
...
void test_x86_backend_explicit_emit_surface_keeps_renamed_global_load_caller_on_asm_path() {
  ...
  expect_true(caller_fn != nullptr,
              "x86 renamed scalar-global-load backend emit regression test needs the lowered zero-argument caller function");
  if (caller_fn == nullptr) {
    return;
  }

  caller_fn->signature.name = "entry_load";
  ...
  expect_true(backend_rendered == direct_rendered,
              "x86 backend selection should keep renamed structured scalar-global-load callers on the explicit backend emit surface");
  expect_contains(direct_rendered, ".globl entry_load\n",
                  "x86 explicit backend emit surface should publish the renamed scalar-global-load caller instead of assuming main");
  expect_contains(direct_rendered, "entry_load:\n",
                  "x86 explicit backend emit surface should preserve the renamed scalar-global-load caller label on the structured asm path");
  expect_contains(direct_rendered, "lea rax, g_counter[rip]\n",
                  "x86 explicit backend emit surface should still form the scalar global address after removing the caller-name anchor");
  expect_contains(direct_rendered, "mov eax, dword ptr [rax]\n",
                  "x86 explicit backend emit surface should still reload the scalar global value for renamed callers");
  expect_not_contains(direct_rendered, "define i32 @entry_load()",
                      "x86 explicit backend emit surface should not fall back to backend IR text for renamed scalar-global-load callers");
}
...
void test_x86_backend_scaffold_accepts_lowered_call_crossing_source_value_rename_without_signature_shims() {
  ...
  expect_true(main_fn != nullptr,
              "x86 lowered call-crossing source rename regression test needs the backend main function");
  if (main_fn == nullptr || main_fn->blocks.empty() || main_fn->blocks.front().insts.size() != 3) {
    return;
  }
  ...
  expect_true(source_add != nullptr && call != nullptr && final_add != nullptr,
              "x86 lowered call-crossing source rename regression test needs the structured source add, direct call, and final add to mutate");
  ...
  expect_contains(rendered, "mov edi, ebx",
                  "x86 backend seam should still marshal the renamed lowered call-crossing source through the ABI argument register");
  expect_contains(rendered, "call add_one",
                  "x86 backend seam should keep the lowered call-crossing direct call on the asm path after a structured source-value rename");
  expect_contains(rendered, "add eax, ebx",
...
int main(int argc, char* argv[]) {
  ...
  RUN_TEST(test_x86_backend_keeps_renamed_typed_two_arg_double_rewrite_slice_on_asm_path);
