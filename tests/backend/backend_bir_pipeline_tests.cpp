...
void test_backend_default_path_uses_riscv_native_entry_when_bir_pipeline_is_not_selected() {
  const auto prepared = c4c::backend::prepare_lir_module_for_target(
      make_bir_return_add_module(), c4c::backend::Target::Riscv64);
  const auto native =
      c4c::backend::riscv::codegen::try_emit_prepared_lir_module(prepared);
  const auto rendered = c4c::backend::emit_module(
  ...
  expect_true(native.has_value(),
              "riscv64 backend default-path test should start from a bounded native-emittable prepared-LIR slice");
  expect_true(
      rendered == c4c::backend::riscv::codegen::peephole_optimize(*native),
      "default riscv64 backend flow should finalize the bounded supported native-emitted slice through the shared RISC-V peephole surface");
  expect_contains(rendered, ".globl main",
                  "default riscv64 backend flow should route the bounded supported tiny-add slice through the native entry seam instead of the shared BIR text fallback");
  expect_contains(rendered, "addi a0, zero, 5",
                  "default riscv64 backend flow should still lower the bounded tiny-add slice through the native activation entry");
  expect_not_contains(rendered, "bir.func",
                      "default riscv64 backend flow should stop reporting the bounded supported tiny-add slice through the BIR text surface once the native entry seam is active");
}
...
void test_backend_riscv64_supported_lir_uses_native_route_while_direct_bir_stays_on_text_route() {
  const auto lir_module = make_bir_return_add_module();
  const auto bir_module = c4c::backend::lower_to_bir(lir_module);
  const auto prepared =
      c4c::backend::prepare_lir_module_for_target(lir_module, c4c::backend::Target::Riscv64);
  const auto native =
      c4c::backend::riscv::codegen::try_emit_prepared_lir_module(prepared);
  ...
  expect_true(native.has_value(),
              "supported riscv64 backend entry should keep exposing the bounded native tiny-add emitter seam for the same prepared-LIR slice");
  expect_true(
      lir_rendered == c4c::backend::riscv::codegen::peephole_optimize(*native),
      "supported riscv64 backend entry should finalize the native-emitted tiny-add slice through the shared RISC-V peephole surface before returning backend text");
  expect_true(lir_rendered != bir_rendered,
              "supported riscv64 backend entry should now route bounded supported LIR through the native activation seam while prelowered BIR stays on the text fallback surface");
  expect_contains(lir_rendered, ".globl main",
                  "supported riscv64 backend entry should expose the native text entry for the bounded tiny-add slice");
  expect_not_contains(lir_rendered, "bir.func",
                      "supported riscv64 backend entry should no longer report the bounded tiny-add LIR slice through the BIR text surface");
  expect_contains(bir_rendered, "bir.func",
                  "prelowered riscv64 BIR input should still stay on the shared BIR text fallback surface after the native activation seam lands");
}
...
void run_backend_bir_pipeline_tests() {
  ...
  RUN_TEST(test_backend_default_path_uses_riscv_native_entry_when_bir_pipeline_is_not_selected);
  ...
  RUN_TEST(test_backend_riscv64_supported_lir_uses_native_route_while_direct_bir_stays_on_text_route);
