...
struct RegallocDeferredBatchResolution {
  RegallocDeferredBatchJoin join;
  std::string_view allocation_stage;
};
...
struct RegallocBindingBatchMatch {
  const prepare::PreparedRegallocBindingDecision* decision = nullptr;
  RegallocDeferredBatchMatch deferred;
...
RegallocDeferredBatchResolution resolve_regalloc_deferred_binding_batch(
    const prepare::PreparedRegallocFunction& function,
    const prepare::PreparedRegallocDeferredBindingBatchSummary& summary);

RegallocDeferredBatchMatch find_regalloc_deferred_binding_attachment_batch_match(
    const prepare::PreparedRegallocFunction& function,
    std::string_view source_kind,
    std::string_view source_name) {
...
RegallocDeferredBatchResolution resolve_regalloc_deferred_binding_batch(
    const prepare::PreparedRegallocFunction& function,
    const prepare::PreparedRegallocDeferredBindingBatchSummary& summary) {
  auto join = join_regalloc_deferred_binding_batch(function, summary);
  return {
      .join = std::move(join),
      .allocation_stage = join.decision == nullptr ? std::string_view{} : join.decision->allocation_stage,
  };
}
...
RegallocBindingBatchMatch find_regalloc_binding_batch_match(
    const prepare::PreparedRegallocFunction& function,
    std::string_view source_kind,
    std::string_view source_name) {
  ...
  if (deferred_batch_match.summary != nullptr) {
    return {
        .deferred = std::move(deferred_batch_match),
        .kind = regalloc_deferred_binding_batch_kind(function, *deferred_batch_match.summary),
    };
  }
  return {};
}
...
std::size_t regalloc_deferred_attachment_count_sum(
  ...
  for (const auto& summary : summaries) {
    total += summary.attachments.size();
  }
  return total;
}
...
std::string_view regalloc_deferred_batch_access_window_prerequisite_state(
    const RegallocDeferredBatchResolution& resolution) {
  if (resolution.join.uses_access_window_owner) {
...
std::string_view regalloc_deferred_batch_ordering_policy(
  ...
  if (resolution.join.contention != nullptr &&
      resolution.join.contention->follow_up_category == "batched_single_point_coordination") {
...
std::string_view regalloc_deferred_batch_home_slot_prerequisite_state(
    const RegallocDeferredBatchResolution& resolution) {
  if (resolution.join.uses_access_window_owner) {
    if (resolution.join.object == nullptr) {
      return {};
    }
    if (resolution.join.object->home_slot_mode == "home_slot_needs_future_analysis") {
      return "prealloc_home_slot_prerequisite_deferred";
    }
    if (resolution.join.object->home_slot_mode == "stable_home_slot_required" ||
        resolution.join.object->home_slot_mode == "stable_home_slot_preferred" ||
        resolution.join.object->home_slot_mode == "single_use_home_slot_ok") {
      return "prealloc_home_slot_prerequisite_satisfied";
    }
    if (resolution.join.object->home_slot_mode == "idle_home_slot_coordination") {
  ...
  if (resolution.join.contention->home_slot_category == "stable_home_slot_required" ||
      resolution.join.contention->home_slot_category == "stable_home_slot_preferred" ||
      resolution.join.contention->home_slot_category == "single_use_home_slot_ok") {
    return "prealloc_home_slot_prerequisite_satisfied";
  }
  if (resolution.join.contention->home_slot_category == "idle_home_slot_coordination") {
    return "no_home_slot_prerequisite";
  }
  return "prealloc_home_slot_prerequisite_deferred";
}
...
std::string_view regalloc_deferred_batch_sync_handoff_state(
  ...
  if (resolution.join.uses_access_window_owner) {
    ...
    if (resolution.join.object->sync_policy == "sync_policy_needs_future_analysis") {
      return "prealloc_sync_handoff_deferred";
    }
    if (resolution.join.object->sync_policy == "restore_before_read" ||
        resolution.join.object->sync_policy == "writeback_after_write" ||
        resolution.join.object->sync_policy == "sync_on_read_write_boundaries" ||
        resolution.join.object->sync_policy == "memory_authoritative") {
      return "prealloc_sync_handoff_ready";
    }
    if (resolution.join.object->sync_policy == "sync_free_coordination") {
  ...
  if (resolution.join.contention->sync_coordination_category == "restore_only_coordination" ||
      resolution.join.contention->sync_coordination_category == "writeback_only_coordination" ||
      resolution.join.contention->sync_coordination_category == "read_write_coordination" ||
      resolution.join.contention->sync_coordination_category == "bidirectional_sync_coordination" ||
      resolution.join.contention->sync_coordination_category == "mixed_sync_coordination") {
    return "prealloc_sync_handoff_ready";
  }
  if (resolution.join.contention->sync_coordination_category == "sync_free_coordination") {
    return "no_sync_handoff_required";
  }
  return "prealloc_sync_handoff_deferred";
}
...
int main() {
  ...
  const auto deferred_attachment_total =
  ...
  if (regalloc_function->deferred_binding_batches.size() != 2 ||
      deferred_attachment_total != 9 ||
      deferred_access_window_batch.summary == nullptr ||
      deferred_coordination_batch.summary == nullptr ||
      regalloc_deferred_batch_candidate_count(*deferred_access_window_batch.summary) +
              regalloc_deferred_batch_candidate_count(*deferred_coordination_batch.summary) !=
          deferred_attachment_total) {
    return fail(
  ...
  const auto scratch_slot_deferred_binding_batch =
      find_regalloc_deferred_binding_attachment_batch_match(
      *regalloc_function, "lowering_scratch_slot", "scratch.slot");
  const auto* local_slot_regalloc = find_regalloc_object(*regalloc_function, "local_slot", "flag.slot");
  const auto local_slot_deferred_binding_batch =
      find_regalloc_deferred_binding_attachment_batch_match(
      *regalloc_function, "local_slot", "flag.slot");
  ...
  if (scratch_slot_deferred_binding_batch.summary == nullptr ||
      regalloc_deferred_binding_batch_kind(*regalloc_function,
                                           *scratch_slot_deferred_binding_batch.summary) !=
          "deferred_access_window_binding_batch" ||
      regalloc_deferred_batch_identity(scratch_slot_deferred_binding_batch.resolution.join,
                                       *scratch_slot_deferred_binding_batch.summary) !=
          "awaiting_access_window_observation" ||
      regalloc_deferred_batch_access_window_prerequisite_state(
          scratch_slot_deferred_binding_batch.resolution) !=
  ...
  if (local_slot_deferred_binding_batch.summary == nullptr ||
      regalloc_deferred_binding_batch_kind(*regalloc_function,
                                           *local_slot_deferred_binding_batch.summary) !=
          "deferred_coordination_binding_batch" ||
      regalloc_deferred_batch_identity(local_slot_deferred_binding_batch.resolution.join,
                                       *local_slot_deferred_binding_batch.summary) !=
          "batched_single_point_coordination" ||
      regalloc_deferred_batch_ordering_policy(
          local_slot_deferred_binding_batch.resolution) !=
  ...
  const auto carry_slot_binding_batch =
      find_regalloc_binding_batch_match(*regalloc_function, "local_slot", "carry.slot");
  ...
  if (carry_slot_binding_batch.kind != "call_boundary_binding_batch") {
    return fail("semantic-BIR regalloc should publish ready call-boundary batch identity through binding_sequence instead of duplicating it in binding_attachments");
  }
  const auto* window_slot_regalloc = find_regalloc_object(*regalloc_function, "local_slot", "window.slot");
  const auto window_slot_binding_batch =
      find_regalloc_binding_batch_match(*regalloc_function, "local_slot", "window.slot");
  ...
  if (window_slot_binding_batch.kind != "local_reuse_binding_batch") {
    return fail("semantic-BIR regalloc should publish ready local-reuse batch identity through binding_sequence instead of duplicating it in binding_attachments");
  }
  const auto* readonly_slot_regalloc =
      find_regalloc_object(*regalloc_function, "local_slot", "readonly.slot");
  const auto readonly_slot_binding_batch =
      find_regalloc_binding_batch_match(*regalloc_function, "local_slot", "readonly.slot");
  ...
  if (readonly_slot_binding_batch.kind != "local_reuse_binding_batch") {
    return fail("semantic-BIR regalloc should publish ready local-reuse batch identity through binding_sequence instead of duplicating it in binding_attachments");
  }
  const auto* callread_slot_regalloc =
      find_regalloc_object(*regalloc_function, "local_slot", "callread.slot");
  const auto callread_slot_binding_batch =
      find_regalloc_binding_batch_match(*regalloc_function, "local_slot", "callread.slot");
  ...
  if (callread_slot_binding_batch.kind != "call_boundary_binding_batch") {
    return fail("semantic-BIR regalloc should publish ready call-boundary batch identity through binding_sequence instead of duplicating it in binding_attachments");
  }
  const auto* callwrite_slot_regalloc =
      find_regalloc_object(*regalloc_function, "local_slot", "callwrite.slot");
  const auto callwrite_slot_binding_batch =
      find_regalloc_binding_batch_match(*regalloc_function, "local_slot", "callwrite.slot");
  ...
  if (callwrite_slot_binding_batch.kind != "call_boundary_binding_batch") {
    return fail("semantic-BIR regalloc should publish ready call-boundary batch identity through binding_sequence instead of duplicating it in binding_attachments");
  }
  const auto* multiwrite_slot_regalloc =
      find_regalloc_object(*regalloc_function, "local_slot", "multiwrite.slot");
  const auto multiwrite_slot_binding_batch =
      find_regalloc_binding_batch_match(*regalloc_function, "local_slot", "multiwrite.slot");
  ...
  if (multiwrite_slot_binding_batch.kind != "local_reuse_binding_batch") {
    return fail("semantic-BIR regalloc should publish ready local-reuse batch identity through binding_sequence instead of duplicating it in binding_attachments");
  }
  ...
  const auto carry_binding_batch =
      find_regalloc_binding_batch_match(*regalloc_function, "local_slot", "carry.slot");
  const auto callread_binding_batch =
      find_regalloc_binding_batch_match(*regalloc_function, "local_slot", "callread.slot");
  const auto callwrite_binding_batch =
      find_regalloc_binding_batch_match(*regalloc_function, "local_slot", "callwrite.slot");
  const auto window_binding_batch =
      find_regalloc_binding_batch_match(*regalloc_function, "local_slot", "window.slot");
  const auto readonly_binding_batch =
      find_regalloc_binding_batch_match(*regalloc_function, "local_slot", "readonly.slot");
  const auto multiwrite_binding_batch =
      find_regalloc_binding_batch_match(*regalloc_function, "local_slot", "multiwrite.slot");
  const auto local_slot_binding_batch =
      find_regalloc_binding_batch_match(*regalloc_function, "local_slot", "flag.slot");
  const auto writeonly_binding_batch =
      find_regalloc_binding_batch_match(*regalloc_function, "local_slot", "writeonly.slot");
  ...
  if (carry_binding_batch.decision == nullptr || callread_binding_batch.decision == nullptr ||
      callwrite_binding_batch.decision == nullptr || window_binding_batch.decision == nullptr ||
      readonly_binding_batch.decision == nullptr ||
      multiwrite_binding_batch.decision == nullptr ||
  ...
  if (local_slot_binding_batch.decision != nullptr || writeonly_binding_batch.decision != nullptr ||
      local_slot_binding_batch.deferred.summary == nullptr ||
      writeonly_binding_batch.deferred.summary == nullptr) {
  ...
  if (carry_binding_batch.kind != "call_boundary_binding_batch" ||
      callread_binding_batch.kind != "call_boundary_binding_batch" ||
      callwrite_binding_batch.kind != "call_boundary_binding_batch" ||
      carry_binding_batch.decision->binding_order_index != 0 ||
      callread_binding_batch.decision->binding_order_index != 1 ||
      callwrite_binding_batch.decision->binding_order_index != 2) {
    return fail(
        "semantic-BIR regalloc should keep binding-ready across-call entries focused on sequencing identity and order");
  }
  if (window_binding_batch.kind != "local_reuse_binding_batch" ||
      readonly_binding_batch.kind != "local_reuse_binding_batch" ||
      multiwrite_binding_batch.kind != "local_reuse_binding_batch" ||
      window_binding_batch.decision->binding_order_index != 0 ||
      readonly_binding_batch.decision->binding_order_index != 1 ||
      multiwrite_binding_batch.decision->binding_order_index != 2) {
    return fail(
  ...
  if (carry_binding_batch.decision->binding_order_index != 0 ||
      callread_binding_batch.decision->binding_order_index != 1 ||
      callwrite_binding_batch.decision->binding_order_index != 2 ||
      window_binding_batch.decision->binding_order_index != 0 ||
      readonly_binding_batch.decision->binding_order_index != 1 ||
