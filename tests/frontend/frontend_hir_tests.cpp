...
c4c::Node* make_hir_ref_overload_record(
  ...
  c4c::Node* record = parser.make_node(c4c::NK_STRUCT_DEF, 0);
  record->name = arena.strdup(std::string(name).c_str());
  record->unqualified_name = arena.strdup(std::string(name).c_str());
  record->unqualified_text_id =
      parser.parser_text_id_for_token(c4c::kInvalidText, name);
  record->namespace_context_id = parser.current_namespace_context_id();
  return record;
}

c4c::TypeSpec make_hir_ref_overload_record_type(
    c4c::Arena& arena,
    c4c::Node* record,
    std::string_view rendered_tag,
    bool lvalue_ref,
    bool rvalue_ref) {
  c4c::TypeSpec ts{};
  ...
  ts.array_size = -1;
  ts.inner_rank = -1;
  ts.is_lvalue_ref = lvalue_ref;
  ts.is_rvalue_ref = rvalue_ref;
  return ts;
}

c4c::Node* make_hir_ref_overload_function(
    c4c::Parser& parser,
    c4c::Arena& arena,
    std::string_view name,
    const c4c::TypeSpec& param_type) {
  c4c::Node* param = parser.make_node(c4c::NK_DECL, 0);
  param->type = param_type;

  c4c::Node* fn = parser.make_node(c4c::NK_FUNCTION, 0);
  fn->name = arena.strdup(std::string(name).c_str());
  fn->unqualified_name = arena.strdup(std::string(name).c_str());
...
void test_hir_ref_overload_grouping_prefers_record_def_over_stale_tag() {
  ...
  c4c::Node* record =
      make_hir_ref_overload_record(parser, arena, "ActualRefOverloadOwner");
  c4c::TypeSpec lvalue_ts =
      make_hir_ref_overload_record_type(arena, record, "StaleLeftOwner", true, false);
  c4c::TypeSpec rvalue_ts =
      make_hir_ref_overload_record_type(arena, record, "StaleRightOwner", false, true);
  c4c::Node* lvalue_fn =
      make_hir_ref_overload_function(parser, arena, "structured_ref_overload", lvalue_ts);
  c4c::Node* rvalue_fn =
      make_hir_ref_overload_function(parser, arena, "structured_ref_overload", rvalue_ts);

  lowerer.collect_ref_overloaded_free_functions({lvalue_fn, rvalue_fn});

  const auto ov_it = lowerer.ref_overload_set_.find("structured_ref_overload");
  expect_true(ov_it != lowerer.ref_overload_set_.end(),
              "ref-overload grouping should use record_def identity before stale rendered tags");
  expect_eq_int(static_cast<int>(ov_it->second.size()), 2,
                "structured ref-overload grouping should keep both overload entries");
}

void test_hir_ref_overload_grouping_rejects_stale_tag_after_record_def_mismatch() {
  c4c::Arena arena;
  c4c::TextTable texts;
  c4c::FileTable files;
  c4c::Parser parser({}, arena, &texts, &files, c4c::SourceProfile::CppSubset);
  ...
  c4c::Node* first_record =
      make_hir_ref_overload_record(parser, arena, "FirstRefOverloadOwner");
  c4c::Node* second_record =
      make_hir_ref_overload_record(parser, arena, "SecondRefOverloadOwner");
  c4c::TypeSpec lvalue_ts =
      make_hir_ref_overload_record_type(arena, first_record, "SharedStaleOwner", true, false);
  c4c::TypeSpec rvalue_ts =
      make_hir_ref_overload_record_type(arena, second_record, "SharedStaleOwner", false, true);
  c4c::Node* lvalue_fn =
      make_hir_ref_overload_function(parser, arena, "reject_stale_ref_overload", lvalue_ts);
  c4c::Node* rvalue_fn =
      make_hir_ref_overload_function(parser, arena, "reject_stale_ref_overload", rvalue_ts);

  lowerer.collect_ref_overloaded_free_functions({lvalue_fn, rvalue_fn});

  expect_true(lowerer.ref_overload_set_.count("reject_stale_ref_overload") == 0,
              "complete structured record mismatch must not fall back to matching rendered tags");
}

void test_hir_ref_overload_grouping_rejects_partial_metadata_rendered_fallback() {
  c4c::Arena arena;
  c4c::TextTable texts;
  c4c::FileTable files;
  c4c::Parser parser({}, arena, &texts, &files, c4c::SourceProfile::CppSubset);
  ...
  c4c::Node* record =
      make_hir_ref_overload_record(parser, arena, "PartialRefOverloadOwner");
  c4c::TypeSpec lvalue_ts =
      make_hir_ref_overload_record_type(arena, record, "SharedPartialOwner", true, false);
  c4c::TypeSpec rvalue_ts =
      make_hir_ref_overload_record_type(arena, nullptr, "SharedPartialOwner", false, true);
  c4c::Node* lvalue_fn =
      make_hir_ref_overload_function(parser, arena, "partial_ref_overload", lvalue_ts);
  c4c::Node* rvalue_fn =
      make_hir_ref_overload_function(parser, arena, "partial_ref_overload", rvalue_ts);

  lowerer.collect_ref_overloaded_free_functions({lvalue_fn, rvalue_fn});

  expect_true(lowerer.ref_overload_set_.count("partial_ref_overload") == 0,
...
void test_lir_dead_internal_store_pointer_uses_link_name_id_before_rendered_text() {
  c4c::codegen::lir::LirModule module = make_link_name_aware_lir_module();
  const c4c::LinkNameId semantic_helper_id =
      module.link_names.intern("semantic_store_pointer_helper");
  const c4c::LinkNameId rendered_shadow_id =
      module.link_names.intern("rendered_store_pointer_shadow");
  expect_true(semantic_helper_id != c4c::kInvalidLinkName &&
                  rendered_shadow_id != c4c::kInvalidLinkName,
              "fixture should allocate stable store pointer reachability LinkNameIds");

  c4c::codegen::lir::LirFunction root;
  root.name = "root_pointer";
  root.link_name_id = module.link_names.intern("root_pointer");
  c4c::codegen::lir::LirBlock root_block;
  root_block.label = "entry";
  root_block.insts.push_back(c4c::codegen::lir::LirStoreOp{
      c4c::codegen::lir::LirTypeRef::integer(32),
      c4c::codegen::lir::LirOperand::integer("0", 0),
      c4c::codegen::lir::LirOperand::global("@rendered_store_pointer_shadow",
                                            semantic_helper_id)});
  root.blocks.push_back(std::move(root_block));
  module.functions.push_back(std::move(root));

  c4c::codegen::lir::LirFunction semantic_helper;
  semantic_helper.name = "semantic_store_pointer_helper";
  semantic_helper.link_name_id = semantic_helper_id;
  semantic_helper.is_internal = true;
  semantic_helper.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(semantic_helper));
  ...
  c4c::codegen::lir::LirFunction rendered_shadow;
  rendered_shadow.name = "rendered_store_pointer_shadow";
  rendered_shadow.link_name_id = rendered_shadow_id;
  rendered_shadow.is_internal = true;
  rendered_shadow.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(rendered_shadow));
  ...
  expect_true(semantic_it != module.functions.end(),
              "store pointer LinkNameId should keep the semantic helper");
  ...
  expect_true(shadow_it == module.functions.end(),
              "stale store pointer spelling should not keep the rendered-name shadow");

  c4c::codegen::lir::LirModule legacy_module = make_link_name_aware_lir_module();
  c4c::codegen::lir::LirFunction legacy_root;
  legacy_root.name = "legacy_root_pointer";
  legacy_root.link_name_id =
      legacy_module.link_names.intern("legacy_root_pointer");
  c4c::codegen::lir::LirBlock legacy_block;
  legacy_block.label = "entry";
  legacy_block.insts.push_back(c4c::codegen::lir::LirStoreOp{
      c4c::codegen::lir::LirTypeRef::integer(32),
      c4c::codegen::lir::LirOperand::integer("0", 0),
      c4c::codegen::lir::LirOperand::raw("@rendered_store_pointer_shadow")});
  legacy_root.blocks.push_back(std::move(legacy_block));
  legacy_module.functions.push_back(std::move(legacy_root));

  c4c::codegen::lir::LirFunction legacy_shadow;
  legacy_shadow.name = "rendered_store_pointer_shadow";
  legacy_shadow.link_name_id =
      legacy_module.link_names.intern("rendered_store_pointer_shadow");
  legacy_shadow.is_internal = true;
  legacy_shadow.can_elide_if_unreferenced = true;
  legacy_module.functions.push_back(std::move(legacy_shadow));
  ...
  c4c::codegen::lir::eliminate_dead_internals(legacy_module);
  expect_true(legacy_module.functions.size() == 2,
              "legacy raw store pointer should keep text-scan compatibility");
}

void test_lir_dead_internal_load_pointer_uses_link_name_id_before_rendered_text() {
  c4c::codegen::lir::LirModule module = make_link_name_aware_lir_module();
  const c4c::LinkNameId semantic_helper_id =
      module.link_names.intern("semantic_load_pointer_helper");
  const c4c::LinkNameId rendered_shadow_id =
      module.link_names.intern("rendered_load_pointer_shadow");
  expect_true(semantic_helper_id != c4c::kInvalidLinkName &&
                  rendered_shadow_id != c4c::kInvalidLinkName,
              "fixture should allocate stable load pointer reachability LinkNameIds");

  c4c::codegen::lir::LirFunction root;
  root.name = "root_load_pointer";
  root.link_name_id = module.link_names.intern("root_load_pointer");
  c4c::codegen::lir::LirBlock root_block;
  root_block.label = "entry";
  root_block.insts.push_back(c4c::codegen::lir::LirLoadOp{
      c4c::codegen::lir::LirOperand::ssa("%loaded", c4c::codegen::lir::LirValueId{1}),
      c4c::codegen::lir::LirTypeRef::integer(32),
      c4c::codegen::lir::LirOperand::global("@rendered_load_pointer_shadow",
                                            semantic_helper_id)});
  root.blocks.push_back(std::move(root_block));
  module.functions.push_back(std::move(root));

  c4c::codegen::lir::LirFunction semantic_helper;
  semantic_helper.name = "semantic_load_pointer_helper";
  semantic_helper.link_name_id = semantic_helper_id;
  semantic_helper.is_internal = true;
  semantic_helper.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(semantic_helper));
  ...
  c4c::codegen::lir::LirFunction rendered_shadow;
  rendered_shadow.name = "rendered_load_pointer_shadow";
  rendered_shadow.link_name_id = rendered_shadow_id;
  rendered_shadow.is_internal = true;
  rendered_shadow.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(rendered_shadow));
  ...
  expect_true(semantic_it != module.functions.end(),
              "load pointer LinkNameId should keep the semantic helper");
  ...
  expect_true(shadow_it == module.functions.end(),
              "stale load pointer spelling should not keep the rendered-name shadow");

  c4c::codegen::lir::LirModule legacy_module = make_link_name_aware_lir_module();
  c4c::codegen::lir::LirFunction legacy_root;
  legacy_root.name = "legacy_root_load_pointer";
  legacy_root.link_name_id =
      legacy_module.link_names.intern("legacy_root_load_pointer");
  c4c::codegen::lir::LirBlock legacy_block;
  legacy_block.label = "entry";
  legacy_block.insts.push_back(c4c::codegen::lir::LirLoadOp{
      c4c::codegen::lir::LirOperand::ssa("%loaded", c4c::codegen::lir::LirValueId{1}),
      c4c::codegen::lir::LirTypeRef::integer(32),
      c4c::codegen::lir::LirOperand::raw("@rendered_load_pointer_shadow")});
  legacy_root.blocks.push_back(std::move(legacy_block));
  legacy_module.functions.push_back(std::move(legacy_root));

  c4c::codegen::lir::LirFunction legacy_shadow;
  legacy_shadow.name = "rendered_load_pointer_shadow";
  legacy_shadow.link_name_id =
      legacy_module.link_names.intern("rendered_load_pointer_shadow");
  legacy_shadow.is_internal = true;
  legacy_shadow.can_elide_if_unreferenced = true;
  legacy_module.functions.push_back(std::move(legacy_shadow));
...
void test_lir_dead_internal_phi_value_uses_link_name_id_before_rendered_text() {
  c4c::codegen::lir::LirModule module = make_link_name_aware_lir_module();
  const c4c::LinkNameId semantic_helper_id =
      module.link_names.intern("semantic_phi_value_helper");
  const c4c::LinkNameId rendered_shadow_id =
      module.link_names.intern("rendered_phi_value_shadow");
  expect_true(semantic_helper_id != c4c::kInvalidLinkName &&
                  rendered_shadow_id != c4c::kInvalidLinkName,
              "fixture should allocate stable phi value reachability LinkNameIds");

  c4c::codegen::lir::LirFunction root;
  root.name = "root_phi_value";
  root.link_name_id = module.link_names.intern("root_phi_value");
  c4c::codegen::lir::LirBlock root_block;
  root_block.label = "entry";
  root_block.insts.push_back(c4c::codegen::lir::LirPhiOp{
      c4c::codegen::lir::LirOperand::ssa("%phi", c4c::codegen::lir::LirValueId{1}),
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::Pointer),
      {{c4c::codegen::lir::LirOperand::global("@rendered_phi_value_shadow",
                                              semantic_helper_id),
        "entry"}}});
  root.blocks.push_back(std::move(root_block));
  module.functions.push_back(std::move(root));
  ...
  c4c::codegen::lir::LirFunction semantic_helper;
  semantic_helper.name = "semantic_phi_value_helper";
  semantic_helper.link_name_id = semantic_helper_id;
  semantic_helper.is_internal = true;
  semantic_helper.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(semantic_helper));
  ...
  c4c::codegen::lir::LirFunction rendered_shadow;
  rendered_shadow.name = "rendered_phi_value_shadow";
  rendered_shadow.link_name_id = rendered_shadow_id;
  rendered_shadow.is_internal = true;
  rendered_shadow.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(rendered_shadow));
  ...
  expect_true(semantic_it != module.functions.end(),
              "PHI incoming value LinkNameId should keep the semantic helper");
  ...
  expect_true(shadow_it == module.functions.end(),
              "stale PHI incoming value spelling should not keep the rendered-name shadow");

  c4c::codegen::lir::LirModule legacy_module = make_link_name_aware_lir_module();
  c4c::codegen::lir::LirFunction legacy_root;
  legacy_root.name = "legacy_root_phi_value";
  legacy_root.link_name_id =
      legacy_module.link_names.intern("legacy_root_phi_value");
  c4c::codegen::lir::LirBlock legacy_block;
  legacy_block.label = "entry";
  legacy_block.insts.push_back(c4c::codegen::lir::LirPhiOp{
      c4c::codegen::lir::LirOperand::ssa("%phi", c4c::codegen::lir::LirValueId{1}),
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::Pointer),
      {{c4c::codegen::lir::LirOperand::raw("@rendered_phi_value_shadow"),
        "entry"}}});
  legacy_root.blocks.push_back(std::move(legacy_block));
  legacy_module.functions.push_back(std::move(legacy_root));

  c4c::codegen::lir::LirFunction legacy_shadow;
  legacy_shadow.name = "rendered_phi_value_shadow";
  legacy_shadow.link_name_id =
      legacy_module.link_names.intern("rendered_phi_value_shadow");
  legacy_shadow.is_internal = true;
  legacy_shadow.can_elide_if_unreferenced = true;
  legacy_module.functions.push_back(std::move(legacy_shadow));
  ...
  c4c::codegen::lir::eliminate_dead_internals(legacy_module);
  expect_true(legacy_module.functions.size() == 2,
              "legacy raw PHI value should keep text-scan compatibility");
}

void test_lir_dead_internal_cast_operand_uses_link_name_id_before_rendered_text() {
  c4c::codegen::lir::LirModule module = make_link_name_aware_lir_module();
  const c4c::LinkNameId semantic_helper_id =
      module.link_names.intern("semantic_cast_operand_helper");
  const c4c::LinkNameId rendered_shadow_id =
      module.link_names.intern("rendered_cast_operand_shadow");
  expect_true(semantic_helper_id != c4c::kInvalidLinkName &&
                  rendered_shadow_id != c4c::kInvalidLinkName,
              "fixture should allocate stable cast operand reachability LinkNameIds");

  c4c::codegen::lir::LirFunction root;
  root.name = "root_cast_operand";
  root.link_name_id = module.link_names.intern("root_cast_operand");
  c4c::codegen::lir::LirBlock root_block;
  root_block.label = "entry";
  root_block.insts.push_back(c4c::codegen::lir::LirCastOp{
      c4c::codegen::lir::LirOperand::ssa("%cast", c4c::codegen::lir::LirValueId{1}),
      c4c::codegen::lir::LirCastKind::PtrToInt,
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::Pointer),
      c4c::codegen::lir::LirOperand::global("@rendered_cast_operand_shadow",
                                            semantic_helper_id),
  ...
  c4c::codegen::lir::LirFunction semantic_helper;
  semantic_helper.name = "semantic_cast_operand_helper";
  semantic_helper.link_name_id = semantic_helper_id;
  semantic_helper.is_internal = true;
  semantic_helper.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(semantic_helper));
  ...
  c4c::codegen::lir::LirFunction rendered_shadow;
  rendered_shadow.name = "rendered_cast_operand_shadow";
  rendered_shadow.link_name_id = rendered_shadow_id;
  rendered_shadow.is_internal = true;
  rendered_shadow.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(rendered_shadow));
  ...
  expect_true(semantic_it != module.functions.end(),
              "cast operand LinkNameId should keep the semantic helper");
  ...
  expect_true(shadow_it == module.functions.end(),
              "stale cast operand spelling should not keep the rendered-name shadow");

  c4c::codegen::lir::LirModule legacy_module = make_link_name_aware_lir_module();
  c4c::codegen::lir::LirFunction legacy_root;
  legacy_root.name = "legacy_root_cast_operand";
  legacy_root.link_name_id =
      legacy_module.link_names.intern("legacy_root_cast_operand");
  c4c::codegen::lir::LirBlock legacy_block;
  legacy_block.label = "entry";
  legacy_block.insts.push_back(c4c::codegen::lir::LirCastOp{
      c4c::codegen::lir::LirOperand::ssa("%cast", c4c::codegen::lir::LirValueId{1}),
      c4c::codegen::lir::LirCastKind::PtrToInt,
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::Pointer),
      c4c::codegen::lir::LirOperand::raw("@rendered_cast_operand_shadow"),
      c4c::codegen::lir::LirTypeRef::integer(64)});
  legacy_root.blocks.push_back(std::move(legacy_block));
  legacy_module.functions.push_back(std::move(legacy_root));

  c4c::codegen::lir::LirFunction legacy_shadow;
  legacy_shadow.name = "rendered_cast_operand_shadow";
  legacy_shadow.link_name_id =
      legacy_module.link_names.intern("rendered_cast_operand_shadow");
  legacy_shadow.is_internal = true;
  legacy_shadow.can_elide_if_unreferenced = true;
  legacy_module.functions.push_back(std::move(legacy_shadow));
  ...
  c4c::codegen::lir::eliminate_dead_internals(legacy_module);
  expect_true(legacy_module.functions.size() == 2,
              "legacy raw cast operand should keep text-scan compatibility");
}

void test_lir_dead_internal_binop_lhs_uses_link_name_id_before_rendered_text() {
  c4c::codegen::lir::LirModule module = make_link_name_aware_lir_module();
  const c4c::LinkNameId semantic_helper_id =
      module.link_names.intern("semantic_binop_lhs_helper");
  const c4c::LinkNameId rendered_shadow_id =
      module.link_names.intern("rendered_binop_lhs_shadow");
  expect_true(semantic_helper_id != c4c::kInvalidLinkName &&
                  rendered_shadow_id != c4c::kInvalidLinkName,
              "fixture should allocate stable binop lhs reachability LinkNameIds");

  c4c::codegen::lir::LirFunction root;
  root.name = "root_binop_lhs";
  root.link_name_id = module.link_names.intern("root_binop_lhs");
  c4c::codegen::lir::LirBlock root_block;
  root_block.label = "entry";
  root_block.insts.push_back(c4c::codegen::lir::LirBinOp{
      c4c::codegen::lir::LirOperand::ssa("%sum", c4c::codegen::lir::LirValueId{1}),
      "add",
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::Pointer),
      c4c::codegen::lir::LirOperand::global("@rendered_binop_lhs_shadow",
                                            semantic_helper_id),
      c4c::codegen::lir::LirOperand::integer("0", 0)});
  root.blocks.push_back(std::move(root_block));
  module.functions.push_back(std::move(root));
  ...
  c4c::codegen::lir::LirFunction semantic_helper;
  semantic_helper.name = "semantic_binop_lhs_helper";
  semantic_helper.link_name_id = semantic_helper_id;
  semantic_helper.is_internal = true;
  semantic_helper.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(semantic_helper));
  ...
  c4c::codegen::lir::LirFunction rendered_shadow;
  rendered_shadow.name = "rendered_binop_lhs_shadow";
  rendered_shadow.link_name_id = rendered_shadow_id;
  rendered_shadow.is_internal = true;
  rendered_shadow.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(rendered_shadow));
  ...
  expect_true(semantic_it != module.functions.end(),
              "binop lhs LinkNameId should keep the semantic helper");
  ...
  expect_true(shadow_it == module.functions.end(),
              "stale binop lhs spelling should not keep the rendered-name shadow");

  c4c::codegen::lir::LirModule legacy_module = make_link_name_aware_lir_module();
  c4c::codegen::lir::LirFunction legacy_root;
  legacy_root.name = "legacy_root_binop_lhs";
  legacy_root.link_name_id =
      legacy_module.link_names.intern("legacy_root_binop_lhs");
  c4c::codegen::lir::LirBlock legacy_block;
  legacy_block.label = "entry";
  legacy_block.insts.push_back(c4c::codegen::lir::LirBinOp{
      c4c::codegen::lir::LirOperand::ssa("%sum", c4c::codegen::lir::LirValueId{1}),
      "add",
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::Pointer),
      c4c::codegen::lir::LirOperand::raw("@rendered_binop_lhs_shadow"),
      c4c::codegen::lir::LirOperand::integer("0", 0)});
  legacy_root.blocks.push_back(std::move(legacy_block));
  legacy_module.functions.push_back(std::move(legacy_root));

  c4c::codegen::lir::LirFunction legacy_shadow;
  legacy_shadow.name = "rendered_binop_lhs_shadow";
  legacy_shadow.link_name_id =
      legacy_module.link_names.intern("rendered_binop_lhs_shadow");
  legacy_shadow.is_internal = true;
  legacy_shadow.can_elide_if_unreferenced = true;
  legacy_module.functions.push_back(std::move(legacy_shadow));
  ...
  c4c::codegen::lir::eliminate_dead_internals(legacy_module);
  expect_true(legacy_module.functions.size() == 2,
              "legacy raw binop lhs should keep text-scan compatibility");
}

void test_lir_dead_internal_binop_rhs_uses_link_name_id_before_rendered_text() {
  c4c::codegen::lir::LirModule module = make_link_name_aware_lir_module();
  const c4c::LinkNameId semantic_helper_id =
      module.link_names.intern("semantic_binop_rhs_helper");
  const c4c::LinkNameId rendered_shadow_id =
      module.link_names.intern("rendered_binop_rhs_shadow");
  expect_true(semantic_helper_id != c4c::kInvalidLinkName &&
                  rendered_shadow_id != c4c::kInvalidLinkName,
              "fixture should allocate stable binop rhs reachability LinkNameIds");

  c4c::codegen::lir::LirFunction root;
  root.name = "root_binop_rhs";
  root.link_name_id = module.link_names.intern("root_binop_rhs");
  c4c::codegen::lir::LirBlock root_block;
  root_block.label = "entry";
  root_block.insts.push_back(c4c::codegen::lir::LirBinOp{
      c4c::codegen::lir::LirOperand::ssa("%sum", c4c::codegen::lir::LirValueId{1}),
      ...
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::Pointer),
      c4c::codegen::lir::LirOperand::integer("0", 0),
      c4c::codegen::lir::LirOperand::global("@rendered_binop_rhs_shadow",
                                            semantic_helper_id)});
  root.blocks.push_back(std::move(root_block));
  module.functions.push_back(std::move(root));

  c4c::codegen::lir::LirFunction semantic_helper;
  semantic_helper.name = "semantic_binop_rhs_helper";
  semantic_helper.link_name_id = semantic_helper_id;
  semantic_helper.is_internal = true;
  semantic_helper.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(semantic_helper));
  ...
  c4c::codegen::lir::LirFunction rendered_shadow;
  rendered_shadow.name = "rendered_binop_rhs_shadow";
  rendered_shadow.link_name_id = rendered_shadow_id;
  rendered_shadow.is_internal = true;
  rendered_shadow.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(rendered_shadow));
  ...
  expect_true(semantic_it != module.functions.end(),
              "binop rhs LinkNameId should keep the semantic helper");
  ...
  expect_true(shadow_it == module.functions.end(),
              "stale binop rhs spelling should not keep the rendered-name shadow");

  c4c::codegen::lir::LirModule legacy_module = make_link_name_aware_lir_module();
  c4c::codegen::lir::LirFunction legacy_root;
  legacy_root.name = "legacy_root_binop_rhs";
  legacy_root.link_name_id =
      legacy_module.link_names.intern("legacy_root_binop_rhs");
  c4c::codegen::lir::LirBlock legacy_block;
  legacy_block.label = "entry";
  legacy_block.insts.push_back(c4c::codegen::lir::LirBinOp{
      c4c::codegen::lir::LirOperand::ssa("%sum", c4c::codegen::lir::LirValueId{1}),
      ...
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::Pointer),
      c4c::codegen::lir::LirOperand::integer("0", 0),
      c4c::codegen::lir::LirOperand::raw("@rendered_binop_rhs_shadow")});
  legacy_root.blocks.push_back(std::move(legacy_block));
  legacy_module.functions.push_back(std::move(legacy_root));

  c4c::codegen::lir::LirFunction legacy_shadow;
  legacy_shadow.name = "rendered_binop_rhs_shadow";
  legacy_shadow.link_name_id =
      legacy_module.link_names.intern("rendered_binop_rhs_shadow");
  legacy_shadow.is_internal = true;
  legacy_shadow.can_elide_if_unreferenced = true;
  legacy_module.functions.push_back(std::move(legacy_shadow));
  ...
  c4c::codegen::lir::eliminate_dead_internals(legacy_module);
  expect_true(legacy_module.functions.size() == 2,
              "legacy raw binop rhs should keep text-scan compatibility");
}

void test_lir_dead_internal_cmp_lhs_uses_link_name_id_before_rendered_text() {
  c4c::codegen::lir::LirModule module = make_link_name_aware_lir_module();
  const c4c::LinkNameId semantic_helper_id =
      module.link_names.intern("semantic_cmp_lhs_helper");
  const c4c::LinkNameId rendered_shadow_id =
      module.link_names.intern("rendered_cmp_lhs_shadow");
  expect_true(semantic_helper_id != c4c::kInvalidLinkName &&
                  rendered_shadow_id != c4c::kInvalidLinkName,
              "fixture should allocate stable cmp lhs reachability LinkNameIds");

  c4c::codegen::lir::LirFunction root;
  root.name = "root_cmp_lhs";
  root.link_name_id = module.link_names.intern("root_cmp_lhs");
  c4c::codegen::lir::LirBlock root_block;
  root_block.label = "entry";
  root_block.insts.push_back(c4c::codegen::lir::LirCmpOp{
      c4c::codegen::lir::LirOperand::ssa("%cmp", c4c::codegen::lir::LirValueId{1}),
      false,
      c4c::codegen::lir::LirCmpPredicate::Eq,
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::Pointer),
      c4c::codegen::lir::LirOperand::global("@rendered_cmp_lhs_shadow",
                                            semantic_helper_id),
      c4c::codegen::lir::LirOperand::raw("null")});
  root.blocks.push_back(std::move(root_block));
  module.functions.push_back(std::move(root));

  c4c::codegen::lir::LirFunction semantic_helper;
  semantic_helper.name = "semantic_cmp_lhs_helper";
  semantic_helper.link_name_id = semantic_helper_id;
  semantic_helper.is_internal = true;
  semantic_helper.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(semantic_helper));
  ...
  c4c::codegen::lir::LirFunction rendered_shadow;
  rendered_shadow.name = "rendered_cmp_lhs_shadow";
  rendered_shadow.link_name_id = rendered_shadow_id;
  rendered_shadow.is_internal = true;
  rendered_shadow.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(rendered_shadow));
  ...
  expect_true(semantic_it != module.functions.end(),
              "cmp lhs LinkNameId should keep the semantic helper");
  ...
  expect_true(shadow_it == module.functions.end(),
              "stale cmp lhs spelling should not keep the rendered-name shadow");

  c4c::codegen::lir::LirModule legacy_module = make_link_name_aware_lir_module();
  c4c::codegen::lir::LirFunction legacy_root;
  legacy_root.name = "legacy_root_cmp_lhs";
  legacy_root.link_name_id =
      legacy_module.link_names.intern("legacy_root_cmp_lhs");
  c4c::codegen::lir::LirBlock legacy_block;
  legacy_block.label = "entry";
  legacy_block.insts.push_back(c4c::codegen::lir::LirCmpOp{
      c4c::codegen::lir::LirOperand::ssa("%cmp", c4c::codegen::lir::LirValueId{1}),
      false,
      c4c::codegen::lir::LirCmpPredicate::Eq,
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::Pointer),
      c4c::codegen::lir::LirOperand::raw("@rendered_cmp_lhs_shadow"),
      c4c::codegen::lir::LirOperand::raw("null")});
  legacy_root.blocks.push_back(std::move(legacy_block));
  legacy_module.functions.push_back(std::move(legacy_root));

  c4c::codegen::lir::LirFunction legacy_shadow;
  legacy_shadow.name = "rendered_cmp_lhs_shadow";
  legacy_shadow.link_name_id =
      legacy_module.link_names.intern("rendered_cmp_lhs_shadow");
  legacy_shadow.is_internal = true;
  legacy_shadow.can_elide_if_unreferenced = true;
  legacy_module.functions.push_back(std::move(legacy_shadow));
  ...
  c4c::codegen::lir::eliminate_dead_internals(legacy_module);
  expect_true(legacy_module.functions.size() == 2,
              "legacy raw cmp lhs should keep text-scan compatibility");
}

void test_lir_dead_internal_cmp_rhs_uses_link_name_id_before_rendered_text() {
  c4c::codegen::lir::LirModule module = make_link_name_aware_lir_module();
  const c4c::LinkNameId semantic_helper_id =
      module.link_names.intern("semantic_cmp_rhs_helper");
  const c4c::LinkNameId rendered_shadow_id =
      module.link_names.intern("rendered_cmp_rhs_shadow");
  expect_true(semantic_helper_id != c4c::kInvalidLinkName &&
                  rendered_shadow_id != c4c::kInvalidLinkName,
              "fixture should allocate stable cmp rhs reachability LinkNameIds");

  c4c::codegen::lir::LirFunction root;
  root.name = "root_cmp_rhs";
  root.link_name_id = module.link_names.intern("root_cmp_rhs");
  c4c::codegen::lir::LirBlock root_block;
  root_block.label = "entry";
  root_block.insts.push_back(c4c::codegen::lir::LirCmpOp{
      c4c::codegen::lir::LirOperand::ssa("%cmp", c4c::codegen::lir::LirValueId{1}),
      ...
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::Pointer),
      c4c::codegen::lir::LirOperand::raw("null"),
      c4c::codegen::lir::LirOperand::global("@rendered_cmp_rhs_shadow",
                                            semantic_helper_id)});
  root.blocks.push_back(std::move(root_block));
  module.functions.push_back(std::move(root));

  c4c::codegen::lir::LirFunction semantic_helper;
  semantic_helper.name = "semantic_cmp_rhs_helper";
  semantic_helper.link_name_id = semantic_helper_id;
  semantic_helper.is_internal = true;
  semantic_helper.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(semantic_helper));
  ...
  c4c::codegen::lir::LirFunction rendered_shadow;
  rendered_shadow.name = "rendered_cmp_rhs_shadow";
  rendered_shadow.link_name_id = rendered_shadow_id;
  rendered_shadow.is_internal = true;
  rendered_shadow.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(rendered_shadow));
  ...
  expect_true(semantic_it != module.functions.end(),
              "cmp rhs LinkNameId should keep the semantic helper");
  ...
  expect_true(shadow_it == module.functions.end(),
              "stale cmp rhs spelling should not keep the rendered-name shadow");

  c4c::codegen::lir::LirModule legacy_module = make_link_name_aware_lir_module();
  c4c::codegen::lir::LirFunction legacy_root;
  legacy_root.name = "legacy_root_cmp_rhs";
  legacy_root.link_name_id =
      legacy_module.link_names.intern("legacy_root_cmp_rhs");
  c4c::codegen::lir::LirBlock legacy_block;
  legacy_block.label = "entry";
  legacy_block.insts.push_back(c4c::codegen::lir::LirCmpOp{
      c4c::codegen::lir::LirOperand::ssa("%cmp", c4c::codegen::lir::LirValueId{1}),
      ...
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::Pointer),
      c4c::codegen::lir::LirOperand::raw("null"),
      c4c::codegen::lir::LirOperand::raw("@rendered_cmp_rhs_shadow")});
  legacy_root.blocks.push_back(std::move(legacy_block));
  legacy_module.functions.push_back(std::move(legacy_root));

  c4c::codegen::lir::LirFunction legacy_shadow;
  legacy_shadow.name = "rendered_cmp_rhs_shadow";
  legacy_shadow.link_name_id =
      legacy_module.link_names.intern("rendered_cmp_rhs_shadow");
  legacy_shadow.is_internal = true;
  legacy_shadow.can_elide_if_unreferenced = true;
  legacy_module.functions.push_back(std::move(legacy_shadow));
  ...
  c4c::codegen::lir::eliminate_dead_internals(legacy_module);
  expect_true(legacy_module.functions.size() == 2,
              "legacy raw cmp rhs should keep text-scan compatibility");
}

void test_lir_dead_internal_select_cond_uses_link_name_id_before_rendered_text() {
  c4c::codegen::lir::LirModule module = make_link_name_aware_lir_module();
  const c4c::LinkNameId semantic_helper_id =
      module.link_names.intern("semantic_select_cond_helper");
  const c4c::LinkNameId rendered_shadow_id =
      module.link_names.intern("rendered_select_cond_shadow");
  expect_true(semantic_helper_id != c4c::kInvalidLinkName &&
                  rendered_shadow_id != c4c::kInvalidLinkName,
              "fixture should allocate stable select cond reachability LinkNameIds");

  c4c::codegen::lir::LirFunction root;
  root.name = "root_select_cond";
  root.link_name_id = module.link_names.intern("root_select_cond");
  c4c::codegen::lir::LirBlock root_block;
  root_block.label = "entry";
  root_block.insts.push_back(c4c::codegen::lir::LirSelectOp{
      c4c::codegen::lir::LirOperand::ssa("%pick", c4c::codegen::lir::LirValueId{1}),
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::I32),
      c4c::codegen::lir::LirOperand::global("@rendered_select_cond_shadow",
                                            semantic_helper_id),
      c4c::codegen::lir::LirOperand::integer("1", 1),
      c4c::codegen::lir::LirOperand::integer("0", 0)});
  root.blocks.push_back(std::move(root_block));
  module.functions.push_back(std::move(root));

  c4c::codegen::lir::LirFunction semantic_helper;
  semantic_helper.name = "semantic_select_cond_helper";
  semantic_helper.link_name_id = semantic_helper_id;
  semantic_helper.is_internal = true;
  semantic_helper.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(semantic_helper));
  ...
  c4c::codegen::lir::LirFunction rendered_shadow;
  rendered_shadow.name = "rendered_select_cond_shadow";
  rendered_shadow.link_name_id = rendered_shadow_id;
  rendered_shadow.is_internal = true;
  rendered_shadow.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(rendered_shadow));
  ...
  expect_true(semantic_it != module.functions.end(),
              "select cond LinkNameId should keep the semantic helper");
  ...
  expect_true(shadow_it == module.functions.end(),
              "stale select cond spelling should not keep the rendered-name shadow");

  c4c::codegen::lir::LirModule legacy_module = make_link_name_aware_lir_module();
  c4c::codegen::lir::LirFunction legacy_root;
  legacy_root.name = "legacy_root_select_cond";
  legacy_root.link_name_id =
      legacy_module.link_names.intern("legacy_root_select_cond");
  c4c::codegen::lir::LirBlock legacy_block;
  legacy_block.label = "entry";
  legacy_block.insts.push_back(c4c::codegen::lir::LirSelectOp{
      c4c::codegen::lir::LirOperand::ssa("%pick", c4c::codegen::lir::LirValueId{1}),
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::I32),
      c4c::codegen::lir::LirOperand::raw("@rendered_select_cond_shadow"),
      c4c::codegen::lir::LirOperand::integer("1", 1),
      c4c::codegen::lir::LirOperand::integer("0", 0)});
  legacy_root.blocks.push_back(std::move(legacy_block));
  legacy_module.functions.push_back(std::move(legacy_root));
  ...
  c4c::codegen::lir::LirFunction legacy_shadow;
  legacy_shadow.name = "rendered_select_cond_shadow";
  legacy_shadow.link_name_id =
      legacy_module.link_names.intern("rendered_select_cond_shadow");
  legacy_shadow.is_internal = true;
  legacy_shadow.can_elide_if_unreferenced = true;
  legacy_module.functions.push_back(std::move(legacy_shadow));
  ...
  c4c::codegen::lir::eliminate_dead_internals(legacy_module);
  expect_true(legacy_module.functions.size() == 2,
              "legacy raw select cond should keep text-scan compatibility");
}

void test_lir_dead_internal_select_true_val_uses_link_name_id_before_rendered_text() {
  c4c::codegen::lir::LirModule module = make_link_name_aware_lir_module();
  const c4c::LinkNameId semantic_helper_id =
      module.link_names.intern("semantic_select_true_helper");
  const c4c::LinkNameId rendered_shadow_id =
      module.link_names.intern("rendered_select_true_shadow");
  expect_true(semantic_helper_id != c4c::kInvalidLinkName &&
                  rendered_shadow_id != c4c::kInvalidLinkName,
              "fixture should allocate stable select true reachability LinkNameIds");

  c4c::codegen::lir::LirFunction root;
  root.name = "root_select_true";
  root.link_name_id = module.link_names.intern("root_select_true");
  c4c::codegen::lir::LirBlock root_block;
  root_block.label = "entry";
  root_block.insts.push_back(c4c::codegen::lir::LirSelectOp{
      c4c::codegen::lir::LirOperand::ssa("%pick", c4c::codegen::lir::LirValueId{1}),
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::I32),
      c4c::codegen::lir::LirOperand::integer("1", 1),
      c4c::codegen::lir::LirOperand::global("@rendered_select_true_shadow",
                                            semantic_helper_id),
      c4c::codegen::lir::LirOperand::integer("0", 0)});
  root.blocks.push_back(std::move(root_block));
  module.functions.push_back(std::move(root));
  ...
  c4c::codegen::lir::LirFunction semantic_helper;
  semantic_helper.name = "semantic_select_true_helper";
  semantic_helper.link_name_id = semantic_helper_id;
  semantic_helper.is_internal = true;
  semantic_helper.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(semantic_helper));
  ...
  c4c::codegen::lir::LirFunction rendered_shadow;
  rendered_shadow.name = "rendered_select_true_shadow";
  rendered_shadow.link_name_id = rendered_shadow_id;
  rendered_shadow.is_internal = true;
  rendered_shadow.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(rendered_shadow));
  ...
  expect_true(semantic_it != module.functions.end(),
              "select true_val LinkNameId should keep the semantic helper");
  ...
  expect_true(shadow_it == module.functions.end(),
              "stale select true_val spelling should not keep the rendered-name shadow");

  c4c::codegen::lir::LirModule legacy_module = make_link_name_aware_lir_module();
  c4c::codegen::lir::LirFunction legacy_root;
  legacy_root.name = "legacy_root_select_true";
  legacy_root.link_name_id =
      legacy_module.link_names.intern("legacy_root_select_true");
  c4c::codegen::lir::LirBlock legacy_block;
  legacy_block.label = "entry";
  legacy_block.insts.push_back(c4c::codegen::lir::LirSelectOp{
      c4c::codegen::lir::LirOperand::ssa("%pick", c4c::codegen::lir::LirValueId{1}),
      c4c::codegen::lir::LirTypeRef(c4c::codegen::lir::LirBuiltinType::I32),
      c4c::codegen::lir::LirOperand::integer("1", 1),
      c4c::codegen::lir::LirOperand::raw("@rendered_select_true_shadow"),
      c4c::codegen::lir::LirOperand::integer("0", 0)});
  legacy_root.blocks.push_back(std::move(legacy_block));
  legacy_module.functions.push_back(std::move(legacy_root));

  c4c::codegen::lir::LirFunction legacy_shadow;
  legacy_shadow.name = "rendered_select_true_shadow";
  legacy_shadow.link_name_id =
      legacy_module.link_names.intern("rendered_select_true_shadow");
  legacy_shadow.is_internal = true;
  legacy_shadow.can_elide_if_unreferenced = true;
  legacy_module.functions.push_back(std::move(legacy_shadow));
  ...
  c4c::codegen::lir::eliminate_dead_internals(legacy_module);
  expect_true(legacy_module.functions.size() == 2,
              "legacy raw select true_val should keep text-scan compatibility");
}

void test_lir_dead_internal_select_false_val_uses_link_name_id_before_rendered_text() {
  c4c::codegen::lir::LirModule module = make_link_name_aware_lir_module();
  const c4c::LinkNameId semantic_helper_id =
      module.link_names.intern("semantic_select_false_helper");
  const c4c::LinkNameId rendered_shadow_id =
      module.link_names.intern("rendered_select_false_shadow");
  expect_true(semantic_helper_id != c4c::kInvalidLinkName &&
                  rendered_shadow_id != c4c::kInvalidLinkName,
              "fixture should allocate stable select false reachability LinkNameIds");

  c4c::codegen::lir::LirFunction root;
  root.name = "root_select_false";
  root.link_name_id = module.link_names.intern("root_select_false");
  c4c::codegen::lir::LirBlock root_block;
  root_block.label = "entry";
  root_block.insts.push_back(c4c::codegen::lir::LirSelectOp{
      c4c::codegen::lir::LirOperand::ssa("%pick", c4c::codegen::lir::LirValueId{1}),
      ...
      c4c::codegen::lir::LirOperand::integer("1", 1),
      c4c::codegen::lir::LirOperand::integer("0", 0),
      c4c::codegen::lir::LirOperand::global("@rendered_select_false_shadow",
                                            semantic_helper_id)});
  root.blocks.push_back(std::move(root_block));
  module.functions.push_back(std::move(root));

  c4c::codegen::lir::LirFunction semantic_helper;
  semantic_helper.name = "semantic_select_false_helper";
  semantic_helper.link_name_id = semantic_helper_id;
  semantic_helper.is_internal = true;
  semantic_helper.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(semantic_helper));
  ...
  c4c::codegen::lir::LirFunction rendered_shadow;
  rendered_shadow.name = "rendered_select_false_shadow";
  rendered_shadow.link_name_id = rendered_shadow_id;
  rendered_shadow.is_internal = true;
  rendered_shadow.can_elide_if_unreferenced = true;
  module.functions.push_back(std::move(rendered_shadow));
  ...
  expect_true(semantic_it != module.functions.end(),
              "select false_val LinkNameId should keep the semantic helper");
  ...
  expect_true(shadow_it == module.functions.end(),
              "stale select false_val spelling should not keep the rendered-name shadow");

  c4c::codegen::lir::LirModule legacy_module = make_link_name_aware_lir_module();
  c4c::codegen::lir::LirFunction legacy_root;
  legacy_root.name = "legacy_root_select_false";
  legacy_root.link_name_id =
      legacy_module.link_names.intern("legacy_root_select_false");
  c4c::codegen::lir::LirBlock legacy_block;
  legacy_block.label = "entry";
  legacy_block.insts.push_back(c4c::codegen::lir::LirSelectOp{
      c4c::codegen::lir::LirOperand::ssa("%pick", c4c::codegen::lir::LirValueId{1}),
      ...
      c4c::codegen::lir::LirOperand::integer("1", 1),
      c4c::codegen::lir::LirOperand::integer("0", 0),
      c4c::codegen::lir::LirOperand::raw("@rendered_select_false_shadow")});
  legacy_root.blocks.push_back(std::move(legacy_block));
  legacy_module.functions.push_back(std::move(legacy_root));

  c4c::codegen::lir::LirFunction legacy_shadow;
  legacy_shadow.name = "rendered_select_false_shadow";
  legacy_shadow.link_name_id =
      legacy_module.link_names.intern("rendered_select_false_shadow");
  legacy_shadow.is_internal = true;
  legacy_shadow.can_elide_if_unreferenced = true;
  legacy_module.functions.push_back(std::move(legacy_shadow));
  ...
  c4c::codegen::lir::eliminate_dead_internals(legacy_module);
  expect_true(legacy_module.functions.size() == 2,
              "legacy raw select false_val should keep text-scan compatibility");
}
...
int (*global_pick)(int) = rendered_shadow;
)cpp");
  ...
  auto* init_ref =
      std::get_if<c4c::hir::DeclRef>(&hir_module.expr_pool[init->expr.value].payload);
  expect_true(init_ref != nullptr,
              "fixture initializer should carry a decl-ref function designator");
  expect_eq(init_ref->name, "rendered_shadow",
            "fixture raw initializer spelling should be the rendered collision");

  init_ref->link_name_id = semantic_external_id;
  ...
  expect_true(lir_global_it != lir_module.globals.end(),
              "fixture global function pointer should lower into a concrete LIR global");
  expect_true(lir_global_it->init_text.find("@semantic_external_helper") != std::string::npos,
              "global initializer function designator should recover the semantic LinkNameId spelling");
  expect_true(lir_global_it->init_text.find("@rendered_shadow") == std::string::npos,
              "global initializer function designator should not trust the drifted raw spelling");
}
...
void test_hir_to_lir_global_rvalue_load_prefers_link_name_ids() {
  c4c::hir::Module hir_module = lower_hir_module(R"cpp(
int semantic_global;
int rendered_shadow = 2;

int read_global(void) { return semantic_global; }
)cpp");
  ...
  bool rewrote_semantic_ref = false;
  for (auto& expr : hir_module.expr_pool) {
    auto* ref = std::get_if<c4c::hir::DeclRef>(&expr.payload);
    if (ref != nullptr && ref->global &&
        ref->global->value == semantic_global->id.value) {
      ref->name = shadow_global->name;
      ref->link_name_id = c4c::kInvalidLinkName;
      rewrote_semantic_ref = true;
    }
  }
  expect_true(rewrote_semantic_ref,
              "fixture reader should contain a concrete semantic global DeclRef");
  semantic_global->name = shadow_global->name;

  const c4c::codegen::lir::LirModule lir_module = c4c::codegen::lir::lower(hir_module);

  const auto reader_it = std::find_if(
      lir_module.functions.begin(), lir_module.functions.end(),
      [&](const c4c::codegen::lir::LirFunction& fn) {
        return fn.link_name_id != c4c::kInvalidLinkName &&
               lir_module.link_names.spelling(fn.link_name_id) == "read_global";
      });
  expect_true(reader_it != lir_module.functions.end(),
              "fixture reader should lower through semantic function identity");

  bool saw_semantic_load = false;
  bool saw_shadow_load = false;
  for (const auto& block : reader_it->blocks) {
    for (const auto& inst : block.insts) {
      const auto* load = std::get_if<c4c::codegen::lir::LirLoadOp>(&inst);
      if (load == nullptr) continue;
      if (load->ptr == "@semantic_global") saw_semantic_load = true;
      if (load->ptr == "@rendered_shadow") saw_shadow_load = true;
    }
  }
  expect_true(saw_semantic_load,
              "global rvalue load lowering should recover the target spelling from GlobalId LinkNameId");
  expect_true(!saw_shadow_load,
              "global rvalue load lowering should not reselect by corrupted rendered global spelling");
}
...
int main() {
  ...
  test_hir_to_lir_global_rvalue_load_prefers_link_name_ids();
  ...
  test_lir_dead_internal_phi_value_uses_link_name_id_before_rendered_text();
  test_lir_dead_internal_cast_operand_uses_link_name_id_before_rendered_text();
  test_lir_dead_internal_binop_lhs_uses_link_name_id_before_rendered_text();
  test_lir_dead_internal_binop_rhs_uses_link_name_id_before_rendered_text();
  test_lir_dead_internal_cmp_lhs_uses_link_name_id_before_rendered_text();
  test_lir_dead_internal_cmp_rhs_uses_link_name_id_before_rendered_text();
  test_lir_dead_internal_select_cond_uses_link_name_id_before_rendered_text();
  test_lir_dead_internal_select_true_val_uses_link_name_id_before_rendered_text();
  test_lir_dead_internal_select_false_val_uses_link_name_id_before_rendered_text();
