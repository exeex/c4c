...
void test_direct_scalar_fmul_lhs_authority() {
  c4c::hir::Module hir_module = lower_hir_module(R"c(
double scalar_fmul_lhs(double x) { return x * 2.0; }
)c");
  const c4c::codegen::lir::LirModule module = c4c::codegen::lir::lower(hir_module);
  const auto find_fmul = [](const auto& function) -> const c4c::codegen::lir::LirBinOp& {
    for (const auto& block : function.blocks) {
      for (const auto& inst : block.insts) {
        if (const auto* binary = std::get_if<c4c::codegen::lir::LirBinOp>(&inst);
            binary != nullptr &&
            binary->opcode.typed() == c4c::codegen::lir::LirBinaryOpcode::FMul) {
          return *binary;
        }
      }
    }
    fail("scalar floating multiply fixture should lower one fmul LirBinOp");
  };
  const auto& function = require_function(module, "scalar_fmul_lhs", false);
  const auto& binary = find_fmul(function);
  expect_true(binary.scalar_lhs_parameter_authority.has_value(),
              "direct scalar fmul LHS must publish native parameter authority");
  expect_true(!binary.scalar_rhs_parameter_authority.has_value(),
              "direct scalar fmul selected LHS row must leave the nonselected RHS unclaimed");
  const auto& authority = *binary.scalar_lhs_parameter_authority;
  const auto& definition = function.native_body_parameter_definitions.front();
  expect_true(binary.lhs.value_id() && authority.value == *binary.lhs.value_id() &&
                  authority.value == definition.value &&
                  ...
                  authority.abi == c4c::codegen::lir::LirNativeBodyParameterAbi::DirectScalar &&
                  authority.role == c4c::codegen::lir::LirScalarBinaryParameterRole::Lhs,
              "direct scalar fmul LHS authority must mirror its current-function definition");
  c4c::codegen::lir::verify_module(module);

  {
    auto candidate = module;
    auto& candidate_function = require_mutable_function(candidate, "scalar_fmul_lhs", false);
    bool verified = false;
    for (auto& block : candidate_function.blocks) {
      for (auto& inst : block.insts) {
        if (auto* candidate_binary = std::get_if<c4c::codegen::lir::LirBinOp>(&inst);
            candidate_binary != nullptr &&
            candidate_binary->opcode.typed() == c4c::codegen::lir::LirBinaryOpcode::FMul) {
          const auto ordinary_rhs =
              c4c::codegen::lir::LirOperand::ssa("%ordinary_fmul_rhs",
                                                 c4c::codegen::lir::LirValueId{999997});
          c4c::codegen::lir::LirBinOp ordinary_producer{
              c4c::codegen::lir::LirOperand::ssa("%ordinary_fmul_rhs",
                                                 c4c::codegen::lir::LirValueId{999997}),
              "fadd",
              candidate_binary->type_str,
              candidate_binary->rhs,
              candidate_binary->rhs};
          candidate_binary->rhs = ordinary_rhs;
          block.insts.push_back(ordinary_producer);
          c4c::codegen::lir::verify_module(candidate);
          verified = true;
          break;
        }
      }
    ...
    expect_true(verified, "mutable scalar fmul fixture should contain one fmul LirBinOp");
  }

  {
    ...
    for (auto& block : candidate_function.blocks) {
      for (auto& inst : block.insts) {
        if (auto* candidate_binary = std::get_if<c4c::codegen::lir::LirBinOp>(&inst);
            candidate_binary != nullptr &&
            candidate_binary->opcode.typed() == c4c::codegen::lir::LirBinaryOpcode::FMul) {
          candidate_binary->rhs = candidate_binary->lhs;
          c4c::codegen::lir::verify_module(candidate);
          verified = true;
          break;
        }
    ...
    expect_true(verified,
                "DirectScalar fmul LHS verifier must allow nonselected same-parameter RHS reuse");
  }
  ...
  const auto rejects = [&](auto mutate, const std::string& message) {
    ...
    fail("mutable scalar fmul fixture should contain one fmul LirBinOp");
  };
  rejects([](auto&, auto&, auto& binary) { binary.scalar_lhs_parameter_authority.reset(); },
          "DirectScalar fmul LHS verifier must reject omitted authority");
  rejects([](auto&, auto& function, auto&) {
            function.native_body_parameter_definitions.clear();
          },
          "DirectScalar fmul LHS verifier must reject missing parameter definition");
  rejects([](auto&, auto&, auto& binary) {
            binary.scalar_lhs_parameter_authority->value =
                c4c::codegen::lir::LirValueId::invalid();
          },
          "DirectScalar fmul LHS verifier must reject invalid parameter identity");
  rejects([](auto&, auto& function, auto&) {
            function.native_body_parameter_definitions.push_back(
                function.native_body_parameter_definitions.front());
          },
          "DirectScalar fmul LHS verifier must reject duplicate parameter definitions");
  rejects([](auto& candidate, auto&, auto& binary) {
            binary.scalar_lhs_parameter_authority->owner =
                candidate.link_names.intern("foreign_fmul_lhs_owner");
          },
          "DirectScalar fmul LHS verifier must reject foreign parameter owner");
  rejects([](auto&, auto&, auto& binary) {
            binary.scalar_lhs_parameter_authority->parameter_index = 1;
          },
          "DirectScalar fmul LHS verifier must reject wrong parameter index");
  rejects([](auto&, auto&, auto& binary) {
            binary.scalar_lhs_parameter_authority->type =
                c4c::codegen::lir::LirTypeRef("float");
          },
          "DirectScalar fmul LHS verifier must reject wrong parameter type");
  rejects([](auto&, auto&, auto& binary) {
            binary.scalar_lhs_parameter_authority->abi =
                c4c::codegen::lir::LirNativeBodyParameterAbi::DirectPointer;
          },
          "DirectScalar fmul LHS verifier must reject wrong parameter ABI");
  rejects([](auto&, auto&, auto& binary) {
            binary.scalar_lhs_parameter_authority->role =
                c4c::codegen::lir::LirScalarBinaryParameterRole::Rhs;
          },
          "DirectScalar fmul LHS verifier must reject wrong parameter role");
  rejects([](auto&, auto&, auto& binary) { binary.opcode = "fdiv"; },
          "DirectScalar fmul LHS verifier must reject non-fmul consumer");
  rejects([](auto&, auto&, auto& binary) {
            binary.lhs = c4c::codegen::lir::LirOperand::ssa(
                "%foreign", c4c::codegen::lir::LirValueId{999999});
          },
          "DirectScalar fmul LHS verifier must reject LHS mismatch");
  rejects([](auto&, auto&, auto& binary) {
  ...
  rejects([](auto&, auto&, auto& binary) {
            ...
            const auto& lhs_authority = *binary.scalar_lhs_parameter_authority;
            binary.scalar_rhs_parameter_authority =
                c4c::codegen::lir::LirScalarBinaryRhsParameterAuthority{
                    .value = lhs_authority.value,
                    .parameter_index = lhs_authority.parameter_index,
                    .type = lhs_authority.type,
                    .owner = lhs_authority.owner,
                    .abi = lhs_authority.abi,
                    .role = c4c::codegen::lir::LirScalarBinaryParameterRole::Rhs};
          },
          "DirectScalar fmul LHS verifier must reject explicit selected RHS authority");
}
...
double scalar_fadd_lhs(double x) { return x + 2.0; }
...

  {
    ...
    expect_true(verified,
                "DirectScalar fadd LHS verifier must allow nonselected same-parameter RHS reuse");
  }
  ...
  rejects([](auto&, auto&, auto&, auto& binary) { binary.opcode = "fdiv"; },
  ...
  rejects([](auto&, auto&, auto&, auto& binary) {
          ...
          "DirectScalar fadd LHS verifier must reject explicit selected RHS authority");
...
void test_direct_scalar_fsub_lhs_authority() {
  c4c::hir::Module hir_module = lower_hir_module(R"c(
double scalar_fsub_lhs(double x) { return x - 2.0; }
)c");
  const c4c::codegen::lir::LirModule module = c4c::codegen::lir::lower(hir_module);
  const auto find_fsub = [](const auto& function) -> const c4c::codegen::lir::LirBinOp& {
    for (const auto& block : function.blocks) {
      for (const auto& inst : block.insts) {
        if (const auto* binary = std::get_if<c4c::codegen::lir::LirBinOp>(&inst);
            binary != nullptr &&
            binary->opcode.typed() == c4c::codegen::lir::LirBinaryOpcode::FSub) {
          return *binary;
        }
      }
    }
    fail("scalar floating subtract fixture should lower one fsub LirBinOp");
  };
  const auto& function = require_function(module, "scalar_fsub_lhs", false);
  const auto& binary = find_fsub(function);
  expect_true(binary.scalar_lhs_parameter_authority.has_value(),
              "direct scalar fsub LHS must publish native parameter authority");
  expect_true(!binary.scalar_rhs_parameter_authority.has_value(),
              "direct scalar fsub selected LHS row must leave the nonselected RHS unclaimed");
  const auto& authority = *binary.scalar_lhs_parameter_authority;
  const auto& definition = function.native_body_parameter_definitions.front();
  expect_true(binary.lhs.value_id() && authority.value == *binary.lhs.value_id() &&
                  authority.value == definition.value &&
                  ...
                  authority.abi == c4c::codegen::lir::LirNativeBodyParameterAbi::DirectScalar &&
                  authority.role == c4c::codegen::lir::LirScalarBinaryParameterRole::Lhs,
              "direct scalar fsub LHS authority must mirror its current-function definition");
  c4c::codegen::lir::verify_module(module);

  {
    auto candidate = module;
    auto& candidate_function = require_mutable_function(candidate, "scalar_fsub_lhs", false);
    bool verified = false;
    for (auto& block : candidate_function.blocks) {
      for (auto& inst : block.insts) {
        if (auto* candidate_binary = std::get_if<c4c::codegen::lir::LirBinOp>(&inst);
            candidate_binary != nullptr &&
            candidate_binary->opcode.typed() == c4c::codegen::lir::LirBinaryOpcode::FSub) {
          candidate_binary->rhs = candidate_binary->lhs;
          c4c::codegen::lir::verify_module(candidate);
          verified = true;
          break;
    ...
    expect_true(verified,
                "DirectScalar fsub LHS verifier must allow nonselected same-parameter RHS reuse");
  }
  ...
  const auto rejects = [&](auto mutate, const std::string& message) {
    ...
    fail("mutable scalar fsub fixture should contain one fsub LirBinOp");
  };
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_lhs_parameter_authority.reset();
          },
          "DirectScalar fsub LHS verifier must reject omitted authority");
  rejects([](auto&, auto& function, auto&, auto&) {
            function.native_body_parameter_definitions.clear();
          },
          "DirectScalar fsub LHS verifier must reject missing parameter definition");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_lhs_parameter_authority->value =
                c4c::codegen::lir::LirValueId::invalid();
          },
          "DirectScalar fsub LHS verifier must reject invalid parameter identity");
  rejects([](auto&, auto& function, auto&, auto&) {
            function.native_body_parameter_definitions.push_back(
                function.native_body_parameter_definitions.front());
          },
          "DirectScalar fsub LHS verifier must reject duplicate parameter definitions");
  rejects([](auto& candidate, auto&, auto&, auto& binary) {
            binary.scalar_lhs_parameter_authority->owner =
                candidate.link_names.intern("foreign_fsub_lhs_owner");
          },
          "DirectScalar fsub LHS verifier must reject foreign parameter owner");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_lhs_parameter_authority->parameter_index = 1;
          },
          "DirectScalar fsub LHS verifier must reject wrong parameter index");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_lhs_parameter_authority->type =
                c4c::codegen::lir::LirTypeRef("float");
          },
          "DirectScalar fsub LHS verifier must reject wrong parameter type");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_lhs_parameter_authority->abi =
                c4c::codegen::lir::LirNativeBodyParameterAbi::DirectPointer;
          },
          "DirectScalar fsub LHS verifier must reject wrong parameter ABI");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_lhs_parameter_authority->role =
                c4c::codegen::lir::LirScalarBinaryParameterRole::Rhs;
          },
          "DirectScalar fsub LHS verifier must reject wrong parameter role");
  rejects([](auto&, auto&, auto&, auto& binary) { binary.opcode = "fdiv"; },
          "DirectScalar fsub LHS verifier must reject non-fsub consumer");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.lhs = c4c::codegen::lir::LirOperand::ssa(
                "%foreign", c4c::codegen::lir::LirValueId{999999});
          },
          "DirectScalar fsub LHS verifier must reject LHS mismatch");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.type_str = c4c::codegen::lir::LirTypeRef("float");
          },
          "DirectScalar fsub LHS verifier must reject type mismatch");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.rhs = binary.lhs;
            const auto& lhs_authority = *binary.scalar_lhs_parameter_authority;
            binary.scalar_rhs_parameter_authority =
          ...
          "DirectScalar fsub LHS verifier must reject explicit selected RHS authority");
  rejects([](auto&, auto&, auto& block, auto& binary) {
            auto duplicate = binary;
            duplicate.result = c4c::codegen::lir::LirOperand::ssa(
                "%duplicate_fsub_lhs", c4c::codegen::lir::LirValueId{999996});
            block.insts.push_back(duplicate);
          },
          "DirectScalar fsub LHS verifier must reject duplicate consumer");
}

void test_direct_scalar_binary_parameter_lhs_preempts_rhs_authority() {
  c4c::hir::Module hir_module = lower_hir_module(R"c(
double scalar_fadd_parameter_pair(double x, double y) { return x + y; }
)c");
  const c4c::codegen::lir::LirModule module = c4c::codegen::lir::lower(hir_module);
  const auto find_fadd = [](const auto& function) -> const c4c::codegen::lir::LirBinOp& {
    for (const auto& block : function.blocks) {
    ...
    fail("two-parameter scalar floating add fixture should lower one fadd LirBinOp");
  };
  const auto& function = require_function(module, "scalar_fadd_parameter_pair", false);
  const auto& binary = find_fadd(function);
  expect_true(binary.scalar_lhs_parameter_authority.has_value(),
              "two-parameter fadd must publish selected LHS authority");
  expect_true(!binary.scalar_rhs_parameter_authority.has_value(),
              "two-parameter fadd selected LHS row must not also publish RHS authority");
  const auto& lhs_definition = function.native_body_parameter_definitions[0];
  const auto& rhs_definition = function.native_body_parameter_definitions[1];
  const auto& authority = *binary.scalar_lhs_parameter_authority;
  expect_true(binary.lhs.value_id() && binary.rhs.value_id() &&
                  authority.value == *binary.lhs.value_id() &&
                  authority.value == lhs_definition.value &&
                  *binary.rhs.value_id() == rhs_definition.value &&
                  authority.parameter_index == lhs_definition.parameter_index &&
                  authority.type == binary.type_str && authority.type == lhs_definition.type &&
                  authority.owner == function.link_name_id &&
                  authority.owner == lhs_definition.owner &&
                  authority.abi == c4c::codegen::lir::LirNativeBodyParameterAbi::DirectScalar &&
                  authority.role == c4c::codegen::lir::LirScalarBinaryParameterRole::Lhs,
              "two-parameter fadd LHS authority must mirror only the LHS definition");
  c4c::codegen::lir::verify_module(module);
}

void test_direct_scalar_unsupported_fdiv_suppresses_parameter_authority() {
  c4c::hir::Module hir_module = lower_hir_module(R"c(
double scalar_fdiv_lhs(double x) { return x / 2.0; }
double scalar_fdiv_rhs(double x) { return 2.0 / x; }
)c");
  const c4c::codegen::lir::LirModule module = c4c::codegen::lir::lower(hir_module);
  const auto expect_fdiv_unclaimed = [](const auto& function, const std::string& message) {
    bool found = false;
    for (const auto& block : function.blocks) {
      for (const auto& inst : block.insts) {
        if (const auto* binary = std::get_if<c4c::codegen::lir::LirBinOp>(&inst);
            binary != nullptr &&
            binary->opcode.typed() == c4c::codegen::lir::LirBinaryOpcode::FDiv) {
          expect_true(!binary->scalar_lhs_parameter_authority.has_value() &&
                          !binary->scalar_rhs_parameter_authority.has_value(),
                      message);
          found = true;
        }
      }
    }
    expect_true(found, message + " should contain an fdiv consumer");
  };
  expect_fdiv_unclaimed(require_function(module, "scalar_fdiv_lhs", false),
                        "unsupported fdiv LHS parameter consumer must stay unclaimed");
  expect_fdiv_unclaimed(require_function(module, "scalar_fdiv_rhs", false),
                        "unsupported fdiv RHS parameter consumer must stay unclaimed");
  c4c::codegen::lir::verify_module(module);
}

void test_direct_scalar_duplicate_fsub_lhs_publishes_first_authority_only() {
  c4c::hir::Module hir_module = lower_hir_module(R"c(
double scalar_duplicate_fsub_lhs(double x) { return (x - 1.0) + (x - 2.0); }
)c");
  const c4c::codegen::lir::LirModule module = c4c::codegen::lir::lower(hir_module);
  const auto& function = require_function(module, "scalar_duplicate_fsub_lhs", false);
  std::size_t fsub_count = 0;
  std::size_t fsub_lhs_authority_count = 0;
  for (const auto& block : function.blocks) {
    for (const auto& inst : block.insts) {
      if (const auto* binary = std::get_if<c4c::codegen::lir::LirBinOp>(&inst);
          binary != nullptr &&
          binary->opcode.typed() == c4c::codegen::lir::LirBinaryOpcode::FSub) {
        ++fsub_count;
        if (binary->scalar_lhs_parameter_authority) ++fsub_lhs_authority_count;
        expect_true(!binary->scalar_rhs_parameter_authority.has_value(),
                    "duplicate fsub LHS fixture must not publish RHS authority");
      }
    }
  }
  expect_true(fsub_count == 2,
              "duplicate fsub LHS fixture should lower two fsub consumers");
  expect_true(fsub_lhs_authority_count == 1,
              "duplicate fsub LHS fixture must publish exactly the first selected row");
  c4c::codegen::lir::verify_module(module);
}

void test_direct_scalar_fmul_rhs_authority() {
  c4c::hir::Module hir_module = lower_hir_module(R"c(
double scalar_fmul_rhs(double x) { return 2.0 * x; }
)c");
  const c4c::codegen::lir::LirModule module = c4c::codegen::lir::lower(hir_module);
  const auto find_fmul = [](const auto& function) -> const c4c::codegen::lir::LirBinOp& {
    for (const auto& block : function.blocks) {
    ...
    fail("scalar floating multiply RHS fixture should lower one fmul LirBinOp");
  };
  const auto& function = require_function(module, "scalar_fmul_rhs", false);
  const auto& binary = find_fmul(function);
  expect_true(!binary.scalar_lhs_parameter_authority.has_value(),
              "direct scalar fmul selected RHS row must leave the nonselected LHS unclaimed");
  expect_true(binary.scalar_rhs_parameter_authority.has_value(),
              "direct scalar fmul RHS must publish native parameter authority");
  const auto& authority = *binary.scalar_rhs_parameter_authority;
  const auto& definition = function.native_body_parameter_definitions.front();
  expect_true(binary.rhs.value_id() && authority.value == *binary.rhs.value_id() &&
                  authority.value == definition.value &&
                  authority.parameter_index == definition.parameter_index &&
                  authority.type == binary.type_str && authority.type == definition.type &&
                  authority.owner == function.link_name_id && authority.owner == definition.owner &&
                  authority.abi == c4c::codegen::lir::LirNativeBodyParameterAbi::DirectScalar &&
                  authority.role == c4c::codegen::lir::LirScalarBinaryParameterRole::Rhs,
              "direct scalar fmul RHS authority must mirror its current-function definition");
  c4c::codegen::lir::verify_module(module);

  const auto rejects = [&](auto mutate, const std::string& message) {
    auto candidate = module;
    auto& candidate_function = require_mutable_function(candidate, "scalar_fmul_rhs", false);
    for (auto& block : candidate_function.blocks) {
      for (auto& inst : block.insts) {
        if (auto* candidate_binary = std::get_if<c4c::codegen::lir::LirBinOp>(&inst);
            candidate_binary != nullptr &&
    ...
    fail("mutable scalar fmul RHS fixture should contain one fmul LirBinOp");
  };
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority.reset();
          },
          "DirectScalar fmul RHS verifier must reject omitted authority");
  rejects([](auto&, auto& function, auto&, auto&) {
            function.native_body_parameter_definitions.clear();
          },
          "DirectScalar fmul RHS verifier must reject missing parameter definition");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->value =
                c4c::codegen::lir::LirValueId::invalid();
          },
          "DirectScalar fmul RHS verifier must reject invalid parameter identity");
  rejects([](auto&, auto& function, auto&, auto&) {
            function.native_body_parameter_definitions.push_back(
                function.native_body_parameter_definitions.front());
          },
          "DirectScalar fmul RHS verifier must reject duplicate parameter definitions");
  rejects([](auto& candidate, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->owner =
                candidate.link_names.intern("foreign_fmul_rhs_owner");
          },
          "DirectScalar fmul RHS verifier must reject foreign parameter owner");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->parameter_index = 1;
          },
          "DirectScalar fmul RHS verifier must reject wrong parameter index");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->type =
                c4c::codegen::lir::LirTypeRef("float");
          },
          "DirectScalar fmul RHS verifier must reject wrong parameter type");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->abi =
                c4c::codegen::lir::LirNativeBodyParameterAbi::DirectPointer;
          },
          "DirectScalar fmul RHS verifier must reject wrong parameter ABI");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->role =
                c4c::codegen::lir::LirScalarBinaryParameterRole::Lhs;
          },
          "DirectScalar fmul RHS verifier must reject wrong parameter role");
  rejects([](auto&, auto&, auto&, auto& binary) { binary.opcode = "fdiv"; },
          "DirectScalar fmul RHS verifier must reject non-fmul consumer");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.rhs = c4c::codegen::lir::LirOperand::ssa(
                "%foreign", c4c::codegen::lir::LirValueId{999999});
          },
          "DirectScalar fmul RHS verifier must reject RHS mismatch");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.type_str = c4c::codegen::lir::LirTypeRef("float");
          },
          "DirectScalar fmul RHS verifier must reject type mismatch");
  rejects([](auto&, auto&, auto&, auto& binary) { binary.lhs = binary.rhs; },
          "DirectScalar fmul RHS verifier must reject selected LHS coherence");
  rejects([](auto&, auto&, auto& block, auto& binary) {
            auto duplicate = binary;
            duplicate.result = c4c::codegen::lir::LirOperand::ssa(
                "%duplicate_fmul_rhs", c4c::codegen::lir::LirValueId{999998});
            block.insts.push_back(duplicate);
          },
          "DirectScalar fmul RHS verifier must reject duplicate consumer");
}

void test_direct_scalar_fadd_rhs_authority() {
  c4c::hir::Module hir_module = lower_hir_module(R"c(
double scalar_fadd_rhs(double x) { return 2.0 + x; }
)c");
  const c4c::codegen::lir::LirModule module = c4c::codegen::lir::lower(hir_module);
  const auto find_fadd = [](const auto& function) -> const c4c::codegen::lir::LirBinOp& {
    for (const auto& block : function.blocks) {
    ...
    fail("scalar floating add RHS fixture should lower one fadd LirBinOp");
  };
  const auto& function = require_function(module, "scalar_fadd_rhs", false);
  const auto& binary = find_fadd(function);
  expect_true(!binary.scalar_lhs_parameter_authority.has_value(),
              "direct scalar fadd selected RHS row must leave the nonselected LHS unclaimed");
  expect_true(binary.scalar_rhs_parameter_authority.has_value(),
              "direct scalar fadd RHS must publish native parameter authority");
  const auto& authority = *binary.scalar_rhs_parameter_authority;
  const auto& definition = function.native_body_parameter_definitions.front();
  expect_true(binary.rhs.value_id() && authority.value == *binary.rhs.value_id() &&
                  authority.value == definition.value &&
                  ...
                  authority.abi == c4c::codegen::lir::LirNativeBodyParameterAbi::DirectScalar &&
                  authority.role == c4c::codegen::lir::LirScalarBinaryParameterRole::Rhs,
              "direct scalar fadd RHS authority must mirror its current-function definition");
  c4c::codegen::lir::verify_module(module);

  const auto rejects = [&](auto mutate, const std::string& message) {
    auto candidate = module;
    auto& candidate_function = require_mutable_function(candidate, "scalar_fadd_rhs", false);
    for (auto& block : candidate_function.blocks) {
      for (auto& inst : block.insts) {
        if (auto* candidate_binary = std::get_if<c4c::codegen::lir::LirBinOp>(&inst);
            candidate_binary != nullptr &&
    ...
    fail("mutable scalar fadd RHS fixture should contain one fadd LirBinOp");
  };
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority.reset();
          },
          "DirectScalar fadd RHS verifier must reject omitted authority");
  rejects([](auto&, auto& function, auto&, auto&) {
            function.native_body_parameter_definitions.clear();
          },
          "DirectScalar fadd RHS verifier must reject missing parameter definition");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->value =
                c4c::codegen::lir::LirValueId::invalid();
          },
          "DirectScalar fadd RHS verifier must reject invalid parameter identity");
  rejects([](auto&, auto& function, auto&, auto&) {
            function.native_body_parameter_definitions.push_back(
                function.native_body_parameter_definitions.front());
          },
          "DirectScalar fadd RHS verifier must reject duplicate parameter definitions");
  rejects([](auto& candidate, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->owner =
                candidate.link_names.intern("foreign_fadd_rhs_owner");
          },
          "DirectScalar fadd RHS verifier must reject foreign parameter owner");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->parameter_index = 1;
          },
          "DirectScalar fadd RHS verifier must reject wrong parameter index");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->type =
                c4c::codegen::lir::LirTypeRef("float");
          },
          "DirectScalar fadd RHS verifier must reject wrong parameter type");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->abi =
                c4c::codegen::lir::LirNativeBodyParameterAbi::DirectPointer;
          },
          "DirectScalar fadd RHS verifier must reject wrong parameter ABI");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->role =
                c4c::codegen::lir::LirScalarBinaryParameterRole::Lhs;
          },
          "DirectScalar fadd RHS verifier must reject wrong parameter role");
  rejects([](auto&, auto&, auto&, auto& binary) { binary.opcode = "fdiv"; },
          "DirectScalar fadd RHS verifier must reject non-fadd consumer");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.rhs = c4c::codegen::lir::LirOperand::ssa(
                "%foreign", c4c::codegen::lir::LirValueId{999999});
          },
          "DirectScalar fadd RHS verifier must reject RHS mismatch");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.type_str = c4c::codegen::lir::LirTypeRef("float");
          },
          "DirectScalar fadd RHS verifier must reject type mismatch");
  rejects([](auto&, auto&, auto&, auto& binary) { binary.lhs = binary.rhs; },
          "DirectScalar fadd RHS verifier must reject selected LHS coherence");
  rejects([](auto&, auto&, auto& block, auto& binary) {
            auto duplicate = binary;
            duplicate.result = c4c::codegen::lir::LirOperand::ssa(
                "%duplicate_fadd_rhs", c4c::codegen::lir::LirValueId{999997});
            block.insts.push_back(duplicate);
          },
          "DirectScalar fadd RHS verifier must reject duplicate consumer");
}

void test_direct_scalar_fsub_rhs_authority() {
  c4c::hir::Module hir_module = lower_hir_module(R"c(
double scalar_fsub_rhs(double x) { return 2.0 - x; }
  ...
  const auto find_fsub = [](const auto& function) -> const c4c::codegen::lir::LirBinOp& {
    ...
    fail("scalar floating subtract RHS fixture should lower one fsub LirBinOp");
  };
  const auto& function = require_function(module, "scalar_fsub_rhs", false);
  const auto& binary = find_fsub(function);
  expect_true(!binary.scalar_lhs_parameter_authority.has_value(),
              "direct scalar fsub selected RHS row must leave the nonselected LHS unclaimed");
  expect_true(binary.scalar_rhs_parameter_authority.has_value(),
              "direct scalar fsub RHS must publish native parameter authority");
  const auto& authority = *binary.scalar_rhs_parameter_authority;
  const auto& definition = function.native_body_parameter_definitions.front();
  expect_true(binary.rhs.value_id() && authority.value == *binary.rhs.value_id() &&
                  authority.value == definition.value &&
                  ...
                  authority.abi == c4c::codegen::lir::LirNativeBodyParameterAbi::DirectScalar &&
                  authority.role == c4c::codegen::lir::LirScalarBinaryParameterRole::Rhs,
              "direct scalar fsub RHS authority must mirror its current-function definition");
  c4c::codegen::lir::verify_module(module);

  const auto rejects = [&](auto mutate, const std::string& message) {
    auto candidate = module;
    auto& candidate_function = require_mutable_function(candidate, "scalar_fsub_rhs", false);
    for (auto& block : candidate_function.blocks) {
      for (auto& inst : block.insts) {
        if (auto* candidate_binary = std::get_if<c4c::codegen::lir::LirBinOp>(&inst);
            candidate_binary != nullptr &&
    ...
    fail("mutable scalar fsub RHS fixture should contain one fsub LirBinOp");
  };
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority.reset();
          },
          "DirectScalar fsub RHS verifier must reject omitted authority");
  rejects([](auto&, auto& function, auto&, auto&) {
            function.native_body_parameter_definitions.clear();
          },
          "DirectScalar fsub RHS verifier must reject missing parameter definition");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->value =
                c4c::codegen::lir::LirValueId::invalid();
          },
          "DirectScalar fsub RHS verifier must reject invalid parameter identity");
  rejects([](auto&, auto& function, auto&, auto&) {
            function.native_body_parameter_definitions.push_back(
                function.native_body_parameter_definitions.front());
          },
          "DirectScalar fsub RHS verifier must reject duplicate parameter definitions");
  rejects([](auto& candidate, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->owner =
                candidate.link_names.intern("foreign_fsub_rhs_owner");
          },
          "DirectScalar fsub RHS verifier must reject foreign parameter owner");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->parameter_index = 1;
          },
          "DirectScalar fsub RHS verifier must reject wrong parameter index");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->type =
                c4c::codegen::lir::LirTypeRef("float");
          },
          "DirectScalar fsub RHS verifier must reject wrong parameter type");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->abi =
                c4c::codegen::lir::LirNativeBodyParameterAbi::DirectPointer;
          },
          "DirectScalar fsub RHS verifier must reject wrong parameter ABI");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.scalar_rhs_parameter_authority->role =
                c4c::codegen::lir::LirScalarBinaryParameterRole::Lhs;
          },
          "DirectScalar fsub RHS verifier must reject wrong parameter role");
  rejects([](auto&, auto&, auto&, auto& binary) { binary.opcode = "fdiv"; },
          "DirectScalar fsub RHS verifier must reject non-fsub consumer");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.rhs = c4c::codegen::lir::LirOperand::ssa(
                "%foreign", c4c::codegen::lir::LirValueId{999999});
          },
          "DirectScalar fsub RHS verifier must reject RHS mismatch");
  rejects([](auto&, auto&, auto&, auto& binary) {
            binary.type_str = c4c::codegen::lir::LirTypeRef("float");
          },
          "DirectScalar fsub RHS verifier must reject type mismatch");
  rejects([](auto&, auto&, auto&, auto& binary) { binary.lhs = binary.rhs; },
          "DirectScalar fsub RHS verifier must reject selected LHS coherence");
  rejects([](auto&, auto&, auto& block, auto& binary) {
            auto duplicate = binary;
            duplicate.result = c4c::codegen::lir::LirOperand::ssa(
                "%duplicate_fsub_rhs", c4c::codegen::lir::LirValueId{999996});
            block.insts.push_back(duplicate);
          },
          "DirectScalar fsub RHS verifier must reject duplicate consumer");
}
}  // namespace
...
int main() {
  ...
  test_direct_scalar_fsub_lhs_authority();
  test_direct_scalar_binary_parameter_lhs_preempts_rhs_authority();
  test_direct_scalar_unsupported_fdiv_suppresses_parameter_authority();
  test_direct_scalar_duplicate_fsub_lhs_publishes_first_authority_only();
  test_direct_scalar_fmul_rhs_authority();
  test_direct_scalar_fadd_rhs_authority();
  test_direct_scalar_fsub_rhs_authority();
