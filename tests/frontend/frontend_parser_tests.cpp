...
void test_parser_visible_type_alias_uses_scope_local_typedef_facade() {
  ...
  expect_eq(parser.resolve_visible_type_name(parser_test_text_id(parser, "Alias")), "Target",
...
void test_parser_visible_type_alias_uses_token_text_id_scope_lookup() {
  ...
  expect_eq(parser.resolve_visible_type_name(alias_text), "Target",
...
void test_parser_visible_type_alias_keeps_qualified_target_resolution() {
  ...
  expect_eq(parser.resolve_visible_type_name(parser_test_text_id(parser, "Alias")), "ns::Target",
...
void test_parser_using_value_import_keeps_structured_target_key() {
  ...
  expect_eq(parser.resolve_visible_value_name(alias_text), "ns::Target",
            "using-import lookup should render from the structured target key");
}
...
void test_parser_using_value_import_prefers_structured_type_over_corrupt_rendered_name() {
  ...
  expect_true(visible_alias != nullptr && visible_alias->base == c4c::TB_INT,
              "using value imports should prefer the structured target type over rendered bridge storage");
}
...
void test_parser_deferred_member_typedef_lookup_uses_member_text_id() {
  ...
  const c4c::Token box_token =
      parser.make_injected_token(seed, c4c::TokenKind::Identifier, "Box");
  const c4c::Token arg_token =
      parser.make_injected_token(seed, c4c::TokenKind::Identifier, "Arg");
  ...
  arg_ts.tag_text_id = owner_token.text_id;
...
void test_parser_using_value_alias_rejects_missing_structured_target_bridge() {
  ...
  parser.register_using_value_alias_for_testing(
      0, compat_alias_text, c4c::QualifiedNameKey{});

  resolved = {};
  expect_true(!parser.lookup_using_value_alias(0, compat_alias_text,
                                              &resolved),
              "using value aliases should reject no-key rendered compatibility bridges");
...
void test_parser_visible_value_alias_rejects_scope_local_target_bridge() {
  ...
  expect_eq(parser.resolve_visible_value_name(alias_text), "Alias",
...
void test_sema_static_member_type_lookup_rejects_stale_rendered_member_after_structured_miss() {
  c4c::Arena arena;
  c4c::TextTable texts;
  c4c::FileTable files;
  c4c::Parser parser({}, arena, &texts, &files, c4c::SourceProfile::CppSubset);
  ...
  const c4c::sema::ValidateResult result = c4c::sema::validate_program(program);
  expect_true(!result.ok,
              "Sema static member lookup should reject stale rendered fallback after structured member miss");
}
...
void test_sema_namespace_owner_resolution_rejects_rendered_fallback_after_structured_miss() {
  c4c::Arena arena;
  c4c::TextTable texts;
  c4c::FileTable files;
  c4c::Parser parser({}, arena, &texts, &files, c4c::SourceProfile::CppSubset);
  ...
  c4c::Node* fn = parser.make_node(c4c::NK_FUNCTION, 2);
  fn->name = arena.strdup("Owner::get");
  ...
  fn->qualifier_segments[0] = arena.strdup("Owner");
  fn->qualifier_text_ids = arena.alloc_array<c4c::TextId>(1);
  fn->qualifier_text_ids[0] = missing_owner_text;
  fn->type = make_ts(c4c::TB_INT);
  fn->body = body;
...
void test_parser_alias_template_info_prefers_structured_key_over_recovery() {
  ...
  parser.register_using_value_alias_for_testing(0, alias_text, bridge_key);
...
void test_consteval_value_lookup_prefers_structured_metadata_over_stale_rendered_name() {
  ...
  c4c::Node structured_miss_text_hit_ref = ref;
  structured_miss_text_hit_ref.name = "stale_structured_miss";
  structured_miss_text_hit_ref.unqualified_name = "TextOnly";
  structured_miss_text_hit_ref.unqualified_text_id = text_only_text;
  structured_miss_text_hit_ref.namespace_context_id = 11;
  rendered["stale_structured_miss"] = 6;

  auto structured_miss_text_hit =
      c4c::hir::evaluate_constant_expr(&structured_miss_text_hit_ref, env);
  expect_true(structured_miss_text_hit.ok(),
              "TextId consteval value lookup should still answer after structured metadata misses");
  expect_eq_int(static_cast<int>(structured_miss_text_hit.as_int()), 5,
                "consteval value lookup should use TextId metadata before rendered fallback");

  c4c::Node metadata_miss_ref = ref;
  metadata_miss_ref.name = "stale_metadata_miss";
  metadata_miss_ref.unqualified_name = "Missing";
  metadata_miss_ref.unqualified_text_id = texts.intern("Missing");
  metadata_miss_ref.namespace_context_id = 12;
  rendered["stale_metadata_miss"] = 7;
...
void test_consteval_value_lookup_keeps_no_metadata_rendered_compatibility() {
  ...
  legacy_node.unqualified_name = "legacy";
  legacy_node.unqualified_text_id = c4c::kInvalidText;
  legacy_node.namespace_context_id = -1;
...
void test_parser_consteval_sizeof_type_preserves_typedef_text_metadata() {
  ...
  c4c::Node* fn = parse_top_level(parser);
  expect_true(fn != nullptr && fn->kind == c4c::NK_FUNCTION,
              "consteval TypeSpec metadata fixture should parse as a function");
  expect_true(fn->n_template_params == 1 && fn->template_param_name_text_ids,
              "consteval TypeSpec metadata fixture should retain template parameter TextIds");
  expect_true(fn->body && fn->body->kind == c4c::NK_BLOCK &&
                  fn->body->n_children == 1 && fn->body->children[0] &&
                  fn->body->children[0]->kind == c4c::NK_RETURN &&
                  fn->body->children[0]->left &&
                  fn->body->children[0]->left->kind == c4c::NK_SIZEOF_TYPE,
              "consteval TypeSpec metadata fixture should return sizeof(T)");
...
void test_parser_qualified_typespec_preserves_text_metadata_over_rendered_spelling() {
  c4c::Lexer lexer("ns::Alias value;\n", c4c::LexProfile::CppSubset);
  const std::vector<c4c::Token> tokens = lexer.scan_all();
  c4c::Arena arena;
  c4c::Parser parser(tokens, arena, &lexer.text_table(), &lexer.file_table(),
                     c4c::SourceProfile::CppSubset);
  ...
  expect_true(parsed.n_qualifier_segments == 1 && parsed.qualifier_text_ids,
              "qualified TypeSpec should carry qualifier TextId metadata");
...
void test_consteval_type_binding_lookup_uses_typespec_text_metadata_without_name_mirrors() {
  ...
  callee->template_arg_types[0] = make_sema_lookup_ts(c4c::TB_LONGLONG);
  ...
  c4c::hir::ConstEvalEnv call_env = c4c::hir::bind_consteval_call_env(
      callee, fn, outer_env, &type_bindings, &nttp_bindings, &by_text);
  type_bindings["stale_rendered_T"] = make_sema_lookup_ts(c4c::TB_INT);

  expect_true(call_env.type_bindings_by_text == &by_text,
              "consteval call binding should expose TypeSpec TextId bindings");
  expect_true(call_env.type_binding_text_ids_by_name == nullptr,
              "consteval TypeSpec lookup fixture should not install rendered-name TextId mirrors");
