...
- inspect the remaining prepare-owned legality surfaces that still only change
  ...
  bookkeeping beyond plain scalar promotion and any load/store address metadata
  that should become a prepared-phase invariant rather than a semantic-lowering
  leftover
...
- signature/storage bookkeeping and call return type text now move with the
  existing `NoTargetFacingI1` promotion; any follow-up invariant expansion
  should still be tied to a concrete prepare-owned transform rather than a
