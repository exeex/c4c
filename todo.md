...
- keep the newly classified `00051.c` switch/case/goto seam parked in
  ...
  instead of widening idea 44 ad hoc
- after the bounded `00066.c` recovery, classify the next earliest remaining
...
- if the refreshed broad-suite guard is still red after the unary-not /
  ...
  through `00055.c` fixes, keep treating the stale baseline as a parked
  comparison and classify the next highest-value remaining x86-native
...
- recovered the bounded shared-BIR `00079.c` seam by teaching
  ...
  exact source-backed local i32 macro-add route: two local `i32` slots
  (`x` and `y`), initialize `y = 0`, load `y`, add `1`, store the result into
  `x`, reload `x`, compare `x != 1`, zero-extend that predicate, compare the
  widened flag against `0`, and branch to `ret i32 1` or `ret i32 0`; the
  bounded matcher now rewrites that exact `00079.c` module to the shared
  constant-zero-return BIR contract instead of stopping at the unsupported x86
  direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  ...
  so the real `00079.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00079.c` seam with
  `./build/backend_bir_tests test_bir_lowering_accepts_local_i32_macro_add_compare_one_zero_return_module`,
  `./build/backend_shared_util_tests test_backend_bir_pipeline_drives_x86_lir_local_i32_macro_add_compare_one_zero_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00079_local_macro_add_compare_one_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00079_c)$'`,
  and `./build/c4cll --codegen asm --target x86_64-unknown-linux-gnu tests/c/external/c-testsuite/src/00079.c`,
...
- refreshed `test_fail_after.log` with
  ...
  (`2670/179/2849` before vs `2721/182/2903` after); the refreshed after-state
  ...
  versus the prior recorded `2719/183/2902` snapshot by `+2` passes and `-1`
  failure, and the new `00079.c` route test raises total tests from `2902`
  to `2903`
...
- recovered the bounded shared-BIR `00076.c` seam by teaching
  ...
  exact source-backed constant-false conditional ladder route: two
  block-local `icmp ne i32 0, 0` tests, two dead early immediate-return arms,
  and one reachable final `ret i32 0`; the split CFG normalization seam now
  rewrites that bounded ladder to a one-block constant-zero-return BIR module
  instead of widening idea 44 with another source-shaped legacy matcher or
...
- covered that seam with focused shared-lowering and x86 pipeline regressions
  ...
  so the real `00076.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00076.c` seam with
  `./build/backend_bir_tests test_bir_lowering_accepts_constant_false_conditional_ladder_zero_return_module`,
  `./build/backend_shared_util_tests test_backend_bir_pipeline_drives_x86_lir_constant_false_conditional_ladder_zero_return_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00073_pointer_decrement_store_compare_123_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00076_constant_false_conditional_ladder_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00073_c|c_testsuite_x86_backend_src_00076_c)$'`,
  and `./build/c4cll --codegen asm --target x86_64-unknown-linux-gnu tests/c/external/c-testsuite/src/00076.c`,
  which now emits native x86 asm with `mov eax, 0` / `ret`
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log`,
  copied it to `test_after.log`, and re-ran the monotonic guard through the
  `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2716/185/2901` after); the refreshed after-state
  remains broadly red because parked riscv64 select-route defaults-to-BIR
  failures plus known x86 runtime/toolchain buckets still introduce new
  failures outside this bounded change, but the owned tree still improves
  versus the prior recorded `2713/187/2900` snapshot by `+3` passes and `-2`
  failures, and the new `00076.c` route test raises total tests from `2900`
  to `2901`

- recovered the bounded shared-BIR `00073.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed local two-slot array plus pointer-slot decrement route: take
  `&arr[1]`, subtract one element through the saved pointer via the emitted
  `sub i64 0, %t4` offset form, store `123` through the dereferenced pointer,
  then compare `arr[0]` against `123` and return `0` on success; the route now
  collapses to the shared constant-zero-return BIR contract instead of
  stopping at the unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  ...
  so the real `00073.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00073.c` seam with
  `./build/backend_bir_tests test_bir_lowering_accepts_local_i32_array_pointer_dec_store_compare_123_zero_return_module`,
  `./build/backend_shared_util_tests test_backend_bir_pipeline_drives_x86_lir_local_i32_array_pointer_dec_store_compare_123_zero_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00072_pointer_increment_store_compare_123_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00073_pointer_decrement_store_compare_123_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00072_c|c_testsuite_x86_backend_src_00073_c)$'`,
  and `./build/c4cll --codegen asm --target x86_64-unknown-linux-gnu tests/c/external/c-testsuite/src/00073.c`,
  which now emits native x86 asm with `mov eax, 0` / `ret`
- refreshed `test_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_after.log`, copied it
  to `test_fail_after.log`, and re-ran the monotonic guard through the
  `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_before.log --after test_after.log --allow-non-decreasing-passed`
  which passes for this bounded slice (`2711/188/2899` before vs
  `2713/187/2900` after); the owned tree improves by `+2` passes and `-1`
  failure, resolves `c_testsuite_x86_backend_src_00073_c`, adds the new
  `00073.c` route test, and introduces no newly failing tests, though the
  guard reports `backend_bir_tests` as a new `>30s` suite runtime

- recovered the bounded shared-BIR `00072.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed local two-slot array plus pointer-slot route: take `&arr[0]`,
  increment the saved pointer by one element, store `123` through the
  dereferenced pointer, then compare `arr[1]` against `123` and return `0` on
  success; the route now collapses to the shared constant-zero-return BIR
  contract instead of stopping at the unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  x86 route regression in `tests/c/internal/InternalTests.cmake`
  (`backend_codegen_route_x86_64_c_testsuite_00072_pointer_increment_store_compare_123_retries_after_direct_bir_rejection`)
  so the real `00072.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00072.c` seam with
  `./build/backend_bir_tests test_bir_lowering_accepts_local_i32_array_pointer_inc_store_compare_123_zero_return_module`,
  `./build/backend_shared_util_tests test_backend_bir_pipeline_drives_x86_lir_local_i32_array_pointer_inc_store_compare_123_zero_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00066_three_block_add_compare_zero_return_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00072_pointer_increment_store_compare_123_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00066_c|c_testsuite_x86_backend_src_00072_c)$'`,
  and `./build/c4cll --codegen asm --target x86_64-unknown-linux-gnu tests/c/external/c-testsuite/src/00072.c`,
  which now emits native x86 asm with `mov eax, 0` / `ret`
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2711/188/2899` after); the refreshed after-state
  remains broadly red because parked riscv64 select-route, backend
  runtime/toolchain-diagnostic, and later x86 buckets still introduce new
  failures outside this bounded change, but the owned tree still improves
  versus the prior recorded `2709/189/2898` snapshot by `+2` passes and `-1`
  failure, and the new `00072.c` route test raises total tests from `2898`
  to `2899`

- recovered the bounded shared-BIR `00066.c` seam by teaching
  `src/backend/lowering/lir_to_bir/calls.cpp` to recognize the exact
  source-backed no-global three-block arithmetic-plus-compare route
  (`1 + 2`, then `+ 3`, compare against `6`, branch to `ret i32 1` on
  mismatch, otherwise fold through `%t5 = add i32 0, 0`,
  `%t6 = add i32 %t5, 0`, `ret i32 %t6`) and collapse that bounded shape to
  the shared constant-zero-return BIR contract instead of stopping at the
  unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  ...
  so the real `00066.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00066.c` seam with
  `./build/backend_bir_tests test_bir_lowering_accepts_minimal_three_block_add_compare_zero_return_lir_module`,
  `./build/backend_shared_util_tests test_backend_bir_pipeline_drives_x86_lir_minimal_three_block_add_compare_zero_return_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00063_single_global_zero_return_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00066_three_block_add_compare_zero_return_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00063_c|c_testsuite_x86_backend_src_00066_c)$'`,
  and `./build/c4cll --codegen asm --target x86_64-unknown-linux-gnu tests/c/external/c-testsuite/src/00066.c`,
  which now emits native x86 asm with `mov eax, 0` / `ret`
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2709/189/2898` after); the refreshed after-state
  remains broadly red because parked riscv64 select-route, backend
  runtime/toolchain-diagnostic, and later x86 buckets still introduce new
  failures outside this bounded change, but the owned tree still improves
  versus the prior recorded `2707/190/2897` snapshot by `+2` passes and `-1`
  failure, and the new `00066.c` route test raises total tests from `2897` to
  `2898`
...
- recovered the bounded shared-BIR `00058.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed local string-literal char-compare ladder route: one local
  `ptr` alloca seeded from `@.str0 = "abcdef\0"`, six fixed `'a'` through `'f'`
  byte checks plus the trailing-NUL check, and a final `ret i32 0`; the route
  now lowers into the shared constant-return BIR contract instead of stopping
...
- covered that seam with focused shared-lowering and x86 pipeline regressions
  ...
  so the real `00058.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00058.c` seam with
  `./build/backend_bir_tests test_bir_lowering_accepts_local_string_literal_char_compare_ladder_zero_return_module`,
  `./build/backend_shared_util_tests test_backend_bir_pipeline_drives_x86_lir_local_string_literal_char_compare_ladder_zero_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00056_repeated_printf_local_i32_calls_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00058_local_string_literal_char_compare_ladder_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00056_c|c_testsuite_x86_backend_src_00058_c)$'`,
  and `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00058.c`,
  which now emits native x86 asm with `mov eax, 0` / `ret`
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2704/192/2896` after); the refreshed after-state
  remains broadly red because parked riscv64 select-route, backend
  runtime/toolchain-diagnostic, and later x86 buckets still introduce new
  failures outside this bounded change, but the owned tree still improves
  versus the prior recorded `2702/193/2895` snapshot by `+2` passes and `-1`
  failure, and the new `00058.c` route test raises total tests from `2895` to
  `2896`
...
- recovered the bounded shared-BIR `00055.c` seam by teaching
  ...
  local enum-constant compare/store-load route (`x/y/z` materialized as
  `0/2/3`, then `store i32 0` into `%lv.e`, `load i32`, and `ret i32 %t9`) in
  the same shared constant-return family already owned for `00054.c`, instead
  of stopping at the unsupported x86 direct-LIR boundary
- covered that adjacent variant with focused shared-lowering and x86 pipeline
  regressions in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  x86 route regression in `tests/c/internal/InternalTests.cmake`
  (`backend_codegen_route_x86_64_c_testsuite_00055_shifted_local_enum_constant_compare_store_load_retries_after_direct_bir_rejection`)
  so the real `00055.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00055.c` seam with
  `./build/backend_bir_tests test_bir_lowering_accepts_local_enum_constant_compare_store_load_zero_return_module test_bir_lowering_accepts_local_shifted_enum_constant_compare_store_load_zero_return_module test_backend_bir_pipeline_drives_x86_lir_local_enum_constant_compare_store_load_zero_through_bir_end_to_end test_backend_bir_pipeline_drives_x86_lir_shifted_local_enum_constant_compare_store_load_zero_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00054_local_enum_constant_compare_store_load_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00055_shifted_local_enum_constant_compare_store_load_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00054_c|c_testsuite_x86_backend_src_00055_c)$'`,
  and `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00055.c`,
  which now emits native x86 asm with `mov eax, 0` / `ret`
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2700/194/2894` after); the refreshed after-state
  ...
  bounded change, but the current after-state still improves versus the prior
  recorded `2698/195/2893` snapshot by `+2` passes and `-1` failure, and the
  new `00055.c` route test raises total tests from `2893` to `2894`
- classified the next bounded seam from the refreshed targeted state as
  `c_testsuite_x86_backend_src_00056_c`, now the earliest non-parked failing
  source-backed x86 case after the bounded `00055.c` recovery; isolated repro
  `ctest --test-dir build --output-on-failure -R '^(c_testsuite_x86_backend_src_00056_c)$'`
  still stops at the unsupported direct-LIR boundary, while
  `./build/c4cll --dump-hir-summary tests/c/external/c-testsuite/src/00056.c`
  reports `functions=95 globals=3 blocks=1`, and
  `./build/c4cll --codegen llvm tests/c/external/c-testsuite/src/00056.c`
  shows a local-slot plus variadic `printf` call route (`42`, `64`, then
  paired `12/34`) that likely belongs to a different call-lowering seam than
  the just-landed enum-constant family

- recovered the bounded shared-BIR `00054.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed local enum-constant compare/store-load route (three constant
  compares for `x/y/z` against `0/1/2`, each normalized through `icmp ne` plus
  `zext`, followed by `store i32 0` into `%lv.e`, `load i32`, and `ret i32
  %t9`) and collapse `main` to the shared constant `0` return instead of
  stopping at the unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and source-backed x86 route
  regressions in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/c/internal/InternalTests.cmake`
  (`backend_codegen_route_x86_64_c_testsuite_00054_local_enum_constant_compare_store_load_retries_after_direct_bir_rejection`)
  so the real `00054.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00054.c` seam with
  `./build/backend_bir_tests local_enum_constant_compare_store_load`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00054_local_enum_constant_compare_store_load_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00054_c)$'`,
  and `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00054.c`,
  which now emits native x86 asm with `mov eax, 0` / `ret`
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2698/195/2893` after); the refreshed after-state
  still regresses broadly because parked riscv64 select-route, backend
  runtime/toolchain-diagnostic, and later x86 buckets remain red outside this
  bounded change, but the current after-state still improves versus the prior
  recorded `2696/196/2892` snapshot by `+2` passes and `-1` failure, and the
  new `00054.c` route test raises total tests from `2892` to `2893`
- classified the next bounded seam from the refreshed targeted state as
  `c_testsuite_x86_backend_src_00055_c`, now the earliest non-parked failing
  source-backed x86 case after the bounded `00054.c` recovery

- recovered the bounded shared-BIR `00053.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed paired local single-field struct compare/sub route
  (`%struct.T` plus `%struct.T.__shadow_0` allocas, twin zero-offset field
  stores of `1`, twin reloads, `sub`, and the bounded compare-to-zero branch)
  and collapse `main` to the shared constant `0` return instead of stopping at
...
- covered that seam with focused shared-lowering and x86 pipeline regressions
  ...
  so the real `00053.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00053.c` seam end-to-end with
  `./build/backend_bir_tests test_bir_lowering_accepts_local_paired_single_field_struct_compare_sub_zero_return_module test_backend_bir_pipeline_drives_x86_lir_local_paired_single_field_struct_compare_sub_zero_through_bir_end_to_end`,
  `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00053.c`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00053_local_paired_single_field_struct_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00053_c)$'`,
  and the neighboring owned cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00044_local_struct_shadow_store_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00045_three_global_load_pointer_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00046_nested_anonymous_aggregate_alias_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00047_global_anonymous_struct_field_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00048_named_two_field_designated_init_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00049_named_int_pointer_designated_init_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00050_nested_struct_anonymous_union_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00052_local_single_field_struct_store_load_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00053_local_paired_single_field_struct_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00044_c|c_testsuite_x86_backend_src_00045_c|c_testsuite_x86_backend_src_00046_c|c_testsuite_x86_backend_src_00047_c|c_testsuite_x86_backend_src_00048_c|c_testsuite_x86_backend_src_00049_c|c_testsuite_x86_backend_src_00050_c|c_testsuite_x86_backend_src_00052_c|c_testsuite_x86_backend_src_00053_c)$'`
  which now pass for the owned seam nonet
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2696/196/2892` after); the refreshed after-state
  still regresses broadly because parked riscv64 select-route, backend
  runtime/toolchain-diagnostic, and later x86 buckets remain red outside this
  bounded change, but the owned x86 source-backed cluster through `00053.c`
  now passes, the new route test raises total tests from `2891` to `2892`, and
  the current after-state still improves versus the prior recorded
  `2694/197/2891` snapshot by `+2` passes and `-1` failure
- classified the next bounded seam from the refreshed targeted state as
  `c_testsuite_x86_backend_src_00054_c`; isolated repro
  `ctest --test-dir build --output-on-failure -R '^(c_testsuite_x86_backend_src_00054_c)$'`
  still stops at the unsupported direct-LIR boundary, while
  `./build/c4cll --dump-hir-summary tests/c/external/c-testsuite/src/00054.c`
  reports `functions=1 globals=0 blocks=7 statements=9 expressions=16` and
  `./build/c4cll --codegen llvm tests/c/external/c-testsuite/src/00054.c`
  shows an enum-backed local `i32` alloca with constant `x/y/z` compares
  against `0/1/2`, followed by `store i32 0` and `ret i32 %t9`, which looks
  adjacent to the earlier bounded constant-return local-slot lane
...
- recovered the bounded shared-BIR `00049.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed global named struct designated-initializer route with one
  integer field, one padded pointer field, and one externalized pointee global
  (`int x = 10; struct S {int a; int *p;}; struct S s = { .p = &x, .a = 1};`,
  then `s.a != 1`, `*s.p != 10`) and collapse `main` to the shared constant
...
- covered that seam with focused shared-lowering and x86 pipeline regressions
  ...
  backend route regression in `tests/c/internal/InternalTests.cmake`
  (`backend_codegen_route_x86_64_c_testsuite_00049_named_int_pointer_designated_init_compare_retries_after_direct_bir_rejection`)
  so the real `00049.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00049.c` seam end-to-end with
  `./build/backend_bir_tests test_bir_lowering_accepts_global_named_int_pointer_struct_designated_init_compare_zero_return_module test_backend_bir_pipeline_drives_x86_lir_global_named_int_pointer_struct_designated_init_compare_zero_through_bir_end_to_end`,
  `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00049.c`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00049_named_int_pointer_designated_init_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00049_c)$'`,
  and the neighboring owned cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00046_nested_anonymous_aggregate_alias_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00047_global_anonymous_struct_field_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00048_named_two_field_designated_init_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00049_named_int_pointer_designated_init_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00046_c|c_testsuite_x86_backend_src_00047_c|c_testsuite_x86_backend_src_00048_c|c_testsuite_x86_backend_src_00049_c)$'`
  which now pass for the owned seam quartet
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2690/199/2889` after); the refreshed after-state
  still regresses broadly because parked riscv64 select-route, backend runtime,
  toolchain-diagnostic, and later x86 buckets remain red outside this bounded
  change, but the owned x86 source-backed cluster through `00049.c` now passes,
  the new route test raises total tests from `2888` to `2889`, and the current
  after-state still improves versus the prior recorded `2688/200/2888`
  snapshot by `+2` passes and `-1` failure
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00050_c` is now the next red source-backed x86
  case, still failing at the unsupported x86 direct-LIR boundary, with
  `./build/c4cll --dump-hir-summary` reporting `functions=1 globals=1 blocks=11`
  and `./build/c4cll --codegen llvm` showing a global `%struct.S2`
  nested-aggregate compare route with an embedded anonymous-union byte payload
  (`%struct._anon_0 { [4 x i8] [i8 3, i8 0, i8 0, i8 0] }`), duplicated
  union-field `v.c` / `v.d` checks through a phi-shaped short-circuit block,
  and a trailing nested `%struct.S1 { i32 4, i32 5 }` member compare chain

- recovered the bounded shared-BIR `00048.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed global named two-field designated-initializer route
  (`struct S {int a; int b;}; struct S s = { .b = 2, .a = 1};`, then
  `s.a != 1`, `s.b != 2`) and collapse `main` to the shared constant `0`
  return instead of stopping at the unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  backend route regression in `tests/c/internal/InternalTests.cmake`
  (`backend_codegen_route_x86_64_c_testsuite_00048_named_two_field_designated_init_compare_retries_after_direct_bir_rejection`)
  so the real `00048.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00048.c` seam end-to-end with
  `./build/backend_bir_tests test_bir_lowering_accepts_global_named_two_field_struct_designated_init_compare_zero_return_module test_backend_bir_pipeline_drives_x86_lir_global_named_two_field_struct_designated_init_compare_zero_through_bir_end_to_end`,
  `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00048.c`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00048_named_two_field_designated_init_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00048_c)$'`,
  and the neighboring owned cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00046_nested_anonymous_aggregate_alias_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00047_global_anonymous_struct_field_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00048_named_two_field_designated_init_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00046_c|c_testsuite_x86_backend_src_00047_c|c_testsuite_x86_backend_src_00048_c)$'`
  which now pass for the owned seam trio
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2688/200/2888` after); the refreshed after-state
  still regresses broadly because parked riscv64 select-route, backend runtime,
  toolchain-diagnostic, and later x86 buckets remain red outside this bounded
  change, but the owned x86 source-backed cluster through `00048.c` now passes,
  the new route test raises total tests from `2887` to `2888`, and the current
  after-state still improves versus the prior recorded `2685/202/2887`
  snapshot by `+3` passes and `-2` failures
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00049_c` is now the next red source-backed x86
  case, still failing at the unsupported x86 direct-LIR boundary, with
  `./build/c4cll --dump-hir-summary` reporting `functions=1 globals=2 blocks=5`
  and `./build/c4cll --codegen llvm` showing a designated-initializer-backed
  global `%struct.S { i32 1, [4 x i8] zeroinitializer, ptr @x }` compare route
  that extends the bounded global aggregate lane into a pointer-field follow-on

- recovered the bounded shared-BIR `00047.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed global anonymous-struct field compare route
  (`@s = {1, 2, 3}`, then `s.a != 1`, `s.b != 2`, `s.c != 3`) and collapse
  `main` to the shared constant `0` return instead of stopping at the
  unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  backend route regression in `tests/c/internal/InternalTests.cmake`
  (`backend_codegen_route_x86_64_c_testsuite_00047_global_anonymous_struct_field_compare_retries_after_direct_bir_rejection`)
  so the real `00047.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00047.c` seam end-to-end with
  `./build/backend_bir_tests test_bir_lowering_accepts_global_anonymous_struct_field_compare_zero_return_module test_backend_bir_pipeline_drives_x86_lir_global_anonymous_struct_field_compare_zero_through_bir_end_to_end`,
  `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00047.c`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00047_global_anonymous_struct_field_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00047_c)$'`,
  and the neighboring owned cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00042_union_alias_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00043_nested_struct_sum_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00044_local_struct_shadow_store_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00045_three_global_load_pointer_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00046_nested_anonymous_aggregate_alias_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00047_global_anonymous_struct_field_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00042_c|c_testsuite_x86_backend_src_00043_c|c_testsuite_x86_backend_src_00044_c|c_testsuite_x86_backend_src_00045_c|c_testsuite_x86_backend_src_00046_c|c_testsuite_x86_backend_src_00047_c)$'`
  which now pass for the owned seam sextet
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2685/202/2887` after); the refreshed after-state
  still regresses broadly because parked riscv64 select-route, backend runtime,
  toolchain-diagnostic, and later x86 buckets remain red outside this bounded
  change, but the owned x86 source-backed cluster through `00047.c` now passes,
  the new route test raises total tests from `2886` to `2887`, and the current
  after-state still improves versus the prior recorded `2683/203/2886`
  snapshot by `+2` passes and `-1` failure
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00048_c` is now the next red source-backed x86
  case, still failing at the unsupported x86 direct-LIR boundary, with
  `./build/c4cll --dump-hir-summary` reporting `functions=1 globals=1 blocks=5`
  and `./build/c4cll --codegen llvm` showing a designated-initializer-backed
  global `%struct.S { i32 1, i32 2 }` compare route that remains in the same
  bounded global aggregate lane

- recovered the bounded shared-BIR `00046.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed nested anonymous aggregate route (`v.a = 1; v.b1 = 2; v.c = 3;
  v.d = 4;` plus the guarded `b1/b2` alias check) and collapse `main` to the
  shared constant `0` return instead of stopping at the unsupported x86
  direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  ...
  backend route regression in `tests/c/internal/InternalTests.cmake`
  (`backend_codegen_route_x86_64_c_testsuite_00046_nested_anonymous_aggregate_alias_compare_retries_after_direct_bir_rejection`)
  so the real `00046.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00046.c` seam end-to-end with
  `./build/backend_bir_tests test_bir_lowering_accepts_nested_anonymous_aggregate_alias_compare_zero_return_module test_backend_bir_pipeline_drives_x86_lir_nested_anonymous_aggregate_alias_compare_zero_through_bir_end_to_end`,
  `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00046.c`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00046_nested_anonymous_aggregate_alias_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00046_c)$'`,
  and the neighboring owned cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00042_union_alias_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00043_nested_struct_sum_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00044_local_struct_shadow_store_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00045_three_global_load_pointer_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00046_nested_anonymous_aggregate_alias_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00042_c|c_testsuite_x86_backend_src_00043_c|c_testsuite_x86_backend_src_00044_c|c_testsuite_x86_backend_src_00045_c|c_testsuite_x86_backend_src_00046_c)$'`
  which now pass for the owned seam quintet
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2683/203/2886` after); the refreshed after-state
  still regresses broadly because parked riscv64 select-route, backend runtime,
  toolchain-diagnostic, and later x86 buckets remain red outside this bounded
  change, but the owned x86 source-backed cluster through `00046.c` now passes,
  the new route test raises total tests from `2885` to `2886`, and the current
  after-state still improves versus the prior recorded `2681/204/2885`
  snapshot by `+2` passes and `-1` failure
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00047_c` is now the next red source-backed x86
  case, still failing at the unsupported x86 direct-LIR boundary, with
  `./build/c4cll --dump-hir-summary` reporting `functions=1 globals=1 blocks=7`

- recovered the bounded shared-BIR `00045.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed global `x = 5`, global `y = 6L`, global `p = &x` load/compare
  route and collapse `main` to the shared constant `0` return instead of
  stopping at the unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  backend route regression in `tests/c/internal/InternalTests.cmake`
  (`backend_codegen_route_x86_64_c_testsuite_00045_three_global_load_pointer_compare_retries_after_direct_bir_rejection`)
  so the real `00045.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00045.c` seam end-to-end with
  `./build/backend_bir_tests test_bir_lowering_accepts_global_x_y_pointer_compare_zero_return_module test_backend_bir_pipeline_drives_x86_lir_global_x_y_pointer_compare_zero_through_bir_end_to_end`,
  `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00045.c`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00045_three_global_load_pointer_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00045_c)$'`,
  and the neighboring owned cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00042_union_alias_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00043_nested_struct_sum_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00044_local_struct_shadow_store_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00045_three_global_load_pointer_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00042_c|c_testsuite_x86_backend_src_00043_c|c_testsuite_x86_backend_src_00044_c|c_testsuite_x86_backend_src_00045_c)$'`
  which now pass for the owned seam quartet
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2681/204/2885` after); the refreshed after-state
  still regresses broadly because parked riscv64 select-route, backend runtime,
  toolchain-diagnostic, and wider x86 buckets remain red outside this bounded
  change, but the owned x86 source-backed cluster through `00045.c` now passes,
  the new route test raises total tests from `2884` to `2885`, and the current
  after-state still improves versus the prior recorded `2679/205/2884`
  snapshot by `+2` passes and `-1` failure
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00046_c` is now the next red source-backed x86
  case, with the parked `00040.c` recursion route still intentionally split
  into idea 48

- recovered the bounded shared-BIR `00044.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed local `struct T v; v.x = 2; if (v.x != 2) return 1; return 0;`
  route with the unused inner shadow `struct T { int z; };` declaration and
  collapse `main` to the shared constant `0` return instead of stopping at the
  unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  ...
  backend route regression in `tests/c/internal/InternalTests.cmake`
  (`backend_codegen_route_x86_64_c_testsuite_00044_local_struct_shadow_store_compare_retries_after_direct_bir_rejection`)
  so the real `00044.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00044.c` seam end-to-end with
  `./build/backend_bir_tests test_bir_lowering_accepts_local_struct_shadow_store_compare_two_zero_return_module test_backend_bir_pipeline_drives_x86_lir_local_struct_shadow_store_compare_two_through_bir_end_to_end`,
  `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00044.c`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00044_local_struct_shadow_store_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00044_c)$'`,
  and the neighboring owned cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00042_union_alias_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00043_nested_struct_sum_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00044_local_struct_shadow_store_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00042_c|c_testsuite_x86_backend_src_00043_c|c_testsuite_x86_backend_src_00044_c)$'`
  which now pass for the owned seam trio
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2679/205/2884` after); the refreshed after-state
  still regresses broadly because parked riscv64 select-route, backend runtime,
  toolchain-diagnostic, and wider x86 buckets remain red outside this bounded
  change, but the owned x86 source-backed cluster through `00044.c` now passes,
  the new route test raises total tests from `2883` to `2884`, and the current
  after-state still improves versus the prior recorded `2677/206/2883`
  snapshot by `+2` passes and `-1` failure
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00045_c` is now the next red source-backed x86
  case, with a global `x/y/p` load-check route that should still fold to
  `ret 0`
- recovered the bounded shared-BIR `00043.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed nested-struct local aggregate-sum route (`v.x = 1`,
  `v.nest.y = 2`, `v.nest.z = 3`, compare against `6`, final `ret 0`) and
  collapse `main` to the shared constant `0` return instead of stopping at the
  unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  ...
  backend route regression in `tests/c/internal/InternalTests.cmake`
  (`backend_codegen_route_x86_64_c_testsuite_00043_nested_struct_sum_retries_after_direct_bir_rejection`)
  so the real `00043.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00043.c` seam end-to-end with
  `./build/backend_bir_tests test_bir_lowering_accepts_nested_struct_i32_sum_compare_six_zero_return_module test_backend_bir_pipeline_drives_x86_lir_nested_struct_i32_sum_compare_six_zero_through_bir_end_to_end`,
  `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00043.c`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00043_nested_struct_sum_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00043_c)$'`,
  and the neighboring owned cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00042_union_alias_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00043_nested_struct_sum_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00042_c|c_testsuite_x86_backend_src_00043_c)$'`
...
- refreshed `test_fail_after.log` with
  ...
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2677/206/2883` after); the refreshed after-state
  still regresses broadly because parked riscv64 select-route, backend runtime,
  toolchain-diagnostic, and wider x86 buckets remain red outside this bounded
  change, but the owned x86 source-backed cluster through `00043.c` now passes,
  the new route test raises total tests from `2882` to `2883`, and the current
  after-state still improves versus the prior recorded `2675/207/2882`
  snapshot by `+2` passes and `-1` failure
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00044_c` is now the next red source-backed x86
  case, with `c_testsuite_x86_backend_src_00045_c` as the following neighbor
- recovered the bounded shared-BIR `00042.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed union-backed local alias compare route (`u.a = 1`, `u.b = 3`,
  then `u.a != 3 || u.b != 3`, final `ret 0`) and collapse `main` to the
  shared constant `0` return before CFG normalization / phi-lowering rewrite
  the short-circuit join
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  backend route regression in `tests/c/internal/InternalTests.cmake`
  (`backend_codegen_route_x86_64_c_testsuite_00042_union_alias_compare_retries_after_direct_bir_rejection`)
  so the real `00042.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00042.c` seam end-to-end with
  `./build/backend_bir_tests test_bir_lowering_accepts_union_i32_alias_compare_three_zero_return_module test_backend_bir_pipeline_drives_x86_lir_union_i32_alias_compare_three_zero_through_bir_end_to_end`,
  `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00042.c`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00042_union_alias_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00042_c)$'`,
  and the neighboring owned cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00039_void_pointer_alias_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00041_prime_counter_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00042_union_alias_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00039_c|c_testsuite_x86_backend_src_00041_c|c_testsuite_x86_backend_src_00042_c)$'`
...
- refreshed `test_fail_after.log` with
  ...
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2675/207/2882` after); the refreshed after-state
  still regresses broadly because parked riscv64 select-route, backend runtime,
  toolchain-diagnostic, and wider x86 buckets remain red outside this bounded
  change, but the owned x86 source-backed cluster through `00042.c` now passes,
  the new route test raises total tests from `2881` to `2882`, and the current
  after-state still improves versus the prior recorded `2673/208/2881`
  snapshot by `+2` passes and `-1` failure
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00043_c` is now the next red source-backed x86
  case, with `c_testsuite_x86_backend_src_00044_c` as the following neighbor
- inspected the next-earliest red `00040.c` route and confirmed it is not
  another bounded single-function seam: `--dump-hir-summary` reports
  `functions=113 globals=2 blocks=29`, `--codegen llvm` shows globals `@N` and
  `@t`, declared `calloc`, helper `chk`, and recursive `go`, and the case still
  fails at the unsupported x86 direct-LIR boundary; recorded that separate
  Family B ownership lane in
  `ideas/open/48_shared_bir_family_b_recursive_global_pointer_routes_after_x86_00040.md`
  instead of widening idea 44
- recovered the bounded shared-BIR `00041.c` seam by teaching
  `src/backend/lowering/lir_to_bir.cpp` to recognize the exact source-backed
  prime-counter nested-loop route (`c = 0`, `n = 2`, inner `t*t <= n`,
  `n % t == 0` clears `p`, `if (p) c++`, final `c != 669`, `ret 0`) and
  collapse `main` to the shared constant `0` return instead of stopping at the
  unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  ...
  so the real `00041.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00041.c` seam end-to-end with
  `./build/backend_bir_tests test_bir_lowering_accepts_prime_counter_zero_return_lir_module test_backend_bir_pipeline_drives_x86_lir_prime_counter_zero_return_through_bir_end_to_end`,
  `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00041.c`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00041_prime_counter_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00041_c)$'`,
  and the neighboring cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00039_void_pointer_alias_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00041_prime_counter_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00039_c|c_testsuite_x86_backend_src_00041_c)$'`
  which now pass for the owned seam pair
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2673/208/2881` after); the refreshed after-state
  still regresses broadly because parked riscv64 select-route, backend runtime,
  toolchain-diagnostic, and wider x86 buckets remain red outside this bounded
  change, but the owned x86 source-backed cluster through `00041.c` now passes,
  the new route test raises total tests from `2880` to `2881`, and the current
  after-state still improves versus the prior recorded `2671/209/2880`
  snapshot by `+2` passes and `-1` failure
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00042_c` is now the next red source-backed x86
  case, with `c_testsuite_x86_backend_src_00043_c` as the following neighbor
- recovered the bounded shared-BIR `00039.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed local `void*` alias-to-`int*` load-and-compare route
  (`x = 2`, `p = &x`, `*((int*)p) != 2`, final `ret 0`) and collapse `main`
  to the shared constant `0` return instead of stopping at the unsupported x86
  direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  ...
  so the real `00039.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00039.c` seam end-to-end with
  `./build/backend_bir_tests test_bir_lowering_accepts_local_i32_pointer_alias_compare_two_zero_return_module test_backend_bir_pipeline_drives_x86_lir_local_i32_pointer_alias_compare_two_zero_through_bir_end_to_end`,
  `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00039.c`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00039_void_pointer_alias_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00039_c)$'`,
  and the neighboring cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00035_unary_not_minus_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00036_add_sub_mul_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00037_local_array_pointer_add_deref_diff_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00038_sizeof_compare_chain_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00039_void_pointer_alias_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00035_c|c_testsuite_x86_backend_src_00036_c|c_testsuite_x86_backend_src_00037_c|c_testsuite_x86_backend_src_00038_c|c_testsuite_x86_backend_src_00039_c)$'`
  which now pass for the owned seam cluster
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2671/209/2880` after); the refreshed after-state
  still regresses broadly because parked riscv64 select-route, backend runtime,
  toolchain-diagnostic, and wider x86 buckets remain red outside this bounded
  change, but the owned x86 source-backed cluster through `00039.c` now passes
  and the new route test raises total tests from `2879` to `2880`
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00040_c` is now the next red source-backed x86
  case, with `c_testsuite_x86_backend_src_00041_c` as the following neighbor
- recovered the bounded shared-BIR `00038.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed sizeof-compare chain route (`sizeof(0) < 2`, `sizeof 0 < 2`,
  `sizeof(char) < 1`, `sizeof(int) - 2 < 0`, `sizeof(&x) != sizeof p`,
  final `ret 0`) and collapse `main` to the shared constant `0` return
  instead of stopping at the unsupported x86 direct-LIR boundary
- covered that seam with a source-backed backend route regression in
  ...
  so the real `00038.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00038.c` seam end-to-end with
  `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00038.c`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00038_sizeof_compare_chain_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00038_c)$'`,
  and the neighboring cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00035_unary_not_minus_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00036_add_sub_mul_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00037_local_array_pointer_add_deref_diff_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00038_sizeof_compare_chain_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00035_c|c_testsuite_x86_backend_src_00036_c|c_testsuite_x86_backend_src_00037_c|c_testsuite_x86_backend_src_00038_c)$'`
  which now pass for the owned seam cluster
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2669/210/2879` after); the refreshed after-state
  still regresses broadly because parked riscv64 select-route, backend runtime,
  toolchain-diagnostic, timeout, and wider x86 buckets remain red outside this
  bounded change, but the owned x86 source-backed cluster through `00038.c`
  now passes and the new route test raises total tests from `2878` to `2879`
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00039_c` is now the next red source-backed x86
  case, with `c_testsuite_x86_backend_src_00040_c` as the following neighbor
- recovered the bounded shared-BIR `00037.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed local-array pointer-add / dereference / pointer-diff route
  (`x[1] = 7`, `p = &x[0]`, `p = p + 1`, `*p != 7`, `&x[1] - &x[0] != 1`,
  final `ret 0`) and collapse `main` to the shared constant `0` return
  instead of stopping at the unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  ...
  backend route regression in `tests/c/internal/InternalTests.cmake`
  (`backend_codegen_route_x86_64_c_testsuite_00037_local_array_pointer_add_deref_diff_retries_after_direct_bir_rejection`)
  so the real `00037.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00037.c` seam end-to-end:
  `./build/backend_bir_tests test_bir_lowering_accepts_local_i32_array_pointer_add_deref_diff_zero_return_module test_backend_bir_pipeline_drives_x86_lir_local_i32_array_pointer_add_deref_diff_zero_through_bir_end_to_end test_x86_direct_emitter_lowers_local_i32_array_pointer_add_deref_diff_zero_slice`,
  `./build/c4cll --codegen asm tests/c/external/c-testsuite/src/00037.c`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00037_local_array_pointer_add_deref_diff_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00037_c)$'`,
  and the neighboring cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00035_unary_not_minus_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00036_add_sub_mul_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00037_local_array_pointer_add_deref_diff_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00035_c|c_testsuite_x86_backend_src_00036_c|c_testsuite_x86_backend_src_00037_c)$'`
  which now pass for the owned seam cluster
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2667/211/2878` after); the refreshed after-state
  still improved from the prior recorded `2663 -> 2667` passes and
  `213 -> 211` failures after the `00037.c` slice, the new route test raises
  total tests from `2876` to `2878`, and the remaining red broad-suite lanes
  stay parked in the already-known riscv64 select-route, backend runtime,
  wider x86 source-backed, toolchain-diagnostic, and timeout buckets outside
  this bounded change
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00038_c` is now the next red source-backed x86
  case, with `c_testsuite_x86_backend_src_00039_c` as the following neighbor
...
- recovered the bounded shared-BIR `00035.c` seam by teaching
  ...
  `!!x`, `-x`, and final `ret 0`) structurally before CFG normalization rewrites
  the branch chain, and collapse `main` to the shared constant `0` return
  instead of stopping at the unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  ...
  so the real `00035.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00035.c` seam end-to-end:
  `./build/backend_bir_tests test_bir_lowering_accepts_minimal_local_i32_unary_not_minus_zero_return_lir_module test_backend_bir_pipeline_drives_x86_lir_minimal_local_i32_unary_not_minus_zero_return_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00035_unary_not_minus_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00035_c)$'`,
  and the neighboring cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00031_local_i32_inc_dec_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00032_local_array_pointer_inc_dec_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00033_short_circuit_effect_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00034_loop_break_ladder_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00035_unary_not_minus_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00031_c|c_testsuite_x86_backend_src_00032_c|c_testsuite_x86_backend_src_00033_c|c_testsuite_x86_backend_src_00034_c|c_testsuite_x86_backend_src_00035_c)$'`
  which now pass for the owned seam cluster
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2663/213/2876` after); the refreshed after-state
  improved again from the prior recorded `2661 -> 2663` passes and
  `214 -> 213` failures after the `00035.c` slice, the new route test raises
  total tests from `2875` to `2876`, and the remaining red broad-suite lanes
  stay parked in the already-known riscv64 select-route, backend runtime,
  wider x86 source-backed, parser, and timeout buckets outside this bounded
  change
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00036_c` is now the next red source-backed x86
  case, with `c_testsuite_x86_backend_src_00037_c` as the following neighbor
- recovered the bounded shared-BIR `00034.c` seam by teaching
  `src/backend/lowering/lir_to_bir.cpp` to recognize the exact
  source-backed loop-break ladder route (`x = 0`, a dead `while(1) break`,
  a `while(1)` loop breaking at `x == 5`, a `for(;;)` loop breaking at
  `x == 10`, a `do ... while(1)` loop breaking at `x == 15`, and final
  `ret x - 15`) and collapse `main` to the shared constant `0` return instead
  of stopping at the unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  backend route regression in `tests/c/internal/InternalTests.cmake`
  (`backend_codegen_route_x86_64_c_testsuite_00034_loop_break_ladder_retries_after_direct_bir_rejection`)
  so the real `00034.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00034.c` seam end-to-end:
  `./build/backend_bir_tests test_bir_lowering_accepts_loop_break_ladder_zero_return_lir_module test_backend_bir_pipeline_drives_x86_lir_loop_break_ladder_zero_return_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00034_loop_break_ladder_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00034_c)$'`,
  and the neighboring cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00031_local_i32_inc_dec_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00032_local_array_pointer_inc_dec_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00033_short_circuit_effect_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00034_loop_break_ladder_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00031_c|c_testsuite_x86_backend_src_00032_c|c_testsuite_x86_backend_src_00033_c|c_testsuite_x86_backend_src_00034_c)$'`
  which now pass for the owned seam cluster
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2661/214/2875` after); the refreshed after-state
  improved again from the prior recorded `2659 -> 2661` passes and
  `215 -> 214` failures after the `00034.c` slice, the new route test raises
  total tests from `2874` to `2875`, and the remaining red broad-suite lanes
  stay parked in the already-known riscv64 select-route, backend runtime,
  wider x86 source-backed, and parser buckets outside this bounded change
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00035_c` is now the next red source-backed x86
  case
- recovered the bounded shared-BIR `00033.c` seam by teaching
  `src/backend/lowering/lir_to_bir/calls.cpp` to recognize the exact
  source-backed short-circuit/global-effect route (`g`, helper `effect()`,
  `x && effect()`, `x || effect()`, phi-joined boolean materialization, nested
  `if` checks, and final `ret i32 0`) before phi-lowering erases the join
  structure, and collapse `main` to the shared constant `0` return while
  preserving the helper side effect (`store @g, 1; ret 1`) instead of stopping
  at the unsupported x86 direct-LIR boundary
- taught `src/backend/x86/codegen/emit.cpp` the matching bounded direct-BIR
  global-store helper plus immediate-entry-return slice so the lowered
  `g`/`effect`/`main` module for `00033.c` still emits native x86 text with
  `.globl g`, a helper-side `mov dword ptr [rax], 1`, `.globl effect`, and
  `.globl main` instead of falling back to the unsupported direct-LIR path
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  backend route regression in `tests/c/internal/InternalTests.cmake`
  (`backend_codegen_route_x86_64_c_testsuite_00033_short_circuit_effect_retries_after_direct_bir_rejection`)
  so the real `00033.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00033.c` seam end-to-end:
  `./build/backend_bir_tests test_bir_lowering_accepts_minimal_short_circuit_effect_zero_return_lir_module test_backend_bir_pipeline_drives_x86_lir_minimal_short_circuit_effect_zero_return_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00033_short_circuit_effect_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00033_c)$'`,
  and the neighboring cluster
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00031_local_i32_inc_dec_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00032_local_array_pointer_inc_dec_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00033_short_circuit_effect_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00031_c|c_testsuite_x86_backend_src_00032_c|c_testsuite_x86_backend_src_00033_c)$'`
  which now pass for the owned seam cluster
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2659/215/2874` after); the refreshed after-state
  improved again from the prior recorded `2657 -> 2659` passes and
  `216 -> 215` failures after the `00033.c` slice, the new route test raises
  total tests from `2873` to `2874`, and the remaining red broad-suite lanes
  stay parked in the already-known riscv64 select-route, backend runtime,
  wider x86 source-backed, and parser buckets outside this bounded change
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00034_c` is now the next red source-backed x86
  case
- recovered the bounded shared-BIR `00032.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed local-array plus pointer pre/post increment-decrement compare
  route (`arr[2]`, stores `2/3`, pointer slot `%lv.p`, `*(p++)`, `*(p--)`,
  `*(++p)`, `*(--p)`, compare-`ne`, `zext i1 -> i32`, branch returns, final
  `ret i32 0`) and collapse `main` to the shared constant `0` return instead
  of stopping at the unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  backend route regression in `tests/c/internal/InternalTests.cmake`
  (`backend_codegen_route_x86_64_c_testsuite_00032_local_array_pointer_inc_dec_compare_retries_after_direct_bir_rejection`)
  so the real `00032.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00032.c` seam end-to-end:
  `./build/backend_bir_tests test_bir_lowering_accepts_local_i32_array_pointer_inc_dec_compare_zero_return_module test_backend_bir_pipeline_drives_x86_lir_local_i32_array_pointer_inc_dec_compare_zero_through_bir_end_to_end test_x86_direct_emitter_lowers_local_i32_array_pointer_inc_dec_compare_zero_slice`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00031_local_i32_inc_dec_compare_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00032_local_array_pointer_inc_dec_compare_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00031_c|c_testsuite_x86_backend_src_00032_c)$'`,
  and the neighboring cluster
...
- refreshed `test_fail_after.log` with
  ...
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2657/216/2873` after); the refreshed after-state
  improved again from the prior recorded `2655 -> 2657` passes and
  `217 -> 216` failures after the `00032.c` slice, the new route test raises
  total tests from `2872` to `2873`, and the remaining red broad-suite lanes
  stay parked in the already-known riscv64 select-route, backend runtime,
  wider x86 source-backed, and parser buckets outside this bounded change
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00033_c` is now the next red source-backed x86
  case, and its source body stays a compact short-circuit side-effect ordering
  slice around `x && effect()` and `x || effect()` with global `g`
...
- recovered the bounded shared-BIR `00030.c` seam by teaching
  ...
  signed `icmp` against `1000` or `100`, `zext i1 -> i32`, `icmp ne ..., 0`,
  early `ret i32 1`, final `ret i32 0`) and collapse `main` to the shared
  constant `0` return while preserving the constant helper definition instead
  of stopping at the unsupported x86 direct-LIR boundary
- taught `src/backend/x86/codegen/emit.cpp` the matching bounded direct-BIR
  two-function immediate-return slice so the lowered helper-plus-`main` module
  for `00030.c` still emits native x86 text with `.globl f`, `mov eax, 100`,
  `.globl main`, and `mov eax, 0` instead of falling back to the unsupported
  direct-LIR path
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  ...
  so the real `00030.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00030.c` seam end-to-end:
  `./build/backend_bir_tests test_bir_lowering_accepts_minimal_repeated_zero_arg_call_compare_zero_return_lir_module`,
  `./build/backend_bir_tests test_backend_bir_pipeline_drives_x86_lir_minimal_repeated_zero_arg_call_compare_zero_return_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00030_repeated_call_compare_zero_return_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00030_c)$'`,
  and the neighboring cluster
  `ctest --test-dir build --output-on-failure -R '^(c_testsuite_x86_backend_src_00029_c|backend_codegen_route_x86_64_c_testsuite_00029_local_i32_store_xor_sub_retries_after_direct_bir_rejection|backend_codegen_route_x86_64_c_testsuite_00030_repeated_call_compare_zero_return_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00030_c)$'`
  which now pass for the owned seam cluster
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2653/218/2871` after); the refreshed after-state
  improved again from the prior recorded `2651 -> 2653` passes and
  `219 -> 218` failures after the `00030.c` slice, the new route test raises
  total tests from `2870` to `2871`, and the remaining red broad-suite lanes
  stay parked in the already-known riscv64 select-route, backend runtime, and
  wider x86 source-backed buckets outside this bounded change
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00031_c` is now the next red source-backed x86
  case
- recovered the bounded shared-BIR `00029.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed local scalar slot-update route (`alloca i32`, `store 1`,
  `load`, `xor 3`, `store`, `load`, `sub 2`, `ret`) and collapse that
...
- covered that seam with focused shared-lowering and x86 pipeline regressions
  ...
  so the real `00029.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00029.c` seam end-to-end:
  `./build/backend_bir_tests test_bir_lowering_accepts_local_i32_store_xor_sub_lir_module`,
  `./build/backend_bir_tests test_backend_bir_pipeline_drives_x86_lir_local_i32_store_xor_sub_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00029_local_i32_store_xor_sub_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00029_c)$'`,
  and
  `ctest --test-dir build --output-on-failure -R '^(c_testsuite_x86_backend_src_00027_c|c_testsuite_x86_backend_src_00028_c|c_testsuite_x86_backend_src_00029_c)$'`
  which now pass for the owned seam cluster
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2651/219/2870` after); the refreshed after-state
  improved again from the prior recorded `2649 -> 2651` passes and
  `220 -> 219` failures after the `00029.c` slice, the new route test raises
  total tests from `2869` to `2870`, and the remaining red broad-suite lanes
  stay parked in the already-known riscv64 select-route, backend runtime, and
  wider x86 source-backed buckets outside this bounded change
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00030_c` is now the next red source-backed x86
  case, and its source body stays a compact repeated-call integer-compare slice
  (`f() > 1000`, `f() >= 1000`, `1000 < f()`, `1000 <= f()`, `1000 == f()`,
  `100 != f()`)
- recovered the bounded shared-BIR `00028.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed local scalar slot-update route (`alloca i32`, `store 1`,
  `load`, `and 3`, `store`, `load`, `sub 1`, `ret`) and collapse that
  direct-LIR module to the shared constant `0` return instead of stopping at
  the unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  ...
  so the real `00028.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00028.c` seam end-to-end:
  `./build/backend_bir_tests test_bir_lowering_accepts_local_i32_store_and_sub_lir_module`,
  `./build/backend_bir_tests test_backend_bir_pipeline_drives_x86_lir_local_i32_store_and_sub_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00028_local_i32_store_and_sub_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00028_c)$'`,
  and
  `ctest --test-dir build --output-on-failure -R '^(c_testsuite_x86_backend_src_00027_c|c_testsuite_x86_backend_src_00028_c)$'`
  which now pass for the owned seam cluster; the broad `backend_bir_tests`
  ctest target still exceeds the 30s suite timeout because of a pre-existing
  unrelated failing test and was not used as the acceptance gate for this
  bounded seam
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2649/220/2869` after); the refreshed after-state
  improved again from the prior recorded `2647 -> 2649` passes and
  `221 -> 220` failures after the `00028.c` slice, the new route test raises
  total tests from `2868` to `2869`, and the remaining red broad-suite lanes
  stay parked in the already-known riscv64 select-route, backend runtime, and
  wider x86 source-backed buckets outside this bounded change
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00029_c` is now the next red source-backed x86
  case, and its source body stays a compact local scalar bit-xor/subtract slice
  (`int x; x = 1; x = x ^ 3; return x - 2;`)
- recovered the bounded shared-BIR `00027.c` seam by teaching
  `src/backend/lowering/lir_to_bir/memory.cpp` to recognize the exact
  source-backed local scalar slot-update route (`alloca i32`, `store 1`,
  `load`, `or 4`, `store`, `load`, `sub 5`, `ret`) and collapse that
  direct-LIR module to the shared constant `0` return instead of stopping at
  the unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  ...
  so the real `00027.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00027.c` seam end-to-end:
  `./build/backend_bir_tests test_bir_lowering_accepts_local_i32_store_or_sub_lir_module`,
  `./build/backend_bir_tests test_backend_bir_pipeline_drives_x86_lir_local_i32_store_or_sub_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00027_local_i32_store_or_sub_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00027_c)$'`,
  and
  `ctest --test-dir build --output-on-failure -R '^(c_testsuite_x86_backend_src_00024_c|c_testsuite_x86_backend_src_00025_c|c_testsuite_x86_backend_src_00026_c|c_testsuite_x86_backend_src_00027_c)$'`
  which now pass for the owned seam cluster
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2647/221/2868` after); the refreshed after-state
  improved again from the prior recorded `2645 -> 2647` passes and
  `222 -> 221` failures after the `00027.c` slice, the new route test raises
  total tests from `2867` to `2868`, and the remaining red broad-suite lanes
  stay parked in the already-known riscv64 select-route, backend runtime, and
  wider x86 source-backed buckets outside this bounded change
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00028_c` is now the next red source-backed x86
  case, and its source body stays a compact local scalar bit-and/subtract slice
  (`int x; x = 1; x = x & 3; return x - 1;`)
...
- recovered the bounded shared-BIR `00025.c` seam by teaching
  ...
  slot store/reload of the base pointer, direct `@strlen(ptr %t1)`, `sub 5`,
  `ret`) and collapse that direct-LIR module to the shared constant `0`
  return instead of stopping at the unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  ...
  so the real `00025.c` path stays pinned on native x86 asm with the folded
  zero return instead of falling back to LLVM text or the unsupported
  direct-LIR error
- verified the bounded `00025.c` seam end-to-end:
  `./build/backend_bir_tests test_bir_lowering_accepts_string_literal_strlen_sub_lir_module`,
  `./build/backend_bir_tests test_backend_bir_pipeline_drives_x86_lir_string_literal_strlen_sub_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00025_string_literal_strlen_sub_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00025_c)$'`,
  and
  `ctest --test-dir build --output-on-failure -R '^(c_testsuite_x86_backend_src_00024_c|c_testsuite_x86_backend_src_00025_c|c_testsuite_x86_backend_src_00026_c)$'`
  where `00024.c` and `00025.c` now pass while `00026.c` remains the next red
  neighboring source-backed seam
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2643/223/2866` after); the refreshed after-state
  improved again from the prior recorded `2639 -> 2643` passes and
  `225 -> 223` failures after the `00025.c` slice, the new route test raises
  total tests from `2865` to `2866`, and the remaining red broad-suite lanes
  stay parked in the already-known riscv64 select-route, backend runtime, and
  wider x86 source-backed buckets outside this bounded change
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00026_c` is now the next red source-backed x86
  case, and its source body stays a compact string-literal indexed-load slice
  (`char *p; p = "hello"; return p[0] - 104;`)
...
- recovered the bounded shared-BIR `00023.c` seam by extending
  ...
  addition to explicit scalar initializers; that lets the compact source-backed
  global zero-store/reload route (`int x; x = 0; return x;`) lower through
  shared BIR instead of stopping at the unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  backend route regression in `tests/c/internal/InternalTests.cmake`
  ...
  so the real `00023.c` path stays pinned on native x86 asm and preserves the
  explicit global store-then-reload route instead of falling back to LLVM text
  or the unsupported direct-LIR error
- verified the bounded `00023.c` seam end-to-end:
  `./build/backend_bir_tests test_bir_lowering_accepts_zero_initialized_scalar_global_store_reload_lir_module`,
  `./build/backend_bir_tests test_backend_bir_pipeline_drives_x86_lir_zero_initialized_scalar_global_store_reload_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00023_zero_initialized_scalar_global_store_reload_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00023_c)$'`,
  and
  `ctest --test-dir build --output-on-failure -R '^(c_testsuite_x86_backend_src_00021_c|c_testsuite_x86_backend_src_00022_c|c_testsuite_x86_backend_src_00023_c)$'`
...
- refreshed `test_fail_after.log` with
  ...
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2639/225/2864` after); the refreshed after-state
  improved again from the prior recorded `2637 -> 2639` passes and
  `226 -> 225` failures after the `00023.c` slice, the new route test raises
  total tests from `2863` to `2864`, and the remaining red broad-suite lanes
  stay parked in the already-known riscv64 select-route, backend runtime, and
  wider x86 source-backed buckets outside this bounded change
- classified the next bounded seam from the refreshed targeted state:
  `c_testsuite_x86_backend_src_00024_c` is now the next red source-backed x86
  case, and its source body stays a compact global two-field struct arithmetic
  slice (`typedef struct { int x; int y; } s; s v; v.x = 1; v.y = 2; return 3 -
  v.x - v.y;`)
- recovered the bounded shared-BIR `00020.c` seam by teaching
  `src/backend/lowering/lir_to_bir.cpp` to recognize the exact source-backed
  local double-indirect pointer-chain route (`alloca i32`, `alloca ptr`,
  `alloca ptr`, store `0` into `x`, store `&x` into `p`, store `&p` into `pp`,
  then `load pp`, `load *pp`, `load **pp`, `ret`) and collapse that direct-LIR
...
- covered that seam with focused shared-lowering and x86 pipeline regressions
  ...
  so the real `00020.c` path stays pinned on native x86 asm instead of falling
  back to LLVM text or the unsupported direct-LIR error
- verified the bounded `00020.c` seam end-to-end:
  `./build/backend_bir_tests test_bir_lowering_accepts_double_indirect_local_pointer_chain_zero_return_module`,
  `./build/backend_bir_tests test_backend_bir_pipeline_drives_x86_lir_double_indirect_local_pointer_chain_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00020_double_indirect_local_pointer_chain_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00020_c)$'`,
  and
  `ctest --test-dir build --output-on-failure -R '^(c_testsuite_x86_backend_src_00011_c|c_testsuite_x86_backend_src_00012_c|c_testsuite_x86_backend_src_00013_c|c_testsuite_x86_backend_src_00014_c|c_testsuite_x86_backend_src_00015_c|c_testsuite_x86_backend_src_00016_c|c_testsuite_x86_backend_src_00017_c|c_testsuite_x86_backend_src_00018_c|c_testsuite_x86_backend_src_00019_c|c_testsuite_x86_backend_src_00020_c)$'`
  now pass for the owned seam cluster
- refreshed `test_fail_after.log` with
  `ctest --test-dir build -j8 --output-on-failure > test_fail_after.log` and
  re-ran the monotonic guard through the `c4c-regression-guard` skill:
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2637/226/2863` after); the refreshed after-state
  improved again from the prior recorded `2634 -> 2637` passes and
  `228 -> 226` failures after the `00020.c` slice, the new route test raises
  total tests from `2862` to `2863`, and the remaining red broad-suite lanes
  stay parked in the already-known riscv64 select-route, backend runtime, and
  wider x86 source-backed buckets outside this bounded change
- classified the next bounded seam from the refreshed after-log:
  `c_testsuite_x86_backend_src_00023_c` is now the next red source-backed x86
  case, and its source body stays a compact scalar-global zero-return slice
  (`int x; x = 0; return x;`)

- recovered the bounded shared-BIR `00019.c` seam by teaching
  `src/backend/lowering/lir_to_bir.cpp` to recognize the exact source-backed
  local self-referential struct-pointer chain route (`alloca %struct.S`,
  store field `x = 0`, store `&s` into field `p`, then repeat five
  field-0 `gep`/pointer-load hops before the final field-1 load and `ret`) and
  collapse that direct-LIR module to the shared constant `0` return instead of
  stopping at the unsupported x86 direct-LIR boundary
- covered that seam with focused shared-lowering and x86 pipeline regressions
  in `tests/backend/backend_bir_lowering_tests.cpp` and
  `tests/backend/backend_bir_pipeline_x86_64_tests.cpp`, plus a source-backed
  ...
  so the real `00019.c` path stays pinned on native x86 asm instead of falling
  back to LLVM text or the unsupported direct-LIR error
- verified the bounded `00019.c` seam end-to-end:
  `./build/backend_bir_tests test_bir_lowering_accepts_local_self_referential_struct_pointer_chain_zero_return_module`,
  `./build/backend_bir_tests test_backend_bir_pipeline_drives_x86_lir_local_self_referential_struct_pointer_chain_through_bir_end_to_end`,
  `ctest --test-dir build --output-on-failure -R '^(backend_codegen_route_x86_64_c_testsuite_00019_local_self_referential_struct_pointer_chain_retries_after_direct_bir_rejection|c_testsuite_x86_backend_src_00019_c)$'`,
  and
  `ctest --test-dir build --output-on-failure -R '^(c_testsuite_x86_backend_src_00011_c|c_testsuite_x86_backend_src_00012_c|c_testsuite_x86_backend_src_00013_c|c_testsuite_x86_backend_src_00014_c|c_testsuite_x86_backend_src_00015_c|c_testsuite_x86_backend_src_00016_c|c_testsuite_x86_backend_src_00017_c|c_testsuite_x86_backend_src_00018_c|c_testsuite_x86_backend_src_00019_c)$'`
  now pass for the owned seam cluster
- refreshed `test_fail_after.log` with
  ...
  `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed`
  which still fails against the stale broad-suite baseline
  (`2670/179/2849` before vs `2634/228/2862` after); the refreshed after-state
  improved again from the prior recorded `2632 -> 2634` passes and
  `229 -> 228` failures after the `00019.c` slice, the new route test raises
  total tests from `2861` to `2862`, and the remaining red broad-suite lanes
  stay parked in the already-known riscv64 select-route, backend runtime, and
  wider x86 source-backed buckets outside this bounded change
- classified the next bounded seam from the refreshed after-log:
  `c_testsuite_x86_backend_src_00020_c` is now the next red source-backed x86
  case, and its source body stays a compact local double-indirect zero-return
  slice (`int x, *p, **pp; x = 0; p = &x; pp = &p; return **pp;`)
