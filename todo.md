...
trailing-join `mul` consumer family now proves authoritative prepared
...
time, starting with `shl` `EdgeStoreSlot` ownership and passthrough proof
...
- Keep Step 3 packets focused on consumer migration proof, not on reopening
  ...
  surfaces.
- The trailing-join `mul` family now has direct and `EdgeStoreSlot` ownership
...
`mul` coverage so the shared prepared joined-branch handoff stays
