...
- Current slice: move one additional internal LLVM-text check fixture off the
  ...
  `variadic_bigints_last` LLVM-text check onto `backend_module_case` and the
...
- continue migrating one additional narrow
  ...
  `variadic_bigints_last.c`, ideally one of the remaining AArch64 mixed
  variadic LLVM-text checks that can switch paths without changing test intent
