...
Step 1 kept the prepared x86 route structural by collapsing
`render_prepared_local_i32_arithmetic_guard_if_supported` onto
`PreparedX86FunctionDispatchContext` end to end. The prepared module emitter
already dispatched through the context surface; this packet removed the
remaining raw-argument overload so the local i32 guard lane now consumes the
canonical prepared function-dispatch handoff directly.
...
extraction. The matching remaining seam is the local i16 arithmetic guard
helper, which still unwraps the function-dispatch context into a long raw
...
- Prefer one coherent instruction-family migration per packet over broad
  emitter rewrites.
- `render_prepared_local_i32_arithmetic_guard_if_supported` now consumes the
  function-dispatch context directly, but other structural helper seams may
  still expose raw bundles; keep Step 1 structural and avoid widening into
  selector logic until those seams are reviewed.
...
i32 guard dispatch-context refactor, and the `^backend_` subset is not fully
