...
compare-join render contract on the trailing-`mul` joined-branch family,
...
at a time, preferably the trailing-`shl` compare-join render-contract helper
...
- The trailing-join arithmetic, `xor`, `and`, `or`, `mul`, `shl`, `lshr`, and
  ...
  The trailing-`xor`, trailing-`and`, trailing-`or`, and trailing-`mul` lanes
  now also have explicit compare-join render-contract helper proof, so the
  next packet should stay on one uncovered residual helper surface instead of
  reopening covered route families.
...
`backend_x86_handoff_boundary` proof passed after adding explicit trailing-`mul`
