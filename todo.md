...
split-predecessor `unsigned char` mixed-then-deeper-affine `select ne` parity
inventory with the adjacent post-add/sub tail and verify whether it stays on
the direct BIR route.
Next target: if the widened mixed-then-deeper-affine `select ne` post-add/sub
tail is also coverage-only, continue to the adjacent post-add/sub/add tail in
the same family until a real direct-BIR lowering gap appears.

Completed this iteration:
- Re-audited the widened two-parameter `unsigned char`
  split-predecessor mixed-then-deeper-affine select inventory and confirmed the
  next adjacent uncovered Step 2 parity gap after the mixed-affine `select ne`
  sweep was the missing `two_param_u8_select_ne` mixed-then-deeper-affine
  post-add route beside the existing `eq` coverage.
- Added
  `tests/c/internal/backend_route_case/two_param_u8_select_ne_split_predecessor_mixed_then_deeper_affine_post_add.c`,
  proving the bounded two-parameter widened not-equal ternary with
  split-predecessor mixed-then-deeper-affine
  `x + 8 - 3` / `y + 11 - 4 + 7` arms plus a join-local `+ 6` tail stays on
  the backend BIR route instead of falling back to legacy LLVM IR text.
- Registered
  ...
  contains `bir.func @choose2_mixed_then_deeper_post_ne_u(i8 %p.x, i8 %p.y) -> i8 {`,
  the split-predecessor mixed/deeper affine `bir.add` and `bir.sub` steps,
  `bir.select ne i8 %p.x, %p.y, %t12, %t16`, the trailing join-local
  `bir.add`, and forbids legacy LLVM IR
...
- Added
  ...
  `test_bir_lowering_accepts_two_param_u8_select_ne_split_predecessor_mixed_then_deeper_affine_phi_post_join_add_slice()`
  in `tests/backend/backend_bir_lowering_tests.cpp` to keep the widened
  mixed-then-deeper split-predecessor not-equal post-add shape covered below
  the backend-route harness.
- Used the new coverage to confirm there was no hidden direct-BIR lowering gap
  in this slice: after aligning the handcrafted backend BIR scaffold with the
  existing widened `u8` zext/i32/trunc pattern, the direct backend route and
  backend BIR lowering both accepted the mixed-then-deeper `ne` post-add shape
  without touching production lowering code.
- Rebuilt `backend_bir_tests` and `c4cll`, reran `./build/backend_bir_tests`,
  ...
  `392/392` backend-labeled tests passing, refreshed `test_fail_after.log` with
  a full `ctest --test-dir build -j8 --output-on-failure` run, and passed the
  regression guard against `test_fail_before.log` with
  `--allow-non-decreasing-passed --timeout-threshold 30 --enforce-timeout`
  (`2822 -> 2830` passed, `0 -> 0` failed, no newly failing tests, no new
  `>30s` cases).

Completed this iteration:
- Re-audited the widened two-parameter `unsigned char` split-predecessor
  mixed-affine `select ne` inventory after the post-add slice and confirmed
  the next adjacent uncovered Step 2 parity gap was the missing mixed-affine
  split-predecessor `two_param_u8_select_ne` post-add-sub route beside the
  existing `eq` coverage.
- Added
  `tests/c/internal/backend_route_case/two_param_u8_select_ne_split_predecessor_mixed_affine_post_add_sub.c`,
  proving the bounded two-parameter widened not-equal ternary with
  split-predecessor mixed-affine `x + 8 - 3` / `y + 11 - 4` arms plus a
  join-local `+ 6 - 2` tail stays on the backend BIR route instead of falling
...
- Registered
  ...
  in `tests/c/internal/InternalTests.cmake`, asserting the emitted text
  contains `bir.func @choose2_mixed_post_chain_ne_u(i8 %p.x, i8 %p.y) -> i8 {`,
  the split-predecessor mixed-affine `bir.add`/`bir.sub` arms,
  `bir.select ne i8 %p.x, %p.y, %t12, %t15`, the trailing join-local
  `bir.add` and `bir.sub`, and forbids legacy LLVM IR
...
- Added
  ...
  to `tests/backend/backend_bir_test_support.*` plus
  `test_bir_lowering_accepts_two_param_u8_select_ne_split_predecessor_mixed_affine_phi_post_join_add_sub_slice()`
  in `tests/backend/backend_bir_lowering_tests.cpp` to keep the widened
  split-predecessor mixed-affine not-equal post-add/sub shape covered below
  the backend-route harness.
- Used the new coverage to confirm there was no hidden direct-BIR lowering gap
  in this slice: the current widened `i8` conditional-phi select lowering
  already accepts split predecessors, mixed-affine arm arithmetic, the `ne`
  predicate, and the join-local `+ 6 - 2` tail, so this iteration advanced
  Step 2 by closing a parity-coverage hole rather than by changing
  `src/backend/lowering/lir_to_bir.cpp`.
- Rebuilt `backend_bir_tests` and `c4cll`, reran `./build/backend_bir_tests`,
  reran
  `backend_codegen_route_riscv64_two_param_u8_select_ne_split_predecessor_mixed_affine_post_add_sub_defaults_to_bir`,
  reran `ctest --test-dir build -L backend --output-on-failure -j8` with
  `391/391` backend-labeled tests passing, refreshed `test_fail_after.log`
  ...
  `--allow-non-decreasing-passed --timeout-threshold 30 --enforce-timeout`
  (`2822 -> 2829` passed, `0 -> 0` failed, no newly failing tests, no new
  `>30s` cases).

Completed this iteration:
- Re-audited the widened two-parameter `unsigned char` split-predecessor select
  inventory after the add/phi/post-add-sub-add slice and confirmed the next
  adjacent uncovered Step 2 parity gap was the missing mixed-affine
  split-predecessor `two_param_u8_select_ne` post-add route beside the
  existing `eq` coverage.
- Added
  `tests/c/internal/backend_route_case/two_param_u8_select_ne_split_predecessor_mixed_affine_post_add.c`,
  proving the bounded two-parameter widened not-equal ternary with
  split-predecessor mixed-affine `x + 8 - 3` / `y + 11 - 4` arms plus a
  join-local `+ 6` tail stays on the backend BIR route instead of falling back
  to legacy LLVM IR text.
- Registered
  `backend_codegen_route_riscv64_two_param_u8_select_ne_split_predecessor_mixed_affine_post_add_defaults_to_bir`
  in `tests/c/internal/InternalTests.cmake`, asserting the emitted text
  contains `bir.func @choose2_mixed_post_ne_u(i8 %p.x, i8 %p.y) -> i8 {`, the
  split-predecessor mixed-affine `bir.add`/`bir.sub` arms,
  `bir.select ne i8 %p.x, %p.y, %t12, %t15`, the trailing join-local
  `bir.add`, and forbids legacy LLVM IR
...
- Added
  ...
  to `tests/backend/backend_bir_test_support.*` plus
  `test_bir_lowering_accepts_two_param_u8_select_ne_split_predecessor_mixed_affine_phi_post_join_add_slice()`
  in `tests/backend/backend_bir_lowering_tests.cpp` to keep the widened
  split-predecessor mixed-affine not-equal post-add shape covered below the
  backend-route harness.
- Used the new coverage to confirm there was no hidden direct-BIR lowering gap
  in this slice: the current widened `i8` conditional-phi select lowering
  already accepts split predecessors, mixed-affine arm arithmetic, the `ne`
  predicate, and the join-local `+ 6` tail, so this iteration advanced Step 2
  by closing a parity-coverage hole rather than by changing
  `src/backend/lowering/lir_to_bir.cpp`.
- Rebuilt `backend_bir_tests` and `c4cll`, reran `./build/backend_bir_tests`,
  reran
  `backend_codegen_route_riscv64_two_param_u8_select_ne_split_predecessor_mixed_affine_post_add_defaults_to_bir`,
  reran `ctest --test-dir build -L backend --output-on-failure -j8` with
  `390/390` backend-labeled tests passing, refreshed `test_fail_after.log`
  with a full `ctest --test-dir build -j8 --output-on-failure` run, and passed
  the regression guard against `test_fail_before.log` with
  `--allow-non-decreasing-passed --timeout-threshold 30 --enforce-timeout`
  (`2822 -> 2828` passed, `0 -> 0` failed, no newly failing tests, no new
  `>30s` cases).
...
Completed this iteration:
- Re-audited the widened two-parameter `unsigned char` split-predecessor
  select inventory after the simpler predecessor-add `select ne` slice and
  identified the next adjacent uncovered Step 2 parity gap: the missing
  split-predecessor add/phi/post-add-sub `two_param_u8_select_ne` route beside
  the existing `eq` coverage.
- Added
  `tests/c/internal/backend_route_case/two_param_u8_select_ne_split_predecessor_add_phi_post_add_sub.c`,
  proving the bounded two-parameter widened not-equal ternary with
  split-predecessor arithmetic plus join-local `+ 6 - 2` stays on the backend
  BIR route instead of falling back to legacy LLVM IR text.
- Registered
  `backend_codegen_route_riscv64_two_param_u8_select_ne_split_predecessor_add_phi_post_add_sub_defaults_to_bir`
  in `tests/c/internal/InternalTests.cmake`, asserting the emitted text
  contains `bir.func @choose2_add_post_chain_ne_u(i8 %p.x, i8 %p.y) -> i8 {`,
  predecessor-local `bir.add` arms, `bir.select ne i8 %p.x, %p.y, %t11, %t13`,
  the join-local `bir.add` and `bir.sub`, and forbids legacy LLVM IR
...
- Added
  ...
  to `tests/backend/backend_bir_test_support.*` plus
  `test_bir_lowering_accepts_two_param_u8_select_ne_split_predecessor_add_phi_post_join_add_sub_slice()`
  in `tests/backend/backend_bir_lowering_tests.cpp` to keep the widened
  split-predecessor not-equal add/sub tail covered below the backend-route
  harness.
- Used the new coverage to confirm there was no hidden direct-BIR lowering gap
  in this slice: the current widened `i8` conditional-phi select lowering
  already accepts split predecessors, the `ne` predicate, and the join-local
  `+ 6 - 2` tail, so this iteration advanced Step 2 by closing a parity
  coverage hole rather than by changing `src/backend/lowering/lir_to_bir.cpp`.
- Rebuilt `backend_bir_tests` and `c4cll`, reran `./build/backend_bir_tests`,
  reran
  `backend_codegen_route_riscv64_two_param_u8_select_ne_split_predecessor_add_phi_post_add_sub_defaults_to_bir`,
  reran `ctest --test-dir build -L backend --output-on-failure -j8` with
  `388/388` backend-labeled tests passing, and refreshed `test_fail_after.log`
  with a full `ctest --test-dir build -j8 --output-on-failure` run showing
  `2826/2826` tests passing.
...
- Re-audited the zero-parameter widened arithmetic-return inventory in
  ...
  widened arithmetic parity gap after the landed `return_udiv_u8` slice was the
  zero-parameter `return_srem_u8` route beside the existing plain
  `return_srem` family.
- Added `tests/c/internal/backend_route_case/return_srem_u8.c`, proving the
  bounded zero-parameter `unsigned char` signed-remainder wrapper reaches the
  backend through BIR instead of falling back to legacy LLVM IR text.
- Registered `backend_codegen_route_riscv64_return_srem_u8_defaults_to_bir` in
  `tests/c/internal/InternalTests.cmake`, asserting the emitted text contains
  `bir.func @choose_srem_u() -> i8 {`, `%t0 = bir.srem i8 14, 5`,
  `bir.ret i8 %t0`, and forbids legacy LLVM IR `define i8 @choose_srem_u()`.
- Added `make_bir_i8_return_srem_module()` to
  `tests/backend/backend_bir_test_support.*` plus
  `test_bir_lowering_accepts_i8_return_srem()` in
  `tests/backend/backend_bir_lowering_tests.cpp` to keep the widened
  zero-parameter `i8` signed-remainder return shape covered below the
  backend-route harness.
- Used the new coverage to expose a real Step 2 direct-BIR gap: the widened
  zero-parameter `i8` signed-remainder return case was not admitted by the
  existing widened-`i8` arithmetic lowering helper.
- Fixed `src/backend/lowering/lir_to_bir.cpp` so
  `try_lower_widened_i8_add_sub_chain_function()` now accepts widened
  constant-only `srem`, preserving the existing parameter restrictions while
  lowering this route directly to `bir.srem i8`.
- Rebuilt `backend_bir_tests` and `c4cll`, reran `./build/backend_bir_tests`,
  reran `backend_codegen_route_riscv64_return_srem_u8_defaults_to_bir` plus
  adjacent `return_srem`/`return_udiv_u8` route coverage, reran
  `ctest --test-dir build -L backend --output-on-failure -j8` with
  `384/384` backend-labeled tests passing, refreshed `test_fail_after.log`
  with a full `ctest --test-dir build -j8 --output-on-failure` run, and passed
  the regression guard against `test_fail_before.log` with
  `--allow-non-decreasing-passed --timeout-threshold 30 --enforce-timeout`
  (`2800 -> 2822` passed, `0 -> 0` failed, no newly failing tests, no new
  `>30s` cases).
...
- Re-audited the zero-parameter widened arithmetic-return inventory in
  `tests/c/internal/InternalTests.cmake` and confirmed the next adjacent
  widened arithmetic parity gap after the landed `return_ashr_u8` slice was the
  zero-parameter `return_sdiv_u8` route beside the existing plain
  `return_sdiv` family.
- Added `tests/c/internal/backend_route_case/return_sdiv_u8.c`, proving the
  bounded zero-parameter `unsigned char` signed-division wrapper reaches the
  backend through BIR instead of falling back to legacy LLVM IR text.
- Registered `backend_codegen_route_riscv64_return_sdiv_u8_defaults_to_bir` in
  `tests/c/internal/InternalTests.cmake`, asserting the emitted text contains
  `bir.func @choose_sdiv_u() -> i8 {`, `%t0 = bir.sdiv i8 12, 3`,
  `bir.ret i8 %t0`, and forbids legacy LLVM IR `define i8 @choose_sdiv_u()`.
- Added `make_bir_i8_return_sdiv_module()` to
  `tests/backend/backend_bir_test_support.*` plus
  `test_bir_lowering_accepts_i8_return_sdiv()` in
  `tests/backend/backend_bir_lowering_tests.cpp` to keep the widened
  zero-parameter `i8` signed-division return shape covered below the
  backend-route harness.
- Used the new coverage to expose a real Step 2 direct-BIR gap: the widened
  zero-parameter `i8` signed-division return case was not admitted by the
  existing widened-`i8` arithmetic lowering helper.
- Fixed `src/backend/lowering/lir_to_bir.cpp` so
  `try_lower_widened_i8_add_sub_chain_function()` now accepts widened
  constant-only `sdiv`, preserving the existing parameter restrictions while
  lowering this route directly to `bir.sdiv i8`.
- Rebuilt `backend_bir_tests` and `c4cll`, reran `./build/backend_bir_tests`,
  reran `backend_codegen_route_riscv64_return_sdiv_u8_defaults_to_bir`, and
  reran `ctest --test-dir build -L backend --output-on-failure -j8` with
  `382/382` backend-labeled tests passing.

Completed this iteration:
- Re-audited the zero-parameter widened arithmetic-return inventory in
  `tests/c/internal/InternalTests.cmake` and confirmed the next adjacent
  widened shift parity gap after the landed `return_lshr_u8` slice was the
  zero-parameter `return_ashr_u8` route beside the existing plain
  `return_ashr` family.
- Added `tests/c/internal/backend_route_case/return_ashr_u8.c`, proving the
  bounded zero-parameter `unsigned char` arithmetic-right-shift wrapper reaches
  the backend through BIR instead of falling back to legacy LLVM IR text.
- Registered `backend_codegen_route_riscv64_return_ashr_u8_defaults_to_bir` in
  `tests/c/internal/InternalTests.cmake`, asserting the emitted text contains
  `bir.func @choose_ashr_u() -> i8 {`, `%t0 = bir.sub i8 0, 16`,
  `%t1 = bir.ashr i8 %t0, 2`, `bir.ret i8 %t1`, and forbids legacy LLVM IR
  `define i8 @choose_ashr_u()`.
- Added `make_bir_i8_return_ashr_module()` to
  `tests/backend/backend_bir_test_support.*` plus
  `test_bir_lowering_accepts_i8_return_ashr()` in
  `tests/backend/backend_bir_lowering_tests.cpp` to keep the widened
  zero-parameter `i8` arithmetic-right-shift return shape covered below the
  backend-route harness.
- Used the new coverage to expose a real Step 2 direct-BIR gap: the widened
  zero-parameter `i8` arithmetic-right-shift return case was not admitted by
  the existing widened-`i8` arithmetic lowering helper.
- Fixed `src/backend/lowering/lir_to_bir.cpp` so
  `try_lower_widened_i8_add_sub_chain_function()` now accepts widened
  constant-only `ashr`, preserving the existing parameter restrictions while
  lowering this route directly to `bir.ashr i8`.
- Rebuilt `backend_bir_tests` and `c4cll`, reran `./build/backend_bir_tests`,
  reran the plain and widened `return_ashr` backend-route tests, reran
  `ctest --test-dir build -L backend --output-on-failure -j8` with
  `381/381` backend-labeled tests passing, refreshed `test_fail_after.log`
  with a full `ctest --test-dir build -j8 --output-on-failure` run, and passed
  the regression guard against `test_fail_before.log` with
  `--allow-non-decreasing-passed --timeout-threshold 30 --enforce-timeout`
  (`2800 -> 2819` passed, `0 -> 0` failed, no newly failing tests, no new
  `>30s` cases).
...
- Re-audited the zero-parameter widened arithmetic-return inventory in
  `tests/c/internal/InternalTests.cmake` and confirmed the next adjacent
  widened shift parity gap after the landed `return_xor_u8` slice was the
  zero-parameter `return_shl_u8` route beside the existing plain `return_shl`
  family.
- Added `tests/c/internal/backend_route_case/return_shl_u8.c`, proving the
  bounded zero-parameter `unsigned char` direct-left-shift wrapper reaches the
  backend through BIR instead of falling back to legacy LLVM IR text.
- Registered `backend_codegen_route_riscv64_return_shl_u8_defaults_to_bir` in
  `tests/c/internal/InternalTests.cmake`, asserting the emitted text contains
  `bir.func @choose_shl_u() -> i8 {`, `%t0 = bir.shl i8 3, 4`, `bir.ret i8 %t0`,
  and forbids legacy LLVM IR `define i8 @choose_shl_u()`.
- Added `make_bir_i8_return_shl_module()` to
  `tests/backend/backend_bir_test_support.*` plus
  `test_bir_lowering_accepts_i8_return_shl()` in
  `tests/backend/backend_bir_lowering_tests.cpp` to keep the widened
  zero-parameter `i8` left-shift return shape covered below the backend-route
  harness.
- Used the new coverage to expose a real Step 2 direct-BIR gap: the widened
  zero-parameter `i8` left-shift return case was not admitted by the existing
  widened-`i8` arithmetic lowering helper.
- Fixed `src/backend/lowering/lir_to_bir.cpp` so
  `try_lower_widened_i8_add_sub_chain_function()` now accepts widened
  constant-only `shl`, preserving the existing parameter restrictions while
  lowering this route directly to `bir.shl i8`.
- Rebuilt `backend_bir_tests` and `c4cll`, reran `./build/backend_bir_tests`,
  reran `backend_codegen_route_riscv64_return_shl_u8_defaults_to_bir`, and
  reran `ctest --test-dir build -L backend --output-on-failure -j8` with
  `379/379` backend-labeled tests passing, then refreshed
  ...
  `--allow-non-decreasing-passed --timeout-threshold 30 --enforce-timeout`
  (`2800 -> 2817` passed, `0 -> 0` failed, no newly failing tests, no new
  `>30s` cases).

Completed this iteration:
- Re-audited the zero-parameter widened arithmetic-return inventory in
  `tests/c/internal/InternalTests.cmake` and confirmed the next adjacent
  widened bitwise parity gap after the landed `return_or_u8` slice was the
  zero-parameter `return_xor_u8` route beside the existing plain `return_xor`
  family.
- Added `tests/c/internal/backend_route_case/return_xor_u8.c`, proving the
  bounded zero-parameter `unsigned char` direct-bitwise-xor wrapper reaches the
  backend through BIR instead of falling back to legacy LLVM IR text.
- Registered `backend_codegen_route_riscv64_return_xor_u8_defaults_to_bir` in
  `tests/c/internal/InternalTests.cmake`, asserting the emitted text contains
  `bir.func @choose_xor_u() -> i8 {`, `%t0 = bir.xor i8 12, 10`,
  `bir.ret i8 %t0`, and forbids legacy LLVM IR `define i8 @choose_xor_u()`.
- Added `make_bir_i8_return_xor_module()` to
  `tests/backend/backend_bir_test_support.*` plus
  `test_bir_lowering_accepts_i8_return_xor()` in
  `tests/backend/backend_bir_lowering_tests.cpp` to keep the widened
  zero-parameter `i8` bitwise-xor return shape covered below the backend-route
  harness.
- Used the new coverage to expose a real Step 2 direct-BIR gap: the widened
  zero-parameter `i8` bitwise-xor return case still lowered as legacy LLVM IR
  `xor i32` plus `trunc i32 -> i8`.
- Fixed `src/backend/lowering/lir_to_bir.cpp` so
  `try_lower_widened_i8_add_sub_chain_function()` now accepts widened
  constant-only `xor`, preserving the existing parameter restrictions while
  lowering this route directly to `bir.xor i8`.
- Rebuilt `backend_bir_tests` and `c4cll`, reran `./build/backend_bir_tests`,
  reran `backend_codegen_route_riscv64_return_xor_u8_defaults_to_bir`, and
  reran `ctest --test-dir build -L backend --output-on-failure -j8` with
  `378/378` backend-labeled tests passing, then refreshed
  `test_fail_after.log` with a full `ctest --test-dir build -j8 --output-on-failure`
  run and passed the regression guard against `test_fail_before.log` with
  `--allow-non-decreasing-passed --timeout-threshold 30 --enforce-timeout`
  (`2800 -> 2816` passed, `0 -> 0` failed, no newly failing tests, no new
  `>30s` cases).

Completed this iteration:
- Re-audited the zero-parameter widened arithmetic-return inventory in
  `tests/c/internal/InternalTests.cmake` and confirmed the next adjacent
  widened bitwise parity gap after the landed `return_and_u8` slice was the
  zero-parameter `return_or_u8` route beside the existing plain `return_or`
  family.
- Added `tests/c/internal/backend_route_case/return_or_u8.c`, proving the
  bounded zero-parameter `unsigned char` direct-bitwise-or wrapper reaches the
  backend through BIR instead of falling back to legacy LLVM IR text.
- Registered `backend_codegen_route_riscv64_return_or_u8_defaults_to_bir` in
  `tests/c/internal/InternalTests.cmake`, asserting the emitted text contains
  `bir.func @choose_or_u() -> i8 {`, `%t0 = bir.or i8 12, 3`, `bir.ret i8 %t0`,
  and forbids legacy LLVM IR `define i8 @choose_or_u()`.
- Added `make_bir_i8_return_or_module()` to
  `tests/backend/backend_bir_test_support.*` plus
  `test_bir_lowering_accepts_i8_return_or()` in
  `tests/backend/backend_bir_lowering_tests.cpp` to keep the widened
  zero-parameter `i8` bitwise-or return shape covered below the backend-route
  harness.
- Used the new coverage to expose a real Step 2 direct-BIR gap: the widened
  zero-parameter `i8` bitwise-or return case still lowered as legacy LLVM IR
  `or i32` plus `trunc i32 -> i8`.
- Fixed `src/backend/lowering/lir_to_bir.cpp` so
  `try_lower_widened_i8_add_sub_chain_function()` now accepts widened
  constant-only `or`, preserving the existing parameter restrictions while
  lowering this route directly to `bir.or i8`.
- Rebuilt `backend_bir_tests` and `c4cll`, reran `./build/backend_bir_tests`,
  reran `backend_codegen_route_riscv64_return_or_u8_defaults_to_bir`, and
  reran `ctest --test-dir build -L backend --output-on-failure -j8` with
  `377/377` backend-labeled tests passing.
...
Completed this iteration:
- Re-audited the zero-parameter widened arithmetic-return inventory in
  `tests/c/internal/InternalTests.cmake` and confirmed the next adjacent parity
  gap after the landed `return_add_u8` / `return_sub_u8` pair was
  `return_mul_u8`.
- Added `tests/c/internal/backend_route_case/return_mul_u8.c`, proving the
  bounded zero-parameter `unsigned char` direct-multiply wrapper reaches the
  backend through BIR instead of falling back to legacy LLVM IR text.
- Registered
  `backend_codegen_route_riscv64_return_mul_u8_defaults_to_bir` in
  `tests/c/internal/InternalTests.cmake`, asserting the emitted text contains
  `bir.func @choose_mul_u() -> i8 {`, `%t0 = bir.mul i8 6, 7`, `bir.ret i8 %t0`,
  and forbids legacy LLVM IR `define i8 @choose_mul_u()`.
- Added `make_bir_i8_return_mul_module()` to
  `tests/backend/backend_bir_test_support.*` plus
  `test_bir_lowering_accepts_i8_return_mul()` in
  `tests/backend/backend_bir_lowering_tests.cpp` to keep the widened
  zero-parameter `i8` multiply-return shape covered below the backend-route
  harness.
- Used the new coverage to expose a real Step 2 direct-BIR gap: the widened
  zero-parameter `i8` multiply-return case still lowered as legacy LLVM IR
  `mul i32` plus `trunc i32 -> i8`.
- Fixed `src/backend/lowering/lir_to_bir.cpp` so
  `try_lower_widened_i8_add_sub_chain_function()` now accepts widened `mul`
  only for constant-only operands, preserving the existing parameter-chain
  restriction while lowering this route directly to `bir.mul i8`.
- Rebuilt `backend_bir_tests` and `c4cll`, reran `./build/backend_bir_tests`,
  reran `backend_codegen_route_riscv64_return_mul_u8_defaults_to_bir`, reran
  `ctest --test-dir build -L backend --output-on-failure -j8` with
  `375/375` backend-labeled tests passing, then refreshed
  `test_fail_after.log` with a full `ctest --test-dir build -j8 --output-on-failure`
  run and passed the regression guard against `test_fail_before.log` with
  `--allow-non-decreasing-passed --timeout-threshold 30 --enforce-timeout`
  (`2800 -> 2813` passed, `0 -> 0` failed, no newly failing tests, no new
  `>30s` cases).
...
- Re-audited the zero-parameter widened arithmetic-return inventory in
  `tests/c/internal/InternalTests.cmake` and confirmed the next adjacent
  parity gap after the landed two-parameter `u8` add slice was the
  zero-parameter `return_add_u8` route beside the existing `return_add` /
  `return_sub` family.
- Added `tests/c/internal/backend_route_case/return_add_u8.c`, proving the
  bounded zero-parameter `unsigned char` direct-add wrapper reaches the backend
  through BIR instead of falling back to legacy LLVM IR text.
- Registered
  `backend_codegen_route_riscv64_return_add_u8_defaults_to_bir` in
  `tests/c/internal/InternalTests.cmake`, asserting the emitted text contains
  `bir.func @choose_add_u() -> i8 {`, `%t0 = bir.add i8 2, 3`, `bir.ret i8 %t0`,
  and forbids legacy LLVM IR `define i8 @choose_add_u()`.
- Added `make_bir_i8_return_add_module()` to
  `tests/backend/backend_bir_test_support.*` plus
  `test_bir_lowering_accepts_i8_return_add()` in
  `tests/backend/backend_bir_lowering_tests.cpp` to keep the widened
  zero-parameter `i8` add-return shape covered below the backend-route harness.
- Confirmed the new coverage passed without any
  `src/backend/lowering/lir_to_bir.cpp` change; the existing widened-`i8`
  arithmetic lowering already handled the zero-parameter add-return shape
  correctly.
- Rebuilt `backend_bir_tests`, reran `./build/backend_bir_tests`, reran
  `backend_codegen_route_riscv64_return_add_u8_defaults_to_bir`, reran
  `ctest --test-dir build -L backend --output-on-failure -j8` with
  `373/373` backend-labeled tests passing, then refreshed
  `test_fail_after.log` with a full `ctest --test-dir build -j8 --output-on-failure`
  run and passed the regression guard against `test_fail_before.log` with
  `--allow-non-decreasing-passed --timeout-threshold 30 --enforce-timeout`
  (`2800 -> 2811` passed, `0 -> 0` failed, no newly failing tests, no new
  `>30s` cases).
...
- Re-audited the zero-parameter widened compare-return inventory in
  `tests/c/internal/InternalTests.cmake` and confirmed the next adjacent signed
  relational parity gap after the landed `return_sle_u8` slice was
  `return_sgt_u8`.
- Added `tests/c/internal/backend_route_case/return_sgt_u8.c`, proving the
  bounded zero-parameter `unsigned char` wrapper around a signed-`char`
  greater-than compare is available to the backend-route harness.
- Registered
  `backend_codegen_route_riscv64_return_sgt_u8_defaults_to_bir` in
  `tests/c/internal/InternalTests.cmake`, asserting the emitted text contains
  `bir.func @choose_sgt_u() -> i8 {`, `bir.sgt i8 7, 3`, `bir.ret i8 %t5`, and
  forbids legacy LLVM IR `define i8 @choose_sgt_u()`.
- Added `make_bir_i8_return_sgt_module()` to
  `tests/backend/backend_bir_test_support.*` plus
  `test_bir_lowering_accepts_i8_return_sgt()` in
  `tests/backend/backend_bir_lowering_tests.cpp` to keep the widened signed
  compare-return `sgt` shape covered below the backend-route harness.
- Confirmed the new coverage passed without any
  `src/backend/lowering/lir_to_bir.cpp` change; the existing widened-`i8`
  compare-return lowering already handled the signed-greater-than predicate
  correctly.
- Rebuilt the tree, reran `backend_bir_tests`, reran
  `backend_codegen_route_riscv64_return_sgt_u8_defaults_to_bir`, reran
  `ctest --test-dir build -L backend --output-on-failure -j8` with
  `370/370` backend-labeled tests passing, then refreshed
  `test_fail_after.log` with a full `ctest --test-dir build -j8 --output-on-failure`
  run and passed the regression guard against `test_fail_before.log` with
  `--allow-non-decreasing-passed --timeout-threshold 30 --enforce-timeout`
  (`2800 -> 2808` passed, `0 -> 0` failed, no newly failing tests, no new
  `>30s` cases).

Completed this iteration:
- Re-audited the zero-parameter widened compare-return inventory in
  `tests/c/internal/InternalTests.cmake` and confirmed the next adjacent signed
  relational parity gap after the landed `return_slt_u8` slice was
  `return_sle_u8`.
- Added `tests/c/internal/backend_route_case/return_sle_u8.c`, proving the
  bounded zero-parameter `unsigned char` wrapper around a signed-`char`
  less-than-or-equal compare is available to the backend-route harness.
- Registered
  `backend_codegen_route_riscv64_return_sle_u8_defaults_to_bir` in
  `tests/c/internal/InternalTests.cmake`, asserting the emitted text contains
  `bir.func @choose_sle_u() -> i8 {`, `bir.sle i8 7, 7`, `bir.ret i8 %t5`, and
  forbids legacy LLVM IR `define i8 @choose_sle_u()`.
- Added `make_bir_i8_return_sle_module()` to
  `tests/backend/backend_bir_test_support.*` plus
  `test_bir_lowering_accepts_i8_return_sle()` in
  `tests/backend/backend_bir_lowering_tests.cpp` to keep the widened signed
  compare-return `sle` shape covered below the backend-route harness.
- Confirmed the new coverage passed without any
  `src/backend/lowering/lir_to_bir.cpp` change; the existing widened-`i8`
  compare-return lowering already handled the signed-less-than-or-equal
  predicate correctly.
- Rebuilt the tree, reran `backend_bir_tests`, reran
  `backend_codegen_route_riscv64_return_slt_u8_defaults_to_bir`, reran
  `backend_codegen_route_riscv64_return_sle_u8_defaults_to_bir`, reran
  `ctest --test-dir build -L backend --output-on-failure -j8` with
  `369/369` backend-labeled tests passing, then refreshed
  `test_fail_after.log` with a full `ctest --test-dir build -j8 --output-on-failure`
  run and passed the regression guard against `test_fail_before.log` with
  `--allow-non-decreasing-passed --timeout-threshold 30 --enforce-timeout`
  (`2800 -> 2807` passed, `0 -> 0` failed, no newly failing tests, no new
  `>30s` cases).
...
- Re-audited the zero-parameter widened compare-return inventory in
  `tests/c/internal/InternalTests.cmake` and confirmed the next adjacent
  unsigned-relational parity gap after the landed `return_ugt_u8` slice was
  `return_uge_u8`.
- Added `tests/c/internal/backend_route_case/return_uge_u8.c`, proving the
  bounded zero-parameter `unsigned char` unsigned-greater-than-or-equal wrapper
  is available to the backend-route harness.
- Registered
  `backend_codegen_route_riscv64_return_uge_u8_defaults_to_bir` in
  `tests/c/internal/InternalTests.cmake`, asserting the emitted text contains
  `bir.func @choose_uge_u() -> i8 {`, `%t1 = bir.uge i8 7, 7`, `bir.ret i8 %t1`,
  and forbids legacy LLVM IR `define i8 @choose_uge_u()`.
- Added `make_bir_i8_return_uge_module()` to
  `tests/backend/backend_bir_test_support.*` plus
  `test_bir_lowering_accepts_i8_return_uge()` in
...
- Confirmed the new coverage passed without any `src/backend/lowering/lir_to_bir.cpp`
  ...
  unsigned-greater-than-or-equal predicate correctly.
- Rebuilt the tree, reran `backend_bir_tests`, reran
  `backend_codegen_route_riscv64_return_uge_u8_defaults_to_bir`, reran the
  adjacent widened compare-return sextet
  ...
  `backend_codegen_route_riscv64_return_uge_u8_defaults_to_bir`), then reran
  `ctest --test-dir build -L backend --output-on-failure -j8` with
  `367/367` backend-labeled tests passing.
- Refreshed `test_fail_after.log` with a full
  `ctest --test-dir build -j8 --output-on-failure` run and passed the
  regression guard against `test_fail_before.log` with
  `--allow-non-decreasing-passed --timeout-threshold 30 --enforce-timeout`
  (`2800 -> 2805` passed, `0 -> 0` failed, no newly failing tests, no new
  `>30s` cases).

Completed this iteration:
- Re-audited the zero-parameter widened compare-return inventory in
  `tests/c/internal/InternalTests.cmake` and confirmed the next adjacent
  unsigned-relational parity gap after the landed `return_ule_u8` slice was
  `return_ugt_u8`.
- Added `tests/c/internal/backend_route_case/return_ugt_u8.c`, proving the
  bounded zero-parameter `unsigned char` unsigned-greater-than wrapper is
  available to the backend-route harness.
- Registered
  `backend_codegen_route_riscv64_return_ugt_u8_defaults_to_bir` in
  `tests/c/internal/InternalTests.cmake`, asserting the emitted text contains
  `bir.func @choose_ugt_u() -> i8 {`, `%t1 = bir.ugt i8 7, 3`, `bir.ret i8 %t1`,
  and forbids legacy LLVM IR `define i8 @choose_ugt_u()`.
- Added `make_bir_i8_return_ugt_module()` to
  `tests/backend/backend_bir_test_support.*` plus
  `test_bir_lowering_accepts_i8_return_ugt()` in
  `tests/backend/backend_bir_lowering_tests.cpp` to keep the widened `i8`
  unsigned-relational compare-return shape covered below the backend-route
  harness.
- Confirmed the new coverage passed without any `src/backend/lowering/lir_to_bir.cpp`
  change; the existing widened-`i8` compare-return lowering already handled the
  unsigned-greater-than predicate correctly.
- Rebuilt the tree, reran `backend_bir_tests`, reran
  `backend_codegen_route_riscv64_return_ugt_u8_defaults_to_bir`, reran the
  adjacent widened compare-return quintet
  (`backend_codegen_route_riscv64_return_eq_u8_defaults_to_bir`,
  `backend_codegen_route_riscv64_return_ne_u8_defaults_to_bir`,
  `backend_codegen_route_riscv64_return_ult_u8_defaults_to_bir`,
  `backend_codegen_route_riscv64_return_ule_u8_defaults_to_bir`,
  `backend_codegen_route_riscv64_return_ugt_u8_defaults_to_bir`), then reran
  `ctest --test-dir build -L backend --output-on-failure -j8` with
  `366/366` backend-labeled tests passing.
- Refreshed `test_fail_after.log` with a full
  `ctest --test-dir build -j8 --output-on-failure` run and passed the
  regression guard against `test_fail_before.log` with
  `--allow-non-decreasing-passed --timeout-threshold 30 --enforce-timeout`
  (`2800 -> 2804` passed, `0 -> 0` failed, no newly failing tests, no new
  `>30s` cases).
