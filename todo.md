...
- current capability family:
  ...
  nineteen-arg integer-class indirect-call families are now green as well, so
  the route now covers the first eleven honest stack-passed callee-arg
  surfaces without reopening wider stack-call ABI work
- current packet shape:
  keep backlog item 5 moving outward from the repaired entry boundary:
  record the completion of the first honest eleven-stack-slot indirect-call
  family on the riscv64 route, then leave any follow-up widening beyond that
  first nineteen-arg integer-class surface to supervisor packet selection
...
- candidate proving surface:
  ...
  riscv64 proof surface is the paired param/local twenty-arg integer-class
  indirect-call signature, `i32 x20 -> i32`, where the final twelve callee
  args are stack-passed on the current route surface
  keep `branch_if_eq.c`, `call_helper.c`, `local_arg_call.c`, and the current
  one-arg through nineteen-arg indirect-call plus `two_arg_*` direct-call
