...
- completed:
  one more materially distinct deeper-both split-predecessor CFG merge route
  case is now explicitly proved on the riscv64 semantic-BIR route instead of
  remaining only under generic asm-unsupported coverage:
  `two_param_select_eq_split_predecessor_deeper_affine_post_add.c` now has a
  dedicated defaults-to-BIR backend-route assertion that checks the deeper
  join-shape snippets where the then-side affine chain stores `%t10` into
  `%t14.phi`, the else-side affine chain stores `%t13` into that same merge
  slot, and the shared post-merge add reloads through
  `%t14 = bir.load_local i32 %t14.phi` before return
  the matching generic riscv64 `asm_unsupported` registration for that same
  stem was removed so the route surface now records this deeper-both
  split-predecessor merge shape as explicit BIR coverage rather than
  fallback-only inventory
  what remains next:
  keep backlog item 1 on the next materially distinct richer merge shape from
  the remaining split-predecessor families, such as one of the still-generic
  post-chain-tail variants, without widening into testcase-shaped route logic
  or drifting back into call-lane work
  ...
  test `#387`, and the standing `^backend_` subset remained monotonic while
  ...
  but the backend subset improved monotonically from `197` passed / `219`
  failed in `test_before.log` to `198` passed / `218` failed in
  `test_after.log` out of `416` with no new failures, so this deeper-both
  split-predecessor merge case is now explicitly covered on the semantic-BIR
  route without adding regression
- completed:
  one more materially distinct mixed-post split-predecessor CFG merge route
  case is now explicitly proved on the riscv64 semantic-BIR route instead of
  remaining only under generic asm-unsupported coverage:
  `two_param_select_eq_split_predecessor_mixed_affine_post_add.c` now has a
  dedicated defaults-to-BIR backend-route assertion that checks the mixed
  join-shape snippets where the then-side affine chain stores `%t9` into
  `%t12.phi`, the else-side affine chain stores `%t11` into that same merge
  slot, and the shared post-merge add reloads through
  `%t12 = bir.load_local i32 %t12.phi` before return
  the matching generic riscv64 `asm_unsupported` registration for that same
  stem was removed so the route surface now records this mixed-post
  split-predecessor merge shape as explicit BIR coverage rather than
  fallback-only inventory
  what remains next:
  keep backlog item 1 on the next materially distinct richer merge shape from
  the remaining split-predecessor families, such as one of the still-generic
  deeper post-chain-tail variants, without widening into testcase-shaped
  ...
  proof status:
  the delegated build succeeded, the new riscv64 route test passed as
  test `#383`, and the standing `^backend_` subset remained monotonic while
  replacing the old failing generic `asm_unsupported` registration:
  the subset still exited nonzero because of pre-existing backend failures,
  but the backend subset improved monotonically from `196` passed / `220`
  failed in `test_before.log` to `197` passed / `219` failed in
  `test_after.log` out of `416` with no new failures, so this mixed-post
  split-predecessor merge case is now explicitly covered on the semantic-BIR
  route without adding regression
- completed:
  one more materially distinct symmetric split-predecessor CFG merge route
  case is now explicitly proved on the riscv64 semantic-BIR route instead of
  remaining only under generic asm-unsupported coverage:
  `two_param_select_eq_split_predecessor_mixed_affine_post_add_sub.c` now has
  a dedicated defaults-to-BIR backend-route assertion that checks the
  symmetric join-shape snippets where the then-side affine chain stores `%t9`
  into `%t12.phi`, the else-side affine chain stores `%t11` into that same
  merge slot, and the shared post-merge add/sub chain reloads through
  `%t12 = bir.load_local i32 %t12.phi` before return
  the matching generic riscv64 `asm_unsupported` registration for that same
  stem was removed so the route surface now records this mixed-affine
  split-predecessor merge shape as explicit BIR coverage rather than
  fallback-only inventory
  what remains next:
  keep backlog item 1 on the next materially distinct richer merge shape from
  the remaining split-predecessor families, such as one of the still-generic
  mixed-post tail or deeper post-chain variants, without widening into
  testcase-shaped route logic or drifting back into call-lane work
  proof command attempted:
  `cmake --build --preset default > test_after.log 2>&1 && ctest --test-dir build -j --output-on-failure -R '^backend_' >> test_after.log 2>&1`
  proof log:
  `test_after.log`
  ...
  replacing the old failing generic `asm_unsupported` registration:
  the subset still exited nonzero because of pre-existing backend failures,
  but the backend subset improved monotonically from `195` passed / `221`
  failed in `test_before.log` to `196` passed / `220` failed in
  `test_after.log` out of `416` with no new failures, so this symmetric
  split-predecessor merge case is now explicitly covered on the semantic-BIR
  route without adding regression
- completed:
  one more materially distinct deeper-both split-predecessor CFG merge route
  case is now explicitly proved on the riscv64 semantic-BIR route instead of
  remaining only under generic asm-unsupported coverage:
  `two_param_select_eq_split_predecessor_deeper_affine_post_add_sub.c` now
  has a dedicated defaults-to-BIR backend-route assertion that checks the
  deeper-both join-shape snippets where the then-side affine chain stores
  `%t10` into `%t14.phi`, the else-side affine chain stores `%t13` into that
  same merge slot, and the shared post-merge add/sub chain reloads through
  `%t14 = bir.load_local i32 %t14.phi` before return
  the matching generic riscv64 `asm_unsupported` registration for that same
  stem was removed so the route surface now records this deeper-both
  split-predecessor merge shape as explicit BIR coverage rather than
  ...
  keep backlog item 1 on the next materially distinct richer merge shape from
  the remaining split-predecessor families, such as one of the still-generic
  mixed-post or post-chain-tail variants, without widening into
  testcase-shaped route logic or drifting back into call-lane work
  proof command attempted:
  `cmake --build --preset default > test_after.log 2>&1 && ctest --test-dir build -j --output-on-failure -R '^backend_' >> test_after.log 2>&1`
  proof log:
  ...
  proof status:
  the delegated build succeeded, the new riscv64 route test passed as
  test `#384`, and the standing `^backend_` subset remained monotonic while
  replacing the old failing generic `asm_unsupported` registration:
  the subset still exited nonzero because of pre-existing backend failures,
  but the backend subset improved monotonically from `194` passed / `222`
  failed in `test_before.log` to `195` passed / `221` failed in
  `test_after.log` out of `416` with no new failures, so this deeper-both
  split-predecessor merge case is now explicitly covered on the semantic-BIR
  route without adding regression
- completed:
  one more materially distinct converse-asymmetric split-predecessor CFG merge
  route case is now explicitly proved on the riscv64 semantic-BIR route
  instead of remaining only under generic asm-unsupported coverage:
  `two_param_select_eq_split_predecessor_deeper_then_mixed_affine_post_add.c`
  now has a dedicated defaults-to-BIR backend-route assertion that checks the
  converse join-shape snippets where the deeper then-side affine chain stores
  `%t10` into `%t13.phi`, the shorter else-side affine chain stores `%t12`
  into that same merge slot, and the shared post-merge add reloads through
  `%t13 = bir.load_local i32 %t13.phi` before return
  the matching generic riscv64 `asm_unsupported` registration for that same
  stem was removed so the route surface now records this converse asymmetric
  split-predecessor merge shape as explicit BIR coverage rather than
  fallback-only inventory
  what remains next:
  keep backlog item 1 on the next materially distinct richer merge shape from
  the remaining split-predecessor families, such as one of the deeper-both or
  post-chain variants, without widening into testcase-shaped route logic or
  drifting back into call-lane work
  proof command attempted:
  `cmake --build --preset default > test_after.log 2>&1 && ctest --test-dir build -j --output-on-failure -R '^backend_' >> test_after.log 2>&1`
  proof log:
  `test_after.log`
  ...
  replacing the old failing generic `asm_unsupported` registration:
  the subset still exited nonzero because of pre-existing backend failures,
  but the backend subset improved monotonically from `193` passed / `223`
  failed in `test_before.log` to `194` passed / `222` failed in
  `test_after.log` out of `416` with no new failures, so this converse
  split-predecessor merge case is now explicitly covered on the semantic-BIR
  route without adding regression
- completed:
...
- completed:
  ...
  LLVM:
  branch-family lowering now materializes supported `phi` values through
  internal local slots, storing predecessor incoming values on the incoming
  edges and loading the merged value in the join block, so a callee chosen by
  `a ? f : g`, then conditionally overwritten by `h`, can stay on the shared
  semantic BIR route before the indirect `bir.call`
  new route proof covers
  `indirect_select_local_override_callee_call.c` as BIR, while
  `branch_if_eq.c`, `call_helper.c`, `local_arg_call.c`, the earlier
  merge-preserved callee route proof, the addressed-global and local-aggregate
  callee route proofs, and the standing one-arg through twenty-five-arg
  indirect-call plus `two_arg_*` direct-call sentinels stayed in the owned
  proof surface
  what remains next:
  keep backlog item 5 on materially new merge-preserved callee semantics,
  such as richer CFG join shapes or additional honest semantic composition
  beyond this first local-override handoff; do not reopen width-only proof
  churn or another adjacent storage-only packet
  proof command attempted:
  `cmake --build --preset default > test_after.log 2>&1 && ctest --test-dir build -j --output-on-failure -R '^backend_' >> test_after.log 2>&1`
  proof log:
  `test_after.log`
  proof status:
  the delegated build succeeded, the new riscv64 route test passed as
  test `#264`, and the standing `^backend_` subset remained monotonic with
  the backend route surface increasing from `415` to `416` while the
  standing failed-test count held at `225`, so this nested
  merge-preserved local-override callee family is now covered on the shared
...
- completed:
  ...
  global aggregate pointer-array access now preserves the constant struct-field
  offset and the dynamic element index together, so a global struct whose
  function-pointer array is initialized from function symbols can lower
  `holder.slots[which](x)` as semantic BIR on the riscv64 backend-route
  surface with `bir.select` over the known function symbols before `bir.call`
  new route proof covers `indirect_global_struct_array_callee_call.c` as BIR,
  ...
  test `#270`, the broad `^backend_` subset still returned non-zero because
  it remains dominated by standing unrelated failures, and the total backend
  route surface increased from `410` to `411` while the standing failed-test
  count held at `225`, so the first addressed global aggregate-owned
  function-pointer array callee family is now covered on the shared
  semantic-BIR lane without a broader regression increase
- completed:
...
- completed:
  ...
  through `alloca` plus struct-field `gep` addressing, so a parameter-carried
  unary callee stored into a local aggregate field and called back through
  that field lowers as semantic BIR on the riscv64 backend-route surface
  without reopening arg-width churn
  new route proof covers `indirect_local_struct_callee_call.c` as BIR, while
  `branch_if_eq.c`, `call_helper.c`, `local_arg_call.c`, the merge-preserved
  callee route proof, the addressed-global callee route proofs, and the
  ...
  proof status:
  the delegated build succeeded, the new riscv64 route test passed as
  test `#266`, the broad `^backend_` subset still returned non-zero because
  it remains dominated by standing unrelated failures, and the total backend
  route surface increased from `406` to `407` while the standing failed-test
  count held at `225`, so the first local stack-backed aggregate
  callee-provenance slice is now covered on the shared semantic-BIR lane
  without a broader regression increase
- completed:
