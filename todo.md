...
`branch_join_adjust_then_ashr` handoff-boundary proof so the trailing-ashr
...
to the next bounded immediate-op family, with `branch_join_adjust_then_and` as
...
- Do not silently fold branch/join ownership work from idea 62 back into this
  route.
- This packet only tightened trailing-ashr compare-join proof coverage;
...
`branch_join_adjust_then_ashr` trailing-ashr compare-join family now proving
