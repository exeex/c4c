...
trailing-join `or` consumer family now proves authoritative prepared
...
time, starting with `mul` `EdgeStoreSlot` ownership and passthrough proof
...
- Keep Step 3 packets focused on consumer migration proof, not on reopening
  ...
  surfaces.
- The trailing-join `or` family now has direct and `EdgeStoreSlot` ownership
...
`or` coverage so the shared prepared joined-branch handoff stays
