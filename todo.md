...
time, moving from the now-covered trailing-join `and` packet into the next
residual shared-helper or uncovered trailing family only if it still lacks
explicit prepared-contract proof.
...
- Keep Step 3 packets focused on consumer migration proof, not on reopening
  ...
  surfaces.
- The trailing-join `and`, `or`, `mul`, `shl`, `lshr`, and `ashr` families now
  have direct and `EdgeStoreSlot` ownership plus passthrough-drift proof, so
  the next packet should target only a truly uncovered residual shared-helper
  surface instead of reopening covered arithmetic or shift families.
