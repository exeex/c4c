...
trailing-join `shl` consumer family now proves authoritative prepared
...
time, starting with `lshr` `EdgeStoreSlot` ownership and passthrough proof
...
- Keep Step 3 packets focused on consumer migration proof, not on reopening
  ...
  surfaces.
- The trailing-join `shl` family now has direct and `EdgeStoreSlot` ownership
  plus passthrough-drift proof, so the next packet should advance to the next
  adjacent trailing opcode instead of reopening covered `or` or `mul` cases.
...
`shl` coverage so the shared prepared joined-branch handoff stays
