...
slot map and indirect local pointer slot set.

- Removed the `BirFunctionLowerer` compatibility aliases for
  `LocalPointerSlots` and `LocalIndirectPointerSlotSet`.
- Updated the affected memory adapter signatures and lowerer member
  declarations to use the memory-owned `c4c::backend::LocalPointerSlots` and
  `c4c::backend::LocalIndirectPointerSlotSet` declarations directly from
...
- `LocalPointerSlots` and `LocalIndirectPointerSlotSet` are now referenced
