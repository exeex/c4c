...
Completed Step 1 mapping for the RV64 call-family cleanup. The first safe extraction target is the narrow prepared-call argument source predicate group in `object_emission.cpp`: `prepared_frame_slot_call_argument_offset`, `stack_slot_offset_for_prior_preserved_gpr_selection`, and the small GPR prior-preservation register helper path that feeds them. These helpers validate already-prepared call argument source facts and frame-slot offsets; they do not own symbol fixups, final call emission, variadic admission, saved-register frame construction, or return movement.

## Suggested Next

Execute Step 2 by moving only the narrow call argument source predicate group into `prepared_call_emit.*`, exposing dependencies explicitly through parameters and reusing existing `prepared_frame_emit.*` utilities where possible. Keep `fragment_for_prepared_call` object-side and call back into the new prepared-call predicates from the existing object-side traversal.

## Watchouts

- `prepared_call_emit.*` already owns simple call emission plus prepared frame-slot address, prior-preserved GPR, callee-saved boundary-effect, byval stack-copy, and simple result movement helpers. The next call extraction should add validation/admission helpers only, not a broad call-fragment owner.
...
No build or test proof was run; this was the delegated mapping-only Step 1 packet. Use this exact command for Step 2 code-changing proof:
