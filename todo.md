...
rooted-only `bir::SelectInst` true/false value bookkeeping in
`alloca_coalescing.cpp`. The active `backend_prepare_stack_layout` test now
proves that pure rooted pointer `select` value merges conservatively mark the
root local slot `address_exposed == true`, `requires_home_slot == true`, and
keep dedicated frame-slot storage, removing the remaining active C++ `select`
escape-model divergence from the retained Rust reference.

## Suggested Next

Continue Step 2 by comparing rooted-only `bir::PhiInst` operand bookkeeping in
`alloca_coalescing.cpp` against `alloca_coalescing.rs`, then decide whether
the active C++ rooted-phi no-home-slot path should remain a documented bounded
divergence or be tightened so pure rooted phi merges also conservatively force
exposure and home-slot storage.
...
- rooted-only `phi` bookkeeping is still looser than the retained Rust model,
  which marks `Select` and `Phi` operands as escape sites generally, so any
  future loosening needs an explicit reference comparison rather than a
  test-only expectation flip
...
after tightening rooted-only `select` true/false value escape handling and
updating the focused rooted-only select activation coverage. Canonical proof
log: `test_after.log`.
