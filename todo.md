...
prepared compare-join ownership coverage from direct pointer-backed
same-module global selected-value roots to one additional bounded family:
pointer-backed same-module global selected-value chains. The existing shared
prepared computed-value contract and Step 3 x86 compare-join consumer already
carried this bounded chain route correctly; the new focused ownership fixtures
now prove both the plain joined-branch path and the
`PreparedJoinTransferKind::EdgeStoreSlot` carrier preserve pointer-root global
provenance, selected-value chain operations, and the authoritative compare-join
return contexts through the shared helper surface.
...
fixed-offset pointer-backed variant that still fits shared prepared data
without opening idea 61 address/frame work. Keep the work in shared consumer
helpers and focused ownership tests, not Step 4 file organization.
...
- The global-root lane still intentionally keeps same-module scalar-load
  ...
  bounded pointer-backed same-module global-root family for both direct loads
  and selected-value chains, but it still does not open fixed-offset
  pointer-backed loads, local-slot/address-based roots, or generic non-global
...
- `test_before.log` remains the narrow baseline for
  ...
  global selected-value chain ownership checks.
...
selected-value chains, the matching compare-join return contexts, the
...
direct pointer-backed compare-join ownership seams.
