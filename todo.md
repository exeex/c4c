...
Step 3 now proves the compare-join handoff boundary in addition to the loop-countdown case: x86
compare-branch coverage strips `parallel_copy_bundles` while preserving published
`join_transfers` and confirms the prepared-module consumer fails closed instead of accepting
phi-edge obligations through a join-only fallback.
...
back for plan review if compare-join was the last unresolved Step 3 fallback probe.
...
- Compare-join and loop-countdown now both prove `join_transfers` alone are not enough to keep
  phi-edge obligations alive after `parallel_copy_bundles` are stripped; treat any future success
  in either shape as a contract regression.
