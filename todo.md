...
prepared compare-join handoff coverage from fixed-offset same-module
`LoadGlobal i32` selected-value roots to the next bounded non-param family:
fixed-offset same-module global-root selected-value chains. The compare-join
ownership fixtures now prove both the plain joined-branch path and the
`PreparedJoinTransferKind::EdgeStoreSlot` carrier still classify offset-4
aggregate `GlobalI32Load` roots plus their immediate add/sub chains through
the shared prepared helpers, and the focused checks confirm the x86 consumer
continues to treat those fixed-offset global-root chains and the trailing xor
...
non-param selected-value root family, with preference now on a pointer-backed
same-module global-root family that still fits shared prepared data without
opening idea 61 address/frame work. Keep the work in shared consumer helpers
and focused ownership tests, not Step 4 file organization.
...
- The global-root lane still intentionally keeps same-module scalar-load
  ...
  aggregate loads plus bounded immediate-op chains, but it still does not open
  local-slot/address-based roots; do not widen this packet into idea 61
  frame/addressing work.
...
- `test_before.log` remains the narrow baseline for
  ...
  global-root selected-value-chain ownership checks.
