...
`check_join_route_consumes_prepared_control_flow` to the trailing join-`shl`
family, so the compare-join route is now proven to keep following prepared
true/false transfer ownership after one trailing left-shift use of the joined
value instead of stopping at the base, trailing-`add`, trailing-`xor`,
trailing-`and`, trailing-`or`, and trailing-`mul` cases.

## Suggested Next

The next small Step 3 packet is broadening the same compare-join ownership
proof to one more trailing-join shift family such as `lshr`, so prepared
...
trailing-`mul`, and trailing-`shl` follow-on cases.
...
- The trailing-`shl` ownership proof reuses the same helper and mutation path
  as the base, trailing-`add`, trailing-`xor`, trailing-`and`, and
  trailing-`or`, and trailing-`mul` cases; keep future families on that
