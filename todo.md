...
Step 2 migrated
`test_parser_alias_template_substitution_prefers_param_text_id()` away from its
direct `alias_placeholder.tag` and `info.aliased_type.tag` writes. The fixture
now uses `tag_text_id` for the alias placeholder and substitutable parameter
identity while retaining optional legacy spelling through
...
The migrated fixture no longer blocks that probe at the old 6761/6778 sites.
The first remaining fixture/test compile boundary moved to
`tests/frontend/frontend_parser_tests.cpp:6802`.

## Suggested Next

Migrate the next direct `TypeSpec::tag` fixture access pair in
`test_parser_alias_template_substitution_does_not_require_param_name_spelling()`
at `tests/frontend/frontend_parser_tests.cpp:6802` and `6816`, preserving the
structured-parameter behavior while making it compile under the temporary field
deletion.
