...
adjacent fixed-offset pointer-backed same-module global selected-value route
without the selected-value chain, including the
...
non-compare-entry-carrier handoff proof for the adjacent pointer-backed
same-module global selected-value route without the fixed-offset variant,
...
- The new proof now locks in non-compare entry-carrier drift for both
  fixed-offset pointer-backed global selected-value routes, with and without
  the selected-value chain, including the `EdgeStoreSlot` carrier shape; the
  adjacent pointer-backed non-offset selected-value route remains a separate
  follow-up packet.
...
fixed-offset pointer-backed selected-value route coverage for non-compare
