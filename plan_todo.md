...
- [x] Added
  ...
  as a parse-only regression for qualified `_v` variable-template expressions
...
- [x] Re-ran the focused `_v` / trait-value parser guardrails plus 12s Step 3
  ...
  generic parser improvement instead of remaining buried inside the EASTL
  stack
- [x] Re-ran the full suite and monotonic regression guard after the `_v`
  shortcut slice; the tree improved monotonically with `2697 -> 2698` passes
  and the same three pre-existing failures
  (`positive_sema_linux_stage2_repro_03_asm_volatile_c`,
  `backend_lir_adapter_aarch64_tests`, and
  `llvm_gcc_c_torture_src_20080502_1_c`)
- [x] Added
...
- the new trait-value shortcut moves the current timeout frontier past the old
  ...
  aliases
- the new `_v` shortcut is now covered by
  `qualified_variable_template_requires_parse.cpp`, but the 12s Step 3 probes
  still time out after reaching later libstdc++ trait code around
  `type_traits:1571`, `type_traits:2635`, and `type_traits:3207`
