...
- [ ] Step 4: Shift target codegen onto backend semantics
  - Notes: Step 4 x86 cutovers continue to replace emitter-local compatibility reshaping with direct lowering-owned parsed-module views. The zero-argument direct-call asm path consumes `ParsedBackendMinimalDirectCallModuleView` directly, `direct_call_add_imm` now emits from `ParsedBackendMinimalDirectCallAddImmModuleView`, and the latest x86 slice removes the remaining backend-IR `two_arg_direct_call` reshaping layer so the x86 asm fast path emits directly from `ParsedBackendMinimalTwoArgDirectCallModuleView`. The newer AArch64 slices now mirror that pattern for the backend-owned structured direct-call family: `src/backend/aarch64/codegen/emit.cpp` first dropped the target-local `direct_call_add_imm` wrapper, then removed the backend-IR `two_arg_direct_call` reshaping layer, and this iteration removes the folded two-argument compatibility wrapper so the asm fast path now dispatches and emits directly from `ParsedBackendMinimalFoldedTwoArgDirectCallModuleView`. The renamed structured AArch64 folded regression now also mutates helper parameter names, helper SSA names, main call SSA names, and both direct-call immediates while still holding the asm path.
...
Current Step: Step 4
Next Step: Inspect the remaining AArch64 structured local/rewrite two-argument direct-call seams for any leftover emitter-local normalization that should move behind shared lowering-owned parsed-module metadata.
Blockers: None

Latest Iteration Note: The AArch64 folded `two_arg_direct_call` Step 4 slice is now landed: the emitter no longer collapses that path through `MinimalDirectCallSlice` and instead emits directly from `ParsedBackendMinimalFoldedTwoArgDirectCallModuleView`, while the renamed structured folded regression now also mutates helper parameter names, helper/main SSA names, and both lowered call immediates without falling off the asm path.

Iteration Update: Tightened `test_aarch64_backend_scaffold_accepts_renamed_structured_two_arg_direct_call_folded_const_ir_without_signature_shims()` so the renamed lowered folded helper body must still render as the direct asm fast path after helper-parameter renames, helper/main SSA renames, and direct-call immediate changes with legacy signature/call-type shims cleared, then removed the AArch64 folded lowered-IR compatibility wrapper by emitting from `c4c::backend::ParsedBackendMinimalFoldedTwoArgDirectCallModuleView` directly in `src/backend/aarch64/codegen/emit.cpp`. The folded asm regression now also checks that the fast path still materializes the two direct-call immediates in `w0`/`w1` before `bl`.
...
Next Slice Hint: Inspect the remaining AArch64 local/rewrite two-argument direct-call seams for the same backend-owned parsed-module cutover pattern now used by the baseline `two_arg_direct_call`, folded `two_arg_direct_call`, and `direct_call_add_imm` asm paths.

Iteration Target: Completed Step 4 AArch64 folded `two_arg_direct_call` cutover. Next iteration should target the next AArch64 structured local/rewrite direct-call seam that still needs the same direct parsed-module pattern.
