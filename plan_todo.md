...
- Exact target for the next iteration: extend `src/backend/ir.*` past the
  newly covered mixed predecessor five-op-chain/five-op-chain
  conditional-join slice into the next bounded asymmetric join seam by
  widening one predecessor-local arithmetic side beyond the current five-op
  `add`/`sub`/`add`/`sub`/`add` depth while preserving the same four-block
  compare/branch/join/return shape and explicit backend-IR emitter path.
- Resume notes:
  - backend-owned IR coverage now proves the bounded
    compare/branch/join/return slice remains valid when one predecessor
    computes the merged `phi` input through a five-op typed `i32 add` then
    `sub` then `add` then `sub` then `add` chain, the opposite predecessor
    now also contributes its own five-op typed `i32 add` then `sub` then
    `add` then `sub` then `add` chain, and the join block itself applies one
    bounded typed `i32 add`
  - both x86 and AArch64 now have focused explicit-backend-IR coverage for
    that asymmetric five-op-chain/five-op-chain plus join-local-add shape
    without falling back to backend-IR text or legacy LIR
  - the next highest-value asymmetric join seam is widening one
    predecessor-local arithmetic side beyond the current five-op
    `add`/`sub`/`add`/`sub`/`add` chain while preserving the same bounded
    join-local scalar use
  - backend-owned IR coverage now proves the bounded
    compare/branch/join/return slice remains valid when one predecessor
    computes the merged `phi` input through a five-op typed `i32 add` then
    `sub` then `add` then `sub` then `add` chain, the opposite predecessor
    contributes its own four-op typed `i32 add` then `sub` then `add` then
    `sub` chain, and the join block itself applies one bounded typed `i32
    add`
  - both x86 and AArch64 now have focused explicit-backend-IR coverage for
    that asymmetric five-op-chain/four-op-chain plus join-local-add shape
    without falling back to backend-IR text or legacy LIR
  - the next highest-value asymmetric join seam is widening the alternate
    predecessor-local arithmetic side beyond the current four-op
    `add`/`sub`/`add`/`sub` chain while preserving the same bounded
    join-local scalar use
  - backend-owned IR coverage now proves the bounded
    compare/branch/join/return slice remains valid when one predecessor
    computes the merged `phi` input through a four-op typed `i32 add` then
    `sub` then `add` then `sub` chain, the opposite predecessor now also
    contributes its own four-op typed `i32 add` then `sub` then `add` then
    `sub` chain, and the join block itself applies one bounded typed `i32
    add`
  - both x86 and AArch64 now have focused explicit-backend-IR coverage for
    that asymmetric four-op-chain/four-op-chain plus join-local-add shape
    without falling back to backend-IR text or legacy LIR
  - the next highest-value asymmetric join seam is widening one
    predecessor-local arithmetic side beyond the current four-op
    `add`/`sub`/`add`/`sub` depth while preserving the same bounded
    join-local scalar use
  - backend-owned IR coverage now proves the bounded
    compare/branch/join/return slice remains valid when one predecessor
    computes the merged `phi` input through a four-op typed `i32 add` then
    `sub` then `add` then `sub` chain, the opposite predecessor contributes
    its own three-op typed `i32 add` then `sub` then `add` chain, and the
    join block itself applies one bounded typed `i32 add`
  - both x86 and AArch64 now have focused explicit-backend-IR coverage for
    that asymmetric four-op-chain/deeper-chain plus join-local-add shape
    without falling back to backend-IR text or legacy LIR
  - the next highest-value asymmetric join seam is widening the alternate
    predecessor-local arithmetic side beyond the current three-op
    `add`/`sub`/`add` chain while preserving the same bounded join-local
    scalar use
  - backend-owned IR coverage now proves the bounded
    compare/branch/join/return slice remains valid when both predecessors
    compute their merged `phi` inputs through three-op typed `i32 add` then
    `sub` then `add` chains, and the join block itself applies one bounded
    typed `i32 add`
  - both x86 and AArch64 now have focused explicit-backend-IR coverage for
    that asymmetric deeper-chain/deeper-chain plus join-local-add shape
    without falling back to backend-IR text or legacy LIR
  - the next highest-value asymmetric join seam is widening one or both
    predecessor-local arithmetic sides beyond the current three-op
    `add`/`sub`/`add` depth while preserving the same bounded join-local
    scalar use
  - backend-owned IR coverage now proves the bounded
    compare/branch/join/return slice remains valid when one predecessor
  ...
  - backend-owned IR coverage now proves the bounded
    ...
    chain, the opposite predecessor also contributes its own two-op typed
    `i32 add` then `sub` chain, and the join block itself applies one bounded
    typed `i32 add`
  - both x86 and AArch64 now have focused explicit-backend-IR coverage for
    that asymmetric predecessor chain/chain plus join-local-add shape without
    falling back to backend-IR text or legacy LIR
  - the next highest-value asymmetric join seam is widening one
    predecessor-local arithmetic side beyond the current two-op `add`/`sub`
    chain while preserving the same bounded join-local scalar use
...
- Completed an additional Step 2 mixed predecessor five-op-chain/five-op-chain
  join slice:
  - added focused backend coverage for the bounded
    compare/branch/join/return shape where one predecessor computes the
    merged `phi` input through a five-op typed `i32 add` then `sub` then
    `add` then `sub` then `add` chain, the alternate predecessor now also
    contributes its own five-op typed `i32 add` then `sub` then `add` then
    `sub` then `add` chain, and the join block itself applies a bounded
    typed `i32 add`
  - verified the existing lowering and explicit-backend-IR emitter paths
    already preserve that asymmetric five-op-chain/five-op-chain join
    contract in printer, validator, x86, and AArch64 coverage without
    widening implementation scope past the planned bounded slice
- Verified the current post-change full-suite regression status in
  ...
  (2377 passing), matching the checked `test_fail_before.log` baseline with
  no newly failing tests.
- Completed an additional Step 2 mixed predecessor five-op-chain/four-op-chain
  join slice:
  - added focused backend coverage for the bounded
    compare/branch/join/return shape where one predecessor computes the
    merged `phi` input through a five-op typed `i32 add` then `sub` then
    `add` then `sub` then `add` chain, the alternate predecessor contributes
    its own four-op typed `i32 add` then `sub` then `add` then `sub` chain,
    and the join block itself applies a bounded typed `i32 add`
  - verified the existing lowering and explicit-backend-IR emitter paths
    already preserve that asymmetric five-op-chain/four-op-chain join
    contract in printer, validator, x86, and AArch64 coverage without
    widening implementation scope past the planned bounded slice
- Verified the current post-change full-suite regression status in
  ...
  (2377 passing), matching the checked `test_fail_before.log` baseline with
  no newly failing tests.
- Completed an additional Step 2 mixed predecessor four-op-chain/four-op-chain
  join slice:
  - added focused backend coverage for the bounded
    compare/branch/join/return shape where one predecessor computes the
    merged `phi` input through a four-op typed `i32 add` then `sub` then
    `add` then `sub` chain, the alternate predecessor now also contributes
    its own four-op typed `i32 add` then `sub` then `add` then `sub` chain,
    and the join block itself applies a bounded typed `i32 add`
  - verified the existing lowering and explicit-backend-IR emitter paths
    already preserve that asymmetric four-op-chain/four-op-chain join
    contract in printer, validator, x86, and AArch64 coverage without
    widening implementation scope past the planned bounded slice
- Verified the current post-change full-suite regression status in
  `test_fail_after.log`: `93% tests passed`, `183 tests failed out of 2560`
  (2377 passing), matching the checked `test_fail_before.log` baseline with
  no newly failing tests.
- Completed an additional Step 2 mixed predecessor four-op-chain/deeper-chain
  join slice:
  - added focused backend coverage for the bounded
    compare/branch/join/return shape where one predecessor computes the
    merged `phi` input through a four-op typed `i32 add` then `sub` then
    `add` then `sub` chain, the alternate predecessor contributes its own
    three-op typed `i32 add` then `sub` then `add` chain, and the join block
    itself applies a bounded typed `i32 add`
  - verified the existing lowering and explicit-backend-IR emitter paths
    already preserve that asymmetric four-op-chain/deeper-chain join contract
    in printer, validator, x86, and AArch64 coverage instead of capping the
    primary predecessor edge at the current three-op chain
- Verified the current post-change full-suite regression status in
  `test_after.log`: `93% tests passed`, `183 tests failed out of 2560`
  (2377 passing), matching the checked `test_before.log` baseline with no
  newly failing tests.
- Completed an additional Step 2 mixed predecessor deeper-chain/deeper-chain join slice:
  - added focused backend coverage for the bounded
    compare/branch/join/return shape where both predecessors compute the
    merged `phi` inputs through three-op typed `i32 add` then `sub` then
    `add` chains, and the join block itself applies a bounded typed `i32 add`
  - verified the existing lowering and explicit-backend-IR emitter paths
    already preserve that asymmetric deeper-chain/deeper-chain join contract
    in printer, validator, x86, and AArch64 coverage instead of capping the
    alternate predecessor edge at the current two-op chain
- Verified the current post-change full-suite regression status in
  `test_fail_after.log`: `93% tests passed`, `183 tests failed out of 2560`
  (2377 passing), matching the checked `test_fail_before.log` baseline with
  no newly failing tests.
...
- Completed an additional Step 2 mixed predecessor chain/chain join slice:
  - added focused backend coverage for the bounded
    ...
    predecessor contributes its own two-op typed `i32 add` then `sub` chain,
    and the join block itself applies a bounded typed `i32 add`
  - verified the existing lowering and explicit-backend-IR emitter paths
    already preserve that asymmetric chain/chain join contract in printer,
    validator, x86, and AArch64 coverage instead of only covering a single-op
    computed alternate edge
- Verified the current post-change full-suite regression status in
  `test_fail_after.log`: `93% tests passed`, `183 tests failed out of 2560`
  (2377 passing), matching the checked `test_fail_before.log` baseline with no
  newly failing tests.
