...
Current Step: Step 3
Next Step: Re-scan the remaining backend-module call-specialized asm slices for residual direct-call seams beyond fixed param and return-type validation, especially vararg/external declaration cases or matcher-local assumptions that still rely on synthesized compatibility data.
Blockers: None

Latest Iteration Note: Shared backend IR validation now rejects direct-global calls whose structured return type or fixed param types disagree with the referenced callee signature, so malformed lowered call contracts are filtered before x86 or AArch64 direct-call asm matching starts.

Iteration Update: Added backend IR validator regressions for structured direct-call callee-signature param mismatches and call-return mismatches, plus x86 and AArch64 emitter regressions that mutate a lowered `add_pair` helper signature after compatibility text is cleared and confirm both targets fall back instead of matching the malformed direct-call asm slice.

Validation: `cmake --build build -j8 --target backend_ir_tests backend_lir_adapter_x86_64_tests backend_lir_adapter_aarch64_tests`, `./build/backend_ir_tests direct_call`, `./build/backend_lir_adapter_x86_64_tests direct_call`, `./build/backend_lir_adapter_aarch64_tests direct_call`, `ctest --test-dir build -R 'backend_ir_tests' --output-on-failure`, `ctest --test-dir build -R 'backend_lir_adapter_x86_64_tests' --output-on-failure`, `ctest --test-dir build -R 'backend_lir_adapter_aarch64_tests' --output-on-failure`, `ctest --test-dir build -j --output-on-failure > test_fail_after.log`, and `python3 .codex/skills/c4c-regression-guard/scripts/check_monotonic_regression.py --before test_fail_before.log --after test_fail_after.log --allow-non-decreasing-passed` completed with the same known steady-state full-suite result of `2667 passed / 2 failed` and no newly failing tests.
