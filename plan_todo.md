...
Current Step: Step 4
Next Step: If regression coverage stays clean, mirror the same `direct_call_add_imm` emitter cutover on AArch64 or continue Step 4 on x86 by replacing the remaining `two_arg_direct_call` compatibility reshaping layer with direct lowering-owned parsed-module metadata.
Blockers: None

Latest Iteration Note: The next Step 4 x86 slice is now landed too: `direct_call_add_imm` no longer rebuilds a target-local compatibility struct and instead emits directly from `ParsedBackendMinimalDirectCallAddImmModuleView`. The tightened regression keeps the renamed lowered helper body on the asm fast path while legacy signature/call-type shims stay cleared.

Iteration Update: Tightened `test_x86_backend_scaffold_accepts_renamed_structured_direct_call_add_imm_ir_without_signature_shims()` so the renamed lowered helper body must still render as the direct asm fast path with legacy signature/call-type shims cleared, then removed the x86-local `direct_call_add_imm` reshaping layer by emitting from `c4c::backend::ParsedBackendMinimalDirectCallAddImmModuleView` directly in `src/backend/x86/codegen/emit.cpp`.
