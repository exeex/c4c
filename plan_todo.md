...
Current Step: Step 3
Next Step: Reassess the remaining target-local structured direct-call matching helpers and decide whether the next Step 3 extraction should pull the shared `match_minimal_structured_direct_call`-style backend-module discovery into lowering, or pivot to a different handwritten-asm matcher that still duplicates backend-owned shape checks.
Blockers: None

Latest Iteration Note: Shared lowering-owned parsing now also covers the structured backend-module call-crossing direct-call seam, and both x86 and AArch64 consume that backend-owned parser before their target-specific regalloc handoff. The remaining direct-call-family duplication is now narrower and concentrated in the target-local helper-discovery/matching utilities that sit below these module-level parsers.

Iteration Update: Added `parse_backend_minimal_call_crossing_direct_call_module()` to `src/backend/lowering/call_decode.hpp`, rewired both `src/backend/x86/codegen/emit.cpp` and `src/backend/aarch64/codegen/emit.cpp` to consume that shared backend-module parser for the structured call-crossing seam, and added shared regression coverage in `tests/backend/backend_lir_adapter_tests.cpp` proving the new seam preserves renamed helper symbols, source/call/final-add SSA values, and the summed source immediate without target-local backend-module scans.
...
Next Slice Hint: Audit the remaining target-local `match_minimal_structured_direct_call*` helper family in `src/backend/x86/codegen/emit.cpp` and `src/backend/aarch64/codegen/emit.cpp` and decide whether the next Step 3 slice should centralize that backend-module discovery under lowering-owned decode, or instead move to a different handwritten-asm matcher that still duplicates structured backend IR shape checks.
