...
Current Step: Step 2
Next Step: Re-audit the remaining AArch64 fallback reparses of `fn.signature_text` and `p->args_str`; if those are the last decode seams, either finish Step 2 with another shared helper slice or promote the generic fallback cleanup into the first Step 3 structured backend-IR slice
Blockers: None
