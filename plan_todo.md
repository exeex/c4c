...
Current Step: Step 2
Next Step: Continue Step 2 by identifying the next AArch64 emitter-local typed decode or signature-text predicate still living in `src/backend/aarch64/codegen/emit.cpp`, then move that seam behind `src/backend/lowering/` without expanding backend IR scope
Blockers: None
