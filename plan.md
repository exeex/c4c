# PreparedFunctionLookups Value Home Projection Runbook

Status: Active
Source Idea: ideas/open/175_prepared_function_lookups_aggregate_privacy.md
Continues: idea 175 after the `address_materializations` aggregate field group was completed in commit `e307da7d1`.

## Purpose

Continue contracting direct `PreparedFunctionLookups` aggregate exposure one
projected field group at a time. The next selected group is `value_homes`.

Goal: replace the AArch64 traversal read of `prepared_lookups.value_homes`
with a narrow `PreparedValueHomeLookups` projection, while leaving the
aggregate available for all unselected fields.

Core Rule: do not hide or replace the aggregate wholesale; this step only owns
the `value_homes` projection and must not become a renamed lowering-plan
aggregate.
...
- Selected field group: `value_homes`
- Owner boundary: target-local `prepare::PreparedValueHomeLookups` built by
  `prepare::make_prepared_value_home_lookups(...)`
- Current direct aggregate read to remove:
  `function_context.value_home_lookups = &prepared_lookups.value_homes` in
  `src/backend/mir/aarch64/codegen/traversal.cpp`
- Expected context projection: `FunctionLoweringContext::value_home_lookups`

## Remaining Aggregate Fields

- `move_bundles`: target-local value-location lookup projection, but it is not
  this packet. Its aggregate-equivalent builder path currently also publishes
  after-call result lane bindings using value-home data.
...
- Do not move memory-access, publication, source-producer, move-bundle, or
  return-chain consumers under this value-home packet.
- Do not change value-home semantics, register allocation, storage plans, move
  records, or final target policy.
...
- `value_homes` is a target/codegen value-location lookup, not a BIR semantic
  owner.
- The selected projection should be built beside the existing
  `call_plan_lookups` and `address_materialization_lookups` projections and
  wired only through the existing `FunctionLoweringContext` pointer.

## Execution Rules

- Keep this packet behavior-preserving.
- Prefer the existing builder in `value_locations.hpp`; do not add a generic
  facade.
...
- Contract includes only when the selected direct field read no longer requires
  them.
- Treat `move_bundles` as a later group unless the supervisor explicitly
  delegates a builder extraction that preserves after-call result lane
  bindings.
...
### Step 1: Confirm selected value-home group

Goal: verify the chosen group is still the narrowest safe continuation after
the completed `address_materializations` packet.

Actions:

- Confirm traversal still reads `prepared_lookups.value_homes` only to populate
  `FunctionLoweringContext::value_home_lookups`.
- Confirm `prepare::make_prepared_value_home_lookups(...)` is available as the
  narrow builder.
- Confirm direct publication/source-producer, memory-access, move-bundle, and
  return-chain consumers remain outside this packet.
- Confirm `move_bundles` is left for a later packet because aggregate-equivalent
  move-bundle lookup construction includes after-call result lane binding
  publication.

Completion check:

- `todo.md` names `value_homes` as the selected field group and points
  execution at Step 2.

### Step 2: Project value-home lookups separately
...
- Build a local `PreparedValueHomeLookups` value with
  `prepare::make_prepared_value_home_lookups(function_context.value_locations)`.
- Assign `function_context.value_home_lookups` from that local projection.
- Keep `function_context.prepared_lookups`, `move_bundle_lookups`, and all
  unselected aggregate field use unchanged.
- Preserve object lifetimes for the full function-lowering loop, matching the
  local `prepared_call_plan_lookups` and
  `prepared_address_materialization_lookups` patterns.

Completion check:

- Production traversal no longer reads `prepared_lookups.value_homes`.
- A compile proof plus the selected value-location/AArch64 lowering subset is
  green.
...
### Step 4: Prove the selected value-home subset and decide next
...
- Run the supervisor-selected proof command for AArch64 value-location/value
  home lowering.
