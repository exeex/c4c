...
Resumed from: last idea-744 plan/todo pair at `48d50e428^`, after corrected prerequisite commit `0d3781e70`
...
- Steps 1-6 are complete: classification `fe3c14691`, focused probes
  `8426c128a`, direct scalar-call result `07303df86`, immediate argument
  `9505f0443`, SSA argument `c0e14731d`, and representative scalar chain
  `e3485e70a`.
- Step 7 ordinary identity packets continued through AArch64 fp128 call-result
  authority at `91ff4cf24`; `d1e7c6a8` then selected the external-double
  packet that became Step 7.32.
- Corrected prerequisite commit `0d3781e70` unblocks Step 7.32 through the
  normal resolved extern `Function` route. Step 7.32 is not complete until its
  remaining producer/verifier/matrix obligations are proven.
- Do not repeat Steps 1-6 or the completed Step 7 packets. Resume only at the
  Step 7.32 return work recorded below.
...
#### Step 7.32 status — resumed external direct scalar-call packet

Idea 746 is closed under the corrected normal-function architecture and commit
`0d3781e70`. A block-scope fixed-void extern declaration now creates a normal
bodyless extern HIR `Function`; its LIR declaration and direct call share
`LinkNameId` and structured signature, and the scalar result owns the
`LirValueId` consumed by the following `FAdd`.

Resume this exact packet without repeating Steps 1-6 or earlier Step 7 work.
Finish the Step 7.32 producer/verifier/matrix contract on the normal resolved
extern `Function` route: bind the accepted positive path, add or confirm the
reachable malformed declaration/signature/result ownership and type-conflict
obligations, and record the exact row disposition in the checked matrix.

Do not create a separate extern-function list, reconstruct facts from text, or
restore the disproven absent-`target_fn` model. Do not mark Step 7.32 complete
until its verifier and matrix obligations are proven.
