...
Consolidate the AArch64 `calls*.cpp` family now that shared prepared call-plan
authority exists, leaving AArch64-local calls code responsible for emission
...
- a shared prepared call-argument source fact exists for selected `BeforeCall`
  moves that source from preservation, local-frame address materialization, or
  byval register-lane materialization
...
- Call printing or effect spelling that belongs in machine-printer or shared MIR
  layers instead of AArch64 emission
...
- Decide whether a valid route exists through an existing shared fact or through
  the newly available prepared call-argument source fact.
...
- Otherwise, `todo.md` names the remaining blocker and the plan stays open or is
  retired by lifecycle decision.
