...
Activated After: closed idea 57

## Purpose

Resume the parked edge/terminator consumer-preservation repair now that the
HFA aggregate return-result owner has been closed, and re-check whether the
source idea's preservation contract is again the current first bad fact.
...
- If the fresh boundary check exposes another non-owned first bad fact, record
  it in `todo.md` and stop for supervisor routing instead of expanding this
  runbook.

## Ordered Steps

### Step 1: Re-establish the Edge-Preservation Failure Boundary After Idea 57

Goal: Confirm what remains after the HFA return-result repair and identify
whether the edge/terminator preservation acceptance criteria can now be judged
directly.
