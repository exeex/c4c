...
Activated from: paused idea 734 after accepted Step 7.23 typed `LirSwitch`
receipt (`0995a3deb`); fresh matching backend guard 5/5 is accepted, while the
current full baseline candidate has 73 failures and is not full-suite green.
...
2. Populate it without parsing `addr` and retain that operand only as a checked
   display mirror.
...
- populate it from existing pointer value identity, never from `addr` text
