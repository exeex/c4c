...
Resumed After: ideas/closed/144_member_template_constructor_specialization_metadata.md

## Purpose

Remove `TypeSpec::tag` as a cross-stage semantic identity field now that the
constructor member-template specialization decomposition has closed.
...
- `ideas/closed/144_member_template_constructor_specialization_metadata.md`
...
- Downstream LIR/BIR/backend carrier gaps exposed only after the field is
  removed.
- Broad validation or full-suite triage after the constructor-specific
  positive-Sema blocker closed.
...
- Do not weaken tests, mark supported cases unsupported, or add named-case
  shortcuts.
- Do not treat the rejected red full-suite candidate from the constructor
  decomposition as canonical parent proof.
...
- The member-template constructor specialization blocker is closed by
  `ideas/closed/144_member_template_constructor_specialization_metadata.md`.
  Remaining red broad-validation state belongs to this parent route or a
  separately split follow-up.
...
- For Step 6, establish a fresh supervisor-selected broad-validation baseline
  before treating any full-suite result as canonical.
...
Actions:
- Resume from the post-constructor-specialization state where
  `^cpp_positive_sema_` is green at 884/884.
- Establish a fresh supervisor-selected broad-validation baseline; the prior
  red full-suite candidate with 24 known failures was rejected as uncanonical.
...
- Escalate to supervisor-selected broad validation when the build and focused
  proofs are green.
- Split any unrelated broad-validation boundary into a separate open idea
  instead of absorbing it into this parent runbook.
