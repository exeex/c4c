...
Repaired after: accepted one-field migration packets through `LirSelectOp.true_val`; closure rejected because durable in-scope select operand migration remains.
...
- Current repair target: `collect_inst_refs` migration for `LirSelectOp.false_val`
  from raw `S(op.false_val)` scanning to
  `collect_operand_ref(op.false_val, refs)`.
- Already accepted under this route: `LirStoreOp.val`, `LirStoreOp.ptr`,
  ...
  `LirCmpOp.rhs`, `LirSelectOp.cond`, and `LirSelectOp.true_val`.
...
- Select `LirSelectOp.false_val` only unless inspection disproves its carrier
  readiness.
- Record the selected field, scanner consumer, proof target, and non-goals in
  `todo.md`.

Completion check:
- `todo.md` confirms `LirSelectOp.false_val` and `collect_inst_refs` as the
...
Actions:
- Change only the selected consumer path: replace `S(op.false_val)` in the
  `LirSelectOp` arm with `collect_operand_ref(op.false_val, refs)`.
